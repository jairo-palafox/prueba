################################################################################
#Versión                    => 1.0.0                                           #
#Fecha ultima modificación  => 29/10/2012                                      #
################################################################################
################################################################################
...
#Modulo            => CNT                                                      #
#Programa          => CNTL03                                                   #
#Objetivo          => Programa lanzador para integrar archivos de confirmación #
...
MAIN
   ...
   --Se genera folio de la operación
   CALL fn_genera_folio(g_proceso_cod, g_opera_cod,v_usuario)
   RETURNING  g_folio
   

   CALL fn_drgandrop_confirmacion_cnt()
   
END MAIN

--Objetivo: Función que permite seleccionar y arrastrar el archivo a integrar
FUNCTION fn_drgandrop_confirmacion_cnt()
   ...
   OPEN WINDOW w_inte_acred WITH FORM "CNTL031"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION integrar
         ...
         IF r_bandera = 0 THEN  
            ...
            CALL fn_actualiza_opera_ini (v_max_pid,g_proceso_cod,g_opera_cod,g_folio,"DISE08",l_v_arch_proceso,v_usuario)
            RETURNING r_b_valida

                                                
            IF r_b_valida = 0 THEN --Actualiza Operación Inicial
               LET l_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED, "/CNTE02.42r ",
                               v_usuario," ",
                               v_max_pid," ",
                               g_proceso_cod," ",
                               g_opera_cod," ",
