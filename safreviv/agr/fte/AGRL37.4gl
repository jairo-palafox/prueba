...
#                    del archivo recurrente para marca y desmarca  #
...
#Objetivo:
#  Se valida la información del archivo invocando la funcion general que carga el archivo a la tabla temporal
MAIN
   DEFINE p_v_usuario       LIKE seg_usuario.usuario,-- usuario firmado al sistema
          ...
          r_c_ruta_listados LIKE seg_modulo.ruta_listados, -- ruta del listados del módulo
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".AGRL37.log")
   ...
   LET v_d_pid = 0
   LET v_i_proceso_cod = g_proc_cod_agr_recurr_marca -- recepción recurrente de marca y desmarca
   LET v_i_opera_cod = 1 -- valida archivo recurrente
   LET v_d_folio = 0
   LET p_b_tipo_carga = 2
   LET v_v_nom_archivo = NULL
   LET v_c_programa_cod = "AGRL37"
   ...
   IF r_b_valida = 0 THEN
      ...
      LET v_s_comando = " fglrun ",r_c_ruta_bin CLIPPED,"/AGRP27 ",
                        p_v_usuario, " ",
                        v_d_pid, " ",
                        v_i_proceso_cod, " ",
                        v_i_opera_cod, " ",
