...
#                    Rechazos DSE que ha sido validado para el     #
#                    módulo de Anualudades Garantizadas            #
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".AGRE09.log")

   DISPLAY "=INICIA AGRE09="
   ...
   LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_dse_integra_rchdse_agr(?,?,?,?)"
   ...
   DISPLAY "TOTAL RECHAZADOS: ",v_r_dse_ctr_arch.tot_rechazados
{
   ...
   LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/AGRP22 ",
                                           p_v_usuario, " ",
                                           p_d_pid, " ",
                                           p_i_proceso_cod, " ",
                                           p_i_opera_cod, " ",
   ...
   SELECT COUNT(*)
     INTO v_cta_rch
     FROM safre_tmp:tmp_dse_rech_det_agr
   ...
   IF v_cta_rch >= 1 THEN
      LET v_cmd = " fglrun ",r_c_ruta_bin CLIPPED,"/ACRP41 ",
                  "safre_tmp:tmp_dse_rech_det_agr", " ",
                  "agr"
       RUN v_cmd
...

{
   ...
   LET v_v_nom_reporte = p_v_usuario CLIPPED, "-AGRL26-", p_d_pid USING "&&&&&", "-", p_i_proceso_cod USING "&&&&&", "-", p_i_opera_cod USING "&&&&&"
   ...
   IF NOT r_b_existe_err THEN
      ...
      LET v_s_titulo_correo = "Proceso: RECEPCIÓN RECHAZO DEVOL SDOS EXC AGR"
      ...
      LET v_s_mens_correo =  "ID Proceso   : ",p_d_pid,"\n",
                             "Proceso      : RECEPCIÓN RECHAZO DEVOL SDOS EXC AGR\n",
                             "Operacion    : INTEGRA RECHAZOS DEVOLUCIÓN SDO EXC AGR\n",
                             "Fecha Inicio : ",TODAY,"\n",
                             "Fecha Fin    : ",TODAY
...
FUNCTION f_genera_rpt_IntegraRechDSE(p_c_ruta_listados, p_v_nom_reporte)
   ...
   IF fgl_report_loadCurrentSettings("AGRE091.4rp") THEN
...
FUNCTION fn_crea_tmp_rech_devol_saldo()
   ...
   DROP TABLE tmp_deudor_rech_devol_saldo_agr

   -- al encontrar un error detiene el programa
   WHENEVER ERROR STOP

   CREATE TABLE tmp_deudor_rech_devol_saldo_agr(id_dse_devolucion  DECIMAL(9,0),
