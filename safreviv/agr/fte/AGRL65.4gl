...
#                     de deudores SAP-FICO en SACI (Lanzador).           #
...
MAIN 
   ...
   CALL STARTLOG(g_usuario CLIPPED|| ".AGRL65.log")
...
FUNCTION carga_archivo()
   ...
   OPEN WINDOW vtn1 WITH FORM "AGRL651"
      INPUT BY NAME v_archivo ATTRIBUTE(UNBUFFERED)
         ...
         ON ACTION ACCEPT 
            ...
            IF(v_archivo IS NOT NULL) THEN
               ...
               IF(v_extension <> "exfi") THEN
                  CALL fn_mensaje ("","La extensión del archivo no es correcta.\nEl archivo debe tener extensión '.exfi'","")
                  LET v_archivo = NULL 
                  NEXT FIELD v_archivo
               ELSE 
                  --*************************************************************************
                  ...
                  IF(bnd_transfer = 1) THEN
                     ...
                     -- Inicializa proceso
                     CALL fn_inicializa_proceso(g_pid,g_proceso_cod,g_opera_cod,"","AGRP45","",g_usuario) RETURNING v_bnd

                     -- Actualiza operación como iniciada
                     CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,g_opera_cod,0,"AGRP45","",g_usuario) RETURNING v_bnd

                     -- Ejecuta lanzado
                     LET v_comando = "nohup fglrun ",v_ruta_binaria CLIPPED,"/AGRP45 ",
                                      g_usuario," ",
                                      g_pid," ",
                                      g_proceso_cod," ",
                                      g_opera_cod," ",
                     ...
                     LET v_mensaje = "Se ejecutó el proceso de actualización deudores SAP-FICO"," \n",
