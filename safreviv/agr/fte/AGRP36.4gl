...
MAIN
   ...
   -- Creación de log
   CALL STARTLOG(g_usuario CLIPPED|| ".AGRP36.log")
   ...
   OPEN WINDOW w_m_marca WITH FORM "AGRP361"
...
FUNCTION fn_act_marca(p_nss,p_situacion)
   ...
   OPEN WINDOW w_marcas WITH FORM "AGRP363"
      ...
      INPUT ARRAY v_arr_marca FROM r_marca.* ATTRIBUTE( WITHOUT DEFAULTS,
         ...
         ON ACTION ACCEPT
            ...
            IF v_bandera = 1 THEN
               ...
               FOR i = 1 TO v_arr_marca.getLength()
                  IF v_arr_marca[i].checkbox = 1 OR v_arr_marca[i].ck_liq = 1 THEN
                     FOR j = 1 TO v_arr_marca.getLength()
                        LET v_ax_arr_marca[j].* = v_arr_marca[i].*
                     END FOR
                  ...
                  IF v_arr_marca[i].ck_int_pr = 1 OR v_arr_marca[i].ck_liq = 1 THEN
                     FOR j = 1 TO v_arr_marca.getLength()
                        LET v_ax_arr_marca[j].* = v_arr_marca[i].*
                     END FOR
                     -- bandera de marca interna
                  ...
                  IF v_arr_marca[i].ck_procesar = 1 THEN
                     FOR j = 1 TO v_arr_marca.getLength()
                        LET v_ax_arr_marca[j].* = v_arr_marca[i].*
                        LET v_ax_arr_marca[j].marca = v_arr_marca[i].marca_prc
                     END FOR
               ...
               DISPLAY "CK PROCESAR ",v_ax_arr_marca[i].ck_procesar
               ...
               FOR i = 1 TO v_ax_arr_marca.getLength()
                  ...
                  IF v_ax_arr_marca[i].checkbox = 1 OR v_arr_marca[i].ck_liq = 1 THEN    -- Si el checkbox esta prendido
                  ...
                  IF v_ax_arr_marca[i].ck_int_pr = 1 OR v_arr_marca[i].ck_liq = 1 THEN
                     DISPLAY "SI ENTRA DONDE DEBE"
                     LET v_ax_arr_marca[i].marca = v_ax_arr_marca[i].marca_prc    -- P R O B A R 
                     LET v_rec_marca.* = v_ax_arr_marca[i].*
                  END IF
               END FOR

               CALL v_ax_arr_marca.clear()   -- Se limpia el arreglo
               DISPLAY "RECORD",v_rec_marca.*
...
FUNCTION fn_act_desmarca(p_nss,p_situacion)
   ...
   OPEN WINDOW w_d_arreglo WITH FORM "AGRP362"
      ...
      FOR i = 1 TO v_arr_desmarca.getLength()
         ...
         LET v_arr_desmarca[i].ck_pr_int   = 0
      END FOR
      ...
      INPUT ARRAY v_arr_desmarca FROM r_desmarca.* ATTRIBUTE( WITHOUT DEFAULTS,
         ...
         ON CHANGE checkbox
            ...
            FOR j = 1 TO v_arr_desmarca.getLength()
               ...
               LET v_arr_desmarca[j].ck_pr_int   = 0
               IF v_arr_desmarca[j].checkbox IS NULL THEN
                  LET v_arr_desmarca[j].checkbox = 1
               END IF
            END FOR
...
{            -- Se valida que no sean marcas internas
            ...
            FOR j = 1 TO v_arr_desmarca.getLength()
               LET v_arr_desmarca[j].checkbox = 0
               LET v_arr_desmarca[j].ck_pr_int = 0
               IF v_arr_desmarca[j].ck_procesar IS NULL THEN
                  LET v_arr_desmarca[j].ck_procesar = 1
               END IF 
            END FOR
         ...
         ON CHANGE ck_pr_int
            -- Si se elige marca Procesar interna, bloquea las demás
            FOR j = 1 TO v_arr_desmarca.getLength()
               LET v_arr_desmarca[j].checkbox    = 0
               LET v_arr_desmarca[j].ck_procesar = 0
               IF v_arr_desmarca[j].ck_pr_int IS NULL THEN
                  LET v_arr_desmarca[j].ck_pr_int = 1
               END IF 
            END FOR

            -- Valida que solo puedan seleccionar una casilla
            IF v_arr_desmarca[v_pos].ck_pr_int = 1 THEN
               FOR j = 1 TO v_arr_desmarca.getLength()
                  IF j <> v_pos THEN
                     LET v_arr_desmarca[j].ck_pr_int = 0
                  END IF
         ...
         ON ACTION ACCEPT
            -- Se valida que no existan casillas nulas 
            FOR i = 1 TO v_arr_desmarca.getLength()
               IF v_arr_desmarca[i].checkbox IS NULL THEN
                  LET v_arr_desmarca[i].checkbox = 0 
               END IF 
               IF v_arr_desmarca[i].ck_procesar IS NULL THEN
                  LET v_arr_desmarca[i].ck_procesar = 0 
               END IF 
               IF v_arr_desmarca[i].ck_pr_int IS NULL THEN
                  LET v_arr_desmarca[i].ck_pr_int = 0 
               END IF 
            END FOR 
            ...
            IF v_bandera = 1 THEN
               FOR i = 1 TO v_arr_desmarca.getLength()
                  IF v_arr_desmarca[i].checkbox = 1 THEN
                     FOR j = 1 TO v_arr_desmarca.getLength()
                        ...
                        DISPLAY "Arreglo aux", v_ax_arr_desmarca[j].*
                     END FOR
                     --LET i = i + 1
                     LET v_bandera_tpo = 0
                  END IF
                  ...
                  IF v_arr_desmarca[i].ck_pr_int = 1 THEN
                     FOR j = 1 TO v_arr_desmarca.getLength()
                        LET v_ax_arr_desmarca[j].* = v_arr_desmarca[j].*
                     END FOR
                     LET v_bandera_tpo = 0 
               ...
               FOR i = 1 TO v_ax_arr_desmarca.getLength()
                  ...
                  IF v_ax_arr_desmarca[i].ck_pr_int = 1 THEN
                     LET v_rec_desmarca.* = v_ax_arr_desmarca[i].*
                  END IF
               END FOR
               ...
               DISPLAY "Datos que se mandan a la función"
               DISPLAY "nss ", p_nss
               DISPLAY "record desmarca", v_rec_desmarca.*
               DISPLAY "situacion",p_situacion
               DISPLAY "bandera tipo", v_bandera_tpo
...
FUNCTION fn_ins_mov_tabla_marca(p_nss,p_rec_marca,p_situacion,p_bandera)
   ...
   LET v_s_qry = "INSERT INTO safre_tmp:act_marca_desm_ag
                       VALUES(?,?,?,?,?,?,?,?,?,?)"
   ...
   IF sqlca.sqlcode < 0 THEN
      CALL fn_mensaje("Alterta","Error al procesar registro","stop")
   ELSE
...
FUNCTION fn_ins_mov_tabla(p_nss,p_rec_marca,p_situacion,p_bandera_tpo)
   ...
   DISPLAY "nss",p_nss
   DISPLAY "num cred"     , p_rec_marca.num_credito
   DISPLAY "tpo cred"     , p_rec_marca.tpo_credito
   DISPLAY "estadp"       , p_rec_marca.estado
   DISPLAY "edo procesar" , p_rec_marca.edo_procesar
   DISPLAY "marca"        , p_rec_marca.marca
   DISPLAY "situacion"    , p_situacion
...
FUNCTION fn_llena_arreglo(p_nss)
   ...
   FOREACH cur_desmar INTO v_arr_desmarca[v_cnt].*
      ...
      IF v_arr_desmarca[v_cnt].marca IS NULL   OR 
         v_arr_desmarca[v_cnt].marca = ""      AND 
         v_arr_desmarca[v_cnt].num_credito = 0 THEN

         DISPLAY "entra al if"
          LET v_s_qry = " SELECT tc.marca_prc
                            FROM cat_tipo_credito tc, cre_acreditado ca, afi_derechohabiente ad
                            WHERE tc.tpo_Credito = ca.tpo_credito
                            AND tc.tpo_originacion = ca.tpo_originacion
                            ...
                            AND ca.estado       = ?
                            AND ca.edo_procesar = ?
                            AND ca.id_cre_acreditado = ? "

         PREPARE prp_cons_marca FROM v_s_qry
         EXECUTE prp_cons_marca USING p_nss,
                                      v_arr_desmarca[v_cnt].num_credito,
                                      v_arr_desmarca[v_cnt].tpo_credito,
                                      v_arr_desmarca[v_cnt].estado,
                                      v_arr_desmarca[v_cnt].edo_procesar,
                                      v_arr_desmarca[v_cnt].id_cre_acreditado
                                 INTO v_arr_desmarca[v_cnt].marca

         DISPLAY "Marca despues del segundo qry : ",v_arr_marca[v_cnt].marca
      END IF
      LET v_cnt = v_cnt + 1
   END FOREACH
