...
FUNCTION fn_proceso_principal()
   ...
   OPEN WINDOW w_consulta_marca WITH FORM "AGRC101"
      ...
      WHILE g_ciclo
         {IF g_fecha IS NULL OR g_fecha = '12/31/1899' THEN
            LET g_fecha = '12/31/1899'
            WHILE g_fecha = '12/31/1899'}
...
DEFINE p_fec_marca         DATE,
       qry_string          STRING,
       v_s_existe_info     SMALLINT,
       v_f_marca           DATE,
       v_situacion         SMALLINT,
       v_tot_marca         INTEGER
...
DEFINE p_fecha             DATE,  ---Fecha de solicitud
       v_s_qry             STRING, --String para armar el query
       v_c_ruta_envio      LIKE seg_modulo.ruta_envio, --Ruta envío del archivo
       v_c_nom_archivo     CHAR(30),
       v_c_ruta_escritura  STRING, --Ruta donde escribirá el archivo
       v_c_fec_archivo     CHAR(8),  --Fecha del nombre de archivo
       v_s_detalle         STRING, --Detalle del archivo
       v_ch_archivo        base.channel, --Canal para escritura del archivo
       v_s_existe_info     SMALLINT  --Bandera para indicar si existe información

DEFINE v_c_nss           CHAR (11), --Número de seguridad social
       v_c_num_credito   CHAR (10), --Número de crédito para la consulta de cta_marca_ws
       v_c_tpo_credito   CHAR (2), --Tipo de crédito de la tabla cta_marca_ws
       v_c_rechazo       CHAR (3), --Motivo de rechazo de acuerdo a código de PROCESAR
       v_c_situacion     CHAR (1), --Situación del registro 
       v_c_accion        CHAR (1) --Acción de acuerdo con situación
  ...
  --Concluye la escritura del archivo
   ...
   IF v_s_existe_info THEN
       ...
       CALL fn_mensaje("Información","Se ha generado el archivo de Consulta de Solicitud de Marcaje a Procesar via WS\n en la ruta"||v_c_ruta_escritura,"information")                     
   ELSE
...
DEFINE    manejador_rpt           om.SaxDocumentHandler, --Contenedor documentos reporte 
          v_i_indice_situacion    SMALLINT,
          v_i_indice_solicitud    SMALLINT,
          v_i_resp_detalle      SMALLINT
