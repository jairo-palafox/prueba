...
#Modulo            => AGR                                             #
#Programa          => AGRL70                                          #
#Objetivo          => Transacción para envío de archivos de solicitud #
#                     de saldos (TA,43 BIS,AG) a PROCESAR.            #
#Autor             => Emilio Abarca, EFP                              #
#Fecha inicio      => 09/JULIO/2018                                   #
#######################################################################
...
MAIN 
   ...
   CALL STARTLOG(g_usuario CLIPPED|| ".AGRL70.log")
   ...
   CALL envia_archivos_sol_sdo()
   
END MAIN

FUNCTION envia_archivos_sol_sdo()
   ...
   OPEN WINDOW vtn1 WITH FORM "AGRL701"
      ...
      INPUT ARRAY arr_archivos FROM tabla1.* ATTRIBUTE(WITHOUT DEFAULTS,APPEND ROW = FALSE,
         ...
         BEFORE INPUT
            LET v_ind_reg = recupera_archivos_servidor()
         ...
         ON ACTION ACCEPT
            ...
            IF(v_confirma = 1) THEN
               ...
               FOR v_f = 1 TO arr_archivos.getLength()
                  IF(arr_archivos[v_f].check_box = 1) THEN
                     INSERT INTO tmp_envio_arh_sol_sdo (
                                            id_proceso,
                                            archivo,
                                            ruta_envio,
                                            f_proceso,
               ...
               -- Inicializa proceso
               CALL fn_inicializa_proceso(g_pid,g_proceso_cod,g_opera_cod,"","AGRL70","",g_usuario) RETURNING v_bnd
               ...
               CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,g_opera_cod,0,"AGRL70","",g_usuario) RETURNING v_bnd
               ...
               -- Ejecuta lanzado
               --En el parámetro 5 se envía "1" haciendo referencia que son archivos de solicitud de saldos a enviar
...
FUNCTION recupera_archivos_servidor()
   ...
   FOREACH crs_sol_sdo INTO r_seg_modulo.modulo_cod,
      ...
      CASE
         ...
         WHEN r_seg_modulo.modulo_cod = "acr"
            LET v_archivo_envia = "sol_ta.acr"
            LET v_id_proceso    = 210

         --ANUALIDADES GARANTIZADAS
         WHEN r_seg_modulo.modulo_cod = "agr"
            LET v_archivo_envia = "sol_ag.agr"
            LET v_id_proceso    = 310

         --43BIS
         WHEN r_seg_modulo.modulo_cod = "grt"
            LET v_archivo_envia = "sol_ug.usog"
            LET v_id_proceso    = 1216
      ...
      IF (os.Path.exists(v_ruta_archivo)) THEN
         ...
         LET v_comando_t = "wc -l ",v_ruta_archivo," > ",r_seg_modulo.ruta_envio CLIPPED,"/wc_arh_sol.unl"
         RUN v_comando_t

         --Obtiene sólo el total de registros del archivo
         LET v_comando_t = "cut -d ' ' -f 1 ",r_seg_modulo.ruta_envio CLIPPED,"/wc_arh_sol.unl"," > ",r_seg_modulo.ruta_envio CLIPPED,"/total_arh_sol.unl"
         RUN v_comando_t

         --Se lee el archivo total_sol_ta.unl
         LET v_arh_total = r_seg_modulo.ruta_envio CLIPPED,"/total_arh_sol.unl"
         ...
         LET v_comando_t = "rm ",r_seg_modulo.ruta_envio CLIPPED,"/wc_arh_sol.unl"
         RUN v_comando_t

         LET v_comando_t = "rm ",r_seg_modulo.ruta_envio CLIPPED,"/total_arh_sol.unl"
         RUN v_comando_t
...
FUNCTION crea_temporal()
   ...
   WHENEVER ERROR CONTINUE
      DROP TABLE tmp_envio_arh_sol_sdo

   WHENEVER ERROR STOP
      CREATE TABLE tmp_envio_arh_sol_sdo (
                        id_proceso   SMALLINT,
                        archivo      CHAR(40),
                        ruta_envio   CHAR(40),
                        f_proceso    DATE,
