...
MAIN
   DEFINE p_v_usuario       LIKE seg_usuario.usuario,-- usuario firmado al sistema
          ...
          v_i_cuenta_regs     INTEGER, -- cuenta el numero de registros
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".AGRL12.log")
   ...
   OPEN WINDOW w_integra_arch_desmar WITH FORM "AGRL021"
            ...
            # La función de integración los detecta y los rechaza 23/11/2012
{
            ...
            -- verifica si existe información de Recurrente (Originación)
            LET v_s_qryTxt = " SELECT COUNT(*)\n",
                             "   FROM safre_tmp:tmp_desmarca_det_agr_01"

            PREPARE prp_slct_cnt_desm01 FROM v_s_qryTxt
            EXECUTE prp_slct_cnt_desm01 INTO v_i_cuenta_regs
            ...
            IF v_i_cuenta_regs > 0 THEN
               ...
               EXECUTE prp_slct_precioAcc INTO v_i_cuenta_regs
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               IF r_b_valida = 0 THEN
                  ...
                  LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/AGRE06 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
...
FUNCTION f_valida_arch_integra()
   ...
   FOREACH cur_tmp_nss INTO v_c_nss_tmp
      ...
      -- se valida el estado
      IF v_i_status = 0 THEN
         EXIT FOREACH
      END IF

      -- se verifica si el registro no fue encontrado en cre acreditado
      IF v_i_cont_regs = 0 THEN
         -- se busca si el id derechohabiente está en estatus requerido
         LET v_s_qryTxt = " SELECT estado\n",
                          "   FROM cre_uso_garantia\n",
                          "  WHERE id_derechohabiente = ",v_d_id_derechohab,"\n",
                          "    AND edo_procesar IN (5, 60, 70)\n",
