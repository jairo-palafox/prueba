...
#Modulo            => AGR                                            #
#Programa          => AGRL74                                         #
#Objetivo          => Lanzador que permite la integración del        #
#                     de actualización marcas para solicitud saldo.  #
#Autor             => Emilio Abarca, EFP                             #
#Fecha inicio      => 28/Agosto/2018                                 #
######################################################################
...
MAIN
   ...
   DEFINE v_tot_tmp         INTEGER                             -- Total de registros en la temporal
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".AGRL74.log")
   ...
   LET v_i_proceso_cod   = 350   -- ACTUALIZACIÓN MARCA ZOLICITUD SALDO
   LET v_i_opera_cod     = 2     -- INTEGRA ARCHIVO SOLICITUD SALDO
   LET v_i_opera_cod_ant = 1     -- VALIDA ARCHIVO SOLICITUD SALDO
   LET v_d_folio         = 0
   LET v_c_programa_cod  = "AGRL74"
   ...
   -- Obtien los archivos disponibles para integrar
   ...
   OPEN WINDOW w_integra_arch_sspr WITH FORM "AGRL021"
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         ON ACTION ACCEPT
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               LET v_s_qryTxt = " SELECT *\n",
                                ...
                                "  WHERE id_proceso  = ",v_i_proceso_cod,"\n",
                                "    AND operacion   = ",v_i_opera_cod_ant,"\n",
                                "    AND estado      = 10\n", -- cargado
                ...
                IF (v_r_cre_ctr_arch.id_cre_ctr_archivo IS NULL) THEN
                   CALL fn_mensaje("Integración SSPR","El archivo está marcado como Rechazado, no se puede continuar\ncon la integración del archivo","stop")
                   CONTINUE DIALOG
                END IF
         ...
         ON ACTION integrar
            ...
            -- Verifica que existan registros a integrar
            SELECT COUNT(*)
              INTO v_tot_tmp
              FROM safre_tmp:tmp_det_marca_sspr;

            IF(v_tot_tmp = 0) THEN
               LET v_s_cadena =  "No se encuentra información de actualización de marcas SSPr\n",
                                 "para los registros a Integrar"
               CALL fn_mensaje("Aviso",v_s_cadena,"stop")
               CONTINUE DIALOG
            END IF

            CALL fn_mensaje("","INTEGRACIÓN ENVIADA","")
            -- se procesan los archivos seleccionados para integrar
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               LET r_b_valida = fn_actualiza_opera_ini(v_d_pid,v_i_proceso_cod,v_i_opera_cod,v_d_folio, 
                                                       v_c_programa_cod,v_v_nomArch_proc, p_v_usuario)

               -- se verifica si fue posible inicializar la operacion
               IF r_b_valida = 0 THEN
                  -- se crea el comando que ejecuta el modulo que reliza la integracion del archivo
                  LET v_s_comando = " nohup time fglrun ",v_c_ruta_bin CLIPPED,"/AGRE18 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
            ...
            LET v_s_mensaje = "Se ha enviado la integración con PID: ",v_d_pid USING "<<<<<<<<<",
   ...
   CLOSE WINDOW w_integra_arch_sspr
