...
#                   devolucion para el módulo de Anualidades Garantizadas      #
#Autor           => Daniel Buendia, EFP                                        #
#Fecha Inicio    => 30 Mayo 2012                                               #
#Modifiaciones                                                                 #
#Autor           => Héctor F. Jiménez                                          #
#Descripción     => Se modifica la salida del reporte para que muestre los     #
...
MAIN
   ...
   DISPLAY " Ejecuta solicitud de devolución sdos exc "
   ...
   LET v_s_titulo_correo = "Proceso: SOLICITUD DE DEVOLUCIÓN DE SALDOS EXCEDENTES ANUALIDADES GARANTIZADAS"
...
FUNCTION fn_reporte_archivo_solicitud_devolucion(p_id_lote,
   ...
   DEFINE v_arch_encabezado      RECORD
             ...
             id_oper                 CHAR(02),   --Identificador de Operación
             ...
             ent_fed_env             CHAR(03),   --Entidad federativa de envío de lote
             fec_present             CHAR(08),   --Fecha de presentación
             cons_lote               CHAR(03),   --Consecutivo del lote en el día
             cve_mod_recept          CHAR(02),   --Clave de modalidad de recepción
             cod_res_oper            CHAR(02),   --Código de resultado de la Operación
   ...
   DEFINE v_arch_detalle         RECORD
             ...
             id_asign                CHAR(01), -- Identificador de Asignación [83-83]
             tpo_devol               CHAR(01), -- Tipo de devolución [84-84]
             rfc_trab_inf            CHAR(13), -- RFC del Trabajador según Infonavit [85-97]
             ...
             sum_aivs97              CHAR(15), -- Número de AIVS 97 [475-489]
             filler4                 CHAR(15), -- filler4 [490-504]
             nss_separ_cta           CHAR(11), -- NSS por separación de cuentas [505-515]
             orig_devol              CHAR(02), -- origen de la devolución [516-517]
             mto_aportac             CHAR(15), -- Monto de Aportaciones 5% [518-532]
             apl_int_aportac         CHAR(15), -- Aplicación de Intereses de Aportaciones 5% [533-547]
             filler5                 CHAR(02), -- Filler [548-549]
             sum_aivs92              CHAR(15), -- Número de AIVS 92 [550-564]
             filler6                 CHAR(18), -- Filler [565-582]
             cod_res_opera           CHAR(02), -- Código Resultado de la Operación [583-584]
             diag_proc               CHAR(15), -- Diagnóstico del proceso [585-599]
             nom_trab_imss           CHAR(50), -- Nombre del Trabajador según IMSS [600-649]
             num_cred_info           CHAR(10), -- Número de Crédito INFONAVIT [650-659]
   ...
   DEFINE v_arch_sumario         RECORD
             ...
             sum_aivs97              CHAR(18), -- Suma de número de AIVS 97 [42-59]
             filler2                 CHAR(60), -- Filler [60-119]
             sum_aivs92              CHAR(18), -- Suma de número de AIVS 92 [120-137]
   ...
   LET v_arch_encabezado.id_oper        = "15"    --CHAR(02 )   --Identificador de Operación
   ...
   LET v_arch_encabezado.ent_fed_env    = "009"   --CHAR(03 )   --Entidad federativa de envío de lote
   LET v_arch_encabezado.fec_present    = v_c_f_presentacion
   LET v_arch_encabezado.cons_lote      = "001"      --CHAR(03 )   --Consecutivo del lote en el día
   LET v_arch_encabezado.cve_mod_recept = "" --"02"  --CHAR(02 )   --Clave de modalidad de recepción
   LET v_arch_encabezado.cod_res_oper   = ""         --CHAR(02 )   --Código de resultado de la Operación
   ...
   FOREACH cur_rep_selicitud_pago INTO v_r_sol_sdos.*
      ...
      LET v_arch_detalle.sum_aivs97      = (v_r_sol_sdos.aivs97 * 1000000) USING "&&&&&&&&&&&&&&&" --SUM(AIVS97) dse_his_devolucion           --(09,06) Número de AIVS 97
      ...
      LET v_arch_detalle.apl_int_aportac = (v_r_sol_sdos.aivs_aportacion * 1000000) USING "&&&&&&&&&&&&&&&"    --(09,06) AIVS_APORTACION EN dse_his_devolucion    --CHAR(09 )  --Aplicación de Intereses de Aportaciones 5%
      LET v_arch_detalle.filler5         = ""
      LET v_arch_detalle.sum_aivs92      = (v_r_sol_sdos.aivs92 * 1000000) USING "&&&&&&&&&&&&&&&"   --(09,06) SUM(AIVS92) dse_his_devolucion           --CHAR(09 )  --Número de AIVS 92
