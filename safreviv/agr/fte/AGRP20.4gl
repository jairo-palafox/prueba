...
#                la agrupacion de DSE para el módulo de Analidades G. #
...
MAIN
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".AGRP20.log")

   DISPLAY "=INICIA AGRP20="
   ...
   LET v_c_tpo_transf = "43" -- 43-DSE Anualidades Garantizadas
...
FUNCTION fn_gen_rpt_agrupacion(p_v_nom_reporte, p_ruta_listados, p_c_tpo_transf, p_d_folio, p_usuario_cod)
   ...
   IF ( fgl_report_loadCurrentSettings("AGRP201.4rp") ) THEN
   ...
   START REPORT rpt_agrupacion_agr TO XML HANDLER v_report_handler

   -- se obtienen los totales para DSE
   CALL fn_obt_agrup_modulo("agr", p_c_tpo_transf, p_d_folio) RETURNING v_r_agrup.tot_reg_dse, v_r_agrup.aivs97_dse, v_r_agrup.pesos97_dse, v_r_agrup.aivs92_dse, v_r_agrup.pesos92_dse
   ...
   FOREACH cur_folio_archivo INTO v_r_rpt_detalle.*
      -- salida del reporte
      OUTPUT TO REPORT rpt_agrupacion_agr(p_usuario_cod, p_d_folio, v_r_rpt_detalle.*, v_r_agrup.*)
   END FOREACH

   -- finaliza el reporte
   FINISH REPORT rpt_agrupacion_agr
END FUNCTION

-- OBJETIVO: Emitir el reporte de liquidacion/preliquidacion
REPORT rpt_agrupacion_agr(p_usuario_cod, p_d_folio, p_r_rpt_detalle, p_r_agrup)
