...
MAIN
   ...
   CALL STARTLOG(m_c_usuario_cod CLIPPED|| ".AGRP34.log")
...
FUNCTION fn_consulta_nss()
   ...
   OPEN WINDOW w_op_derechohab WITH FORM "AGRP341"
   ...
   SELECT COUNT(*)
     ...
     FROM cre_acreditado c, cat_tipo_credito t
    WHERE c.id_derechohabiente = v_r_afi_derechohabiente.id_derechohabiente
      AND c.tpo_originacion IN (1,3,4,5,15)
...
FUNCTION fn_selec_derechohabiente(p_s_condicion)
   ...
   OPEN WINDOW w_lista_derechohabiente WITH FORM "AGRP342"
...
FUNCTION fn_selec_credito(p_id_derechohabiente, p_c_nss)
   ...
   LET v_sqlqry = " SELECT c.id_cre_acreditado, c.id_derechohabiente, c.tpo_originacion, c.tpo_credito,\n",
                  "        c.num_credito, estado, c.edo_procesar, c.f_otorga, c.sdo_deudor\n",
                  "   FROM cre_acreditado c, cat_tipo_credito t\n",
   ...
   OPEN WINDOW w_lista_creditos WITH FORM "AGRP343"
...
FUNCTION  fn_despliega_info_derechohab(p_r_afi_derechohabiente, p_d_id_cre_acreditado)
   ...
   OPEN WINDOW w_des_derechohab WITH FORM "AGRP344"
      ...
      DIALOG ATTRIBUTES (FIELD ORDER FORM, UNBUFFERED)
         ...
         ON ACTION ACCEPT
            ...
            LET v_s_qryTxt = " SELECT COUNT(*)\n",
                             "   FROM cre_acreditado c, cat_tipo_credito t\n",
                             "  WHERE c.id_cre_acreditado = ",p_d_id_cre_acreditado,"\n",
                             "    AND c.estado IN (140,900,220,145)\n",
                             "    AND c.edo_procesar IN (120,5)",
                             "    AND c.tpo_credito = t.tpo_credito ",
                             "    AND c.tpo_originacion = t.tpo_originacion ",
                             "    AND t.id_deudor = 1 "
