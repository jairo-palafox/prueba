...
#                    ción de rechazo de devolucion de saldos               #
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".AGRP22.log")

   DISPLAY "=INICIA AGRP22="
   DISPLAY " USUARIO       : ",p_v_usuario
   DISPLAY " PID           : ",p_d_pid
   DISPLAY " FOLIO         : ",p_d_folio USING "#########&"
   DISPLAY " ARCHIVO       : ",p_v_arch_proceso
   DISPLAY " CONCILIACIÓN DE RECHAZOS DE DEV. DE SDOS. EXC. AGR"
   ...
   IF fgl_report_loadCurrentSettings("AGRP221.4rp") THEN  -- if  the file loaded OK
   ...
   LET v_d_pid_arch = fn_max_pid(g_proc_cod_agr_rech_dse, 2)
   ...
   LET v_s_qryTxt = " SELECT *\n",
                    ...
                    "    AND proceso_cod = ",g_proc_cod_agr_rech_dse,"\n",
                    "    AND opera_cod = 2\n",
   ...
   # salida del reporte
   OUTPUT TO REPORT reporte_Rechazo_ds(p_v_usuario, v_folio_archivo, v_r_rpt_res.*, v_r_pago_cred_trad_inf.*, v_r_pago_cred_trad_afo.*)
   ...
   LET v_s_titulo_correo = "Proceso: CONCILIACIÓN DE RECHAZO DE DEVOLUCIÓN DE SALDOS EXCEDENTES ANUALIDADES GARANTIZADAS"
...
REPORT reporte_Rechazo_ds(p_v_usuario, p_d_folio, p_r_res, p_r_pago_cred_trad_inf, p_r_pago_cred_trad_afo)
   ...
   ON EVERY ROW
      ...
      DECLARE cur_dia CURSOR FOR   SELECT t.diagnostico[1,3]||" - "||c.desc_rechazo diag,
                                          COUNT(*)
                                     FROM safre_tmp:tmp_dse_rech_det_agr t , cat_rechazo c
