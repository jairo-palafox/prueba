...
#Modulo            =>AGR                                           #
#Programa          =>AGRE05                                        #
#Objetivo          =>Programa que permite la integración del archi-#
#                    vo de Solicitudes no Atentidas del módulo AG  #
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".AGRE05.log")

   DISPLAY "=INICIA AGRE05="
   ...
   LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_agr_integra_noaten(?,?,?,?)"
   ...
   LET v_s_comando = " nohup time fglrun ",v_c_ruta_bin CLIPPED,"/AGRP10 ",
                                           p_v_usuario, " ",
                                           p_d_pid, " ",
                                           p_i_proceso_cod, " ",
                                           p_i_opera_cod, " ",
   ...
   RUN v_s_comando
{
   -- se comenta la finalización de la operación ya que está la ejecuta el programa lanzado
...
FUNCTION fn_crea_tmp_no_aten()
   ...
   DROP TABLE tmp_deudor_no_aten_agr

   WHENEVER ERROR STOP

   CREATE TABLE tmp_deudor_no_aten_agr(id_cre_acreditado DECIMAL(9,0),
