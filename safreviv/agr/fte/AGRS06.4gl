...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".AGRS06.log")

   DISPLAY "=INICIA AGRS06="
   ...
   IF fgl_report_loadCurrentSettings("AGRS061.4rp") THEN  -- if  the file loaded OK
{
      -- verifica si el lanzador fue la liquidación del proceso
      IF p_i_proceso_cod = g_proc_cod_agr_liquidacion THEN
         LET v_c_programa_cod = "AGRS06"
      END IF
      -- veirifica si el lanzador fue archivo de salida
      IF p_i_proceso_cod = g_proc_cod_agr_arch_amort72 THEN
         LET v_c_programa_cod = "AGRL36"
      END IF
}
   ...
   FOREACH cur_cta_movimiento INTO v_r_cta_movimiento.*
      ...
      IF v_r_cta_movimiento.sum_monto_pesos IS NULL THEN
         LET v_r_cta_movimiento.sum_monto_pesos = 0
      END IF
   ...
   LET v_nom_fin = "A_cred_f_ag." || v_c_extension
   ...
   DISPLAY " Nombre del archivo amortización Fondo 72 a enviar: ",v_nom_fin
   DISPLAY ""
   DISPLAY " Ejecutando envío interfaz a ALS"

   LET v_ejecuta_sh = "\n sh /opt/Interpel/Scripts/A_cred_f_ag.sh"
   RUN v_ejecuta_sh
...
FUNCTION fn_obt_nss_numCredito(p_id_afi_fondo72)
   DEFINE p_id_afi_fondo72     LIKE afi_fondo72.id_afi_fondo72, --id del fondo72
          v_d_num_credito      LIKE cre_acreditado.num_credito, -- registro de afi derechohabiente
          v_i_nss              LIKE afi_derechohabiente.nss, -- numero de seguridad social
