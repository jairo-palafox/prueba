...
#Módulo       => AGR                                                           #
#Programa     => AGRL58                                                        #
#Objetivo     => Programa que permite la integración del archivo               #
#                para homologación de creditos y marcas procesar               #
#Autor        => Jose Eduardo Ventura                                          #
#Fecha inicio => 05 Agosto de 2016                                             #
################################################################################
...
#Objetivo:
#  Proceso que permite la ejecución del proceso de integracion de la informacion
#  del archivo de créditos liquidados del área de Cartera
MAIN
   ...
   DEFINE v_arr_integrados DYNAMIC ARRAY OF RECORD -- arreglo que contiene los archivos integrados
      archivo             LIKE glo_ctr_archivo.nombre_archivo,
      fecha               LIKE glo_ctr_archivo.f_actualiza,
      folio               LIKE glo_ctr_archivo.folio,
      fecha_act           LIKE glo_ctr_archivo.f_actualiza,
      tot_registros       INTEGER,
      folio_1             LIKE glo_ctr_archivo.folio
   END RECORD

   DEFINE v_r_cre_ctr_arch          RECORD LIKE glo_ctr_archivo.* -- registro del control de archivo  
   ...
   DEFINE v_si_cuenta_valor         SMALLINT -- variable que indica si existe el precio de acción
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".AGRL60.log")
   ...
   OPEN WINDOW w_integra_arch_cred_liq WITH FORM "AGRL601"
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         BEFORE DIALOG
            -- se oculta el botón de Integrar
         ...
         ON ACTION ACCEPT
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               -- se cuenta el total de registros en archivo
               LET v_s_qryTxt = " SELECT COUNT(*)
                                    FROM safre_tmp:tmp_hom_mca_prcr "

               PREPARE prp_cnt_tot FROM v_s_qryTxt
               EXECUTE prp_cnt_tot INTO v_tot_registros

               -- se realiza la consulta a la información del archivo
               LET v_s_qryTxt = " SELECT *\n",
                                "   FROM glo_ctr_archivo\n",
                                "  WHERE proceso_cod = ",v_i_proceso_cod,"\n",
                                "    AND opera_cod = 1 \n",
                                "    AND estado = 1 \n", -- cargado
                                "    AND nombre_archivo = '",v_v_nomArch_proc CLIPPED,"'"
                ...
                IF v_r_cre_ctr_arch.nombre_archivo IS NULL THEN
                   ...
                   CALL fn_mensaje("Integración Recurrente",v_s_mensaje,"stop")
                   CONTINUE DIALOG
                END IF

                -- se asigna la informacion del archivo integrado
                LET v_arr_integrados[v_i_iter].archivo = v_r_cre_ctr_arch.nombre_archivo
                LET v_arr_integrados[v_i_iter].fecha = v_r_cre_ctr_arch.f_actualiza
                LET v_arr_integrados[v_i_iter].folio = v_r_cre_ctr_arch.folio
                LET v_arr_integrados[v_i_iter].fecha_act = v_r_cre_ctr_arch.f_actualiza
                LET v_arr_integrados[v_i_iter].tot_registros = v_tot_registros
                LET v_arr_integrados[v_i_iter].folio_1 = v_r_cre_ctr_arch.folio
            END FOR
         ...
         ON ACTION integrar
            ...
            -- en caso de no existir registroa a procesar se informa que debe de haber al menos uno
            IF v_i_num_arch = 0 THEN
               LET v_s_mensaje =  "No se han seleccionado archivos a intergrar"
               CALL fn_mensaje("Aviso",v_s_mensaje,"stop")
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               LET v_v_nomArch_proc = v_arr_integrados[v_i_iter].archivo
               ...
               IF r_b_valida = 0 THEN
                  UPDATE glo_ctr_archivo
                   ...
                   WHERE proceso_cod = v_i_proceso_cod
                     AND opera_cod   = 1
                     AND estado      = 1
                     AND nombre_archivo = v_v_nomArch_proc
                  -- se crea el comando que ejecuta el modulo que reliza la integracion del archivo
                  LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/AGRE17 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
   ...
   CLOSE WINDOW w_integra_arch_cred_liq
