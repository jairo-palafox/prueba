...
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".AGRL34.log")
   ...
   LET m_i_proceso_cod = g_proc_cod_agr_reverso_dse -- reverso dev sdo exc agr
   LET m_i_opera_cod = 1 -- reverso dev sdo exc agr
   LET v_d_pid = 0
   LET v_d_folio = 0
   LET m_c_programa_cod = "AGRL34"
   LET v_c_op_arch_ent = NULL
   LET v_c_op_arch_sal = NULL
   LET v_ci_op_arch_tran = NULL
   LET m_c_tpo_transf = '43' --43-DSe Anualiades Garantizadas
   ...
   CALL fn_rutas("agr") RETURNING m_c_ruta_bin, r_c_ruta_listados
   ...
   OPEN WINDOW w_reverso WITH FORM "AGRL341"
...
FUNCTION fn_reversa_arch_entrada(p_c_op_arch_ent)
   ...
   OPEN WINDOW w_arch_entrada WITH FORM "AGRL342"
            ...
            ON ACTION ACCEPT
               ...
               IF r_b_valida = 0 THEN
                  ...
                  IF r_b_valida = 0 THEN
                     ...
                     LET v_s_comando = " nohup time fglrun ",m_c_ruta_bin CLIPPED,"/AGRR04 ",
                                                             p_v_usuario, " ",
                                                             v_d_pid, " ",
                                                             m_i_proceso_cod, " ",
                                                             m_i_opera_cod, " ",
...
FUNCTION fn_valida_ext_archivo(p_c_op_arch_ent, p_c_nom_archivo)
   ...
   CASE p_c_op_arch_ent
      WHEN "opt1" -- Devolución de Saldos Excedentes
         LET v_i_proceso_cod = g_proc_cod_agr_dse -- devolución de saldos excedentes agr
         LET v_i_opera_cod = 1 -- reverso dev sdo exc agr
      ...
      WHEN "opt2" -- Recepción Rechazo Devolución Saldos
         LET v_i_proceso_cod = g_proc_cod_agr_rech_dse -- recepción rechazo devolución saldos exc agr
      ...
      WHEN "opt3" -- Recepción Confirmación Saldos
         LET v_i_proceso_cod = g_proc_cod_agr_conf_dse -- recepción confirmación saldos exc agr
      ...
      WHEN "opt4" -- Salida Solicitud de Saldos
         LET v_i_proceso_cod = g_proc_cod_agr_arch_solic_dse -- generación solicitud devolución sdo exc agr
         LET v_i_opera_cod = 1 -- solicitud devolución saldo excedente agr
...
FUNCTION fn_reversa_transacciones(p_c_op_arch_tran)
   ...
   OPEN WINDOW w_arch_transaccion WITH FORM "AGRL344"
            ...
            SELECT COUNT(*)
             ...
             WHERE id_dse_grp_devolucion IN (
                   SELECT id_dse_grp_devolucion
                    ...
                    WHERE folio = v_d_folio -- folio del archivo
               ...
               ELSE
                  ...
                  IF r_b_valida = 0 THEN
                     ...
                     IF r_b_valida = 0 THEN
                        ...
                        LET v_s_comando = " nohup time fglrun ",m_c_ruta_bin CLIPPED,"/AGRR05 ",
                                                                p_v_usuario, " ",
                                                                v_d_pid, " ",
                                                                m_i_proceso_cod, " ",
                                                                m_i_opera_cod, " ",
...
FUNCTION fn_reversa_arch_salida()
   ...
   OPEN WINDOW w_arch_salida WITH FORM "AGRL343"
   -- se deplega el nombre del proceso a reversar en el grupo
   LET v_ui_ventana = ui.Window.getCurrent()
   LET v_ui_forma = v_ui_ventana.getForm()
   CALL v_ui_forma.setElementText("grevent",v_nom_grupo)
            ...
            ON ACTION ACCEPT
               ...
               IF r_b_valida = 0 THEN
                  ...
                  IF r_b_valida = 0 THEN
                     ...
                     LET v_s_comando = " nohup time fglrun ",m_c_ruta_bin CLIPPED,"/AGRR06 ",
                                                             p_v_usuario, " ",
                                                             v_d_pid, " ",
                                                             m_i_proceso_cod, " ",
                                                             m_i_opera_cod, " ",
