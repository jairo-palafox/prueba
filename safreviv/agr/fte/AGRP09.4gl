...
#                    control de archivos correspondiente para el        #
#                    proceso No Atendidas del módulo de Anualidades G.  #
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".AGRP09.log")

   DISPLAY "=INICIA AGRP09="
   ...
   IF v_i_estado_cod = 4 THEN
      ...
      LET v_i_estado = 10
   ELSE
      --DISPLAY "ESTADO INVALIDO EN GLO CTR ARCHIVO"
      LET v_i_estado = 30
   END IF

   -- se invoca la funcion que inserta el registro en control de archivos
   LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_agr_insrt_ctr_arch_noaten(?,?,?)"
