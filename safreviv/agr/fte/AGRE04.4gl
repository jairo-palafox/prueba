...
#                    de Solicitudes para el módulo de Anualidades G.    #
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".AGRE04.log")

   DISPLAY "=INICIA AGRE04="
   ...
   LET v_s_qryTxt = " SELECT f_presentacion, consec_lote\n",
                    "   FROM safre_tmp:tmp_devoluc_enc_agr"
   ...
   --LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_agr_integrar_devoluciones(?,?,?,?)"
   LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_agr_integra_devol_solic(?,?,?,?)"
   ...
   LET v_s_comando = " nohup time fglrun ",v_c_ruta_bin CLIPPED,"/AGRP08 ",
                                           p_v_usuario, " ",
                                           p_d_pid, " ",
                                           p_i_proceso_cod, " ",
                                           p_i_opera_cod, " ",
...
{
   -- se comenta la finalización de la operación ya que está la ejecuta el programa AGRP08
    -- ===========================================================================
    -- ===========================================================================
    -- ===========================================================================
    -- se invoca la finalizacion de la operacion
    CALL fn_actualiza_opera_fin(p_d_pid        ,
                                p_i_proceso_cod,
                                p_i_opera_cod  )
   ...
   IF v_si_resultado <> 0 THEN
      ...
      CALL fn_desplega_inc_operacion(v_si_resultado)
...
#Objetivo: Función que crea la tabla temporal de la integración de devoluciones
FUNCTION fn_crea_tmp_devoluc()
   ...
   DROP TABLE tmp_deudor_devoluc_agr

   WHENEVER ERROR STOP

   CREATE TABLE tmp_deudor_devoluc_agr(id_cre_acreditado DECIMAL(9,0),
      ...
      FOREACH cur_slct_idDer_modCod INTO v_d_id_derechohab, v_c_modulo_cod, v_referencia
         ...
         IF v_c_modulo_cod = "AG" THEN
            ...
            LET v_s_qryTxt = " SELECT folio_liquida\n",
                             ...
                             "  WHERE estado >= 140\n",
                             "    AND edo_procesar = 70\n",
                             "    AND id_cre_acreditado = ",v_referencia
