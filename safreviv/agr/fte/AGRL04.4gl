...
#                    archivo de entrada Rechazo de Saldos AGR      #
#Autor             =>Daniel Buendia, EFP                           #
#Fecha inicio      =>11 Marzo 2012                                 #
####################################################################
...
# Objetivo: Programa que permite la selección de archivos Rechazo de saldos para su correspondiente integración
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".AGRL04.log")
   ...
   LET v_i_proceso_cod = g_proc_cod_agr_rech_saldos -- recepción rechazo de saldos ag
   ...
   OPEN WINDOW w_integra_arch_rech WITH FORM "AGRL021"
...
LET r_b_valida = 1
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               IF r_b_valida = 0 THEN
                  ...
                  LET v_s_comando = " nohup time fglrun ",v_c_ruta_bin CLIPPED,"/AGRE02 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
      ...
      FOREACH cur_id_derechohab INTO v_d_id_derechohab, v_c_modulo_cod
         ...
         IF v_c_modulo_cod = "AG" THEN
            ...
            LET v_s_qryTxt = " SELECT estado\n",
                             ...
                             "    AND estado IN(140,145,218,20,25)\n",
...
FUNCTION f_id_cre_ctr_arh()

   DEFINE v_s_qryTxt        STRING   -- contiene una sentencia sql a ejecutar
   DEFINE v_exe             SMALLINT -- resultado de la ejecución dek qry

   LET v_s_qryTxt = " SELECT id_cre_ctr_archivo\n",
                    "   FROM cre_ctr_archivo\n",
                    "  WHERE operacion NOT IN (1,6,9,14)\n",
                    " INTO TEMP tmp_cre_ctr_archivo"

   PREPARE prp_id_arh FROM v_s_qryTxt
   EXECUTE prp_id_arh INTO v_exe
