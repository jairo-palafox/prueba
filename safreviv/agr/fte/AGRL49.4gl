...
#Modulo            =>AGR                                           #
#Programa          =>AGRL49                                        #
#Objetivo          =>Programa que permite la integración de información  #
#                    del archivo recurrente a la tabla temporal    #
#Autor             =>Esteban Sánchez, EFP                          #
#Fecha inicio      =>26 Septiembre 2013                            #
#Modificación      =>NA                                            #
#Fecha             =>NA                                            #
#                  =>NA                                            #
####################################################################
...
DEFINE v_tipo_proceso    SMALLINT, -- Forma como ejecutara el programa 
   ...
   CALL STARTLOG (v_usurio CLIPPED|| ".DISL49.log")
   
   --Obtiene el nombre del archivo
   LET v_nom_archivo = ""

   DISPLAY "g_proceso_cod ",g_proceso_cod," g_opera_cod", g_opera_cod," v_usurio ",v_usurio
   ...
   CALL fn_drgandrop_reenvioWS()
   
END MAIN

FUNCTION fn_drgandrop_reenvioWS()
DEFINE l_arr_arch_pend    DYNAMIC ARRAY OF VARCHAR(100), -- Archivos pendientes 
       ...
       l_arr_integrados   DYNAMIC ARRAY OF RECORD -- Detalle archivos a integrar
            folio              LIKE cta_marca_ws.folio_archivo,
            nom_archivo        VARCHAR(100),
            f_presenta         VARCHAR(10),
            tot_reg            INTEGER 
       END RECORD,
   ...
   CLOSE WINDOW SCREEN 
   OPEN WINDOW w_inte_reenvio WITH FORM "AGRL491"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION accept
         ...
         FOR l_i_iter = 1 TO l_i_num_arch
            ...
            LET l_arr_integrados[l_i_iter].f_presenta = TODAY USING "dd-mm-yyyy"
     ...
     {ON ACTION deshacer
      ...
      ON ACTION integrar
         ...
         IF r_bandera = 0 THEN  
            ...
            CALL fn_actualiza_opera_ini(v_max_pid,g_proceso_cod, g_opera_cod,g_folio,"AGRE14",l_v_arch_proceso,v_usurio)
            RETURNING r_b_valida
                                                
            IF r_b_valida = 0 THEN
            
               LET l_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/AGRE14.42r ",
                                v_usurio," ",
                                v_max_pid," ",
                                g_proceso_cod," ",
                                g_opera_cod," ",
            ...
            END IF
         ELSE
            -- Enva mensaje con la descripciOn del Error
            CALL fn_muestra_inc_operacion(r_bandera)
         END IF
         EXIT DIALOG

   END DIALOG
CLOSE WINDOW w_inte_reenvio
END FUNCTION
