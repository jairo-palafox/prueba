...
OPEN WINDOW v_consulta WITH FORM "DPEC130"
   DIALOG   ATTRIBUTES(UNBUFFERED)
      ...
      ON ACTION accept
         ...
         ON ACTION reporte
         DISPLAY "Reporte Conciliación "
            --IF v_folio IS NULL THEN
            --   CALL fn_mensaje("Atención","Debe seleccionar un folio para generar el reporte","stop")
            --ELSE
               CALL fn_dpe_ejecuta_generacion_archivo(v_folio, v_estado, v_relacion, g_usuario_cod)
            --END IF 
   END DIALOG 
...
FUNCTION fn_dpe_ejecuta_generacion_archivo(p_folio, p_estado, p_relacion, p_usuario_cod)
DEFINE p_folio          DECIMAL(9,0), 
       p_estado         SMALLINT, 
       p_relacion       SMALLINT, 
   ...
   -- se verifica si se puede continuar con la operacion
          LET v_s_comando = " fglrun ",g_reg_modulo.ruta_exp CLIPPED,"/DPES07 ",
                             p_usuario_cod, " ",
                             g_pid  , " " ,
                             g_proceso_cod , " " ,
                             g_opera_cod ," ",
