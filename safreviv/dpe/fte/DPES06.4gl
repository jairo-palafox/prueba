...
DEFINE p_pid            LIKE bat_ctr_operacion.pid, -- PID del proceso
   ...
   CALL fn_archivo_salida_saci_procesar(p_usuario_cod, p_folio)
   
END MAIN

#Objetivo: Genera la estructura del archivo de salida con las diferencias 
FUNCTION fn_archivo_salida_saci_procesar(p_usuario_cod, p_folio)
...
DEFINE v_v_nom_archivo     CHAR(40),     -- nombre del archivo de salida
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".DPES06.log")
...
DISPLAY "------------ E M P I E Z A   L A    G E N E R A C I Ó N    D E    A R C H I V O ----------"
   ...
   LET v_v_nom_archivo = "RPT_SACI_VS_PROCESAR_"||v_fecha_dia||"_"||p_folio||".txt"

   LET v_v_ruta_nomarch = g_reg_modulo.ruta_listados CLIPPED||"/"||v_v_nom_archivo
...
------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   PREPARE prp_cons_detalles FROM v_QryTxt
   DECLARE cur_cons_detalles CURSOR FOR prp_cons_detalles  
   
   LET v_s_tot_registros = 0

   FOREACH cur_cons_detalles INTO rec_detalles.v_PROCESAR        ,
...
------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   PREPARE prp_cons_detalles_sin_relacion FROM v_QryTxt
   DECLARE cur_cons_detalles_sin_relacion CURSOR FOR prp_cons_detalles_sin_relacion  
   ...
   ELSE
      ...
      CALL fn_mensaje("Atención","Se ha generado el reporte, verificar la ruta","info" )
      CALL fn_mensaje("Atención",v_v_ruta_nomarch,"info" )
   END IF
