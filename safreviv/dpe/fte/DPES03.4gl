...
#OBJETIVO: Generar el archivo de salida para DPE
...
DEFINE v_tot_sumario INTEGER,
   ...
   DEFINE v_id_dpe_referencia_trab    DECIMAL(18,6)
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".DPES03.log")
...
------   ------------------
         ...
         SELECT a.id_dpe_referencia,
                "02",               ---- tipo_reg
                a.reg_patronal_imss,   ---- num_reg_pat_imss       
                a.rfc_patron,          ---- rfc_patron
                a.periodo_pago,        ---- per_pago
                a.folio_sua,           ---- folio_sua
                a.razon_social,        ---- nombre_razon_soc        
                a.numero_solicitud,    ---- num_solicitud           
                a.tipo_cotizacion,     ---- tipo_cotizacion         
                (select count(*) from dpe_sol_trabajador where id_dpe_patron = a.id_dpe_referencia) num_traba_sol,    ---- num_trab_sol    ,        
                a.f_pago,              ---- fec_pago        
                --(select b.f_actualiza from glo_folio b where b.folio_referencia = a.folio ) fec_valor_viv,    ---- fec_valor_viv 	,        
                a.f_valor_viv,
                fn_habil_siguiente(a.f_valor_viv + 1,1) fecha_rcv,      ---- fec_valor_rcv        
                "01",                ---- resultado_op         
         ...
         FOREACH cur_dpe_patron 
            ...
            SELECT a.id_dpe_referencia,      -- referencia del trabajador
                   "03",                     -- tipo_reg,
                   "03",                     -- id_servicio,
                   a.reg_patronal_imss,      -- num_reg_pat_imss,
                   b.rfc_patron,             -- rfc_patron,
                   a.periodo_pago,           -- per_pago,
                   b.folio_sua,              -- folio_sua,
                   a.nss,                    -- nss_aportacion,
                   a.rfc,                    -- rfc_trabajador,
                   a.curp,                   -- curp,
                   a.imp_retiro_dev,         -- imp_ret_devolver,
                   a.imp_act_retiro_dev,     -- imp_act_recargos,
                   a.imp_cv_pat_dev,         -- imp_ces_vejez,
                   a.imp_cv_trab_dev,        -- imp_act_ces_vejez,
                   "           ",            -- v_nss_unificador
                   a.imp_act_cv_dev,         -- imp_plusv_retiro,
                   a.imp_act_retiro_dev,     -- imp_minus_retiro,
                   a.imp_cv_pat_dev,         -- imp_comisiones_afore,
                   a.imp_cv_trab_dev,        -- imp_total_apo_patronal,
                   a.imp_act_cv_dev,         -- imp_plusv_apo_patronal,
                   "000",                    -- clave_afore_trabajador,
                   a.avis_viv_dev,           -- num_aplicaciones_inter,
                   a.resul_op,                -- se cambia estado_solicitud por resul_op
                   a.diagnostico,            -- diagnostico1,
                   "000",                    -- diagnostico2,
                   "000",                    -- diagnostico3,
                   " "                       -- filler
            FROM   dpe_sol_trabajador a, dpe_patron b
            WHERE  a.folio = p_folio
            AND    a.id_dpe_patron = v_id_dpe_referencia
            AND    b.folio = a.folio
            AND    b.id_dpe_referencia = a.id_dpe_patron

            FOREACH cur_dpe_sol_trabajador_aceptadas 
               INTO v_id_dpe_referencia_trab,
                    v_r_dpe_sol_trabajador.v_tipo_reg,
               ...
               --Si la operacion se rechaza, cambia diagnostico  
               IF  v_r_dpe_sol_trabajador.v_resultado_operacion = 2 THEN
                   LET v_r_dpe_sol_trabajador.v_diagnostico1 = 382
               END IF

               IF  v_r_dpe_sol_trabajador.v_resultado_operacion IS NULL THEN
                  LET v_r_dpe_sol_trabajador.v_resultado_operacion = 0
               END IF
               ...
               IF  v_r_dpe_sol_trabajador.v_resultado_operacion = 1 AND 
                   ...
                   SELECT aivs_viv_dev
                   INTO   v_r_dpe_sol_trabajador.v_num_aplicaciones_inter
                   FROM   dpe_resp_procesar
                   WHERE  id_dpe_referencia  = v_id_dpe_referencia_trab 
                   AND    folio = p_folio
               END IF  
            ...
            SELECT reg_patronal_imss,  -- num_reg_pat_imss,	      
                   rfc,                -- rfc_patron,	            
                   periodo_pago,       -- per_pago,	              
                   folio_sua,          -- folio_sua,	              
             ...
             WHERE folio = p_folio
               ...
               AND reg_patronal_imss = v_r_dpe_patron.num_reg_pat_imss
            ...
            FOREACH cur_dpe_sum_patron 
               ...
               WHERE  id_dpe_patron = v_id_dpe_referencia
               ...
               SELECT SUM(aivs_viv_dev)
               INTO   v_r_dpe_sum_patron.v_num_aplicaciones_inter
               FROM   dpe_resp_procesar
               WHERE  id_dpe_referencia in (SELECT a.id_dpe_referencia 
                                            FROM dpe_sol_trabajador a
                                            WHERE a.id_dpe_patron = v_id_dpe_referencia
                                            AND estado_solicitud = 1)
