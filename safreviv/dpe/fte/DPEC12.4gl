...
#                por pagos indebidos solo INFONAVIT                                     #
...
--   CALL ui.interface.loadstyles("../../mdt/bin/mdtstyle")
   ...
   CALL fn_genera_consulta_integra_INFONAVIT(p_usuario_cod)

END MAIN       

{ ======================================================================
Clave: DPEC08
Nombre: fn_genera_consulta_integra_INFONAVIT
Fecha creacion: 04/05/2012
  Genera consulta de datos integrados DPE solo INFONAVIT
======================================================================
}

FUNCTION fn_genera_consulta_integra_INFONAVIT(p_usuario_cod)
DEFINE p_usuario_cod       LIKE seg_usuario.usuario_cod, -- clave del usuario
   ...
   OPEN WINDOW win_ConsInfonavit WITH FORM "DPEC120"
   -- Recupera punteros a ventana para control de grupos
      ...
      WHERE g.proceso_cod = g_proceso_cod_dpe_infonavit
      ...
      ON ACTION ACCEPT
         ...
         CALL fn_muestra_consulta_integra_INFONAVIT(p_usuario_cod, v_folio, 
...
===========================================================================
Clave: DPEC08
Nombre: fn_muestra_consulta_integra_INFONAVIT
Fecha creacion: 04/05/2012
Narrativa del proceso que realiza:
"CONSULTA DE DIAGNOSTICOS DE DEVOLUCION DE PAGOS EN EXCESO SOLO INFONAVIT"
==========================================================================
}

FUNCTION fn_muestra_consulta_integra_INFONAVIT(p_usuario_cod, p_folio, 
   ...
   DIALOG ATTRIBUTES (UNBUFFERED)
      ...
      ON ACTION reporte
         ...
         # Se indica que el reporte usara la plantilla creada
           IF fgl_report_loadCurrentSettings("DPEC12.4rp") THEN
              CALL fgl_report_selectDevice("PDF")
              LET v_nom_reporte = p_usuario_cod CLIPPED || "-DPEC12-","00000","-","00000","-","00000"||".pdf"
              ...
              CALL fn_reporte_integracion_INFONAVIT(p_usuario_cod, p_folio, 
                                                    p_nss, p_estado_solicitud, 
                                                    p_credito, p_f_gen_archivo)
...
DEFINE p_folio                   LIKE glo_folio.folio,
   ...
   -- Llenado de los totales de la solicitud
   --LET v_s_sql = "SELECT SUM(a.aportacion_reg), SUM(a.amortizacion_reg),",
   --              "\n     SUM(a.aportacion_sol), SUM(a.amortizacion_sol),",
   --              "\n      SUM(CASE a.estado_solicitud WHEN 1 THEN 1 ELSE 0 END),",
   --              "\n      SUM(CASE a.estado_solicitud WHEN 2 THEN 1 ELSE 0 END)",
   --              "\n FROM dpe_sol_soloinfonavit a, afi_derechohabiente b,",
   --              "\n      dpe_soloinfonavit_arch c",
...
======================================================================
Clave: DPEC08
Nombre: fn_reporte_integracion_INFONAVIT
Fecha creacion: 07/05/2012
Narrativa del proceso que realiza:
...
FUNCTION fn_reporte_integracion_INFONAVIT(p_usuario_cod, p_folio, 
                                          p_nss, p_estado_solicitud, 
                                          p_credito, p_f_gen_archivo)
...
DEFINE p_usuario_cod       LIKE seg_usuario.usuario_cod, -- clave del usuario
   ...
   START REPORT reporte_diagnostico_INFONAVIT TO XML HANDLER v_manejador_rpt
   FOREACH CuRp_ObtResumen_solicitudes INTO v_r_rep_solicitudes.*
      ...
      -- Se llena el reporte con la información recuperada
         OUTPUT TO REPORT reporte_diagnostico_INFONAVIT(p_usuario_cod, p_folio, 
                                                        p_nss, cadena_estado_solicitud, 
                                                        p_credito, p_f_gen_archivo,
                                                        v_r_rep_solicitudes.*,
                                                        v_r_rep_aceptadas.*,
                                                        v_r_rep_rechazadas.*,
                                                        v_r_rep_total.*)
   END FOREACH
   FINISH REPORT reporte_diagnostico_INFONAVIT
   FREE CuRp_ObtResumen_solicitudes
   RETURN v_si_indice
END FUNCTION

REPORT reporte_diagnostico_INFONAVIT(p_usuario_cod, p_folio, p_nss, p_cadena_solicitud, 
                                     p_credito, p_f_gen_archivo, v_r_rep_solicitudes,
                                     v_r_rep_aceptadas, v_r_rep_rechazadas, v_r_rep_total)
