...
#Modulo       => UNI                                                           #
#Programa     => UNIL73                                                        #
#Objetivo     => Lanzador de la integración Unificación Complementaria         #
...
DEFINE p_usuario    LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
   ...
   CALL fn_drgandrop_integra_uni_cuenta(p_usuario)
END MAIN

FUNCTION fn_drgandrop_integra_uni_cuenta(p_usuario)
DEFINE p_usuario LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
   ...
   OPEN WINDOW w_inte_devol WITH FORM "UNIL440"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION ACCEPT
         ...
         FOR l_i_iter = 1 TO l_i_num_arch
            ...
            -- calcula total de solicitudes
             ...
             PREPARE pre_tot_familias FROM v_s_qry
             DECLARE cur_tot_familias CURSOR FOR pre_tot_familias 

             LET v_tot_familias = 0
             FOREACH cur_tot_familias INTO rec_tot_regs.nss_unificador
                LET v_tot_familias = v_tot_familias + 1
             END FOREACH

             SELECT COUNT (v_nss_unificado)
             INTO   v_tot_ctas_unificadas
             FROM   safre_tmp:tmp_uni_det_compl

            --se asigna el total de solicitudes
            LET l_arr_integrados[l_i_iter].tot_familias =  v_tot_familias  
            LET l_arr_integrados[l_i_iter].total_unificadores = v_tot_familias
      ...
      ON ACTION integrar
         ...
         CALL fn_ventana_confirma("Atención",
              ...
              " Unificación Complementaria?",
              "quest") RETURNING v_respuesta

        IF v_respuesta = 1 THEN
           CALL fn_uni_ejecuta_integracion(p_usuario, l_v_arch_proceso)
           EXIT DIALOG
        END IF
...
END FUNCTION -- fn_drgandrop_integra_uni_cuenta

#OBJETIVO: Ejecutar el programa lanzado de la integración UNI Recurrente
FUNCTION fn_uni_ejecuta_integracion(p_usuario, p_nombre_archivo)
DEFINE p_usuario      LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
   ...
   IF ( r_bnd_valida_opera = 0 ) THEN
      -- Inicio operacion.
      CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,g_opera_cod,0,"UNIL73","",p_usuario)
      RETURNING r_bnd_opera_ini
      
      IF (r_bnd_opera_ini = 0) THEN	
         LET v_s_comando = " nohup time fglrun ",g_reg_modulo.ruta_bin CLIPPED,"/UNIP23 ",
                            p_usuario, " ",
                            g_pid  , " " ,
                            g_proceso_cod , " " ,
                            g_opera_cod ," ",
