...
#Modulo       => UNI                                                           #
#Programa     => UNIL64                                                        #
#Objetivo     => Lanzador indicador de crédito y desmarca                      #
...
MAIN 
   ...
   OPEN WINDOW w_folio_indicadores WITH FORM "UNIL640"
...
FUNCTION fn_muestra_resumen_indicador(p_folio)
   DEFINE p_folio                         DECIMAL(9,0),
          ...
          v_total_desmarca_unificado      INTEGER,
          v_g_total_desmarca_unificador   INTEGER,
          v_g_total_desmarca_unificado    INTEGER,
          v_i_res                         INTEGER,
          v_f_txt                         STRING,
          r_tot_registros                 INTEGER,
          r_g_tot_registros               INTEGER    
   ...
   DECLARE cur_det_resumen CURSOR FOR SELECT folio_unificacion
                                      FROM   uni_det_unificador
                                      WHERE  folio_liquidacion = p_folio
                                      GROUP BY 1
   FOREACH cur_det_resumen INTO   v_folio_unificacion
      ...
      EXECUTE prp_res_indicadores USING v_folio_unificacion, 
       ...
       LET r_g_tot_registros = r_g_tot_registros +  r_tot_registros;
       LET v_g_total_desmarca_unificador = v_g_total_desmarca_unificador + v_total_desmarca_unificador;  
       LET v_g_total_desmarca_unificado  = v_g_total_desmarca_unificado  + v_total_desmarca_unificado;
   END FOREACH 
   ...
   DISPLAY v_g_total_desmarca_unificador,
           v_g_total_desmarca_unificado
        TO txt_tot_des_dor,
           txt_tot_des_do
   
   RETURN v_g_total_desmarca_unificador,
          v_g_total_desmarca_unificado,
          v_folio_unificacion
END FUNCTION
#OBJETIVO: Ejecutar el lanzado de desmarca y herencia de crédito.
################################################################################
FUNCTION fn_uni_ejecuta_indicadores_IMSS(p_folio_unificacion,
   ...
   IF (r_bnd_fin_oper = 0) THEN
      ...
      LET v_s_comando = " nohup time fglrun ",g_reg_modulo.ruta_exp CLIPPED,"/UNIP22 ",
                          p_usuario_cod, " ",
                          g_pid  , " " ,
                          g_proceso_cod , " " ,
                          g_opera_cod ," ",
