...
#Modulo            =>UNI                                                       #
#Programa          =>UNIS10                                                    #
#Objetivo          =>Programa que ejecuta el extractor de aceptados para TRM   #
#Fecha inicio      => 06/06/2012                                               #
################################################################################
--==============================================================================
-- Version: 1.0.0
-- Fecha ultima modificacion: 04/Nov/2015 --Se agrega ind_procedencia
--==============================================================================
...
MAIN
   ...
   DISPLAY " Parámetros de Consulta para Generar Archivo de Aceptados "
   DISPLAY " " 
   ...
   CALL fn_archivo_salida_trm(p_usuario_cod,
                              p_folio,
                              p_fecha_ini,
                              p_fecha_fin)
...
FUNCTION fn_archivo_salida_trm(p_usuario_cod,
                               p_folio,
                               p_fecha_ini,
                               p_fecha_fin)
   ...
   DEFINE v_r_det_unificador  RECORD
          ...
          v_rec_ind_procedencia    SMALLINT,
          v_rec_nss_unificado      CHAR(11),
          id_derechohabiente_cta1  DECIMAL(9,0)
   END RECORD
       
   -- se crear el archivo log
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".UNIS10.log")
   ...
   LET v_busca_archivo = "UNI_CTRM"||v_d_hoy 
   ...
   AND     v_fecha_fin
   AND     folio <> 42406
   INTO TEMP tmp_folios_unificacion
   
   LET v_s_sql = "\n SELECT a.*, c.nsscta1, c.id_derechohabiente ",   
   ...
   FOREACH cur_registros_unificacion INTO v_r_det_unificador.*
      ...
      END IF
         ...
         WHERE  id_derechohabiente = v_r_det_unificador.id_derechohabiente_cta1
         AND    marca = 150
         
         IF v_marca_cont = 0 THEN
            LET v_marca_cont = 0
