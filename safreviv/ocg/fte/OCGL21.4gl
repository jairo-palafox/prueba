...
MAIN 
   ...
   CALL STARTLOG(g_usuario CLIPPED || ".OCGL21.log")
   ...
   CALL arh_saldos_cofi()
   
END MAIN 

FUNCTION arh_saldos_cofi()
...
OPEN WINDOW vtn1 WITH FORM "OCGL211"
   ...
   MENU ""
      ...
      ON ACTION ACCEPT
         LET v_respuesta = fn_ventana_confirma("","¿Está seguro de generar el archivo de saldos de créditos vigentes cofinavit?","")
         ...
         ELSE
            ...
            ELSE 
               -- Inicializa proceso
               CALL fn_inicializa_proceso(v_pid,g_proceso_cod,g_opera_cod,"","OCGP23","",g_usuario) RETURNING v_valida_operacion

               -- Actualiza operación como inicio
               CALL fn_actualiza_opera_ini(v_pid,g_proceso_cod,g_opera_cod,"","OCGP23","",g_usuario) RETURNING v_valida_operacion
               ...
               LET v_notifica = "Se ejecutó la generación del archivo de saldos cofinavit"," \n",
