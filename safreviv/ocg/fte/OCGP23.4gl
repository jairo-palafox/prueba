...
#Modulo            => OCG                                                     #
#Programa          => OCGP                                                    #
#Objetivo          => Lnzado archivo de saldos de créditos #
#                     Cofinavit.                                              #
#Autor             => Emilio Abarca EFP                                       #
#Fecha inicio      => 27/Marzo/2017                                           #
###############################################################################
...
MAIN
   ...
   CALL STARTLOG(p_usuario CLIPPED|| ".OCGP23.log")
   ...
   CALL fn_display_proceso(0,"INICIA GENERACIÓN ARCHIVO DE SALDOS COFINAVIT") 
   ...
   ELSE 
     DISPLAY "" 
     DISPLAY " => Se ha generado el archivo de saldos cofinavit con nombre : SDO43VIG_COFI.txt "
     DISPLAY " => El archivo de encuentra en la ruta /safreviv_int/ocg/envio "
     DISPLAY " => El respaldo del archivo SDO43VIG_COFI.txt se ha realizado correctamente "
     DISPLAY " "

     DISPLAY " PROCESO EJECUTADO CORRECTAMENTE"
     
     CALL fn_display_proceso(1,"FIN GENERACIÓN ARCHIVO DE SALDOS COFINAVIT") 
...
FUNCTION genera_arh_saldos()
   ...
   DEFINE arr_saldos_cofi DYNAMIC ARRAY OF RECORD
      nss                 CHAR(11) ,
   ...
   LET v_arh_salida   = v_ruta_envio CLIPPED,"/SDO43VIG_COFI.txt"
   LET v_arh_sal_resp = v_ruta_envio CLIPPED,"/SDO43VIG_COFI_" CLIPPED, TODAY USING "ddmmyyyy" CLIPPED,".txt"  
   ...
   CALL arr_saldos_cofi.clear()
   
   FOREACH crsr_consulta INTO arr_saldos_cofi[k].nss,
                              arr_saldos_cofi[k].id_derechohabiente,
                              arr_saldos_cofi[k].cve_ent_financiera,
                              arr_saldos_cofi[k].num_ctr_int_ef
   ...
   FOR i = 1 TO arr_saldos_cofi.getLength()
      
         IF(arr_saldos_cofi[i].id_derechohabiente IS NOT NULL) THEN
   
            -- Obtiene precio monto pesos
            SELECT ROUND(SUM(monto_acciones) * v_precio_fondo,2)
               INTO v_monto_pesos
               FROM cta_movimiento
              WHERE id_derechohabiente = arr_saldos_cofi[i].id_derechohabiente
                AND subcuenta       = 4
            ...
            LET v_detalle = arr_saldos_cofi[i].cve_ent_financiera USING "&&&","|",
                            arr_saldos_cofi[i].nss,"|",
                            arr_saldos_cofi[i].num_ctr_int_ef,"|", 
                            v_monto_pesos,"|",
