...
#Modulo            => OCG                                                #
#Programa          => OCGP16                                             #
#Objetivo          => Programa que republica un crédito no reconocido    #
#                     por la Entidad Financiera.                         #
#Autor             => Emilio Abarca Sánchez, EFP                         #
#Fecha inicio      => 24 OCTUBRE 2016                                    #
##########################################################################
...
MAIN
   ...
   CALL STARTLOG(g_usuario CLIPPED|| ".OCGP16.log")
...
FUNCTION ocg_principal()
   ...
   OPEN WINDOW vtn1 WITH FORM "OCGP161"
            ...
            ELSE
               ...
               ELSE 
                  ...
                  ELSE 
                     ...
                     --Se verifica que sea un crédito no reconocido por la EF
                     SELECT COUNT(*)
                        INTO v_exist_tramite
                        FROM ocg_tramite
                       WHERE id_derechohabiente = v_id_derechohabiente
                         AND diagnostico = 1      --Valido
                         AND estado      = 70     --Aceptado
                         AND situacion   = 25     --Crédito no reconocido
                         AND tpo_credito IN (7,8); #Tipo de credito Cofinavit

                     IF(v_exist_tramite = 0) THEN
                        CALL fn_mensaje("Información","No se ha encontrado ningun crédito no reconocido para este trabajador","about")
                     ELSE
                        --Carga información de dh 
...
FUNCTION carga_inf_dh_ocg(p_nss,p_id_derechohabiente)
   ...
   OPEN WINDOW vtn2 WITH FORM "OCGP162"
      ...
      SELECT tmt.id_ocg_tramite,
       ...
       WHERE id_derechohabiente     = p_id_derechohabiente
         ...
         AND tmt.diagnostico     = 1   # válido
         AND tmt.estado          = 70  # Aceptado
         AND tmt.situacion       = 25  # No reconocido
         AND tmt.tpo_credito IN (7,8); # Crédito cofinavit 
      ...
      INPUT BY NAME nva_ent_financiera ATTRIBUTE(UNBUFFERED, ACCEPT = FALSE, CANCEL = FALSE )
         ...
         ON ACTION Republicar
            IF(nva_ent_financiera IS NULL) THEN 
               ...
               CONTINUE INPUT 
            ELSE 
               LET v_answer = fn_ventana_confirma("Información","¿Esta seguro de republicar este crédito no reconocido?","about")
               IF(v_answer = 0) THEN
               ...
               ELSE 
                  ...
                  INSERT INTO ocg_his_tramite
                             ...
                             VALUES(v_id_ocg_tramite,
                                    v_cve_ef,  --EF Original
                                    p_id_derechohabiente,
                                    1, --------|
                                    v_estado,
                                    25,--------|Indicadores que fue un crédito no reconocido
                                    TODAY,
                                    g_usuario);
                  ...
                  UPDATE ocg_detalle
                   ...
                   WHERE id_ocg_detalle = v_id_ocg_detalle --id de ocg_tramite
                    AND  id_derechohabiente = p_id_derechohabiente
                    AND  subproceso = 1
                    AND  nss =  p_nss;
