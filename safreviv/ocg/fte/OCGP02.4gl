...
MAIN
   ...
   DEFINE v_sb2_aceptados       INTEGER 
   DEFINE v_sb2_rechazados      INTEGER
   DEFINE v_estado              SMALLINT
   DEFINE v_tot_rch_sp2_val     INTEGER
   ...
   LET v_proceso_cod     = 3903
   LET v_opera_cod       = 1
   LET v_programa_cod    = "OCGP02"
   ...
   LET v_sb2_aceptados   = 0
   LET v_sb2_rechazados  = 0
   LET v_estado          = 0 
   
   -- se crea el archivo log
   CALL STARTLOG(p_usuario CLIPPED|| ".OCGP02.log")

   DISPLAY "=INICIA OCGP02="
   ...
   LET v_s_qry = "EXECUTE FUNCTION fn_ocg_valida_subpr2_formalizacion(?)"

   PREPARE prp_exe_fn_proceso FROM v_s_qry
   EXECUTE prp_exe_fn_proceso USING v_id_ocg_ctr_arch 
                               INTO v_sp_error,v_cnt_aceptados, v_cnt_rechazados 

   -- Recupera registros rechazados    
   SELECT COUNT(*)
      INTO v_sb2_rechazados
      FROM ocg_formalizacion fz,
           ocg_detalle dt
     WHERE fz.id_ocg_detalle = dt.id_ocg_detalle
       AND fz.diagnostico  = 02
       AND dt.id_ocg_ctr_archivo = v_id_ocg_ctr_arch
       AND dt.subproceso   = 2
       AND dt.f_proceso    = TODAY;
       --AND fz.tpo_credito IN (7,8,"A","C",NULL)
       
   -- Recupera registros aceptados
   SELECT COUNT(*)
     INTO v_sb2_aceptados
     FROM ocg_formalizacion fz,
          ocg_detalle dt
    WHERE fz.id_ocg_detalle = dt.id_ocg_detalle
      AND fz.diagnostico = 01
      AND dt.id_ocg_ctr_archivo = v_id_ocg_ctr_arch
      AND dt.subproceso  = 2
      AND dt.f_proceso   = TODAY;

   --Rechazos SP2 en validación
   SELECT tot_sp2_rch_val
     INTO v_tot_rch_sp2_val
     FROM safre_tmp:tmp_ocg_cifras;
   
   DISPLAY "\n=> TOTAL ACEPTADOS:",v_sb2_aceptados    
   DISPLAY "=> TOTAL RECHAZOS OPERATIVOS:",v_sb2_rechazados
   DISPLAY "=> TOTAL RECHAZOS VALIDACIÓN:",v_tot_rch_sp2_val
   
   -- verifica si ocurrió un error durante el proceos de marcaje
   IF v_sp_error <> 0 THEN
      -- Ocurrió un error, se muestra el error
      DISPLAY "OCURRIÓ UN ERROR EN EL PROCESO DE FORMALIZACIÓN: ",v_sp_error
