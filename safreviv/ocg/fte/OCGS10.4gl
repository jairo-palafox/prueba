         ...
         IF v_cadena IS NOT NULL OR
            ...
            FOREACH cur_transaccion INTO arr_transaccion[a].*
               ...
               IF arr_transaccion[a].concepto = "407" OR
                  ...
                  LET arr_transaccion[a].concepto = "Uso de Garantía"
               ELSE
                  IF arr_transaccion[a].concepto = "508" THEN
                     LET arr_transaccion[a].concepto = "Devolución AS"
                  ELSE
                     IF arr_transaccion[a].concepto = "608" THEN
                        LET arr_transaccion[a].concepto = "Devolución UG"
                     ELSE
                        LET arr_transaccion[a].concepto = "Aportación Subsecuente"
                     END IF
                  END IF
               END IF
...
FUNCTION fn_archivo_full()
   ...
   LET v_qry_t = "
      ...
      UPDATE tmp_det_ex
         SET desc_concepto_ocg = "Aportación Subsecuente"
       WHERE concepto IN (807,107,307,817,117,317)

      UPDATE tmp_det_ex
         SET desc_concepto_ocg = "Uso Garantía"
       WHERE concepto IN (407,417)

      UPDATE tmp_det_ex
         SET desc_concepto_ocg = "Devolución AS"
       WHERE concepto IN (508)

      UPDATE tmp_det_ex
         SET desc_concepto_ocg = "Devolución UG"
