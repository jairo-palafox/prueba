...
#                     de archivos de Recepcion de acreditados migración       #
###############################################################################
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".OCGE93.log")

   DISPLAY "=INICIA OCGE93="
   ...
   ELSE
      ...
      LET v_s_qryTxt = "EXECUTE FUNCTION fn_insrt_ctr_arch_mig_acred(?,?,?,?)"
      ...
      IF r_b_estatus_proc <> 0 OR v_i_estado = 30 THEN
         ...
         DISPLAY " RETORNO DE LA ACTUALIZACIÓN: " ,r_b_valida
      ELSE
         DISPLAY " TOTAL DE REGISTROS ACREDITADOS: ",v_i_tot_reg_det
