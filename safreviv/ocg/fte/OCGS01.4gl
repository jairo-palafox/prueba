...
--******************************************************************************
-- modificación para trasnferncia de archivo 24/04/2019

   -- Ejecución del Script de envío del archivo a TRM
   LET v_env_comando = "sh"," ","/opt/Interpel/Scripts/SOA18068-Prod.sh" #Envío de Producción
   --LET v_env_comando = "sh"," ","/opt/Interpel/Scripts/SOA18068-QA.sh" #Envío QA 
...
FUNCTION fn_archivo_43bis()
   ...
   LET v_qry_garantia_2 = "SELECT ocg.id_ocg_solicitud_ug,
      ...
      -- cadena para archivo de salida con detalle de pagos de SP003 usos de Garantía en tabla de transacción
        {
   ...
   --in (40,60) condición para estado
   ...
   -- cadena para archivo de salida con detalle para registros rechazados en validación
...
--******************************************************************************
   ...
   IF v_cta_l > 0 THEN
      -- se llena arreglo con datos de tabla liquidación
      --LET a = a +1
      ...
      FOREACH cur_liquidacion INTO arr_trabajador[a].id_ocg,
         ...
         END IF
-- se actualiza la situación a 158 13/03/2017
