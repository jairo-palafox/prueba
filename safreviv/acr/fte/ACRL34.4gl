...
#                     archivo de entrada Rechazo Devoluciones       #
#                     Saldos Excedentes                             #
...
# Objetivo: Programa que permite la selección del archivo Rechazo Devolución
...
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".ACRL34.log")
   ...
   LET v_i_proceso_cod   = g_proc_cod_acr_rech_dse -- recepción rechazo devoluciÓn saldos exc acr
   LET v_i_opera_cod     = 2 -- integra rechazos devolución sdo exc acr
   LET v_i_opera_cod_ant = 1 -- Valida Rechazo Saldos
   LET v_c_tpo_transf    = "56" -- Tipo de transferencia (56-Rechazo de saldos)
   LET v_d_folio         = 0
   LET v_c_programa_cod  = "ACRE34"
   ...
   OPEN WINDOW w_integra_arch_rechdse WITH FORM "ACRL321"
         ...
         ON ACTION integrar
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               IF r_b_valida = 0 THEN
                  ...
                  LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/ACRE17 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
...
FUNCTION f_valida_arch_integra()
   ...
   SELECT COUNT(*)
     INTO v_i_cont_regs
     FROM safre_tmp:tmp_rch_devolucion c
    WHERE c.nss NOT IN(
       SELECT t.nss
         FROM safre_tmp:tmp_ctr_devolucion t
         INNER JOIN safre_tmp:tmp_rch_devolucion s
         ON t.nss = s.nss)
