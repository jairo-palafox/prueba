...
-- Modificado el 24-09-2015 José Eduardo Ventura
--===============================================================
...
#                   Confirmación Devolución Saldos que fue validado#
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".ACRE18.log")

   DISPLAY "=INICIA ACRE18="
   ...
   LET v_s_qryTxt = "EXECUTE PROCEDURE fn_integra_conf_dev_sdos(?,?,?,?)"
   ...
   DISPLAY "TOTAL RECHAZADOS: ",v_r_dse_ctr_arch.tot_rechazados
{
   ...
   LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/ACRP30 ",
                                           p_v_usuario, " ",
                                           p_d_pid, " ",
                                           p_i_proceso_cod, " ",
                                           p_i_opera_cod, " ",
   ...
   RUN v_s_comando
{
   ...
   LET v_v_nom_reporte = p_v_usuario CLIPPED, "-ACRL36-", p_d_pid USING "&&&&&", "-", p_i_proceso_cod USING "&&&&&", "-", p_i_opera_cod USING "&&&&&"
   ...
   IF NOT r_b_existe_err THEN
      ...
      LET v_s_titulo_correo = "Proceso: RECEPCIÓN CONFIRMACIÓN SALDOS EXC ACR"
      ...
      LET v_s_mens_correo =  "ID Proceso   : ",p_d_pid,"\n",
                             "Proceso      : RECEPCIÓN CONFIRMACIÓN SALDOS EXC ACR\n",
...
FUNCTION f_genera_rpt_IntegraConfDSE(p_c_ruta_listados, p_v_nom_reporte)
   ...
   IF fgl_report_loadCurrentSettings("ACRE181.4rp") THEN
...
FUNCTION fn_crea_tmp_conf_devol_saldo()
   ...
   DROP TABLE tmp_deudor_conf_devol_saldo

   -- al encontrar un error detiene el programa
   WHENEVER ERROR STOP

   CREATE TABLE tmp_deudor_conf_devol_saldo(id_dse_devolucion  DECIMAL(9,0),
