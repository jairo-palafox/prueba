...
#                    transacciones (preliquidacion, liquidacion y  #
#                    saldos remanentes)                            #
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".ACRR11.log")

   DISPLAY "=INICIA ACRR11="
   ...
   CASE p_c_op_arch_tran
      WHEN "opt8" -- Preliquidación
         CALL fn_reversa_preliquidacion_acr(p_d_folio, p_i_tpo_originacion) RETURNING r_codigo_sql

      WHEN "opt9" -- Liquidación
         CALL fn_reversa_liquidacion_acr(p_d_folio, p_i_tpo_originacion, p_v_usuario, p_i_proceso_cod) RETURNING r_codigo_sql

      WHEN "opt10" -- Saldos Remanentes
         CALL fn_reversa_saldos_remanentes_acr(p_d_folio, p_i_tpo_originacion) RETURNING r_codigo_sql
...
#Objetivo: Realiza el reverso de la pre liquidación ACR
FUNCTION fn_reversa_preliquidacion_acr(p_folio, p_i_tpo_originacion)
   ...
   PREPARE prp_fn_reverso_preliquidacion_acr FROM "EXECUTE FUNCTION fn_reversa_preliquidacion_acr(?,?)"
   EXECUTE prp_fn_reverso_preliquidacion_acr USING p_folio, p_i_tpo_originacion
                                              INTO r_sql_reverso
...
#Objetivo: Realiza el reverso de la liquidación ACR
FUNCTION fn_reversa_liquidacion_acr(p_folio, p_i_tpo_originacion, p_v_usuario, p_i_proceso_cod)
   DEFINE p_folio             LIKE glo_folio.folio                -- folio
   DEFINE p_i_tpo_originacion LIKE cre_acreditado.tpo_originacion -- tipo de originación
   DEFINE p_v_usuario         LIKE seg_usuario.usuario            -- usuario firmado al sistema
   DEFINE p_i_proceso_cod     LIKE cat_proceso.proceso_cod        -- proceso que llama las funciones
   ...
   --CALL fn_reverso_reg_cnt(p_folio, p_i_proceso_cod) RETURNING r_sts_rev_cont
   ...
   PREPARE prp_reverso_liquidacion_acr FROM "EXECUTE FUNCTION fn_reverso_liquidacion_acr(?,?)"
   EXECUTE prp_reverso_liquidacion_acr USING p_folio, p_i_tpo_originacion
                                        INTO r_sql_reverso
...
#Objetivo: Realiza el reverso de saldos remanentes acr
FUNCTION fn_reversa_saldos_remanentes_acr(p_folio, p_i_tpo_originacion)
   DEFINE p_folio             LIKE glo_folio.folio                -- folio
   DEFINE p_i_tpo_originacion LIKE cre_acreditado.tpo_originacion -- tipo de originación
   DEFINE r_sql_reverso       SMALLINT                            -- código de error
   ...
   PREPARE prp_fn_reverso_remanentes_acr FROM "EXECUTE FUNCTION fn_reversa_saldo_remanente_acr(?)"
   EXECUTE prp_fn_reverso_remanentes_acr USING p_i_tpo_originacion
                                          INTO r_sql_reverso
...
FUNCTION fn_reversa_tbls_gen(p_c_op_arch_tran)
   ...
   CASE p_c_op_arch_tran
      WHEN "opt8" -- Preliquidación
         LET v_i_proceso_cod_rev = g_proc_cod_acr_liquidacion
         LET v_i_opera_cod_rev = 1
      WHEN "opt9" -- Liquidación
         LET v_i_proceso_cod_rev = g_proc_cod_acr_liquidacion
         LET v_i_opera_cod_rev = 2
      WHEN "opt10" -- Saldos Remanentes
         LET v_i_proceso_cod_rev = g_proc_cod_acr_sdos_reman
         LET v_i_opera_cod_rev = 1
      OTHERWISE
         DISPLAY "REVERSA TRANSACCIONES - OPCIÓN NO RECONOCIDA"
   END CASE
