...
#                    control de archivos de Recurrente del módulo de    #
#                    Anualidades Garantizadas                           #
#Autor             =>Daniel Buendia, EFP                                #
#Fecha inicio      =>18 ENERO 2012                                      #
#########################################################################
...
MAIN
   DEFINE v_i_estado_cod    LIKE bat_ctr_proceso.estado_cod, -- estado del proceso
          ...
          v_i_tot_reg_det   LIKE cre_ctr_archivo.tot_registros, -- total de registros en detalle
          v_s_qryTxt        STRING, -- guarda una sentencia SQL a ejecutar
          r_si_cod_error    SMALLINT, -- codigo de error en caso de excepción
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".ACRP15.log")

   DISPLAY "=INICIA ACRP15="
   ...
   LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_insrt_acr_ctr_arch_recurr(?,?,?)"
   ...
   EXECUTE prp_procd_ins_cre_ctr_arch USING p_v_arch_proceso, v_i_estado, p_v_usuario
                                       INTO r_si_cod_error, r_b_estatus_proc, v_i_tot_reg_det
...
FUNCTION fn_finaliza_procesos()
   DEFINE v_i_proceso_cod   LIKE cat_proceso.proceso_cod, -- codigo del proceso
          ...
          v_ind_tipo_ejec   LIKE bat_ctr_operacion.ind_tipo_ejecucion, -- tipo de ejecución
          v_s_qryTxt        STRING, -- se asigna una sentencia sql a ejecutar
          v_dt_tiempo       DATETIME YEAR TO SECOND, -- variable con fecha y hora
          v_arch_nohup      STRING, -- ruta y archivo del nohup
          v_arch_errnohup   STRING, -- ruta y archivo del errnohup
          v_comando         STRING, -- comando a ejecutar en consola
   ...
   CALL fn_rutas("bat") RETURNING r_c_ruta_bin, r_c_ruta_listados
   ...
   PREPARE prp_tpo_ejec FROM v_s_qryTxt
   EXECUTE prp_tpo_ejec INTO v_ind_tipo_ejec

   IF v_ind_tipo_ejec = 0 THEN
      -- se cambia el log de la operacion, para que la funcion de correo lo tome
      -- correctamente y no provoque error
      LET v_arch_nohup = r_c_ruta_listados CLIPPED,
                         "/finnohup:",
                         p_d_pid USING "&&&&&",":",
                         p_i_proceso_cod USING "&&&&&",":",
                         p_i_opera_cod USING "&&&&&"

      LET v_arch_errnohup = r_c_ruta_listados CLIPPED,
                            "/errnohup:",
                            p_d_pid USING "&&&&&",":",
                            p_i_proceso_cod USING "&&&&&",":",
                            p_i_opera_cod USING "&&&&&"

      LET v_comando = "mv "||v_arch_nohup||" "||v_arch_errnohup
