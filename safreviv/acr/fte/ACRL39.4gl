...
#                    conciliación de devolucion de saldos          #
#                    correspondiente                               #
#Autor             =>Mauricio Sanchez, EFP                         #
#Fecha inicio      =>07 FEFRERO 2012                               #
####################################################################
DATABASE safre_viv
GLOBALS "ACRG10.4gl"
...
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".ACRL39.log")
   ...
   LET v_i_proceso_cod = g_proc_cod_acr_concilia_dse -- Conciliación de devolucion de saldos
   LET v_i_opera_cod = 1 # Conciliación acr
   LET v_d_pid = 0
   LET v_d_folio = 0
   LET v_v_nom_archivo = "N/A"
   LET v_c_programa_cod = "ACRL39"
   ...
   OPEN WINDOW w_genera_conciliacion_dev WITH FORM "ACRL391"
   INPUT v_cmb_operacion, v_d_folio WITHOUT DEFAULTS FROM cmb_opera, folio ATTRIBUTES(UNBUFFERED)
      ...
      ON ACTION ACCEPT
         ...
         IF v_d_folio IS NULL OR v_d_folio = 0 THEN
            CALL fn_mensaje("Conciliación","Debe capturar un folio válido para conciliar","stop")
            CONTINUE INPUT
         END IF
         ...
         CASE v_cmb_operacion
            WHEN "01" -- Rechazo de saldos excedentes
               LET v_s_comando = v_s_comando,"ACRP29 "

            WHEN "02" -- Confirmacion de devolucion de saldos
               LET v_s_comando = v_s_comando,"ACRP30 "
...
DEFINE v_v_nombre_combo VARCHAR(20),   -- nombre del combobox
   ...
   # Opciones del combo
   CALL cb.addItem("01","15 - Rechazo de saldos")
   CALL cb.addItem("02","15 - Confirmación de saldos")
END FUNCTION
