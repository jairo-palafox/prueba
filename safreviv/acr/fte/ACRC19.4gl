...
DEFINE p_tipo_ejecucion  SMALLINT, -- forma como ejecutara el programa
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".ACRC19.log")
   ...
   LET g_tpo_originacion = 1 --acreditados

   -- se invoca la funcion general de consulta de rechazos de acreditados
   CALL fn_consulta_rechazos_acr()
END MAIN

## ventana que muestra las opciones para consultas de rechazos de acreditados ##
FUNCTION fn_consulta_rechazos_acr()
   DEFINE v_op_consulta CHAR(4)

   OPEN WINDOW w_op_consulta WITH FORM "ACRC191"
...
DEFINE v_s_condicion        STRING,
   ...
   OPEN WINDOW w_op_derechohab WITH FORM "ACRC192"   
       ...
       
{
   ...
   IF ( v_bandera = TRUE ) THEN
      IF v_nss IS NULL THEN
         ...
         EXECUTE prp_cta_nombre INTO v_cuenta_regs

         IF ( v_cuenta_regs = 0 ) THEN 
            CALL fn_mensaje("Aviso","No existe información del acreditado","stop")

            LET v_bandera = FALSE
         ELSE
            -- se verifica si aparece una o mas ocasiones en los rechazos
            IF ( v_cuenta_regs > 1 ) THEN
      ...
      IF v_bandera THEN
         ...
         EXECUTE prp_cuenta_nss INTO v_cuenta_regs
...
FUNCTION fn_consulta_archivo()
   ...
   OPEN WINDOW w_op_archivo WITH FORM "ACRC195"
   ...
   INPUT v_id_cre_ctr_archivo FROM cmb_archivo ATTRIBUTE (UNBUFFERED) 
      ...
      BEFORE INPUT
         ...
         LET v_sqlqry = " SELECT nom_archivo, id_cre_ctr_archivo\n",
                        ...
                        "  WHERE id_proceso IN (301,",g_id_proceso_acr,")\n",
                        "    AND operacion IN (1,6,9,14,21)\n"
         ...
         ELSE
            SELECT nom_archivo
             ...
             WHERE id_cre_ctr_archivo = v_id_cre_ctr_archivo
               AND id_proceso IN (301, g_id_proceso_acr)
            
            -- se consulta si existen registros rechazados para la fecha dada
            SELECT COUNT(*)
              INTO v_conteo
              FROM cre_rch_acreditado
             WHERE id_cre_ctr_archivo = v_id_cre_ctr_archivo
            ...
            IF ( v_conteo = 0 ) THEN
               CALL fn_mensaje("Atención","No existen créditos rechazados para el archivo seleccionado","info")

               CONTINUE INPUT
            END IF

            -- se invoca la función que despliega la información solicitada
            CALL fn_despliega_por_archivo(v_id_cre_ctr_archivo, v_nom_archivo, v_conteo)   
         END IF           
...
FUNCTION fn_despliega_por_archivo(p_id_cre_ctr_archivo, p_nom_archivo, p_cta_reg)
   ...
   OPEN WINDOW w_des_archivo WITH FORM "ACRC196"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm()
      LET v_sqlqry = " SELECT b.desc_estado,\n",
                     "        c.originacion_desc,\n",
                     "        SUM(a.sdo_deudor),\n",
                     "        d.f_proceso,\n",
                     "        d.nom_archivo\n",
      ...
      FOREACH cur_arch INTO v_rec_archivo.*
            ...
            LET v_ar_archivo[v_indice].num_registros    = p_cta_reg
      ...
      DISPLAY ARRAY v_ar_archivo TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("ACRC196.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-ACRC192-","00000","-","00000","-","00000" 
...
FUNCTION fn_consulta_fecha()
   ...
   OPEN WINDOW w_op_fecha WITH FORM "ACRC197"
   ...
   ATTRIBUTES (UNBUFFERED)
      ON ACTION ACCEPT
         ...
         IF ( v_fecha_proceso IS NULL ) THEN
            ...
            CONTINUE INPUT
         ELSE
            -- se consulta si existen archivos cargados para la fecha dada
            SELECT COUNT(*)
              INTO v_conteo
              FROM cre_ctr_archivo
            ...
            -- si no hay al menos un archivo no se muestran datos
            IF ( v_conteo = 0 ) THEN
               CALL fn_mensaje("Atención","No existen créditos rechazados en la fecha dada","info")
               CONTINUE INPUT
            END IF

            -- se invoca la función que despliega la información solicitada
            CALL fn_despliega_por_fecha(v_fecha_proceso, v_conteo)
         END IF           
...
###Funcion que muestra los registros encontrados para el archivo consultado
FUNCTION fn_despliega_por_fecha(p_fecha_proceso, p_i_num_registros)
   DEFINE  p_fecha_proceso       DATE, -- fecha de proceso elegida
           p_i_num_registros     INTEGER, -- numero de registros rechazados
           v_sqlqry              STRING,           
           v_indice              SMALLINT,        
           v_rec_archivo         RECORD 
              estado             VARCHAR(40),
  ...
  CALL v_ar_archivo.clear()
   ...
   OPEN WINDOW w_des_archivo WITH FORM "ACRC196"
      ...
      FOREACH cur_arch_fecha INTO v_rec_archivo.*
         ...
         LET v_ar_archivo[v_indice].f_proceso        = v_rec_archivo.f_proceso
         LET v_ar_archivo[v_indice].nom_archivo      = v_rec_archivo.nom_archivo
         LET v_ar_archivo[v_indice].num_registros    = p_i_num_registros
      ...
      DISPLAY ARRAY v_ar_archivo TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("ACRC196.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-ACRC193-","00000","-","00000","-","00000" 
               CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
...
FUNCTION fn_consulta_tipo_rechazo()
   ...
   OPEN WINDOW w_op_archivo WITH FORM "ACRC198"
   ...
   INPUT v_estado FROM cmb_estado ATTRIBUTE (UNBUFFERED) 
      ...
      ON ACTION ACCEPT
         ...
         ELSE
            ...
            CALL fn_despliega_por_tpo_rechazo(v_estado, v_conteo)   
         END IF           
...
###Funcion que muestra los registros encontrados para el archivo consultado
FUNCTION fn_despliega_por_tpo_rechazo(p_estado, p_i_num_registros)
   DEFINE  p_estado              LIKE cat_rch_acreditado.estado, -- estado de rechazo           
           p_i_num_registros     INTEGER, -- numero de registros rechazados
           v_sqlqry              STRING,           
           v_indice              SMALLINT,           
           v_rec_archivo         RECORD 
   ...
   OPEN WINDOW w_des_rechazo WITH FORM "ACRC196"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm() 
      LET v_sqlqry = " SELECT b.desc_estado,\n",
                     "        c.originacion_desc,\n",
      ...
      DISPLAY ARRAY v_ar_archivo TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("ACRC196.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-ACRC194-","00000","-","00000","-","00000" 
               CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
...

{
   ...
   LET v_sqlqry = " SELECT UNIQUE A.nss,\n",
                  "        C.desc_credito,\n",
                  "        A.num_credito,\n",
                  "        D.edo_credito_desc, ",      
                  "        A.f_otorga,\n",
                  ...
                  "        E.desc_estado\n",
                  "   FROM cre_rch_acreditado A,\n",
                  "  OUTER cat_tipo_credito C,\n",
                  "  OUTER cat_estado_credito D,\n",
                  "  OUTER cat_rch_acreditado E\n", 
                  "  WHERE ", p_s_condicion,
                  "    AND A.tpo_credito = C.tpo_credito\n",
                  "    AND A.edo_credito = D.edo_credito\n",
                  "    AND A.tpo_rch     = E.estado\n"
   ...
   OPEN WINDOW w_list_derechohabiente WITH FORM "ACRC194" 
...
FUNCTION fn_despliega_por_derechohab(v_nss)
  ...
  OPEN WINDOW w_des_derechohab WITH FORM "ACRC193" 
     ...
     DISPLAY ARRAY v_arr_nss_rechazados TO tbl_nss_rechazados.*
        ...
        ON ACTION reporte
           ...
           # Se indica que el reporte usara la plantilla creada
           IF fgl_report_loadCurrentSettings("ACRC191.4rp") THEN 
              CALL fgl_report_selectDevice("PDF") 
              LET v_nom_reporte = p_usuario_cod CLIPPED || "-ACRC191-","00000","-","00000","-","00000" 
              CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
        ...
        BEFORE ROW
           ...
           PREPARE prp_slctFrst_descCred2 FROM v_s_sqlqry
...
DEFINE p_rec_archivo        RECORD 
   ...
   FIRST PAGE HEADER
      LET v_titulo_reporte = "CONSULTA DE RECHAZOS POR ARCHIVO ACR" 
      LET v_fecha_reporte = TODAY      
      PRINTX v_fecha_reporte USING "dd-mm-yyyy"             
      PRINTX p_usuario_cod 
   ...
   ON EVERY ROW
      ...
      PRINTX p_rec_archivo.num_registros
END REPORT
...
DEFINE p_rec_archivo        RECORD 
   ...
   FIRST PAGE HEADER
      LET v_titulo_reporte = "CONSULTA DE RECHAZOS POR FECHA ACR" 
      LET v_fecha_reporte = TODAY      
      PRINTX v_fecha_reporte USING "dd-mm-yyyy"             
      PRINTX p_usuario_cod 
      PRINTX p_rec_archivo.nom_archivo  
...
DEFINE p_rec_archivo        RECORD 
   ...
   FIRST PAGE HEADER
      LET v_titulo_reporte = "CONSULTA DE POR TIPO DE RECHAZO ACR" 
      LET v_fecha_reporte = TODAY      
      PRINTX v_fecha_reporte USING "dd-mm-yyyy"             
      PRINTX p_usuario_cod 
      PRINTX p_rec_archivo.nom_archivo  
