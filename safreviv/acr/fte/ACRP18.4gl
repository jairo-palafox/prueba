...
#                    mación de acr deudor vs tmp deudor rechazo saldos  #
#Autor             =>Daniel Buendia, EFP                                #
#Fecha inicio      =>26 ENERO 2012                                      #
#Autor modifica    =>Emilio Abarca, EFP.                                #
#Fecha modifica    =>07 Mayo 2018.                                      #
#Objetivo modifica =>Adecuación a reporte PDF y generación archivo de   #
#                    de salida rechazos Operación 01 ACR                #
#########################################################################
...
GLOBALS "ACRG10.4gl"
   ...
   DEFINE arr_inf_det_rch DYNAMIC ARRAY OF RECORD 
      estado          SMALLINT,
      estado_desc     CHAR(40),
      t_registros     INTEGER,
      aivs92          DECIMAL(20,2),
      ...
      porc_concatena  CHAR(12)
   END RECORD
   -- Arreglo detalle diagnosticos de rechazos
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".ACRP18.log")

   DISPLAY " "
   DISPLAY "=INICIA ACRP18="
   DISPLAY " CONCILIACIÓN DE RECHAZO DE SALDOS ACR"
   ...
   LET v_si_operacion = 1 -- Rechazo de saldos
   ...
   LET v_d_pid_arch = fn_max_pid(g_proc_cod_acr_rech_saldos, 2)
   ...
   LET v_s_qryTxt = " SELECT *\n",
                    ...
                    "    AND proceso_cod = ",g_proc_cod_acr_rech_saldos,"\n",
                    "    AND opera_cod = 2"
   ...
   LET v_reporte_bin = v_ruta_bin CLIPPED,"/ACRP181.4rp"
   ...
   LET v_s_titulo_correo = "Proceso: CONCILIACIÓN DE RECHAZO DE SALDOS TRANSFERENCIA DE ACREDITADOS"
   ...
   LET v_s_mens_correo =  "ID Proceso   : ",p_d_pid,"\n",
                          ...
                          "Operacion    : RECHAZO DE SALDOS ACR\n",
                          "Fecha Inicio : ",TODAY,"\n",
                          "Fecha Fin    : ",TODAY
...
FUNCTION obtiene_info_rpt()
   ...
   SELECT his.id_cre_acreditado,
    ...
    WHERE his.id_cre_acreditado  = acr.id_cre_acreditado
      AND his.edo_procesar       = 90 --SALDO RECHAZADO
   ...
   FOREACH crs_op01 INTO r_op01_rch.id_cre_acreditado,
                          r_op01_rch.id_derechohabiente,
                          r_op01_rch.nss,
                          r_op01_rch.diagnostico,
                          r_op01_rch.estado
      ...
      FOREACH crs_saldo_92 USING r_op01_rch.nss 
      ...
      FOREACH crs_saldo_97 USING r_op01_rch.nss 
      ...
      -- Guarda en temporal
      INSERT INTO safre_tmp:tmp_op01_acr(
                                id_cre_acreditado ,
                                id_derechohabiente,
                                nss               ,
                                aivs92            ,
                                ...
                                diagnostico       ,
                                estado)
                       VALUES (r_op01_rch.id_cre_acreditado,
                                r_op01_rch.id_derechohabiente,
                                r_op01_rch.nss,
                                v_acciones92,
                                v_pesos92,
   ...
   LET arr_inf_det_rch[1].estado         = NULL
   LET arr_inf_det_rch[1].estado_desc    = NULL 
   LET arr_inf_det_rch[1].t_registros    = 0
   LET arr_inf_det_rch[1].aivs92         = 0
   LET arr_inf_det_rch[1].aivs97         = 0
   LET arr_inf_det_rch[1].porc_concatena = "0.0 %"
   
   --Recupera la información de la temporal
   DECLARE crs_rch_tmp CURSOR FOR 
   SELECT cat.estado,
           cat.estado_desc,
           SUM(tmp.aivs92),
           SUM(tmp.aivs97),
           COUNT(*)
   FROM safre_tmp:tmp_op01_acr tmp,
        cat_maq_credito cat
  ...
  ORDER BY cat.estado;
   ...
   FOREACH crs_rch_tmp INTO arr_inf_det_rch[a].estado,
                             arr_inf_det_rch[a].estado_desc,
                             arr_inf_det_rch[a].aivs92,
                             arr_inf_det_rch[a].aivs97,
                             arr_inf_det_rch[a].t_registros

      LET r_total_global.t_registros = r_total_global.t_registros + arr_inf_det_rch[a].t_registros
      LET r_total_global.aivs92      = r_total_global.aivs92 + arr_inf_det_rch[a].aivs92
      LET r_total_global.aivs97      = r_total_global.aivs97 + arr_inf_det_rch[a].aivs97
   ...
   IF(arr_inf_det_rch[arr_inf_det_rch.getLength()].estado IS NULL) THEN
      CALL arr_inf_det_rch.deleteElement(arr_inf_det_rch.getLength()) 
   END IF 
   ...
   DECLARE crs_detalle_rch CURSOR FOR 
   SELECT cat.estado,
          cat.estado_desc,
          SUM(tmp.aivs97),
          SUM(tmp.pesos97),
   ...
   FOREACH crs_detalle_rch INTO arr_det[a].estado,
      ...
      # Obtiene desc. del diagnostico
      -- Si el diagnostico contiene los 3 dígitos
      IF(LENGTH(arr_det[a].causal) < 3) THEN
         LET arr_det[a].causal = arr_det[a].causal USING "&&&"
      END IF

      -- Busca en cat_Rechazo_causal
      SELECT desc_causal
        INTO arr_det[a].causal_desc
        FROM cat_rechazo_causal
       WHERE causal  = arr_det[a].causal
         AND entidad = "PRC";
...
REPORT genera_PDF()
   ...
   FIRST PAGE HEADER
      ...
      LET v_desc_operacion = "Recepción rechazos de saldo ACR"  
      ...
      PRINTX v_r_bat_ctr_opera.fecha_fin --Fin del proceso
   ...
   ON EVERY ROW
      #Detalle global
      FOR f=1 TO arr_inf_det_rch.getLength()
         PRINTX arr_inf_det_rch[f].estado_desc  
         PRINTX arr_inf_det_rch[f].t_registros       
         PRINTX arr_inf_det_rch[f].aivs92        
         PRINTX arr_inf_det_rch[f].aivs97   
         #calcula porcentaje
         LET v_aux_porcentaje = 0
         LET v_aux_porcentaje = (arr_inf_det_rch[f].t_registros / r_total_global.t_registros) * 100
         LET arr_inf_det_rch[f].porc_concatena = v_aux_porcentaje CLIPPED,"%"
         PRINTX arr_inf_det_rch[f].porc_concatena
      END FOR 
...
FUNCTION genera_archivo_salida()
   ...
   LET v_arh_salida   = "Detalle_TA01_",TODAY USING "yyyymmdd",".cta" CLIPPED 
   ...
   LET v_qry_salida = "SELECT nss,
                              ...
                              estado,
                              diagnostico
                         FROM safre_tmp:tmp_op01_acr;"
                            
      
   PREPARE prp_arh_op01 FROM v_qry_salida
   DECLARE crs_arh_op01 CURSOR FOR prp_arh_op01
   ...
   FOREACH crs_arh_op01 INTO r_arh_op01.nss,
                              r_arh_op01.aivs92,
                              r_arh_op01.aivs97,
                              r_arh_op01.estado,
      ...
      CASE 
         WHEN  r_arh_op01.estado = 20 OR r_arh_op01.estado = 140 
            LET r_arh_op01.tipo  = "DE"
...
FUNCTION crea_temporal()
   ...
   WHENEVER ERROR CONTINUE 
      DROP TABLE tmp_op01_acr
   WHENEVER ERROR STOP
      CREATE TABLE tmp_op01_acr(id_cre_acreditado  DECIMAL(9,0),
                                 id_derechohabiente DECIMAL(9,0),
                                 nss                CHAR(11),
                                 aivs92             DECIMAL(20,2),
