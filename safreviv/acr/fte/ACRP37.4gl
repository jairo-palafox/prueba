...
GLOBALS
   ...
   DEFINE g_folio                   LIKE deo_preliquida.folio_liquida -- folio de la operación
   DEFINE g_nombre_archivo          LIKE glo_ctr_archivo.nombre_archivo -- nombre dle archivo
...
MAIN
   ...
   DEFINE v_b_error                 SMALLINT -- bandera que indica si ocurrió error en la función
   ...
   CALL STARTLOG(g_usuario_cod CLIPPED|| ".ACRP37.log")
   
   DISPLAY "=INICIA ACRP37="
   ...
   CALL fn_display_proceso(0, "PRELIQUIDACION DE DSE")

   -- se asume que el proceso termina correctamente
   LET v_i_resultado = 0

   -- se contruye el enuncionado SQL
   LET v_s_sql = "EXECUTE FUNCTION fn_dse_preliquida_devolucion(?,?,?,?)"
   ...
   DISPLAY " = PRELIQUIDACIÓN DSE (ACR)="
   ...
   LET v_s_qryTxt = " SELECT pre.subcuenta, sub.subcuenta_desc, pre.movimiento, mto.movimiento_desc,\n",
                    ...
                    "   FROM dse_preliquida pre, cat_subcuenta sub, cat_movimiento mto\n",
...
FUNCTION cre_tabla_dse_preliquda()
   ...
   LET v_s_qryTxt = " DROP TABLE dse_preliquida;"
   ...
   LET v_s_qryTxt = " CREATE TABLE dse_preliquida\n",
...
# Objetivo: Función que ejcuta el programa que genera el reporte de preliquidación
FUNCTION fn_ejec_reporte_preliquida()
   ...
   --LET v_c_programa_cod = "ACRL37" -- programa lanzador
   LET v_v_tabla    = "dse_preliquida" -- tabla de preliquidación
