...
#                     Rechazos Devolucio Saldos que ha sido validado #
...
#Objetivo: Función que realiza la integración del archivo de rechazo devolucion saldos
MAIN
   ...
   DEFINE v_r_dse_ctr_arch RECORD
      tot_registros                 LIKE dse_ctr_archivo.tot_registros,  -- número total de registros
      tot_aceptados                 LIKE dse_ctr_archivo.tot_aceptados,  -- número de registros aceptados
      tot_rechazados                LIKE dse_ctr_archivo.tot_rechazados -- número de registros rechazados
   END RECORD
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".ACRE17.log")

   DISPLAY "=INICIA ACRE17="
   ...
   CALL fn_crea_tmp_rech_devol_saldo()
   ...
   -- se crea el comando que ejecuta la conciliación de Rechazo de devolución
   LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/ACRP29 ",
                                           p_v_usuario, " ",
                                           p_d_pid, " ",
                                           p_i_proceso_cod, " ",
                                           p_i_opera_cod, " ",
...
-- comando para generar archivo de texto con detalle de rechazos

   SELECT COUNT(*)
     INTO v_cta_rch
     FROM safre_tmp:tmp_rch_devolucion

   IF v_cta_rch >= 1 THEN
      LET v_cmd = " fglrun ",r_c_ruta_bin CLIPPED,"/ACRP41 ",
                  "safre_tmp:tmp_rch_devolucion", " ",
                  "acr"
       RUN v_cmd

       DISPLAY "Se generó archivo .txt con detalle de rechazos"
   END IF

{
   DISPLAY " GENERA REPORTE CIFRAS CONTROL"
   -- se asigna el nombre del reporte
   LET v_v_nom_reporte = p_v_usuario CLIPPED, "-ACRE34-", p_d_pid USING "&&&&&", "-", p_i_proceso_cod USING "&&&&&", "-", p_i_opera_cod USING "&&&&&"

   -- se invoca la funcion que genera el reporte del proceso de Intergración
   CALL f_genera_rpt_IntegraRechDSE(r_c_ruta_listados, v_v_nom_reporte) RETURNING r_b_existe_err
   ...
   IF NOT r_b_existe_err THEN
      ...
      LET v_s_titulo_correo = "Proceso: RECEPCIÓN RECHAZO DEVOL SDOS EXC ACR"
      ...
      LET v_s_mens_correo =  "ID Proceso   : ",p_d_pid,"\n",
                             "Proceso      : RECEPCIÓN RECHAZO DEVOL SDOS EXC ACR\n",
                             "Operacion    : INTEGRA RECHAZOS DEVOLUCIÓN SDO EXC ACR\n",
                             "Fecha Inicio : ",TODAY,"\n",
                             "Fecha Fin    : ",TODAY
...
FUNCTION f_genera_rpt_IntegraRechDSE(p_c_ruta_listados, p_v_nom_reporte)
   ...
   IF fgl_report_loadCurrentSettings("ACRE171.4rp") THEN
   ...
   START REPORT reporte_integra_rch_dse TO XML HANDLER v_manejador_rpt

   -- salida del reporte
   OUTPUT TO REPORT reporte_integra_rch_dse(v_r_rpt_res.*)

   -- finaliza el reporte
   FINISH REPORT reporte_integra_rch_dse

   RETURN v_b_existe_err
END FUNCTION

#OBJETIVO: Genera el reporte de Integración de Recurrente
REPORT reporte_integra_rch_dse(p_r_res)
...
#Objetivo: Función que crea la tabla temporal de la integración de rechazo devolucion saldos
FUNCTION fn_crea_tmp_rech_devol_saldo()
   ...
   DROP TABLE tmp_deudor_rech_devol_saldo

   -- al encontrar un error detiene el programa
   WHENEVER ERROR STOP

   CREATE TABLE tmp_deudor_rech_devol_saldo(id_dse_devolucion  DECIMAL(9,0),
