...
#                   ción de Rechazo de Devolucion de Saldos Exced. ACR     #
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".ACRP29.log")

   DISPLAY "=INICIA ACRP29="
   DISPLAY " USUARIO       : ",p_v_usuario
   DISPLAY " PID           : ",p_d_pid
   DISPLAY " FOLIO         : ",p_d_folio USING "#########&"
   DISPLAY " ARCHIVO       : ",p_v_arch_proceso
   DISPLAY " CONCILIACIÓN DE RECHAZO DE DEV. DE SDOS. EXC. ACR"
   ...
   LET v_s_qryTxt = " SELECT UNIQUE folio_archivo\n",
                    "   FROM safre_tmp:tmp_deudor_rech_devol_saldo"
   ...
   IF fgl_report_loadCurrentSettings("ACRP291.4rp") THEN  -- if  the file loaded OK
   ...
   LET v_d_pid_arch = fn_max_pid(g_proc_cod_acr_rech_dse, 2)
   ...
   LET v_s_qryTxt = " SELECT *\n",
                    ...
                    "    AND proceso_cod = ",g_proc_cod_acr_rech_dse,"\n",
                    "    AND opera_cod = 2\n",
   ...
   LET v_s_titulo_correo = "Proceso: CONCILIACIÓN RECHAZO DEVOLUCIÓN DE SALDOS EXCEDENTES ACREDITADOS"
...
#OBJETIVO: Genera el reporte de la conciliación de Rechazo de saldos
REPORT reporte_Rechazo_ds(p_v_usuario,
   ...
   DEFINE p_d_fol_arch        LIKE glo_ctr_archivo.folio -- número de folio
   ...
   ON EVERY ROW
      ...
      DECLARE cur_dia CURSOR FOR   SELECT t.diagnostico[1,3]||" - "||c.desc_rechazo diag,
                                          COUNT(*)
                                     FROM safre_tmp:tmp_rch_devolucion t , cat_rechazo c
