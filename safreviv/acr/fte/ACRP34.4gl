...
#                     liquidacion de acreditados                           #
#Autor             => Daniel Buendia, EFP                                  #
#Fecha inicio      => 22 Marzo 2012                                        #
...
DEFINE p_v_usuario                  LIKE seg_usuario.usuario, -- nombre del usuario
       ...
       p_c_programa_cod             LIKE cat_operacion.programa_cod --nombre del programa que lanza el reporte
...
MAIN
   ...
   -- se obtienen las descripciones del proceso y la operación
   ...
   CALL fn_reporte_preliquidacion_acr(TRUE, r_ruta_listados, v_v_nom_reporte, v_c_opera_desc)
...
FUNCTION fn_reporte_preliquidacion_acr(p_b_despliegue_pantalla, p_ruta_listados, p_nom_reporte, p_c_opera_desc)
   ...
   FOREACH cur_grupo USING p_d_folio INTO arr_grupo[v_i].*
      ...
      FOREACH cur_subcuenta USING p_d_folio, arr_grupo[v_i].grupo_regimen
   ...
   ##### FONDO DE ARHORRO 72 ####
   ##############################
   IF p_i_proceso_cod <> g_proc_cod_acr_liquida_dse THEN 
   LET j = 1
   ...
   LET v_qry_string = " SELECT UNIQUE fa.movimiento, fa.movimiento|| ' '||mov.movimiento_desc\n",
                      "   FROM safre_tmp:tmp_acr_fondo72 fa, cat_movimiento mov\n",
   ...
   FOREACH cur_glo_fondo72 INTO arr_nivel2[j].movimiento, arr_nivel2[j].descripcion
      LET v_qry_string = " SELECT SUM(importe)\n",
                         "   FROM safre_tmp:tmp_acr_fondo72\n",
                         "  WHERE folio_liquida = ",p_d_folio,"\n",
                         "    AND movimiento = ",arr_nivel2[j].movimiento
      ...
      LET arr_datos[v_contador].desc_regimen      = "FONDO AHORRO 72"
   ...
   IF ( fgl_report_loadCurrentSettings("ACRP341.4rp") ) THEN  -- if  the file loaded OK
...
REPORT rpt_liquidacion_acr(p_c_opera_desc, p_r_encabezado, p_r_datos)
   ...
   FORMAT   
      ...
      BEFORE GROUP OF p_r_datos.subcuenta
         IF p_r_datos.desc_regimen = "SALDO DEUDOR" OR p_r_datos.desc_regimen = "FONDO AHORRO 72" THEN 
