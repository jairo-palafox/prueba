...
MAIN
   ...
   LET v_i_proceso_cod      = g_proc_cod_acr_liquidacion -- liquidación transferencia acreditados
   ...
   IF v_cuenta_accion = 0 THEN
      LET v_s_mensaje = "No es posible realizar la preliquidación\n",
   ...
   IF v_cta_reg = 0 THEN
      LET v_s_mensaje = "No es posible realizar la preliquidación\n",
                        "ya que no se encontró información para preliquidar"
      CALL fn_mensaje("Aviso",v_s_mensaje,"stop")
      EXIT PROGRAM
   END IF
...
FUNCTION fn_llena_arbol_montos_pre_acr(p_folio, v_tabla, p_si_tpo_originacion)
   ...
   LET qry_string = " SELECT ts.subcuenta, ts.subcuenta||' '||ts.subcuenta_desc ",
                    ...
                    "  WHERE ts.subcuenta IN (4,8,42,44,49) ",
...
FUNCTION fn_despliega_desc_pre(p_proceso_cod, p_opera_cod)
   ...
   -- se obtiene la descripción de la operación
   SELECT opera_desc
     INTO v_opera_desc
     FROM cat_operacion
    WHERE proceso_cod = p_proceso_cod
