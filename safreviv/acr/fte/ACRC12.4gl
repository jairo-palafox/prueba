...
#Modulo            => ACR                                                     #
#Programa          => ACRC12                                                  #
#Objetivo          => CONSULTA DE LIQUIDACION DE ACR (NO USA LA GENERICA)     #
...
MAIN
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".ACRC12.log")
   ...
   LET g_proceso_cod = g_proc_cod_acr_liquidacion -- liquidacion de ACR
   LET g_opera_cod   = 2 -- operacion 1

   -- se invoca la funcion general de consulta de liquidacion
   CALL fn_consulta_folios_liquida(p_usuario_cod, g_proceso_cod, g_opera_cod)

END MAIN

FUNCTION fn_consulta_folios_liquida(p_usuario, p_proceso_cod, p_opera_cod)
   ...
   OPEN WINDOW vent_folios WITH FORM "ACRC121"
      ...
      ON ACTION ACCEPT
         IF ( g_consulta.f_liquida IS NULL AND
            ...
            CALL fn_mensaje("Consulta",
                            ...
                            "about")
         ELSE
            -- se construye la cadena de condición
            LET v_condicion = "1=1\n"
   ...
   IF NOT INT_FLAG THEN
      IF fn_valida_folio_liquida(v_condicion,v_tabla, p_proceso_cod, p_opera_cod) THEN
         CALL fn_folios_consulta_liquida(v_tabla, p_proceso_cod, p_opera_cod, p_usuario)
      ELSE
         CALL fn_mensaje("Consulta",
...
FUNCTION fn_despliega_desc_liquida(p_proceso_cod, p_opera_cod)
...
FUNCTION fn_valida_folio_liquida(v_condicion, v_tabla, p_proceso_cod, p_opera_cod)
...
FUNCTION fn_folios_consulta_liquida(v_tabla, p_proceso_cod, p_opera_cod, p_usuario_cod)
   ...
   DEFINE p_c_programa_cod   LIKE cat_operacion.programa_cod -- nombrel del programa origen
   ...
   LET v_nom_reporte = p_usuario_cod CLIPPED || "-",p_c_programa_cod CLIPPED,"-",g_pid USING "&&&&&","-",p_proceso_cod USING "&&&&&", "-", p_opera_cod USING "&&&&&"
