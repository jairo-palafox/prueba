...
DEFINE p_tipo_ejecucion SMALLINT, -- forma como ejecutara el programa
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".ACRC10.log")
   ...
   CALL fn_consulta_historico_acr()
END MAIN

## ventana que muestra las opciones para consultas de acreditados ##
FUNCTION fn_consulta_historico_acr()
   DEFINE v_op_consulta CHAR(4)

   OPEN WINDOW w_op_consulta WITH FORM "ACRC101"
...
DEFINE v_s_condicion        STRING,
   ...
   OPEN WINDOW w_op_derechohab WITH FORM "ACRC102"   
...
DEFINE v_s_condicion  STRING,
   ...
   OPEN WINDOW w_op_estado WITH FORM "ACRC104"
   CONSTRUCT v_s_condicion
          ON estado,
             edo_procesar
        FROM cmb_estado,
...
DEFINE v_s_condicion        STRING,
   ...
   OPEN WINDOW w_op_historica WITH FORM "ACRC106"
   CONSTRUCT v_s_condicion
          ON estado,
             edo_procesar,
             diagnostico
...
FUNCTION fn_consulta_archivo()
   ...
   OPEN WINDOW w_op_archivo WITH FORM "ACRC109"
   INPUT v_id_cre_ctr_archivo FROM cmb_archivo ATTRIBUTE (UNBUFFERED) 
      ...
      ON ACTION ACCEPT
         ...
         -- se invonca la función que desplega la información requerida
...
FUNCTION fn_despliega_archivo(p_id_cre_ctr_archivo, p_nom_archivo)
   ...
   OPEN WINDOW w_des_archivo WITH FORM "ACRC105"
      ...
      -- se inicializan valores antes del ciclo
      ...
      DISPLAY ARRAY v_ar_archivo TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("ACRC104.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-ACRC104-","00000","-","00000","-","00000" 
...
FUNCTION fn_obt_derechohabiente(p_s_condicion)
   ...
   OPEN WINDOW w_list_derechohabiente WITH FORM "ACRC108" 
...
FUNCTION fn_despliega_derechohab(p_id_derechohabiente)
  ...
  -- se obtiene la descripción del estado
  LET v_des_estado = fn_obt_desc_estado(v_estado)

  -- se obtiene la descripción del estado procesar
  LET v_des_procesar = fn_obt_desc_estado(v_edo_procesar)
  ...
  OPEN WINDOW w_des_derechohab WITH FORM "ACRC103" 
     LET  w = ui.Window.getCurrent()
     LET  f_w = w.getForm()  
...
FUNCTION fn_despliega_estado(p_s_condicion)
   ...
   OPEN WINDOW w_des_estado WITH FORM "ACRC105"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm() 
      LET v_sqlqry = " SELECT estado,edo_procesar",
                     "   FROM cre_acreditado", 
...
FUNCTION fn_despliega_historica(p_s_condicion)
   ...
   OPEN WINDOW w_des_historica WITH FORM "ACRC107"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm() 
