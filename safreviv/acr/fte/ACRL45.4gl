...
#  del archivo de fondo de ahorro 72 en la base de datos de historicos (his transferencia)
MAIN
   DEFINE p_v_usuario         LIKE seg_usuario.usuario,-- usuario firmado al sistema
          ...
          END RECORD,
          v_r_cre_ctr_arch    RECORD LIKE dse_ctr_archivo.*, -- registro del control de archivo  
          ...
          v_c_tpo_trasf       LIKE dse_ctr_archivo.tpo_transferencia, -- trasferencia
          ...
          --v_v_nom_reporte   VARCHAR(50), -- nombre del reporte
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".ACRL45.log")
   ...
   LET v_i_opera_cod = 2 -- integra archivo fondo ahorro 72
   LET v_i_opera_cod_ant = 1 -- valida archivo fondo ahorro 72
   LET v_c_tpo_trasf = "15" -- Trasferencia de Acreditados
   LET v_si_id_proceso = g_id_proceso_acr -- Transferencia de Acreditados
   LET v_d_folio = 0
   LET v_c_programa_cod = "ACRL45"
   ...
   OPEN WINDOW w_integra_arch_recurr WITH FORM "ACRL321"
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         BEFORE DIALOG
            -- se oculta el botón de integración
         ...
         ON ACTION ACCEPT
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               LET v_s_qryTxt = " SELECT *\n",
                                ...
                                "  WHERE tpo_transferencia = '",v_c_tpo_trasf,"'\n",
                                "    AND estado = 10\n", -- cargado
                ...
                IF v_r_cre_ctr_arch.lote IS NULL THEN
                   ...
                   CALL fn_mensaje("Integración fondo de ahorro 72",v_s_mensaje,"stop")
                   CONTINUE DIALOG
                END IF
         ...
         ON ACTION integrar
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               IF r_b_valida = 0 THEN
                  ...
                  LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/ACRE19 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
