...
#Modulo            =>ACR                                           #
#Programa          =>ACRC13                                        #
#Objetivo          =>Programa que reliza la consulta de historicos #
#                    de devolucion de saldos                       #
#Autor             =>Mauricio Sanchez, EFP                         #
#Fecha inicio      =>3 FEBRERO 2012                                #
####################################################################
...
MAIN
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".ACRC13.log")
   ...
   LET g_tpo_transferencia = 15 --devolucion de acreditados
...
FUNCTION fn_consulta_historico_dev()
   ...
   OPEN WINDOW w_op_consulta WITH FORM "ACRC131"
...
FUNCTION fn_consulta_derechohabiente_dev()
   ...
   OPEN WINDOW w_op_derechohab WITH FORM "ACRC132"
...
FUNCTION fn_consulta_estado_dev()
   ...
   OPEN WINDOW w_op_estado WITH FORM "ACRC134"
...
FUNCTION fn_consulta_historica_dev()
   ...
   OPEN WINDOW w_op_historica WITH FORM "ACRC136"
   CONSTRUCT v_s_condicion
          ON estado,
             edo_procesar,
             diagnostico
...
FUNCTION fn_consulta_archivo_dev()
   ...
   OPEN WINDOW w_op_archivo WITH FORM "ACRC139"
   ...
   INPUT v_folio FROM cmb_archivo ATTRIBUTE (UNBUFFERED)
      ...
      BEFORE INPUT
         LET v_tpo_transferencia = "15" -- 15 - DSE Transferencia de Acreditados
...
FUNCTION fn_despliega_archivo_dev(p_folio, p_nom_archivo)
   ...
   OPEN WINDOW w_des_archivo WITH FORM "ACRC135"
      ...
      DISPLAY ARRAY v_ar_archivo TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("ACRC134.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-ACRC134-","00000","-","00000","-","00000" 
...
FUNCTION fn_obt_derechohabiente(p_s_condicion)
   ...
   OPEN WINDOW w_list_derechohabiente WITH FORM "ACRC138" 
...
FUNCTION fn_despliega_derechohab_dev(p_id_derechohabiente)
  ...
  SELECT estado_desc
   ...
   OPEN WINDOW w_des_derechohab WITH FORM "ACRC133"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm()
      ...
      DISPLAY --p_id_derechohabiente,
         ...
         # Se indica que el reporte usara la plantilla creada
           IF fgl_report_loadCurrentSettings("ACRC131.4rp") THEN 
              CALL fgl_report_selectDevice("PDF") 
              LET v_nom_reporte = p_usuario_cod CLIPPED || "-ACRC131-","00000","-","00000","-","00000" 
              CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
...
FUNCTION fn_despliega_estado_dev(p_s_condicion)
   ...
   OPEN WINDOW w_des_estado WITH FORM "ACRC135"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm()
      ...
      DISPLAY ARRAY v_ar_estado TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            # Se indica que el reporte usara la plantilla creada
            IF fgl_report_loadCurrentSettings("ACRC132.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-ACRC132-","00000","-","00000","-","00000" 
               CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
...
FUNCTION fn_despliega_historica_dev(p_s_condicion)
   ...
   OPEN WINDOW w_des_historica WITH FORM "ACRC137"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm()  
      ...
      DISPLAY ARRAY v_ar_historica TO tabla_historica.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            # Se indica que el reporte usara la plantilla creada
            IF fgl_report_loadCurrentSettings("ACRC133.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-ACRC133-","00000","-","00000","-","00000" 
               CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
