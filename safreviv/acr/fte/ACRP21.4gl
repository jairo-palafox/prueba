...
#                    ahorro 72 para el módulo de Transferencia de Acred.#
...
MAIN 
   ...
   CALL STARTLOG(p_v_usuario_cod CLIPPED|| ".ACRP21.log")
   
   DISPLAY "=INICIA ACRP21="
   DISPLAY " USUARIO    : ",p_v_usuario_cod
   DISPLAY " PID        : ",p_d_pid
   DISPLAY " FOLIO      : ",p_d_folio USING "#########&"

   -- se inicializan variables
   LET v_c_programa_cod = "ACRP21"
   ...
   IF v_i_cuenta_regs = 0 THEN
      DISPLAY " ADVERTENCIA: No existieron registro para liquidar en fondo ahorro 72"
   ...
   LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_acr_liquida_fondo72(?)"
   ...
   CALL fn_registro_contable_20(p_d_folio, p_d_pid, p_i_proceso_cod, p_i_opera_cod)
   ...
   LET v_s_qryTxt = " SELECT sdo.subcuenta, sub.subcuenta_desc, sdo.movimiento, mov.movimiento_desc, SUM(sdo.importe)\n",
                    ...
                    "  WHERE sdo.subcuenta = sub.subcuenta\n",
                    "    AND sdo.movimiento = mov.movimiento\n",
                    "    AND sdo.folio_liquida = ",p_d_folio,"\n",
                    "  GROUP BY 1,2,3,4\n",
                    "  ORDER BY 1,3"
...
FUNCTION fn_registro_contable_20(v_folio_registro, p_pid, p_proceso_cod, p_opera_cod)
  ...
  LET p_cve_proceso_cnt = 20  --Traspaso - Transferencia de Acreditados SAR 97
  LET p_transaccion     = 0   --Sin transaccion
  LET r_bnd_proceso_cnt = 0   --Bandera del stored del registro de traspaso - transferencia de acreditados
  ...
  PREPARE prp_reg_contable
    FROM "EXECUTE PROCEDURE fn_cnt_acr_97(?,?,?,?,?)"
  ...
  IF r_bnd_proceso_cnt = 1 THEN
     ...
     IF v_cuenta_contable > 0 THEN
        DISPLAY "El registro contable de traspaso - transferencia de acreditados se realizó exitosamente."
     ELSE
