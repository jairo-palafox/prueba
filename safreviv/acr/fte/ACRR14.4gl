...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".ACRR14.log")

   DISPLAY "=INICIA ACRR14="
   ...
   CASE p_c_op_arch_ent
      WHEN "opt5" -- Agrupación
         LET v_si_proceso_cod_rev = g_proc_cod_acr_agrupacion_dse -- agrupación registros con devolución acr
         LET v_si_opera_cod_rev   = 1 -- agrupa registros con devolución
         CALL fn_reverso_agrupacion(p_d_folio) RETURNING r_b_valida

      WHEN "opt6" -- Preliquidación
         LET v_si_proceso_cod_rev = g_proc_cod_acr_liquida_dse -- liquidación devolución saldos exc acr
         LET v_si_opera_cod_rev   = 1 -- preliquida devolución de saldos
         CALL fn_reverso_preliquida_dseacr(p_d_folio) RETURNING r_b_valida

      WHEN "opt7" -- Liquidación
         LET v_si_proceso_cod_rev = g_proc_cod_acr_liquida_dse -- liquidación devolución saldos exc acr
         LET v_si_opera_cod_rev   = 2 -- liquida devolución de saldos
         CALL fn_reverso_liquida_dseacr(p_d_folio) RETURNING r_b_valida

      WHEN "opt9" -- Liquidación Fondo ahorro 72
         LET v_si_proceso_cod_rev = g_proc_cod_acr_liquida_f72 -- liquidación fondo 72
         LET v_si_opera_cod_rev   = 1 -- liquidación fondo 72
         CALL fn_reverso_liquida_fondo72(p_d_folio) RETURNING r_b_valida
   END CASE
...
FUNCTION fn_reverso_agrupacion(p_d_folio)
   DEFINE p_d_folio             LIKE glo_ctr_archivo.folio, -- numero de folio
...
FUNCTION fn_reverso_preliquida_dseacr(p_d_folio)
...
FUNCTION fn_reverso_liquida_dseacr(p_d_folio)
   DEFINE p_d_folio           LIKE glo_ctr_archivo.folio, -- numero de folio
...
#Objetivo: Función que ejecuta el reverso de la liquidación
FUNCTION fn_reverso_liquida_fondo72(p_d_folio)
   DEFINE p_d_folio             LIKE glo_ctr_archivo.folio, -- numero de folio
          r_codigo_sql          SMALLINT
   ...
   DISPLAY "REVERSO LIQUIDACIÓN FONDO 72"
   -- Ejecuta el SP que realiza el reverso
   PREPARE prp_reverso_liquidaFondo_dse FROM "EXECUTE FUNCTION safre_viv:fn_reverso_liquidacion_fondo72_dse(?)"
   EXECUTE prp_reverso_liquidaFondo_dse USING p_d_folio
                                        INTO r_codigo_sql
