...
DEFINE p_v_usuario         LIKE seg_usuario.usuario, -- usuario firmado al sistema
       ...
       m_d_folio           LIKE bat_ctr_operacion.folio -- folio
...
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".ACRL40.log")
   ...
   LET m_i_proceso_cod = g_proc_cod_acr_reverso_dse -- reverso dev sdo exc
   LET m_i_opera_cod = 1 -- reverso dev sdo exc
   LET v_d_pid = 0
   LET m_d_folio = 0
   LET m_c_programa_cod = "ACRL40"
   LET v_c_op_arch_ent = NULL
   LET v_c_op_arch_sal = NULL
   LET v_ci_op_arch_tran = NULL
   LET m_c_tpo_transf = '15' -- 15-DSE Transferencia de Acreditados
   ...
   CALL fn_rutas("acr") RETURNING m_c_ruta_bin, r_c_ruta_listados
   ...
   OPEN WINDOW w_reverso WITH FORM "ACRL401"
...
FUNCTION fn_reversa_arch_entrada(p_c_op_arch_ent)
   DEFINE p_c_op_arch_ent     VARCHAR(5), -- contiene la opcion a reversar de Archivos entrada
          ...
          v_folio             LIKE dse_ctr_archivo.folio, -- folio
   ...
   CASE p_c_op_arch_ent
      WHEN "opt1"
         ...
         LET v_i_proceso_cod_rev = g_proc_cod_acr_dse
         LET v_i_opera_cod_rev = 2
      WHEN "opt2"
         LET v_nom_grupo = "Recepción Rechazo Devolución"
         LET v_i_proceso_cod_rev = g_proc_cod_acr_rech_dse
         LET v_i_opera_cod_rev = 2
      WHEN "opt3"
         LET v_nom_grupo = "Recepción Confirmación Devolución"
         LET v_i_proceso_cod_rev = g_proc_cod_acr_conf_dse
         LET v_i_opera_cod_rev = 2
      WHEN "opt8"
         LET v_nom_grupo = "Recepción Fondo de Ahorro 72"
         LET v_i_proceso_cod_rev = g_proc_cod_acr_fondo72
         LET v_i_opera_cod_rev = 2
   ...
   -- se obtiene PID del proceso y operación a reversar
   LET v_d_pid_rev = fn_max_pid(v_i_proceso_cod_rev, v_i_opera_cod_rev)

   OPEN WINDOW w_arch_entrada WITH FORM "ACRL402"
      ...
      INPUT v_v_nom_archivo, v_i_lote, v_fecha_lote, v_folio
       FROM nom_archivo, lote, f_lote, folio
      ATTRIBUTES ( UNBUFFERED ) 
      
       ON ACTION ACCEPT
         -- se validan las variables ingresadas
         IF v_v_nom_archivo IS NULL AND v_i_lote IS NULL AND
            v_fecha_lote IS NULL AND v_folio IS NULL THEN
         ...
         IF ( v_folio IS NOT NULL ) THEN
            LET v_s_condicion = v_s_condicion, "AND folio ='", v_folio, "'\n"
         END IF
         EXIT INPUT 
   ...
   IF v_ban_salir THEN
      -- se canceló el reverso, regresa a la ventana principal
      RETURN
   END IF
      ...
      DISPLAY ARRAY v_ar_tbl_archivos TO tabla_reverso.*
         ON ACTION ACCEPT
            ...
            CASE p_c_op_arch_ent
               ...
               WHEN "opt8" -- Recepción Confirmación Devolución
                  CALL fn_valida_dev_fondo(v_d_folio_arch) RETURNING v_b_continua
               OTHERWISE -- En otro caso
                  CALL fn_mensaje("Reverso","Opción no reconocida","stop")
                  CONTINUE DISPLAY
            END CASE
            ...
            IF r_b_valida = 0 THEN
               ...
               IF r_b_valida = 0 THEN
                  ...
                  LET v_s_comando = " nohup time fglrun ",m_c_ruta_bin CLIPPED,"/ACRR13 ",
                                                          p_v_usuario, " ",
                                                          v_d_pid, " ",
                                                          m_i_proceso_cod, " ",
                                                          m_i_opera_cod, " ",
...
FUNCTION fn_valida_ext_archivo(p_c_op_arch_ent, p_c_nom_archivo)
   ...
   CASE p_c_op_arch_ent
      ...
      WHEN "opt2" -- Recepción Rechazo Devolución Saldos
         LET v_i_proceso_cod = g_proc_cod_acr_rech_dse -- Recepción Rechazo Devolución Saldos
      ...
      WHEN "opt3" -- Recepción Confirmación Saldos
         LET v_i_proceso_cod = g_proc_cod_acr_conf_dse -- Recepción Confirmación Saldos
      ...
      WHEN "opt4" -- Salida Solicitud de Saldos
         LET v_i_proceso_cod = g_proc_cod_acr_arch_solic_dse -- generación solicitud devolución sdo exc acr
         LET v_i_opera_cod = 1 -- solicitud devolución saldo excedente acr
      ...
      WHEN "opt8" -- Devolución Fondo Ahorro 72
         LET v_i_proceso_cod = g_proc_cod_acr_fondo72 -- Recepción fondo de ahorro 72
         LET v_i_opera_cod = 1 -- valida archivo fondo ahorro 72
...
FUNCTION fn_reversa_transacciones(p_c_op_arch_tran)
   DEFINE p_c_op_arch_tran    VARCHAR(5), -- contiene la opcion a reversar transacciones
          v_d_folio_liquida   LIKE glo_ctr_archivo.folio, -- numero de folio
          v_d_pid             LIKE bat_ctr_proceso.pid, -- identificador del proceso
          ...
          v_i_cta_reg_dset    INTEGER --contador de registros de la tabla dse transferencia
          
   -- se inicializan variables
   LET v_d_pid = 0
   LET v_d_folio_liquida = NULL

   OPEN WINDOW w_arch_transaccion WITH FORM "ACRL404"
   --se captura el folio a reversar
   INPUT v_d_folio_liquida WITHOUT DEFAULTS 
      ...
      ON ACTION ACCEPT   
         ...
         IF p_c_op_arch_tran = "opt5" THEN
             ...
             WHERE id_dse_grp_devolucion IN (
                   SELECT id_dse_grp_devolucion
                    ...
                    WHERE tpo_transferencia = m_c_tpo_transf
                      AND folio = v_d_folio_liquida) -- folio del archivo
            ...
            ELSE
               ...
               IF v_i_cta_reg = 0 THEN
                  CALL fn_mensaje("Reverso de Transacciones","No hay registros localizados en agrupación ó los registros ya preliquidados o liquidados","stop")   
                  CONTINUE INPUT 
               ELSE
                  -- se invoca la funcion que genera el PID
                  CALL fn_genera_pid(m_i_proceso_cod,m_i_opera_cod,p_v_usuario) RETURNING v_d_pid 
                  ...
                  IF r_b_valida = 0 THEN
                     ...
                     IF r_b_valida = 0 THEN
                        ...
                        LET v_s_comando = " nohup time fglrun ",m_c_ruta_bin CLIPPED,"/ACRR14 ",
                                                                p_v_usuario, " ",
                                                                v_d_pid, " ",
                                                                m_i_proceso_cod, " ",
                                                                m_i_opera_cod, " ",
         ...
         IF p_c_op_arch_tran = "opt6" THEN 
            ...
            SELECT COUNT(*)
              INTO v_i_cta_reg
              FROM dse_agrupa_devolucion
             WHERE folio_liquida = v_d_folio_liquida
            ...
            ELSE
               ...
               SELECT COUNT(*)
                 ...
                 FROM dse_agrupa_devolucion 
                WHERE estado = 130
                  AND folio_liquida = v_d_folio_liquida
         ...
         IF p_c_op_arch_tran = "opt7" THEN 
            ...
            ELSE
               ...
               ELSE
                 ...
                 --VPD Revisar si la validacion se ocupa para este proceso
         ...
         -- se selecciono la opcion de liquidacion fondo 72
         IF p_c_op_arch_tran = "opt9" THEN 
            -- se consulta si existe el folio a reversar
            SELECT COUNT(*)
              INTO v_i_cta_reg
              FROM cta_fondo72 
...
FUNCTION fn_reversa_arch_salida()
   ...
   OPEN WINDOW w_arch_salida WITH FORM "ACRL403"
   -- se deplega el nombre del proceso a reversar en el grupo
   LET v_ui_ventana = ui.Window.getCurrent()
   LET v_ui_forma = v_ui_ventana.getForm()
   CALL v_ui_forma.setElementText("grevent",v_nom_grupo)
            ...
            ON ACTION ACCEPT
               ...
               IF r_b_valida = 0 THEN
                  ...
                  IF r_b_valida = 0 THEN
                     ...
                     LET v_s_comando = " nohup time fglrun ",m_c_ruta_bin CLIPPED,"/ACRR15 ",
                                                             p_v_usuario, " ",
                                                             v_d_pid, " ",
                                                             m_i_proceso_cod, " ",
                                                             m_i_opera_cod, " ",
...
FUNCTION fn_valida_rech_dev(p_d_folio_arch)
   ...
   -- se verifica el contador de regsitros
   IF v_i_cont_regs = 0 THEN
      CALL fn_mensaje("Reverso","No se puede reversar el proceso ya que no se encuentra la\ninformación en el status requerido","stop")
...
# Objetivo: Se valida el proceso de reverso de Confirmación de Devolución de Saldos
FUNCTION fn_valida_dev_fondo(p_d_folio_arch)
   DEFINE p_d_folio_arch  LIKE glo_folio.folio, -- folio del archivo seleccionado
          v_b_val_proceso SMALLINT, -- indica si el proceso de reverso procede o no
          v_i_cont_regs   INTEGER, -- contador de registros
          v_s_qryTxt      STRING -- guarda una sentencia sql a ejecutar
   ...
   PREPARE prp_val_fondo_ahorro FROM v_s_qryTxt
   EXECUTE prp_val_fondo_ahorro INTO v_i_cont_regs
