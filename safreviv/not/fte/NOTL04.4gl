...
MAIN
   ...
   WHERE proceso_cod = PROC_OMISOS_TRM
...
FUNCTION fn_drgandrop_bdnsviv()
   DEFINE   l_arr_arch_pend    DYNAMIC ARRAY OF VARCHAR(100), -- Archivos pendientes 
            ...
            l_arr_integrados   DYNAMIC ARRAY OF RECORD -- Detalle archivos a integrar
               ...
               f_presenta     DATE,
               t_aportacion   DECIMAL(15,2),
               t_amortizacion DECIMAL(15,2),
   ...
   LET l_s_qryTxt = "SELECT nombre_archivo ",
                    ...
                    "AND proceso_cod = ", PROC_OMISOS_TRM, " ",
                    "AND estado = 1"
   ...
   OPEN WINDOW w_inte_cbd WITH FORM "NOTL041"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION integrar
         ...
         FOR l_i_iter = 1 TO l_i_num_arch
            ...
            EXECUTE exe_consulta_sumario INTO   l_arr_integrados[l_i_iter].f_presenta,
                                                l_arr_integrados[l_i_iter].t_registros

            SELECT SUM(aportacion)/100, SUM(amortizacion)/100
            INTO l_arr_integrados[l_i_iter].t_aportacion, l_arr_integrados[l_i_iter].t_amortizacion
      ...
      ON ACTION ACCEPT
         ...
         CALL fn_ventana_confirma("Atención",
                    "¿Desea ejecutar el proceso de Integracion del archivo de Omisos TRM?",
...
FUNCTION fn_ejecuta_integracion(p_nombre_archivo, v_ruta_ejecutable, v_ruta_listados)
   ...
   CALL fn_valida_operacion(v_pid,PROC_OMISOS_TRM,OP_INTEGRA_ARCHIVO) RETURNING v_resultado
   IF v_resultado = 0 THEN
      LET v_folio = 1
      
      # Inicia operación
      CALL fn_actualiza_opera_ini(v_pid,PROC_OMISOS_TRM,OP_INTEGRA_ARCHIVO,v_folio,"NOTP04",
                            p_nombre_archivo,p_usuario) RETURNING v_resultado
      
      LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/NOTP04 ",
                                       p_usuario," ",
                                       v_pid," ",
                                       PROC_OMISOS_TRM," ",
                                       OP_INTEGRA_ARCHIVO," ",
