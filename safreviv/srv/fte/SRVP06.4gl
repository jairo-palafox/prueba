...
PRIVATE FUNCTION fn_datos_edo_cuenta(p_nss)
   ...
   LET consulta_datos = "  SELECT FIRST 1 ",
                           ...
                           "LEFT JOIN afi_domicilio domicilio ON (domicilio.id_derechohabiente = afiliado.id_derechohabiente AND domicilio.ind_envio IN ('1','X') ) ",
   ...
   LET consulta_anterior = "SELECT ",
                           ...
                           "WHERE mov.subcuenta IN (4,8,40,41,42,43,44,45,46,49)  ",
                           "AND mov.f_liquida < ? ",
   ...
   LET consulta_movtos = " SELECT ",
                           ...
                           "AND movimientos.subcuenta IN (4,8,40,41,42,43,44,45,46,49) ",
   ...
   LET consulta_subcuentas = "SELECT subcuenta, subcuenta_desc FROM cat_subcuenta WHERE subcuenta IN (4,8,40,41,42,43,44,45,46,49)"
#INITCAP(subcuenta_desc)
   ...
   FOREACH anterior USING v_parametros.periodo_inicio, id_derechohabiente INTO v_totales.*
      CASE v_totales.subcuenta
         ...
         WHEN 46
            LET v_parametros.anterior_excedente = v_totales.suma
         WHEN 49
            LET v_parametros.abono_fortalecimiento = v_totales.suma
      END CASE
   END FOREACH
   ...
   FOREACH periodo USING v_parametros.periodo_inicio, v_parametros.periodo_fin, id_derechohabiente INTO v_totales.*
      CASE v_totales.subcuenta
         ...
         WHEN 46
            IF v_totales.tipo_mov = 1 THEN
               LET v_parametros.abono_excedente = v_totales.suma
            ELSE
               LET v_parametros.cargo_excedente = v_totales.suma
            END IF
         WHEN 49
            IF v_totales.tipo_mov = 1 THEN
               LET v_parametros.abono_fortalecimiento = v_totales.suma
            ELSE
               LET v_parametros.cargo_fortalecimiento = v_totales.suma
            END IF 
      END CASE
   END FOREACH
   ...
   FOREACH subcuentas INTO v_subcuenta, v_desc_subcuenta
      CASE v_subcuenta
         ...
         WHEN 46
            LET v_parametros.desc_excedente = v_desc_subcuenta
         WHEN 49
            LET v_parametros.desc_fortalecimiento = v_desc_subcuenta
      END CASE
   END FOREACH
   ...
   LET v_parametros.final_excedente =    v_parametros.anterior_excedente
                                       +  v_parametros.cargo_excedente
                                       +  v_parametros.abono_excedente


   LET v_parametros.final_fortalecimiento =    v_parametros.anterior_fortalecimiento
                                            +  v_parametros.cargo_fortalecimiento
                                            +  v_parametros.abono_fortalecimiento
   ...
   LET v_parametros.saldo_total =   v_parametros.final_72 
                                 ...
                                 +  v_parametros.final_excedente
                                 +  v_parametros.final_fortalecimiento
...
PRIVATE FUNCTION fn_inicio()
   ...
   LET v_parametros.anterior_excedente = 0
   LET v_parametros.cargo_excedente = 0
   LET v_parametros.abono_excedente = 0
