################################################################################
# Version: 1.0.0                                                               #
# Fecha ultima modificacion: 06/06/2014                                        #
################################################################################
################################################################################
...
MAIN
   ...
   WHERE proceso_cod = PROC_BDNSVIV
...
FUNCTION fn_drgandrop_bdnsviv()
   DEFINE   l_arr_arch_pend    DYNAMIC ARRAY OF VARCHAR(100), -- Archivos pendientes 
            ...
            l_arr_integrados   DYNAMIC ARRAY OF RECORD -- Detalle archivos a integrar
               ...
               f_presenta    DATE,
               t_registros   DECIMAL(15,0),
               t_sdo_viv97   DECIMAL(15),
               t_aivs_viv97  DECIMAL(18),
               t_sdo_viv92   DECIMAL(15),
               t_aivs_viv92  DECIMAL(18),
               t_fondo72     DECIMAL(15)
            END RECORD,
   ...
   OPEN WINDOW w_inte_cbd WITH FORM "CBDL121"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION integrar
         ...
         FOR l_i_iter = 1 TO l_i_num_arch
            ...
            EXECUTE exe_consulta_sumario INTO   l_arr_integrados[l_i_iter].t_registros,
                                                l_arr_integrados[l_i_iter].f_presenta,
                                                l_arr_integrados[l_i_iter].t_sdo_viv97,
                                                l_arr_integrados[l_i_iter].t_sdo_viv92,
                                                l_arr_integrados[l_i_iter].t_fondo72,
                                                l_arr_integrados[l_i_iter].t_aivs_viv97,
                                                l_arr_integrados[l_i_iter].t_aivs_viv92
         END FOR
...
FUNCTION fn_ejecuta_integracion(p_nombre_archivo, v_ruta_ejecutable, v_ruta_listados)
   ...
   CALL fn_valida_operacion(v_pid,PROC_BDNSVIV,OP_INTEGRA_ARCHIVO) RETURNING v_resultado
   IF v_resultado = 0 THEN
      LET v_folio = 1
      
      # Inicia operación
      CALL fn_actualiza_opera_ini(v_pid,PROC_BDNSVIV,OP_INTEGRA_ARCHIVO,v_folio,"CBDP12",
                            p_nombre_archivo,p_usuario) RETURNING v_resultado
      
      LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/CBDP12 ",
                                       p_usuario," ",
                                       v_pid," ",
                                       PROC_BDNSVIV," ",
                                       OP_INTEGRA_ARCHIVO," ",
