...
#                     BDNSVIV QUE TIENEN SALDO NEGATIVO                       #
#Fecha Inicio      => 20/08/2014                                              #
###############################################################################
...
MAIN
   ...
   IF v_resultado = 0 THEN
      ...
      END IF
      DISPLAY "*******************************************************************"
      DISPLAY "Termino el proceso que exporta los registros con saldo negativo en BDNSVIV"
      DISPLAY " FECHA              : ",TODAY USING 'dd-mm-yyyy'
      DISPLAY " HORA               : ",TIME(CURRENT)
      DISPLAY ""
      DISPLAY "La salida se encuentra en el archivo:"
      DISPLAY v_ruta_envio CLIPPED, "/SALDO_NEGATIVO_BDNSVIV_", v_formato_fecha ,".negativos"
      DISPLAY "*******************************************************************"
   ELSE
      DISPLAY "*******************************************************************"
...
PRIVATE FUNCTION fn_genera_salida()
   ...
   LET v_nombre_archivo = v_ruta_envio CLIPPED,"/EXPORTA_SALDO_NEGATIVO_BDNSVIV.sql"
   ...
   CALL v_archivo.writeLine('unload TO ' || v_ruta_envio CLIPPED || '/SALDO_NEGATIVO_BDNSVIV_' || v_formato_fecha || '.negativos')
   ...
   CALL v_archivo.writeLine('WHERE acciones92 < 0 OR acciones97 < 0 ')
...
PRIVATE FUNCTION fn_genera_reporte()
   ...
   INITIALIZE vhandler TO NULL
	...
	LET v_nombre = v_ruta_ejecutable CLIPPED, "/CBDP24.4rp"
...
REPORT rep_cifras (p_folio) 
   ...
   FIRST PAGE HEADER
      ...
      WHERE acciones92 < 0 OR acciones97 < 0

      SELECT SUM(acciones97)
      INTO v_sdo_97
      FROM cbd_detalle_bdnsviv
      WHERE acciones97 < 0

      IF v_sdo_97 IS NULL THEN
         LET v_sdo_97 = 0
      END IF

      SELECT SUM(acciones92)
      INTO v_sdo_92
      FROM cbd_detalle_bdnsviv
      WHERE acciones92 < 0

      IF v_sdo_92 IS NULL THEN
         LET v_sdo_92 = 0
      END IF
