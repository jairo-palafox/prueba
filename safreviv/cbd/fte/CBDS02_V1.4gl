...
PRIVATE DEFINE v_consulta_bdnsviv         STRING
...
PRIVATE FUNCTION fn_genera_archivo()
   ...
   LET v_consulta_saldo =  "SELECT ",
                           ...
                           "AND sdo.subcuenta IN (4,8,40,42,44)"
   PREPARE exe_consulta_saldo FROM v_consulta_saldo
   DECLARE cur_consulta_saldo CURSOR FOR exe_consulta_saldo

   #Consulta de indicadores para las cuentas IMSS se mandan los indicadores que llegaron en la BDNSVIV
   ...
   FOREACH cur_consulta_trabajador INTO v_id_derechohabiente,
      ...
      IF v_id_derechohabiente IS NOT NULL THEN
         ...
         LET v_detalle.f_paivs = v_fcorte              USING "ddmmyy"

         #Se establece la fecha de saldo para vivienda 97,92 y fondo 72
         LET v_detalle.f_ssv97 = v_fcorte              USING "ddmmyy"
         LET v_detalle.f_ssv92 = v_fcorte              USING "ddmmyy"
         LET v_detalle.f_fondo_anterior = v_fcorte     USING "ddmmyy"
         ...
         #Se valida si el nss tuvo algun saldo para integrarlo en el archivo
         IF v_ind_saldo_imss = 1 OR v_ind_saldo_inf = 1 THEN
            #Se manda a ejecutar la funcion que llena los campos de indicadores
            CALL fn_llena_indicadores()

            #Se inserta el registro en el archivo correspondiente
            CALL fn_inserta_registro()
            
         END IF   #FIN v_ind_saldo_imss = 1 OR v_ind_saldo_inf = 1
...
PRIVATE FUNCTION fn_llena_indicadores()
   #Indicadores para los trabajadores IMSS
   IF v_ind_saldo_imss = 1 THEN
      EXECUTE exe_consulta_bdnsviv USING  v_detalle.nss 
                                    INTO  v_detalle.tipo_trabajador,                      #tipo_trabajador
                                          ...
                                          v_detalle.estatus_acreditados_43su              #ind_acr_43bis_sub
                                                            
   END IF   #FIN Indicadores IMSS
...
PRIVATE FUNCTION fn_inicializa_detalle()
   ...
   LET v_detalle.f_fondo_anterior           = 0 USING '&&&&&&'
   ...
   LET v_detalle.estatus_trabajador_credito = '0'
   LET v_detalle.solicitud_credito          = '0'
   LET v_detalle.f_credito                  = 0 USING '&&&&&&&&'
   LET v_detalle.f_aivs_retiro              = 0 USING '&&&&&&&&'
   LET v_detalle.f_ultimo_movimiento        = 0 USING '&&&&&&&&'
