...
#                     BDNSVIV QUE NO SE ENCUENTRAN EN SAFRE                   #
...
MAIN
   ...
   IF v_resultado = 0 THEN
      ...
      END IF
      DISPLAY "*******************************************************************"
      DISPLAY "Termino el proceso que exporta las cuentas sin registro en SAFRE"
      DISPLAY " FECHA              : ",TODAY USING 'dd-mm-yyyy'
      DISPLAY " HORA               : ",TIME(CURRENT)
      DISPLAY ""
      DISPLAY "La salida se encuentra en el archivo:"
      DISPLAY v_ruta_envio CLIPPED, "/CUENTAS_SIN_REGISTRO_SAFRE_", v_formato_fecha ,".sin_registro"
      DISPLAY "*******************************************************************"
   ELSE
      DISPLAY "*******************************************************************"
...
PRIVATE FUNCTION fn_genera_salida()
   ...
   LET v_nombre_archivo = v_ruta_envio CLIPPED,"/EXPORTA_CUENTA_SIN_REGISTRO.sql"
   ...
   CALL v_archivo.writeLine('unload TO ' || v_ruta_envio CLIPPED || '/CUENTAS_SIN_REGISTRO_SAFRE_' || v_formato_fecha || '.sin_registro')
   ...
   CALL v_archivo.writeLine('WHERE id_derechohabiente IS NULL ')
...
PRIVATE FUNCTION fn_genera_reporte()
   ...
   INITIALIZE vhandler TO NULL
	...
	LET v_nombre = v_ruta_ejecutable CLIPPED, "/CBDP21.4rp"
...
REPORT rep_cifras (p_folio) 
   ...
   FIRST PAGE HEADER
      ...
      WHERE id_derechohabiente IS NULL
