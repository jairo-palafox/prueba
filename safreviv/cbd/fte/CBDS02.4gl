...
MAIN
   ...
   ELSE
      ...
      ELSE
         ...
         --CALL fn_genera_archivo() RETURNING v_resultado
         ...
         IF v_resultado = 0 THEN
            ...
            DISPLAY " HORA               : ",TIME(CURRENT)
            DISPLAY "*******************************************************************"
            CALL fn_ejecuta_genera_archivo_EFIRISS()
         ELSE
            CALL fn_error_opera(p_pid,p_proceso_cod,p_opera_cod)
               RETURNING r_resultado_opera
         END IF
...
FUNCTION fn_genera_device(p_ruta_envio,p_archivo_salida,p_usuario,p_tipo_archivo)
   ...
   IF p_tipo_archivo = ARCHIVO_IMSS THEN
      LET v_cadena = p_ruta_envio CLIPPED, '/SALDOS_', v_fcorte USING "YYYYMMDD", '.saldos'
      LET v_archivo_imss = v_cadena CLIPPED
   ELSE
      LET v_cadena = p_ruta_envio CLIPPED, '/SALDOS_INFONAVIT_', v_fcorte USING "YYYYMMDD", '.saldoinfonavit'
...
FUNCTION fn_genera_query(p_proyecto, p_ruta_envio, p_archivo_salida, p_usuario, p_tipo_archivo)
   ...
   IF p_tipo_archivo = ARCHIVO_IMSS THEN
      LET v_query = v_query, "FROM cbd_saldo_mensual_imss\""
   ELSE
      LET v_query = v_query, "FROM cbd_saldo_mensual_inf\""
   END IF
...
FUNCTION fn_ejecuta_genera_archivo_EFIRISS()--Función que ejecuta el proceso EXTRACTOR DE APORTACIONES EFIRISS
   ...
   ELSE
      ...
      CALL fn_inicializa_proceso(v_pid,v_proceso_cod,v_opera_cod,0,
                                             "CBDS03",v_nom_archivo,v_usuario)
                                    RETURNING r_resultado_opera
      IF ( r_resultado_opera <> 0 ) THEN
         CALL fn_muestra_inc_operacion(r_resultado_opera)
      ELSE
         # Inicia operación
         CALL fn_actualiza_opera_ini(v_pid,v_proceso_cod,v_opera_cod,v_folio,"CBDS03",
         ...
         IF(r_resultado_opera)THEN
            CALL fn_muestra_inc_operacion(r_resultado_opera)
         ELSE
            LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/CBDS03.42r ",
                                             v_usuario," ",
                                             v_pid," ",
                                             v_proceso_cod," ",
                                             v_opera_cod," ",
