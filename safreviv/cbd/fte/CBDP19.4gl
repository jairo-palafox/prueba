...
#Modulo            => CBD                                                     #
#Programa          => CBDP19                                                  #
#Objetivo          => PROGRAMA PARA GENERAR EL ARCHIVO DE REGISTROS DUPLICADOS#
...
MAIN
   ...
   DEFINE v_num_duplicados                INTEGER
   ...
   #Validamos que existan registros duplicados
   SELECT COUNT(*) 
   INTO v_num_duplicados
   FROM cbd_duplicado_bdnsviv

   IF v_num_duplicados IS NOT NULL AND v_num_duplicados > 0 THEN
      ...
      IF v_resultado = 0 THEN
         ...
         DISPLAY "*******************************************************************"
         DISPLAY "Termino el proceso que exporta los registros duplicados de la BDNSVIV"
         DISPLAY " FECHA              : ",TODAY USING 'dd-mm-yyyy'
         DISPLAY " HORA               : ",TIME(CURRENT)
         DISPLAY ""
         DISPLAY "La salida se encuentra en el archivo:"
         DISPLAY v_ruta_envio CLIPPED, "/BDNSVIV_DUPLICADOS_", v_formato_fecha ,".dup_bdnsviv"
         DISPLAY "*******************************************************************"
      ELSE
         DISPLAY "*******************************************************************"
   ...
   ELSE
      DISPLAY "*******************************************************************"
      DISPLAY " El archivo BDNSVIV integrado en el sistema NO tiene NSSs duplicados"
      DISPLAY " FECHA              : ",TODAY USING 'dd-mm-yyyy'
      DISPLAY " HORA               : ",TIME(CURRENT)
      DISPLAY "*******************************************************************"
      CALL  fn_actualiza_opera_fin(p_pid,p_proceso_cod,p_opera_cod)
...
PRIVATE FUNCTION fn_genera_salida()
   ...
   LET v_nombre_archivo = v_ruta_envio CLIPPED,"/EXPORTA_DUPLICADOS.sql"
   ...
   CALL v_archivo.writeLine('unload TO ' || v_ruta_envio CLIPPED || '/BDNSVIV_DUPLICADOS_' || v_formato_fecha || '.dup_bdnsviv')
   CALL v_archivo.writeLine('SELECT')
   CALL v_archivo.writeLine('nss,')
   CALL v_archivo.writeLine('cve_afore,')
   CALL v_archivo.writeLine('to_char(acciones92,\'-##############&.&&&&&&\'),')
   CALL v_archivo.writeLine('to_char(acciones97,\'-##############&.&&&&&&\')')
   CALL v_archivo.writeLine('FROM cbd_duplicado_bdnsviv ')
   CALL v_archivo.writeLine('ORDER BY nss ')
...
PRIVATE FUNCTION fn_genera_reporte()
   ...
   INITIALIZE vhandler TO NULL
	...
	LET v_nombre = v_ruta_ejecutable CLIPPED, "/CBDP19.4rp"
