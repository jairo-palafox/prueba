...
#Modulo            => CUENTAS                                                 #
#Programa          => CBDL03                                                  #
#Objetivo          => EXTRACCION DE SALDOS ESPECIALES                         #
...
MAIN
    ...
    OPEN WINDOW cbdl191 WITH FORM "CBDL191"

    LET ventana = ui.Window.forName("cbdl191")
    LET forma = ventana.getform()

    CALL fn_captura_parametros() RETURNING v_bandera 
    
    CLOSE WINDOW cbdl191
   ...
   INPUT v_fcorte FROM fcorte ATTRIBUTES (UNBUFFERED, WITHOUT DEFAULTS) 
      ON ACTION accept
         IF v_fcorte IS NULL THEN
            CALL fn_mensaje("Extracción de saldos especiales",
...
PRIVATE FUNCTION fn_genera_archivo()
   ...
   ELSE
      ...
      CALL fn_inicializa_proceso(v_pid,v_proceso_cod,v_opera_cod,0,
                                 "CBDP06",v_nom_archivo,p_usuario)
      ...
      ELSE
         # Inicia operación
         CALL fn_actualiza_opera_ini(v_pid,v_proceso_cod,v_opera_cod,null,"CBDP06",
