...
#Modulo       => CBD                                                            #
#Programa     => CBDP25                                                         #
#Objetivo     => BLOQUEAR NSS POR INCONSISTENCIA EN EL DATO                     #        
...
MAIN 
    DEFINE v_ruta                     STRING --Ruta donde se va a generar el archivo de información 
   ...
   DISPLAY "Buscando nss inconsistentes..."
   WHENEVER ERROR CONTINUE
     CALL fn_marca_nss() 
     WHENEVER ERROR STOP
     DISPLAY ""
     DISPLAY "Termina la ejecucion de la funcion que busca nss inconsistentes"
     DISPLAY ""
...
FUNCTION fn_marca_nss()
    DEFINE v_query  STRING--Funcion que carga el arreglo con la información de los nss utilizada para
    DEFINE v_cont INTEGER --crear archivo posteriormente

    LET v_query="execute function fn_cbd_marca_nss_inconsistentes()"
    ...
    FOREACH cur_function INTO g_arr_nss_invalido[v_cont].* --Se retorna toda la información 
                                                           --de los nss y se guarda en el arreglo 
                                                           --de informaciòn de nss a marcar
        LET v_cont = v_cont+1
    END FOREACH 
...
--Se concatena toda la informacón del arreglo en un un string para ser escrito en el archivo 
    ...
    FOR  v_cont=1 TO g_arr_nss_invalido.getLength()-1
        ...
        CALL channel_1.writeLine(v_nss_informacion)--Se escribe el string modificado anteriormente
