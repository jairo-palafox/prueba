...
#Modulo            => CUENTAS                                                 #
#Programa          => CTAC001                                                 #
#Objetivo          => Consulta de catalogo de movimientos                     #
...
MAIN
   ...
   LET p_nombre_menu = "Consulta de movimientos"
   CALL ui.Interface.setText(p_nombre_menu)

   OPEN WINDOW ctac0011 WITH FORM "CTAC0011"
      LET ventana = ui.Window.forName("ctac0011")
   ...
   IF NOT INT_FLAG THEN
      ...
      FOREACH cur_busca_movimientos INTO  v_lista_movimientos[i].movimiento,
         ...
         IF v_lista_movimientos[i].movimiento IS NOT NULL AND v_lista_movimientos[i].movimiento > 0 THEN
            ...
            SELECT MAX(id_cat_his_movimiento) 
            INTO v_id_bitacora
            FROM cat_his_movimiento
            WHERE movimiento = v_lista_movimientos[i].movimiento
            ...
            IF v_id_bitacora IS NOT NULL AND v_id_bitacora > 0 THEN
               ...
               FROM cat_his_movimiento cat
               INNER JOIN cat_tpo_modifica tpo ON tpo.tpo_modifica = cat.tpo_modifica
               WHERE cat.id_cat_his_movimiento = v_id_bitacora
            ELSE
               SELECT 
                  mov.f_actualiza,
                  mov.usuario
               INTO
                  ...
                  v_lista_movimientos[i].usuario
               FROM cat_movimiento mov
               WHERE mov.movimiento = v_lista_movimientos[i].movimiento
