...
MAIN
   ...
   DEFINE v_fecha                    DATE 
   DEFINE v_ind_desmarca             SMALLINT
   ...
   CALL fn_ejecuta_solicitud_desmarca()
    ...
    # Ejecuta función de desmarca de Créditos en Garantía 43BIS
   LET v_qry_fn = "EXECUTE FUNCTION fn_confirma_desmarca_grt(?)"

   PREPARE prp_confirma_desmarca FROM v_qry_fn
   EXECUTE prp_confirma_desmarca USING v_fecha
                                  INTO v_ind_desmarca

   IF(v_ind_desmarca <> 0) THEN
      DISPLAY "OCURRIÓN UN ERROR EN LA EJECUCIÓN DE LA DESMARCA" 
   END IF 
   ...
   DISPLAY "*******************************************************************"
   DISPLAY ""
   DISPLAY "Terminó el envío se solicitudes de desmarca a Procesar: "
   DISPLAY ""
   DISPLAY " PROCESO            : ",v_proceso_desc
   DISPLAY " OPERACIÓN          : ",v_opera_desc
   DISPLAY " FECHA              : ",TODAY USING 'dd-mm-yyyy'
...
PRIVATE FUNCTION fn_configura_ws()
   ...
   #La clave 'cre_2' del catálogo de clientes de webServices corresponde a la solicitud de desmarca
...
PRIVATE FUNCTION fn_respuesta_desmarca(p_solicitud, p_respuesta)
  ...
  EXECUTE exe_inserta_historico USING p_solicitud.id_derechohabiente,
   ...
   IF  p_respuesta.resultOperacion = 1 AND
      (p_respuesta.diagProceso = '   ' OR
       p_respuesta.diagProceso = '') THEN
   ...
   IF p_respuesta.diagProceso = '01' THEN
      ...
      #se desmarca la cuenta con la marca del Instituto indicando que Procesar ya desmarcó al trabajador
