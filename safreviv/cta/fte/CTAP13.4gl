...
#Modulo            => CTA                                                     #
#Programa          => CTAP12                                                  #
#Objetivo          => PROGRAMA PARA GENERAR INFORMACION DE PAGOS PARA DWH     #
...
MAIN
   ...
   DISPLAY ""
   DISPLAY ""
   DISPLAY "EXTRACCIÓN DE INFORMACIÓN DE PAGOS "
   LET v_query = "EXECUTE PROCEDURE sp_dwh_extractor_pagos(?) "
   PREPARE prp_ext_indica FROM v_query
   EXECUTE prp_ext_indica USING v_f_corte
                           INTO v_error, v_isam, v_descripcion
   IF v_error <> 0 THEN
      DISPLAY "SACI-EXCEPCION EJECUCION sp_dwh_extractor_pagos"
   ...
   ELSE
      ...
      DISPLAY ""
      DISPLAY "*******************************************************************"
      DISPLAY "SACI-DWH TERMINO CORRECTAMENTE LA EXTRACCIÓN DE PAGOS"
      DISPLAY " FECHA              : ",TODAY USING 'dd-mm-yyyy'
      DISPLAY " HORA               : ",TIME(CURRENT)
      DISPLAY "*******************************************************************"
   END IF
