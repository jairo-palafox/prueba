...
MAIN
   ...
   DISPLAY "*******************************************************************"
         ...
         LET v_query = "EXECUTE PROCEDURE sp_dwh_crea_tabla_aclaratorios()"
         PREPARE prp_crea_tablas FROM v_query
         EXECUTE prp_crea_tablas INTO v_error, v_isam, v_descripcion
         IF v_error <> 0 THEN
            DISPLAY "SACI-EXCEPCION EJECUCION sp_dwh_crea_tabla_aclaratorios"
         ...
         ELSE
            ...
            DISPLAY ""
            DISPLAY "EXTRACCIÓN DE INFORMACIÓN DE INDICADORES "
            DISPLAY ""
            DISPLAY ""
            DISPLAY "EXTRACCIÓN DE INFORMACIÓN DE  ACLARATORIOS SP"
            LET v_query = "EXECUTE PROCEDURE sp_dwh_extractor_aclaratorios() "
            PREPARE prp_ext_indica FROM v_query
            EXECUTE prp_ext_indica  INTO v_error, v_isam, v_descripcion
            IF v_error <> 0 THEN
               DISPLAY "SACI-EXCEPCION EJECUCION sp_dwh_extractor_aclaratorios"
               DISPLAY "ERROR       ",v_error
               DISPLAY "ISAM ERROR  ",v_isam
               DISPLAY "DESCRIPCION ",v_descripcion
               CALL fn_error_opera(p_pid,p_proceso_cod,p_opera_cod) 
