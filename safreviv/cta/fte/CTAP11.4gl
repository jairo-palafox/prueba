...
MAIN
   ...
   LET v_query = "EXECUTE PROCEDURE sp_dwh_crea_tablas()"
   PREPARE prp_crea_tablas FROM v_query
   EXECUTE prp_crea_tablas INTO v_error, v_isam, v_descripcion
   IF SQLCA.SQLCODE <> 0 THEN
      DISPLAY "SACI-EXCEPCION EJECUCION sp_dwh_crea_tablas"
   ...
   ELSE
      ...
      LET v_query = "EXECUTE PROCEDURE sp_dwh_extractor_indicadores(?) "
      PREPARE prp_ext_indica FROM v_query
      EXECUTE prp_ext_indica USING v_f_corte
                              INTO v_error, v_isam, v_descripcion
      IF v_error <> 0 THEN
         DISPLAY "SACI-EXCEPCION EJECUCION sp_dwh_extractor_indicadores"
         DISPLAY "ERROR       ",v_error
         DISPLAY "ISAM ERROR  ",v_isam
         DISPLAY "DESCRIPCION ",v_descripcion
         CALL fn_error_opera(p_pid,p_proceso_cod,p_opera_cod) 
