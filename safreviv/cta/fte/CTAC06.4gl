...
DEFINE v_ind_estado_cuenta SMALLINT,
       ...
       r_valida_nss        SMALLINT,
       v_busca_nss         SMALLINT,
       v_consulta_rojo     CHAR (11)
...
--******************************************************************************
   ...
   IF v_datos.nombre_completo IS NOT NULL THEN
      SELECT nss
      INTO v_datos.nss
      FROM afi_fondo72
      WHERE nombre = v_datos.nombre_completo
         ...
         IF v_datos.rfc IS NOT NULL THEN
            ...
            IF v_rfc_filtro.getLength() > 10 THEN  #RFC > 10
               ...
               IF v_respuesta = 1 THEN #Se ejecuta la consulta con RFC10
                  ...
                  select a.rfc [1,10] 
                    INTO v_consulta_rojo
                    from afi_derechohabiente a,cat_nss_rojo c
                   where a.nss = c.nss
                     and c.estado_rojo = 1
                     and a.rfc[1,10] = v_rfc10

                    DISPLAY v_consulta_rojo
                 IF v_rfc10 = v_consulta_rojo THEN 
                    CALL fn_mensaje("Atención",v_msj_alerta,"stop")
                ELSE 
         ...
         ELSE
            ...
            DIALOG ATTRIBUTES(UNBUFFERED)
               ...
               BEFORE ROW 
                  ...
                  CALL fn_credito_vivienda(v_id_dh_f72, 0)
