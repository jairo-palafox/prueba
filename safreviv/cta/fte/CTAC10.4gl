...
PRIVATE FUNCTION fn_nueva_busqueda()
   ...
   CONSTRUCT v_condicion ON consec_cuenta, nss_afo_recep, rfc_afo_recep, tia.curp, nombre_trabajador,folio,f_movimiento
                         FROM consec_cuenta, nss, rfc, curp, nombre,folio_liquida_1,fecha_liquida
      BEFORE CONSTRUCT
         CLEAR FORM
      ...
      ON ACTION ACCEPT
         ...
         LET v_datos.folio= GET_FLDBUF(folio_liquida_1)
         LET v_datos.f_movimiento= GET_FLDBUF(fecha_liquida)
         LET INT_FLAG = FALSE
--******************************************************************************
         ...
         LET string_fecha=GET_FLDBUF(fecha_liquida)
         LET string_folio=GET_FLDBUF(folio_liquida_1)
   ...
   IF GET_FLDBUF(fecha_liquida) IS NOT NULL THEN
...
--******************************************************************************
  ...
  { 
      IF (v_datos.consec_cuenta IS NOT NULL OR 
        ...
        LET v_consulta_cliente = "SELECT COUNT(*) FROM afi_decreto WHERE ",v_condicion 

        PREPARE exe_consulta_existe_usuario_pendientes FROM v_consulta_cliente
        EXECUTE exe_consulta_existe_usuario_pendientes INTO v_cuenta_usuarios
}
      ...
      END FOREACH
{
      IF v_cuenta_existe_cliente==1 THEN
             LET v_consulta_cliente =   "SELECT ",
                                    "afi.id_decreto, ",
                                    "afi.consec_cuenta," ,
                                     "CASE WHEN tia.nss_icefa LIKE '%     %' THEN \n",
                                    "afi.nss ELSE tia.nss_icefa END CASE, \n",
                                    "CASE WHEN tia.rfc_icefa LIKE '%     %' THEN \n",
                                    "afi.rfc ELSE tia.rfc_icefa END CASE, \n",
      ...
      IF v_cuenta_existe_cliente>=1 THEN
        IF v_bandera_aceptar==1 THEN
            ...
            DISPLAY ARRAY v_lista_clientes_cursor TO lista_clientes.*  ATTRIBUTES (CANCEL=FALSE)
      ...
      IF v_cuenta_existe_cliente==0 AND NOT string_nss.equals('*') AND NOT string_rfc.equals('*') AND NOT string_curp.equals('*') AND
       ...
       --   EXECUTE exe_consulta_existe_cliente INTO v_cuenta_existe_cliente
          ...
          IF v_cuenta_existe_cliente==2 THEN
                CALL fn_mensaje("Consulta Decreto",
                         "Se encontraron registros ejecutados \n con el criterio de búsqueda. \n",
                         "about")
                RETURN 1
          END IF
...
--******************************************************************************
 ...
 --  CALL fn_datos_generales()
   ...
   DIALOG   ATTRIBUTES(UNBUFFERED)
      ...
      DISPLAY ARRAY v_lista_decreto TO --f_movimiento,folio,origen_traspaso,aivs_viv92,sdo_viv92
      lista_decreto.* END DISPLAY
      ...
      BEFORE DIALOG
         ...
         DISPLAY v_datos.f_movimiento    TO fecha_liquida
         DISPLAY v_datos.folio    TO folio_liquida_1
...
PRIVATE FUNCTION fn_consulta_decreto()
   ...
   LET v_consulta_decreto =   "SELECT ",
                              ...
                              "WHERE tia.id_decreto = ? AND tia.result_operacion= '10' "
   ...
   SELECT SUM(tia.aivs_viv92), SUM(tia.aivs_viv92)
   INTO v_saldo_total, v_acciones_total
   FROM tia_det_traspaso tia
   WHERE tia.id_decreto = v_datos.id_decreto
