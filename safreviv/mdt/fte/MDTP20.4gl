...
#                     acreditados con origen recurrente                        # 
#Autor             => Hugo César Ramírez García                                #
#Fecha inicio      => Agosto 09, 2012                                          #
################################################################################
DATABASE safre_viv
...
DEFINE v_conteo_detalle     INTEGER,
   ...
   SELECT cant_reg_detalle
     INTO v_total_sum
     FROM safre_tmp:tmp_mdt_sum_recurrente
    WHERE 1 = 1
...
DEFINE v_consulta   STRING,
   ...
   PREPARE prp_crea_tbl_tmp FROM "CREATE TEMP TABLE mdt_tmp_inconsistencias_val(mensaje CHAR(254))"
   EXECUTE prp_crea_tbl_tmp

   LET v_consulta = "EXECUTE PROCEDURE sp_valida_carga_recurrente()"
   ...
   SELECT COUNT(*) 
     INTO v_conteo_inc
     FROM mdt_tmp_inconsistencias_val
    WHERE 1 = 1
