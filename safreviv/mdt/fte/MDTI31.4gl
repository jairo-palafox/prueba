...
DEFINE p_folio                LIKE glo_folio.folio,
   ...
   IF(fgl_report_loadCurrentSettings(v_ruta_ejecutable CLIPPED||"/MDTI311.4rp"))THEN
      # Salida del reporte
      CALL fgl_report_selectDevice("PDF")
      LET v_nombre_reporte = p_usuario_cod CLIPPED, "-",p_archivo_batch,"-",--"-MDTP31-", 
                             p_pid USING "&&&&&", "-", 
                             p_proceso_cod USING "&&&&&", "-", 
      ...
      CASE p_estado
         WHEN 50 # Abonos
            LET v_texto_rpt = "LIQUIDACIÓN ABONO MANDATOS"
            LET v_consulta = "\n SELECT tpo.tpo_mandato,",
                             ...
                             "\n        JOIN mdt_ctr_aplica_mandato apctr",
                             "\n     ON apl.id_ctr_aplica_mandato = apctr.id_ctr_aplica_mandato",
                             "\n        JOIN",
