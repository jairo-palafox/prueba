...
#Modulo            => MDT                                                      #
#Programa          => MDTL21                                                   #
#Objetivo          => Integracion mandatos con origen recurrente ACR           #
#Autor             => Hugo César Ramírez Gracía                                #
#Fecha Inicio      => Junio 11 2012                                            #
#Modifico     => Alexandro Hollmann, EFP - Nota: Cambio definicion de temporal #
...
DEFINE p_tipo_ejecucion    SMALLINT,
   ...
   LET v_proceso = 1303 # validacion mandatos origen recurrente
   LET v_operacion = 2
   ...
   OPEN WINDOW w_solicitud_mandatos WITH FORM g_ruta_bin CLIPPED||"/MDTL211"
...
DEFINE v_archivo             LIKE glo_ctr_archivo.nombre_archivo
   ...
   OPEN WINDOW w_detalle_solicituides WITH FORM v_ruta_ejecutable CLIPPED||"/MDTL212"
      #Se asigna el titulo de la ventana
      IF(p_titulo IS NOT NULL)THEN
          CALL ui.Interface.setText(p_titulo)
         LET v_ventana = ui.Window.getCurrent()
      ...
      DISPLAY ARRAY v_detalle_solicitudes TO tbl_solicitudes.* 
        ...
        BEFORE DISPLAY
           LET v_consulta = "\n SELECT rec.cve_mandato, rec.fec_ini_mandato, COUNT(rec.nss)",
                            "\n FROM safre_tmp:tmp_acr_transf_30 rec  ",
                            "\n GROUP BY 1,2 "
        ...
        ON ACTION Aceptar
          -- LET v_pid = 0
           ...
           ELSE
              ...
              LET v_folio = 0
              CALL fn_actualiza_opera_ini(v_pid,v_proceso,v_operacion,v_folio,'MDTL21','',p_usuario_cod)
                    RETURNING r_bandera
              IF(r_bandera = 0)THEN
                 CALL fn_recupera_arch_cargado(v_proceso,v_operacion) RETURNING v_archivo
                 #Construye comando
                 LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/MDTE21.42r ",
                                                 p_usuario_cod," ",
                                                 v_pid," ",
                                                 v_proceso," ",
                                                 v_operacion," ",
