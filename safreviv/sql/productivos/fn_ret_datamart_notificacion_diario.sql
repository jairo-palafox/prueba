...
CREATE FUNCTION "safreviv".fn_ret_datamart_notificacion_diario(p_folio_datamart DECIMAL(9,0),p_folio_notificacion DECIMAL(9,0))
    ...
    FOREACH SELECT id_datamart,
			...
			WHERE  rd.regimen           = 73 
			  AND  folio                = p_folio_datamart
        		...
        		--      1    - ENVIADO
        		--      3    - YA TUVO RETIRO
        		--      4    - SUMA DE SALDOS MENOR A 0
        		--      5    - MARCA NO CONVIVE
        		--      6    - UN ID_DATAMART PROCESADO MAS RECIENTEMENTELO NOTIFICARA

        		-- Se actualiza a 6 si el NSS esta intentado ser procesado por un id_datamart anterior
        		UPDATE ret_notificacion
        		SET    ind_envio = 6
        		WHERE  nss       = v_nss
        		AND    ind_envio IS NULL; -- Se excluira del proceso

				-- Si ya fue procesado
	    		UPDATE tmp_ret_notificacion
        		SET    ind_envio = 6
        		WHERE  nss = v_nss
        		AND    ind_envio IS NULL; -- Se excluira del proceso
