...
CREATE FUNCTION "safreviv".fn_notifica_pagos(p_folio_proc_operativo  DECIMAL(9,0),
...
RETURNING SMALLINT, INTEGER, INTEGER, INTEGER
	...
	--	   AND cta.id_derechohabiente = ind.id_derechohabiente
     --    AND cta.folio = p_folio_proc_operativo
      ...
      IF (v_imp_am_cre = 0 OR v_imp_am_cre IS NULL) THEN
         IF (v_imp_ap_pat IS NOT NULL AND v_imp_ap_pat <> 0) THEN
            LET v_query = 'INSERT INTO ' || v_nom_tabla || '(folio,nss,aportacion,bimestre,ejercicio) VALUES ('
										  || p_folio_notificacion || ',"' || v_nss || '","' || TRIM(TO_CHAR(v_imp_ap_pat,"###,###,##&.&&")) ||'","' ||v_bimestre[5,6]|| '","' ||v_bimestre[1,4]|| '")';
			EXECUTE IMMEDIATE v_query;
         END IF
      END IF
	END FOREACH;
   ...
   RETURN 0, v_correo, v_correo, v_correo;
END FUNCTION;
