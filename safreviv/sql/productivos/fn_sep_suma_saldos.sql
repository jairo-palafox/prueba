...
CREATE FUNCTION "safreviv".fn_sep_suma_saldos(v_derechohabiente DECIMAL (9,0), v_subcuenta SMALLINT) RETURNING DECIMAL(12,2) AS saldo;
...
DEFINE v_qry        CHAR(254);
   ...
   FOREACH SELECT tabname 
		   ...
		   LET v_qry = "SELECT NVL(SUM(monto_pesos),0) FROM "||curr_table||" WHERE subcuenta = ? AND movimiento =  381 AND id_derechohabiente = ?";
		   ...
		   LET v_qry = "SELECT NVL(SUM(monto_pesos),0) FROM "||curr_table||" WHERE subcuenta = ? AND movimiento =  382 AND id_derechohabiente = ?";
