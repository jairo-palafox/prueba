...
CREATE PROCEDURE "safreviv".sp_reversa_desmarca_decreto( p_id_decreto     DECIMAL(9,0),
                                      p_marca        SMALLINT,
...
--   set debug file to "rev_desmarca_cuenta.log";
--   trace on;
  ...
  FOREACH
    ...
    INTO   v_fecha_ini    ,
           ...
           v_marca_resulta,
           v_ind_habilita
      FROM sfr_marca_historica_decreto c, 
           sfr_marca t
  ...
  IF v_registros = 1 THEN
     ...
     IF ( v_ind_habilita > 0 ) THEN  -- Habilita o Inhabilita
        ...
        DELETE                              -- Borra registro inhabilitado en
             FROM sfr_marca_activa_decreto                -- marcas activas
        ...
        DELETE                              -- Borra registro in/hablitado de
          FROM sfr_marca_historica_decreto          -- historico
         WHERE id_decreto   = p_id_decreto
           AND marca        = v_marca_resulta
           AND n_referencia = p_n_referencia
...

{
     ...
     UPDATE sfr_marca_historica_decreto                  -- Restaura valores en historico
        SET f_fin        = NULL    ,
     ...
     INSERT INTO sfr_marca_activa_decreto             -- Restaura registro en marcas activas
     SELECT id_decreto    ,
            marca         ,
            n_referencia  ,
            f_inicio      ,
