...
CREATE PROCEDURE "safreviv".sp_dwh_extractor_reverso_opera()
RETURNING INTEGER, INTEGER, CHAR(200);
   ...
   CREATE TEMP TABLE tmp_mov_rev_op(f_liquida DATE ,
   ...
   LET v_query = "INSERT INTO tmp_mov_rev_op " ||
      "SELECT f_liquida, id_derechohabiente, "||
        ...
        " WHERE subcuenta IN (4,8,44) "||
          " AND movimiento IN (521,841,1212,1722) ";
   ...
   CREATE INDEX xtmp_mov_rev_op ON tmp_mov_rev_op (folio_liquida, id_derechohabiente);

   INSERT INTO safre_sdo@vivws_tcp:saci_reverso_opera
        SELECT c.nss,
               b.monto_pesos,
               b.monto_acciones,
               YEAR(f_liquida),
   ...
   INSERT INTO safre_sdo@vivws_tcp:glo_ctr_dwh
        SELECT TODAY,
               "saci_reverso_opera",
               TODAY,
               ( SELECT COUNT(*) FROM safre_sdo@vivws_tcp:saci_reverso_opera ),
               1
          FROM SYSTABLES
         WHERE tabid = 1;
END PROCEDURE
