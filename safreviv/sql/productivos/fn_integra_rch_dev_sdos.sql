...
CREATE FUNCTION "safreviv".fn_integra_rch_dev_sdos(p_d_pid DECIMAL(9,0),
                                        p_v_usuario CHAR(20),
   ...
   DEFINE tmp_saldo_vivienda92          DECIMAL(13,2); -- 13 enteros 2 decimales
   ...
   DEFINE v_ax_tpo_transf_rchdse        CHAR(2); -- tipo de transferencia rechazos DSE
   ...
   LET v_ax_tpo_transf_rchdse = "56"; -- 56-Rechazo de DSE ACR
   ...
   SELECT consecutivo_lote
     INTO v_ax_lote
     FROM safre_tmp:tmp_rch_devolucion_enc;
   ...
   FOREACH
    SELECT *
      ...
      FOREACH
         ...
         LET dseh_tpo_transferencia     = v_ax_tpo_transf_rchdse;
         ...
         -- se inserta registro
         INSERT INTO safre_tmp:tmp_deudor_rech_devol_saldo (
                     id_dse_devolucion,
                     id_derechohabiente,
                     nss,
                     folio_archivo)
         ...
         IF tmp_diagnostico[10,12] IS NOT NULL AND tmp_diagnostico[10,12] <> "   " AND
            ...
            INSERT INTO dse_his_devolucion (
                ...
                VALUES (dseh_id_dse_grp_devolucion,
                        ...
                        dseh_f_proceso);
         ELSE
            -- si el diagnóstico fue nulo continúa con el siguiente registro
            CONTINUE FOREACH;
         END IF
   ...
   -- actualiza estadísticas a la tabla de históricos DSE
   ...
   EXECUTE PROCEDURE fn_act_dse_ctr_archivo(p_d_folio, p_d_pid, v_ax_tpo_transf_rchdse, v_ax_id_lote_acpt, v_ax_id_lote_rech);
