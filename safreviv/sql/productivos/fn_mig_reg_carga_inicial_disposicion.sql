...
CREATE FUNCTION "safreviv".fn_mig_reg_carga_inicial_disposicion(p_usuario_cod CHAR(20), p_folio DECIMAL(9,0),
...
RETURNING INTEGER, INTEGER, VARCHAR(250)
   ...
   DEFINE tmp_ret_det_valor_partidicpaion   DECIMAL(14,0);
...
--TRACE "verificando los primeros tres campos";
      ...
      FROM
         ret_matriz_derecho
      WHERE
             tpo_retiro           = tmp_ret_det_tpo_retiro
      ...
      FROM
         cat_afore
      WHERE
         afore_cod = tmp_ret_det_cve_afore;
      ...
      LET v_pesos_aiv97 = ret_disposicion_aivs_viv97 * 1; -- falta el valor del fondo
      LET v_pesos_aiv92 = ret_disposicion_aivs_viv92 * 1; -- falta el valor del fondo
   ...
   --TRACE "rechazando registros por causa de errores";
   ...
   IF ( v_reg_det_rechazados > 0 ) THEN
      ...
      WHERE
         ...
         FROM
            afi_derechohabiente
         WHERE
            id_derechohabiente = ret_disposicion_id_derechohabiente;
      ...
      --LET v_si_resultado = v_error_det_lote_invalido;
      --LET isam_err = 0;
      --LET v_c_msj = "Se rechaza el lote por error en validación de detalle. Puede revisar los rechazos en la consulta general.";
   ...
   LET ret_cza_disposicion_f_valor_transferencia = TODAY; 
   LET ret_cza_disposicion_precio_fondo          = 0.0; 
	 LET ret_cza_disposicion_total_registros       = 0; 
	 LET ret_cza_disposicion_total_importe         = 0; 
