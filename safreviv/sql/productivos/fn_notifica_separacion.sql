...
CREATE FUNCTION "safreviv".fn_notifica_separacion(p_folio_proc_operativo  DECIMAL(9,0),
...
RETURNING SMALLINT, INTEGER, INTEGER, INTEGER
   ...
   FOREACH 
		SELECT
         sep.id_derechohabiente,
         afi.nss
      INTO 
         v_id_derechohabiente,
         v_nss
      FROM sep_notifica_op27 sep
      INNER JOIN afi_derechohabiente afi ON afi.id_derechohabiente = sep.id_derechohabiente
      WHERE  sep.id_derechohabiente IN (SELECT noti.id_derechohabiente FROM afi_ind_notifica noti WHERE noti.tpo_notificacion IN (1,2))
      AND sep.folio = p_folio_proc_operativo
      AND sep.ind_notificado = 0
