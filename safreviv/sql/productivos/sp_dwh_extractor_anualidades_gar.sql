...
CREATE PROCEDURE "safreviv".sp_dwh_extractor_anualidades_gar()
RETURNING INTEGER, INTEGER, CHAR(200);
   ...
   CREATE TEMP TABLE tmp_mov_anu(f_liquida DATE ,
   ...
   LET v_query = "INSERT INTO tmp_mov_anu " ||
   ...
   CREATE INDEX xtmp_mov_anu ON tmp_mov_anu (folio_liquida, id_derechohabiente);

   INSERT INTO safre_sdo@vivws_tcp:saci_anualidades_gar
        SELECT c.nss,
               a.num_credito,
               b.monto_pesos,
               b.monto_acciones,
   ...
   INSERT INTO safre_sdo@vivws_tcp:glo_ctr_dwh
        SELECT TODAY,
               "saci_anualidades_gar",
               TODAY,
               ( SELECT COUNT(*) FROM safre_sdo@vivws_tcp:saci_anualidades_gar ),
               1
          FROM SYSTABLES
         WHERE tabid = 1;
   RETURN r_sql_err, r_isam_err, r_err_txt;
