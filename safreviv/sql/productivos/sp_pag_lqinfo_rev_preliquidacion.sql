...
CREATE PROCEDURE "safreviv".sp_pag_lqinfo_rev_preliquidacion( 
                 p_folio               DECIMAL(9,0),
  ...
  DEFINE v_marca                SMALLINT;
   ...
   -- se borran los datos de la tabla de excepcion
   DELETE
     FROM pag_excep_preliquida
    WHERE folio = p_folio;
    
   DELETE
     FROM pag_excep_preliq_pag
    WHERE folio = p_folio;
    
   DELETE
     FROM pag_lqinfo_preliquida
