...
--TRACE ON;
  ...
  FOREACH
    ...
    IF v_destino_ap_viv = 2 THEN  --10
       ...
       IF v_tipo_trabajador = 2 THEN
          --##Se valida si no tiene marca confirmada, si es el caso se envía la información a dis_info_inconsistentes
          IF v_edo_credito = 0 THEN -- Existe crédito vigente
             IF v_marca_confirmada IS NULL THEN --Crédito No Confirmado
                INSERT INTO dis_info_inconsistente VALUES(v_derechohabiente_pag,
                                                          v_id_referencia,
                                                          p_folio_disp,
                                                          v_destino_ap_viv,
    ...
    ELSE -- Del destino de la aportacion 
       ...
       IF v_destino_ap_viv = 1 THEN -- Dispersión a INFONAVIT
          ...
          --Se agrega validación para verificar si es el nuevo tipo de crédito Mejoravit
  ...
  LET v_char = "  Preliquidación Especial aclaratorio finalizo correctamente";
