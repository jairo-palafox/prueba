...
CREATE FUNCTION "safreviv".fn_consulta_marca_ws(p_nss CHAR(11))
RETURNING SMALLINT, CHAR(11), CHAR(4), VARCHAR(140), DECIMAL(12,2), DECIMAL(12,2)
      ...
      IF vcodResp <> "2000" AND
         ...
         IF vcodResp = "208" OR
            ...
            SELECT desc_rechazo
              INTO vdescResp
              FROM cat_rechazo_mf
             WHERE cod_rechazo = vcodResp[2,3];
         ELSE
            IF EXISTS (SELECT cod_rechazo
                         FROM cat_rechazo_mf
                        WHERE cod_rechazo = vcodResp[2,4]) THEN
               SELECT desc_rechazo
                 INTO vdescResp
                 FROM cat_rechazo_mf
                WHERE cod_rechazo = vcodResp[2,4];
            ELSE
               LET vcodRch = vcodResp[2,4];

               IF EXISTS (SELECT desc_estado
                            FROM cat_rch_acreditado
                           WHERE estado = vcodRch) THEN
   ...
   IF vbnd = 0 THEN
      ...
      IF vcodResp = 208 THEN
         ...
         LET vdescResp = "El saldo de la subcuenta de vivienda registrado en el Infonavit es $0.0";
   ...
   IF vbnd = 0 THEN
      IF EXISTS (  ---Se verifica si está ya inciado el trámite
         ...
         FOREACH
            SELECT h.diagnostico, h.id_cre_tramite
              INTO vcodResp, v_id_cre_tramite
         ...
         IF vcodResp[1] = "3" THEN
            IF vcodResp = "3000" OR vcodResp = "3241" THEN
               IF vcodResp = "3241" THEN
            ...
            ELSE
               IF vcodResp = "3-1" THEN
                  LET vdescResp = "FALLA EN COMUNICACIÓN CON PROCESAR, SE GENERARÁ REINTENTO EN 24 HORAS";
               ELSE
                  IF vcodResp = "208" OR
                     vcodResp = "211" OR
                     vcodResp = "212" OR
                  ...
                  ELSE
                     IF EXISTS (SELECT cod_rechazo
                        ...
                        SELECT desc_rechazo
                         ...
                         WHERE cod_rechazo = vcodResp[2,4];
                     ELSE
                        IF EXISTS (SELECT desc_rechazo
                                     FROM cat_rechazo
                                    WHERE cod_rechazo = vcodResp[2,4]
                                      AND tpo_rechazo = "RCH") THEN
               ...
               END IF
            END IF
         ELIF vcodResp = "2213" THEN
            IF EXISTS (SELECT w.diagnostico
                         FROM cre_tramite c,
                              cre_acreditado a,
                              cta_his_marca_ws w
                        WHERE c.id_cre_tramite     = v_id_cre_tramite
                          ...
                          AND w.situacion          = 2
                          AND w.marca_procesar     = "04"
                          AND w.diagnostico        = "   "
                          AND w.f_actualiza       >= a.f_otorga) THEN

               SELECT w.diagnostico
                INTO  v_diagnostico
                FROM cre_tramite c,
                     cre_acreditado a,
                     cta_his_marca_ws w
               WHERE c.id_cre_tramite     = v_id_cre_tramite
                 ...
                 AND w.situacion          = 2
                 AND w.marca_procesar     = "04"
                 AND w.f_actualiza       >= a.f_otorga;

               IF v_diagnostico = "   " OR v_diagnostico = "241" THEN
                  LET vcodResp  = "3000";
                  LET vdescResp = "MARCA PROCEDENTE";
               ELSE
                  LET vcodResp = "3"||v_diagnostico;
               ...
               UPDATE cre_his_tramite
                  SET diagnostico    = vcodResp
                WHERE id_cre_tramite = v_id_cre_tramite;
            ELSE
               LET vdescResp = "INFONAVIT: MARCA CRÉDITO EN TRÁMITE";
               LET vdescResp = "Solicitud de marca en la Afore en trámite. Intentar en las 48 horas siguientes";
