...
CREATE PROCEDURE "safreviv".sp_dis_apo_subs5(v_folio DECIMAL(9,0))
...
END EXCEPTION
  ...
  --SET DEBUG FILE TO '/ds/safreviv_int/dis/sp_dis_apo_subs5.TRACE';
  --TRACE ON;
  ...
  SELECT apo.id_dis_interface_ef,
         ...
         afi.ap_materno_af,
         afi.curp
  FROM   tmp_dis_ctr_aps_tns apo,
         afi_derechohabiente afi
  WHERE  apo.id_derechohabiente = afi.id_derechohabiente
  INTO TEMP tmp_dis_apo_subs5;

  UPDATE STATISTICS FOR TABLE tmp_dis_apo_subs5;  
  ...
  FOREACH
    ...
    INTO   v_id_dis_interface_ef,
           ...
           v_curp
    FROM   tmp_dis_ctr_aps_tns apo,
           tmp_dis_apo_subs5 afi
    WHERE  apo.id_dis_interface_ef = afi.id_dis_interface_ef
    ...
    --Identifica Tipo de Crédito
    IF (v_tpo_credito_ocg = "7") OR
       (v_tpo_credito_ocg = "8") THEN
       LET v_tpo_credito = 5;
    END IF
    ...
    IF (v_resultado  = 0     OR 
       ...
       LET v_estado   = 21; -- 21 ~V Confirmado No Acreditado
    ...
    ELSE ------------ACREDITADO existe en BD
       ...
       IF v_bnd_existe_transacciones = 1 THEN --------------SI existe(NSS) en BD TRANSACCIONES
          ...
          IF v_resultado = 1 THEN -----------------------SI esta VIGENTE
             IF v_tpo_credito = 2 THEN ------------- Apoyo INFONAVIT (ANT 7)
                LET v_concepto = 107; -- 107 . Recurrente por Facturar                  
                --TRANSACCIÓN RECURRENTE POR FACTURAR
                   
                IF v_fecha_carga = "05-17-2011" THEN
                   LET v_estado   = 22; -- 22 . Confirmado Pago Anticipado
                   LET v_concepto = 0;
                END IF
             END IF

             IF v_tpo_credito = 5 THEN ------------- COFINAVIT 08 (ANT 8) 
                LET v_concepto = 807; -- 807 . Recurrente por Facturar                   
                --TRANSACCIÓN RECURRENTE POR FACTURAR 
          ...
          ELSE--------------------------------------------------Estado DIFERENTE A VIGENTE
             ...
             IF v_tpo_credito = 2 THEN ------------- Apoyo INFONAVIT (ANT 7)
                LET v_concepto = 108; -- 108 . Devolución                   
                --TRANSACCIÓN DEVOLUCIÓN  
             END IF

             IF v_tpo_credito = 5 THEN ------------- COFINAVIT 08 (ANT 8)
                LET v_concepto = 808; -- 808 . Devolución                   
                --TRANSACCIÓN DEVOLUCIÓN
             END IF
          END IF
       ELSE ------------------------------------------------ NO existe en BD TRANSACCIONES
          ...
          IF v_resultado = 1 THEN --SI esta VIGENTE
             IF v_tpo_credito = 2 THEN ------------- Apoyo INFONAVIT (ANT 7)
                --LET v_concepto = 109; -- 109 . Alta por Facturar
                LET v_concepto = 107; -- 107 . Recurrente por Facturar
             END IF
            
             IF v_tpo_credito = 5 THEN ------------- COFINAVIT 08 (ANT 8)
                LET v_concepto = 807; -- 807 . Recurrente por Facturar 
                --LET v_concepto = 809; -- 809 . Alta por Facturar
             END IF

             --TRANSACCIÓN ALTA POR FACTURAR            
          ELSE -- Estado DIFERENTE A VIGENTE
             IF v_tpo_credito = 2 THEN ------------- Apoyo INFONAVIT (ANT 7)
                LET v_concepto = 108; -- 108 . Devolución
             END IF
