...
CREATE FUNCTION "safreviv".fn_cre_insrt_ctr_arch_homologa(p_c_nom_archivo CHAR(40),
   ...
   LET v_ax_id_proceso    = 334; -- 334 -Homologación TRM SAFRE
   LET v_ax_error         = 0;
   LET v_isam_err         = 0;
   ...
   IF p_i_estado = 10 THEN
      -- se cuenta el número de registros insertados en la tabla de detalle
      -- no envían sumario, se asume que ésta validación es procedente
      LET v_ax_cont_regs = 1;

      -- se valida la existencia en sumario
      IF v_ax_cont_regs <> 1 THEN
         --TRACE("ERROR: NO EXISTE SUMARIO");
         -- se asigna estatus erróneo ya que no se encontraron registros en detallle
         LET p_i_estado = 30;
      ...
      ELSE
         ...
         IF v_ax_sum_regs_01 <> v_ax_cont_regs_01 THEN
            --TRACE("ERROR: NO HAY REGISTROS EN EL ARCHIVO DE HOMOLOGACIÓN DE TRM - Saci SAFRE");
            -- se asigna estatus errÓneo ya que no se encontraron registros detalle
            LET p_i_estado = 30;
   ...
   -- si el archivo está marcado como Válido, continua con el conteo de regs sin originación
   IF p_i_estado = 10 THEN
      -- se cuenta el número de registros insertados en la temporal (31-HOMOLOGACIÓN TRM-SAFRE)
      SELECT COUNT(*)
        INTO v_ax_cont_regs
        FROM safre_tmp:tmp_homologa_trm;
      ...
      IF v_ax_tot_registros = 0 THEN
         ...
         -- se asigna estatus errúneo ya que no se encontraron registros detalle
         LET p_i_estado       = 30;
   ...
   -- si el archivo está marcado como Válido, se inserta registro en la tabla de control
   IF p_i_estado = 10 THEN
