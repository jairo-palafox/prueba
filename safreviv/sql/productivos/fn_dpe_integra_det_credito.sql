...
CREATE FUNCTION "safreviv".fn_dpe_integra_det_credito(p_usuario_cod CHAR(20),
                                             p_pid DECIMAL(9,0),
...
LET v_resultado_saldo                 = 0;
   ...
   --SET DEBUG FILE TO "/ds/safreviv_int/BD/trace.dpe.integra_det_infonavit.txt";
   -- [Error]
   ...
   FOREACH
      ...
      IF (v_det_tpo_registro <> 2 OR v_det_tpo_registro IS NULL) THEN
         ...
         EXECUTE PROCEDURE sp_dpe_inserta_rechazo(
            p_folio,2,v_d_id_referencia,"No",84,
      ...
      IF (v_det_nss IS NULL) THEN
         -- ERROR de detalle.
         EXECUTE PROCEDURE sp_dpe_inserta_rechazo(
            p_folio,2,v_d_id_referencia,"No",85,
            "NSS -"||v_det_nss);
         LET v_si_resultado = 1;
         LET v_det_estado_solicitud = 2; -- Rechazada
      END IF
      
      --trace "Valida el credito";
      LET err_txt = "Valida el credito";
      IF (v_det_credito IS NULL) THEN
         -- ERROR de detalle.
         EXECUTE PROCEDURE sp_dpe_inserta_rechazo(
            p_folio,2,v_d_id_referencia,"No",86,
            "Credito -"||v_det_credito);
         LET v_si_resultado = 1;
         LET v_det_estado_solicitud = 2; -- Rechazada   
      END IF
      ...
      IF (v_det_periodo_pago IS NULL) THEN
         -- ERROR de detalle.
         EXECUTE PROCEDURE sp_dpe_inserta_rechazo(
            p_folio,2,v_d_id_referencia,"No",87,
      ...
      IF(v_det_date_fec_pago > TODAY OR v_det_date_fec_pago IS NULL)THEN
         -- ERROR de detalle.
         EXECUTE PROCEDURE sp_dpe_inserta_rechazo(
            p_folio,2,v_d_id_referencia,"No",88,
      ...
      IF(v_id_derechohabiente IS NULL)THEN
         -- ERROR de detalle.
         EXECUTE PROCEDURE sp_dpe_inserta_rechazo(
            p_folio,2,v_d_id_referencia,"No",89,
      ...
      --trace "Verificar suficiencia de saldo en subcuenta vivienda 97";
      LET err_txt = "Verificar suficiencia de saldo en subcuenta vivienda 97";
      --EXECUTE FUNCTION fn_saldo_dia(v_det_nss,v_id_derechohabiente,
      --                                        v_subcuenta, 
      --                                        NULL) -- fecha saldo en nulo indica la fecha actual
      --   INTO v_resultado_saldo, v_d_saldo_disponible_aivs, v_saldo_pesos;
         
      -- Saldo disponible para el nss
      --IF v_resultado_saldo = 0 THEN
      ...
      --   -- Error solicitud en estatus de rechazada por falta de saldo
      --      EXECUTE PROCEDURE sp_dpe_inserta_rechazo( p_folio,2, 
      --                    v_d_id_referencia,"No",90,
      --                    "Saldo insuficiente-"||v_d_saldo_disponible_aivs);
      --            
      --         LET v_si_resultado = 1;
      --         LET v_det_estado_solicitud = 2; -- Rechazada
      ...
      LET err_txt = "Al insertar detalle en dpe_sol_soloinfonavit";
         INSERT INTO dpe_sol_soloinfonavit(id_dpe_referencia,
          ...
          VALUES(v_d_id_referencia,  
                 ...
                 v_det_enti_recaudadora,
                 v_det_nrp,
                 v_det_aportacion_registrada/100,
                 v_det_amortiza_registrada/100,
                 v_det_aportacion_solicitada/100,
                 v_det_amortiza_solicitada/100,
                 v_det_llave_sdd,
                 v_det_folio_sua,
                 v_det_cve_rechazo,
