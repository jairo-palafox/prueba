...
CREATE FUNCTION "safreviv".fn_dse_integra_confdse_agr(p_d_pid DECIMAL(9,0),
                                           p_v_usuario CHAR(20),
                                           p_v_arch_proceso CHAR(100),
                                           p_d_folio DECIMAL(9))
   RETURNING SMALLINT
   -- REGISTRO tmp dse confirm det agr
   ...
   DEFINE tmp_num_aplic_interes97     DECIMAL(15); -- 9 enteros 6 decimales
   DEFINE tmp_filler4                 DECIMAL(15); -- 13 enteros 2 decimales
   DEFINE tmp_nss_separacion          DECIMAL(11);
   DEFINE tmp_origen_devolucion       DECIMAL(2);
   DEFINE tmp_monto_aportaciones      DECIMAL(15); -- 13 enteros 2 decimales
   DEFINE tmp_aplic_interes_aporta    DECIMAL(15); -- 9 enteros 6 decimales
   DEFINE tmp_filler5                 CHAR(2);
   DEFINE tmp_num_aplic_interes92     DECIMAL(15); -- 9 enteros 6 decimales
   DEFINE tmp_filler6                 DECIMAL(18);   
   DEFINE tmp_cod_res_operacion       CHAR(2);
   DEFINE tmp_diagnostico             CHAR(15);
   DEFINE tmp_nombre_imss             CHAR(50);
   DEFINE tmp_num_credito_infonavit   DECIMAL(10);   
   DEFINE tmp_filler7                 CHAR(71);
   ...
   DEFINE v_ax_edo_proc_agrup         SMALLINT; -- estado en el registro de his devolución
   DEFINE v_ax_estado                 SMALLINT; -- estado a insertar en his
   DEFINE v_ax_tpo_transf_dse         CHAR(2); -- tipo de transferencia DSE
   DEFINE v_ax_tpo_transf_conf_dse    CHAR(2); -- tipo de transferencia CONF DSE
   ...
   --SET DEBUG FILE TO '/ds/safreviv_int/BD/dseIntegConfDevsSdos_agr.trace';
   --TRACE ON;

   -- se inicializa el contador de registros
   --LET v_ax_proceso_cod = 8;
   --LET v_ax_opera_cod = 11;
   LET v_ax_tpo_transf_dse = "43"; -- 43-DSE Anualidades Garantizadas
   LET v_ax_tpo_transf_conf_dse = "39"; -- 39- Confirmación de DSE AGR
   ...
   SELECT consecutivo_lote
     INTO v_ax_lote
     FROM safre_tmp:tmp_dse_confirm_enc_agr;
   ...
   END IF;
      ...
      -- se obtienen los datos de la temporal de confirmación de devolucion
   FOREACH
    SELECT *
      ...
      FOREACH
         ...
         SELECT id_dse_grp_devolucion,edo_procesar
           ...
           FROM safre_tmp:tmp_ctr_devolucion_agr
          WHERE nss = tmp_nss

         --en caso de ser nulo el identificador se asigna 0, VPD: que hacer en caso que no se encuentre el id
         IF v_ax_id_dse_grp_devolucion IS NULL THEN
            LET v_ax_id_dse_grp_devolucion = 0;
         END IF
         ...
         LET dseh_tpo_transferencia     = v_ax_tpo_transf_conf_dse;
         ...
         LET dseh_pesos97               = 0;
         LET dseh_aivs92                = tmp_num_aplic_interes92/1000000;
         LET dseh_pesos92               = 0;
         LET dseh_monto_aportacion      = tmp_monto_aportaciones/100;
         LET dseh_aivs_aportacion       = tmp_aplic_interes_aporta/1000000;
         LET dseh_nss_separacion        = tmp_nss_separacion;
         LET dseh_edo_procesar          = 120;
         ...
         -- se ejecuta el sp que actualiza el registro correspondiente de la tabla dse agrupa devolucion
         -- a estado 120 - Saldo Transferido
         ...
         -- se inserta registro
         INSERT INTO safre_tmp:tmp_deudor_conf_devol_saldo_agr (
                     id_dse_devolucion,
                     id_derechohabiente,
                     nss,
                     folio_archivo)
   ...
   EXECUTE PROCEDURE fn_act_dse_ctr_archivo(p_d_folio, p_d_pid, v_ax_tpo_transf_conf_dse, v_ax_id_lote_acpt, v_ax_id_lote_rech);
