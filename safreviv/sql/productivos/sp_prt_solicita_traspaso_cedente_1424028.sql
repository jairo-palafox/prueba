...
CREATE PROCEDURE "safreviv".sp_prt_solicita_traspaso_cedente_1424028(p_id_prt_sol_ced VARCHAR(10),
...
DEFINE v_resto_pesos                            DECIMAL(22,2) ;
DEFINE v_resto_aivs                             DECIMAL(22,2) ;
DEFINE v_aivs_credito                           DECIMAL(22,2) ;
...
-->
 ...
 IF (v_saldo_aivs_viv97 <= 0 OR v_saldo_aivs_viv97 IS NULL ) THEN 
    LET v_saldo_aivs_viv97  = 0;
    LET v_saldo_pesos_viv97 = 0;
 ELSE
 ...
 LET v_saldo_aivs_viv97 = 11040.33;
 LET v_saldo_pesos_viv97 = 29296.40;
 LET v_saldo_aivs_viv92 = 0;
 LET v_saldo_pesos_viv92 = 0;


 LET v_saldo_pesos_97_92 = v_saldo_pesos_viv97 + v_saldo_pesos_viv92;

 -- se envia como maximo el saldo de Infonavit para no quebrantar la cuenta si el crédito de fovissste es mayor
 -- o se envia como maximo el monto del credito fovissste si el saldo en infonavit 92 y 97 es mayor

   IF v_saldo_pesos_viv92 >= v_s_saldo_insoluto_credito_fovissste THEN   
      -- si el saldo 92 es mayor al monto del credito se solicita solo el monto del credito

      LET v_aivs_credito = v_s_saldo_insoluto_credito_fovissste / v_precio_fondo;

      LET v_s_pesos_saldo_viv92_infonavit = v_s_saldo_insoluto_credito_fovissste;
      LET v_s_aivs_saldo_viv92_infonavit  = v_aivs_credito;
      LET v_s_pesos_saldo_viv97_infonavit = 0;
      LET v_s_aivs_saldo_viv97_infonavit  = 0;
      LET v_t_tpo_operacion               = "01";

   ELIF  v_saldo_pesos_viv92 < v_s_saldo_insoluto_credito_fovissste THEN  

      -- si sar 92 no es suficiente se obtiene el resto exedente de 92 para tomarse de 97
      LET v_resto_pesos = 0;
      LET v_resto_aivs  = 0;

      LET v_resto_pesos  = v_s_saldo_insoluto_credito_fovissste - v_saldo_pesos_viv92;
      LET v_resto_aivs   = v_resto_pesos / v_precio_fondo ; 

         -- si el saldo 97 es mayor a cero
         IF v_saldo_pesos_viv97 > v_resto_pesos THEN  
            -- si el saldo 97 es mayor al resto se envia la suma de 92 + el resto 
            LET v_s_pesos_saldo_viv92_infonavit = v_saldo_pesos_viv92;
            LET v_s_aivs_saldo_viv92_infonavit  = v_saldo_aivs_viv92;
            LET v_s_pesos_saldo_viv97_infonavit = v_resto_pesos;
            LET v_s_aivs_saldo_viv97_infonavit  = v_resto_aivs;
            LET v_t_tpo_operacion               = "01";

         ELIF v_saldo_pesos_viv97 <= v_resto_pesos THEN 
            -- si el saldo 97 es menor o igual al resto se solicita 92 + 97 completos
            LET v_s_pesos_saldo_viv92_infonavit = v_saldo_pesos_viv92;
            LET v_s_aivs_saldo_viv92_infonavit       = v_saldo_aivs_viv92;
...
LET v_s_aivs_saldo_viv97_infonavit = 11040.33;
LET v_s_pesos_saldo_viv97_infonavit = 29296.40;
LET v_s_aivs_saldo_viv92_infonavit = 0;
LET v_s_pesos_saldo_viv92_infonavit = 0;
