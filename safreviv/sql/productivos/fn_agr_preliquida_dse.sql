...
CREATE FUNCTION "safreviv".fn_agr_preliquida_dse(p_d_folio DECIMAL(10),
   ...
   DEFINE v_tpo_transferencia        CHAR(2);  -- variable para el tipo de transferencia de AG "43"
   ...
   --SET DEBUG FILE TO '/ds/safreviv_int/BD/dsePreliqDSE.trace';
   --TRACE ON;
   ...
   FOREACH
      SELECT id_dse_grp_devolucion,
             ...
             folio_liquida,
             aivs97,
             ROUND(aivs97*p_precio_fondo,2),
             aivs92,
             ROUND(aivs92*p_precio_fondo,2),
             monto_aportacion,
             aivs_aportacion,
             nss_separacion,
             edo_procesar,
       ...
       WHERE tpo_transferencia = v_tpo_transferencia
         AND edo_procesar IN (20, 5, 70, 60)
         AND estado = 20
      ...
      IF dsea_pesos92 > 0 THEN
         ...
         LET pre_monto_pesos    = dsea_pesos92; -- Monto en pesos del registro origen

         -- se inserta el registro en la tabla de preliquidacion
         INSERT INTO safre_viv:dse_ag_preliquida(
                     f_liquida,
                     id_derechohabiente,
                     subcuenta,
                     fondo_inversion,
      ...
      IF ( dsea_pesos97 > 0 ) THEN
         ...
         LET pre_monto_pesos    = dsea_pesos97; -- Monto en pesos del registro origen
         ...
         AND estado = 15;
{
      -- se actualiza el estado del registro en dse_agrupa a 130
      UPDATE safre_viv:dse_devolucion
         SET folio_referencia = p_d_folio
       WHERE id_derechohabiente = dsea_id_derechohabiente
         AND tpo_transferencia = v_tpo_transferencia
         AND estado = 15;
}
   ...
   -- se actualiza el folio indicando que ya se preliquido
