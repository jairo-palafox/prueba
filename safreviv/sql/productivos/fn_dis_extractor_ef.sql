...
CREATE PROCEDURE "safreviv".fn_dis_extractor_ef(p_folio DECIMAL(9,0))
...
--SET DEBUG FILE TO '/ds/safreviv_int/dis/respaldo/PRODINF-711/fn_dis_extractor_ef.TRACE';
--TRACE ON;
...
LET error_info               = "";
  ...
  DROP TABLE IF EXISTS tmp_dis_ef;
  CREATE TABLE tmp_dis_ef (nss                CHAR(11) NOT NULL,
                           periodo_pago       CHAR(6),
                           f_pago             DATE, 
                           nrp                CHAR(11),
  ...
  DROP TABLE IF EXISTS tmp_nss_id_ef;
	
  SELECT afi.nss, ef.*
  FROM   afi_derechohabiente afi, dis_interface_ef ef
  WHERE  afi.id_derechohabiente = ef.id_derechohabiente
  AND    ef.folio_liquida       = p_folio
  INTO TEMP tmp_nss_id_ef;
	
  UPDATE STATISTICS FOR TABLE tmp_nss_id_ef;
  ...
  FOREACH
    ...
    INTO   v_nss,
           ...
           v_f_interface,            
           v_tipo_interface         
    FROM   tmp_nss_id_ef afi 
    ORDER BY afi.ind_liquidacion DESC, afi.periodo_pago
       
    INSERT INTO tmp_dis_ef VALUES (v_nss,
                                   v_periodo_pago,           
                                   v_f_pago,                 
                                   v_nrp,                    
                                   v_imp_ap_pat,            
  ...
  --TRACE 'Finaliza fn_dis_extractor_ef con valor '||v_bnd_proceso;
   
  LET v_char = "Terminado Extractor Dispersión Entidades Financieras";
