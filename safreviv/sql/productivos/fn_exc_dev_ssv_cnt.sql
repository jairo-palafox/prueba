...
create procedure "safreviv".fn_exc_dev_ssv_cnt(p_folio_liquida      DECIMAL(9,0),  --Folio de liquidación del proceso
...
DEFINE v_monto_sum_dev       DECIMAL(20,2);  --Suma Monto Devolución a Patrones
...
DEFINE v_monto_no_disp       DECIMAL(20,2);  --Monto no dispersados
  ...
  LET v_monto_sum_dev       = 0;

  --SET DEBUG FILE TO '/ds/safreviv_int/BD/fn_exc_dev_ssv_cnt.trace';
  ...
  -- Transacción 122 Excepciones Devolución SSV
  ...
  -- Cuenta Contable Excepciones Devolución SSV Carga Ini
  -- (Abono)
  SELECT SUM(monto_pesos)
  INTO   v_monto_dev_ssv1
  FROM   safre_viv:cta_movimiento
  WHERE  folio_liquida = p_folio_liquida
  AND    movimiento   IN (1811,721,672)
  AND    f_liquida     = p_f_liquida;
  IF DBINFO('sqlca.sqlerrd2') == 0 THEN
     LET v_monto_dev_ssv1 = 0;
  END IF

  -- Cuenta Contable Devol Sub Viv - Trab No Oblig IMSS
  -- (Abono)
  SELECT SUM(monto_pesos)
  INTO   v_monto_sum_dev
  FROM   safre_viv:cta_movimiento
  WHERE  folio_liquida = p_folio_liquida
  AND    movimiento    = 2052
  AND    subcuenta    IN (4,8,47)
  AND    f_liquida     = p_f_liquida;
  IF DBINFO('sqlca.sqlerrd2') == 0 THEN
     LET v_monto_sum_dev = 0;
  END IF
  ...
  FOREACH
    ...
    WHERE  cod_transaccion_cnt IN (122)
    ...
    IF v_cta_contable = '2504070005' THEN
       LET v_monto_pesos = v_monto_sum_dev;
    END IF

    -- Cuenta Contable Carga Inicial (Abono)
    IF v_cta_contable = '2504090001' THEN
       LET v_monto_pesos = v_monto_dev_ssv1; 
    END IF

    -- Cuenta Contable Vivienda 97 (Cargo)
