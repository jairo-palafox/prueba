...
CREATE FUNCTION "safreviv".fn_genera_id_archivo(p_proceso SMALLINT)
RETURNING DECIMAL(9,0), DECIMAL(9,0)
   ...
   IF p_proceso = 201 THEN
      LET v_nom_archivo = "ORIG_TA_esp_"||v_fecha[4,5]||v_fecha[1,2]||v_fecha[7,10]||".rca";
   END IF

   IF p_proceso = 302 THEN
      LET v_nom_archivo = "SOL_AG_esp_"||v_fecha[4,5]||v_fecha[1,2]||v_fecha[7,10]||".uag";
   END IF

   IF p_proceso = 1202 THEN
      LET v_nom_archivo = "SOL_UG_esp_"||v_fecha[4,5]||v_fecha[1,2]||v_fecha[7,10]||".usog";
   END IF

   SELECT r.id_cre_ctr_archivo, folio_archivo
     INTO v_id_cre_ctr_archivo, v_folio_archivo
     FROM cre_ctr_archivo r
    WHERE r.id_proceso = p_proceso
