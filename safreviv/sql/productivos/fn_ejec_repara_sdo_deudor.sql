...
CREATE FUNCTION "safreviv".fn_ejec_repara_sdo_deudor()
   ...
   SELECT tpo_registro,
    ...
    WHERE tpo_registro IN ("01", "20")
      ...
      UPDATE safre_viv:cre_acreditado
         SET sdo_deudor = tmp_saldo_calculado
        WHERE id_derechohabiente = tmp_id_derechohabiente
          AND id_cre_acreditado = id_cre_acreditado
          AND num_credito = tmp_num_credito
          AND f_otorga = tmp_fec_otorga
          AND tpo_credito = tmp_tpo_credito
          AND estado < 900
          AND id_cre_ctr_archivo <> 1;
   END FOREACH
