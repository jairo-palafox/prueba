...
CREATE FUNCTION "safreviv".fn_mig_ret_carga_inicial_spess(p_usuario_cod CHAR(20),
                                  p_folio DECIMAL(9,0),
   ...
   -- codigos de rechazdo
   DEFINE v_cod_rechazo_matriz_derecho_noexiste SMALLINT;
   
   -- se configura el regreso del codigo de error
   ON EXCEPTION SET sql_err, isam_err, err_txt
      LET v_si_resultado = sql_err;
    
      RETURN v_si_resultado, isam_err, err_txt, tmp_ret_spes_nss;
   END EXCEPTION


   -- se inician los valores de las constantes de los codigos de rechazo
   LET v_cod_rechazo_matriz_derecho_noexiste = 1; -- la combinacion de los valores del tipo de retiro no existe
      ...
      ELSE 	
        ...
        INSERT INTO ret_datamart_rch (
           ...
           VALUES ( 
                    ...
                    ret_data_mart_importe_viv72         ,
                    ret_data_mart_cve_afore             ,
                    v_cod_rechazo_matriz_derecho_noexiste
                   );
         --LET v_mensajeno_insert = v_mensajeno_insert + 1;
      END IF 
   END FOREACH;
