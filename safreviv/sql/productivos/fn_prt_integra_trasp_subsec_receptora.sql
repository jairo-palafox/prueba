...
CREATE FUNCTION "safreviv".fn_prt_integra_trasp_subsec_receptora(p_usuario     CHAR(20),
                                                      p_pid         DECIMAL(9,0),
...
DEFINE v_cambio_pesos   INTEGER;
   ...
   --SET DEBUG FILE TO "/ds/safreviv_int/BD/fn_prt_integra_trasp_subsec_receptora.trace";
   --TRACE ON;
   ...
   LET v_tipo_traspaso = '02'; -- APORTACIONES SUBSECUENTES PORTABILIDAD
   ...
   FOREACH SELECT tipo_registro,
      ...
      ELSE
         ...
         SELECT MAX( NVL(id_prt_solicitud_receptora,0))
          ...
          WHERE nss = v_nss
            AND estado = p_estado_solicitud; -- Solicitud liquidada
      ...
      -- Valida que el monto transferido no sea mayor al saldo insoluto registrado en la solicitud
      --IF( v_monto_pesos_fovissste > v_sdo_insoluto_his )THEN
      --   LET v_rechaza_registro = 1;
      --   LET v_motivo_rechazo = "MONTO TRASPASO NO VALIDO";
      --END IF
      ...
      IF( v_rechaza_registro = 1)THEN
         ...
         INSERT INTO safre_tmp:tmp_prt_integrados_subsec VALUES(v_tpo_operacion,v_nss,1,v_monto_pesos_tmp,v_motivo_rechazo);
      ELSE
         LET v_estado = 0; -- Inicial o registrado
         LET v_resultado_op = "01";
         INSERT INTO safre_tmp:tmp_prt_integrados_subsec VALUES(v_tpo_operacion,v_nss,0,v_monto_pesos_tmp,v_motivo_rechazo);
      END IF
