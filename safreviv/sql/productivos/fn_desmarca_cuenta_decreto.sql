...
CREATE FUNCTION "safreviv".fn_desmarca_cuenta_decreto( p_id_decreto      DECIMAL(9,0),
   ...
   ON EXCEPTION
      ...
      IF sql_err = -691 THEN
         ...
         IF v_des_excep IS NULL THEN
            CALL sp_error_marca(sql_err, isam_err, error_info,
                             p_id_decreto, 0,p_marca_entra,p_n_referencia,
                             p_usuario, "desmarca_cuenta",v_hora_actual);
            LET v_des_excep = error_info ;
         ELSE

            INSERT INTO sfr_excep_marca_decreto VALUES (
                        p_id_decreto,  -- nss
                        p_marca_entra ,      -- marca
                        v_fecha_hoy   ,      -- f_ini
                        v_hora_actual ,      -- h_ini
      ...
      LET v_des_excep = 'DESMARCAJE:'||v_des_excep||' '||p_id_decreto||' '||p_marca_entra;
   ...
   FOREACH
      SELECT f_inicio         ,
        ...
        INTO v_fecha_ini      ,
             ...
             v_ind_saldo      ,
             v_ind_habilita
        FROM sfr_marca_activa_decreto c, 
             sfr_marca t
       WHERE c.id_decreto = p_id_decreto
   ...
   --- Borra marca activa
      ...
      IF v_ind_habilita = 2 OR 
         v_ind_habilita = 3 THEN
          ...
          UPDATE sfr_marca_historica_decreto
          ...
          INSERT INTO sfr_marca_historica_decreto VALUES (
                                             p_id_decreto,  -- p_id_decreto
         ...
         IF ( v_ind_habilita = 1 )  THEN  -- Verifica si inhabilita cuenta
            ...
            IF (p_estado_marca = 0 ) THEN -- Desmarcaje procedente
               ...
               EXECUTE FUNCTION fn_marca_cuenta (
                                                 p_id_decreto ,  -- id_derechohabiente
                ...
                IF r_cod_rechazo = 0 THEN
{
                   UPDATE afi_derechohabiente
                    ...
                    WHERE id_derechohabiente = p_id_decreto;
}
                END IF
