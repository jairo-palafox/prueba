...
CREATE FUNCTION "safreviv".fn_repara_sdo_deudor()
   ...
   SELECT tpo_registro,
     ...
     INTO tmp_tpo_registro,
          ...
          tmp_tpo_credito,
          tmp_estado
     FROM safre_tmp:tmp_repara_sdo_deudor
    WHERE tpo_registro IN ("01", "20")
      ...
      IF v_ax_id_derechohabiente IS NOT NULL THEN
         ...
         IF v_ax_id_cre_acreditado IS NOT NULL THEN
            ...
            END IF
         ELSE
            -- se busca el registro en la tabla de rechazos
            IF EXISTS (
            SELECT nss
             ...
             WHERE nss = tmp_nss
               AND num_credito = tmp_num_credito
               AND f_otorga = tmp_fec_otorga
               AND tpo_credito = tmp_tpo_credito) THEN
               -- se asigna que el registro no existe en cre_acreditado pero sí en rechazo
               LET v_ax_edo_registro = 4;
            ELSE
               -- se asigna que el registro no existe en cre_acreditado nií en rechazo
               LET v_ax_edo_registro = 5;
            END IF
         END IF
      ELSE
      ...
      UPDATE safre_tmp:tmp_repara_sdo_deudor
         SET id_derechohabiente = v_ax_id_derechohabiente,
             id_cre_acreditado  = v_ax_id_cre_acreditado,
             saldo_anterior     = v_ax_sdo_deudor,
