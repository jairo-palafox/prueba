...
CREATE FUNCTION "safreviv".fn_agr_insrt_ctr_arch_devoluc(p_c_nom_archivo CHAR(40),
                                              p_i_estado SMALLINT,
                                              p_c_usuario CHAR(20))
   ...
   --SET DEBUG FILE TO '/ds/safreviv_int/BD/agrCtrArch_devol.trace';
   --TRACE ON;

   -- se inicializan variables
   LET v_b_estatus_proc = 0;
   LET v_ax_operacion = 6; -- 6-Devoluciones de solicitudes
   LET v_ax_id_proceso = 301; -- 301-Anualidades Garantizadas

   -- se obtiene el lote y la fecha
   SELECT f_presentacion, consec_lote
   ...
   IF p_i_estado = 10 THEN
      SELECT COUNT(*)
        INTO v_ax_tot_regs_aux
        FROM safre_tmp:tmp_devoluc_sum_agr;
   ...
   IF p_i_estado = 10 THEN
      IF EXISTS (
      SELECT orig_tpo_transf
        FROM safre_tmp:tmp_devoluc_det_agr
       WHERE orig_tpo_transf NOT IN (
