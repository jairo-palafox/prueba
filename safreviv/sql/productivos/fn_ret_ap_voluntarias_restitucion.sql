...
CREATE FUNCTION "safreviv".fn_ret_ap_voluntarias_restitucion(p_folio_restitucion DECIMAL(10,0))
...
DEFINE v_c_msj                          VARCHAR(200);
   ...
   UPDATE glo_folio
    ...
    WHERE folio = p_folio_restitucion;
   ...
   LET v_movimiento = 611; -- ABONO DE APORTACIONES VOLUNTARIAS
   ...
   FROM
      ret_voluntaria
   WHERE --folio            = p_folio
      estado_solicitud = 100 -- rechazadas
           
      -- retiro por aportacion voluntaria
      --se concatena la cadena RETIRO
      LET ret_preliquida_origen = "RESTITUCION APVOL";
      ...
      LET ret_preliquida_folio_liquida      = p_folio_restitucion;
      ...
      SET    estado_solicitud = 120 -- restituida
   ...
   IF ( v_num_regs_preliquidados < 1 ) THEN
      LET v_si_resultado = 0;
      LET isam_err = 0;
      LET v_c_msj = 'No se preliquidaron registros. No se encontraron solicitudes no pagadas.';
   END IF
