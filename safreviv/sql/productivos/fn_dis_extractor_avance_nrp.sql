...
CREATE PROCEDURE "safreviv".fn_dis_extractor_avance_nrp(p_periodo_pago CHAR(06))

RETURNING SMALLINT, SMALLINT, CHAR(70);

--Última modificación 20082015
...
DEFINE v_sel_det_nrp		LVARCHAR(350);
...
--SET DEBUG FILE TO '/ds/safreviv_int/dis/fn_dis_extractor_avance_nrp.TRACE';
--TRACE ON;
...
LET v_folio_cta_hp			= "";
   ...
   DROP TABLE IF EXISTS tmp_avance_abierto_nrp;
 
   DROP TABLE IF EXISTS tmp_avance_nrp;
   CREATE TABLE tmp_avance_nrp ( folio_liquida      DECIMAL(9,0),
								 nss				CHAR(11),
   ...
   DROP TABLE IF EXISTS tmp_dis_pagos_avance_nrp;
   CREATE TABLE tmp_dis_pagos_avance_nrp ( id_derechohabiente DECIMAL(9,0),
										   folio_sua          DECIMAL(6,0),
										   periodo_pago       CHAR(06),
										   f_pago             DATE,
										   nrp                CHAR(11),
   ...
   LET v_sel_det_nrp =  " SELECT det.folio, "||
	...
	IF p_periodo_pago IS NULL OR p_periodo_pago = 0 THEN
		LET v_sel_det_nrp = v_sel_det_nrp ||" INTO TEMP tmp_avance_abierto_nrp ";
	ELSE
		LET v_sel_det_nrp = v_sel_det_nrp ||" AND det.periodo_pago = '"||p_periodo_pago||"'"||
											" INTO TEMP tmp_avance_abierto_nrp ";
	END IF
	
	EXECUTE IMMEDIATE v_sel_det_nrp;
	
	SELECT DISTINCT(ava.id_derechohabiente), afi.nss
	FROM tmp_avance_abierto_nrp ava, afi_derechohabiente afi
	WHERE ava.id_derechohabiente = afi.id_derechohabiente
	INTO TEMP tmp_id_nss1;
	
	CREATE INDEX xie1tmp_id_nss1 ON tmp_id_nss1(id_derechohabiente) IN dis_ix_dbs;
	...
	FOREACH
		...
		FROM cta_his_pagos cta,
			 tmp_id_nss1 tid
		WHERE  cta.id_derechohabiente  	= tid.id_derechohabiente
		AND    cta.ind_liquidacion      NOT IN (1,6)
		
		INSERT INTO tmp_dis_pagos_avance_nrp VALUES( v_id_dh_aux,  
													 v_folio_sua,	         
													 v_periodo_pago,       	   
													 v_f_pago,            	  
													 v_nrp,
	...
	CREATE INDEX xie1tmp_dis_pagos_avance_nrp ON tmp_dis_pagos_avance_nrp(id_derechohabiente ,periodo_pago) IN dis_ix_dbs;
   
	SET INDEXES FOR tmp_dis_pagos_avance_nrp ENABLED;
	UPDATE STATISTICS FOR TABLE tmp_dis_pagos_avance_nrp;
   ...
   --TRACE 'Finaliza fn_dis_extractor_avance_nrp con valor '||v_bnd_proceso;
   
   LET v_char = "Terminado Extractor Avance Abierto por NRP";
