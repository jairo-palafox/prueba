...
create procedure "safreviv".fn_dev_ssv_92_97_cnt12(p_folio_liquida      DECIMAL(9,0),  --Folio de liquidación del proceso
...
DEFINE v_bnd_inserta         SMALLINT;       --Estatus para inserción
  ...
  --SET DEBUG FILE TO '/ds/safreviv_int/BD/fn_dev_ssv_92_97_cnt12.trace';
  ...
  -- a restituir
  -- (Cargo)
  {SELECT SUM(grp.pesos92)
  INTO   v_monto_restituir
  FROM   safre_viv:dse_agrupa_devolucion grp
  WHERE  grp.folio_liquida = p_folio_liquida;
  IF DBINFO('sqlca.sqlerrd2') == 0 THEN
     LET v_monto_restituir = 0;
  END IF}

  -- Cuenta Contable 5102020300 Intereses complementarios
  -- subcta de Vivienda
  -- (Cargo)
  -- Falta obtener movimiento (No genera intereses)
  --IF v_cta_contable = '5102020300' THEN;
     --SELECT SUM(monto_pesos)
     --INTO   v_monto_intereses
  ...
  FOREACH
    ...
    -- Cuenta Contable 2504060001 Saldo 92-97 EN AFORE
    -- (Abono) SV a restituir mas Intereses complementarios SV
    SELECT SUM(monto_pesos)
    INTO   v_monto_pesos
    FROM   safre_viv:cta_movimiento
    WHERE  f_liquida     = p_f_liquida
