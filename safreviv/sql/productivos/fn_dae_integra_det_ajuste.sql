...
CREATE FUNCTION "safreviv".fn_dae_integra_det_ajuste(p_usuario    CHAR(20),
                                          p_pid            DECIMAL(9,0),
   ...
   DEFINE v_tmp_nss_ajustar        CHAR(11);
   ...
   --SET DEBUG FILE TO "/safreviv_int/archivos/dae_integra_ajustes.trace";
   --TRACE ON;
   ...
   LET v_fecha_liquidacion     = 0;

FOREACH
   SELECT nss_ajustar
   INTO   v_tmp_nss_ajustar
   ...
   IF v_tmp_nss_ajustar = "00000000000" THEN
   ...
   ELSE
      ...
      IF v_id_derechohabiente_afi IS NULL THEN
         ...
         LET v_total_rechazados = v_total_rechazados + 1;
      ELSE
         --Si id_derechohabiente no tiene amortización
         IF v_id_derechohabiente_afi IS NULL THEN
            LET v_rch_folio             = p_folio;
            LET v_rch_tipo_registro     = 2;
            LET v_rch_resul_opera       = "02";
            LET v_rch_diagnostico       = 16;
            LET v_rch_campo_valor       = "NO TIENE AMORTIZACIÓN";
         ...
         ELSE
            ...
            EXECUTE FUNCTION fn_saldo_dia(v_tmp_nss_ajustar, v_id_derechohabiente_afi,46, TODAY)
            INTO v_resultado, v_saldo_aivs, v_saldo_pesos;

            LET v_marca_ajuste = 403;
            --Si saldo > 0
            --IF v_resultado = 0 AND v_saldo_pesos > 0 THEN
            IF v_resultado = 0 AND v_saldo_aivs > 0 THEN --Se cambia validación de pesos por validación de AIVS --AG 26/08/2014.
...
--########################################################
                  IF v_det_status_retiro = 1 OR v_det_status_retiro = 3 THEN
                     FOREACH
                        --Consulta si derechohabiente tiene amortización
                        SELECT id_derechohabiente,
                               nss,
                               id_dae_referencia,
                               importe_amort,
...
--########################################################
            ...
            ELSE
               ...
               LET v_total_rechazados = v_total_rechazados + 1;
            END IF --Si saldo > 0
         END IF -- Si DH no tiene amortización
