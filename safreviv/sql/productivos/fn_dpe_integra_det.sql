...
CREATE FUNCTION "safreviv".fn_dpe_integra_det(p_usuario_cod    CHAR(20), 
                                   p_pid            DECIMAL(9,0), 
  ...
  RETURNING INTEGER,
   ...
   DEFINE v_si_status_sumario_patron  SMALLINT;
   DEFINE v_si_status_sumario_exceso  SMALLINT;
   ...
   --SET DEBUG FILE TO "/home/safreviv/trace.dpe.integra.txt";
   --TRACE ON;
   ...
   FOREACH
      ...
      IF(v_si_tipo_archivo = 1)THEN          
         ...
         -- <Por dafault NO existen aclaraciones>
         LET v_si_contador = 0;
         
         IF(v_si_contador>0)THEN
            -- Solicitud en estatus de pendiente
            -- [Solicitud en estatus de pendiente por aclaraciones]
            EXECUTE PROCEDURE sp_dpe_inserta_rechazo(p_folio, 
                                                     03 , 
                                                     v_id_dpe_referencia, 
                                                     'NO' , 
            ...
            -- Solicitud pendiente por aclaraciones [Paso 1.8]
            LET v_si_status_solicitud = v_si_pendiente;
            LET v_dpe_diagnostico     = 100;
         ELSE
            -- # [Verificar suficiencia de saldo en subcuenta vivienda 97]            
            ...
            EXECUTE FUNCTION fn_saldo_dia(v_tmp_nss,
               ...
               INTO v_resultado, 
                    v_d_saldo_disponible_aivs, 
                    v_saldo_pesos;

            --Valida si el derechohabiente tiene algún crédito
            SELECT id_credito                  
            INTO   v_id_credito_ad                  
            FROM   afi_derechohabiente                  
            WHERE  id_derechohabiente = v_id_derechohabiente;                 

            --Si TIENE, se rechaza por falta de saldo
            IF v_id_credito_ad <> 0 THEN             
               -- #[Solicitud en estatus de rechazada por falta de saldo]
               EXECUTE PROCEDURE sp_dpe_inserta_rechazo( p_folio, 
                                                         03 , 
                                                         v_id_dpe_referencia, 
                                                         'NO' , 
               ...
               LET v_b_registro_correcto = 1;
               -- # [Paso 1.7 Se envía la solicitud a estatus rechazada]
               LET v_si_status_solicitud = v_si_rechazada;
               LET v_dpe_diagnostico     = 21;               
            END IF 
            ...
            IF(v_d_saldo_disponible_aivs>0)THEN
               -- #-- Saldo valido es superior a cero
               -- #-- Se envía la solicitud a estatus aceptada [Paso 1.6]
               -- #LET v_si_status_solicitud = v_si_aceptada;
            ELSE -- verifica saldos
               -- Se rechaza por no existir saldo al día de la validación.
               ...
               LET v_si_status_solicitud = v_si_rechazada;
               LET v_dpe_diagnostico     = 21;
            END IF -- verifica saldos
         END IF-- aclaraciones         
      ELSE 
         -- Por default solicitud aceptada [Paso 1.6]
         LET v_si_status_solicitud = v_si_aceptada;
      END IF
      
      -- [Obtener el importe equivalente al total de acciones a la fecha de hoy]
      LET err_txt = 'Al generar importe en pesos del total de AIVs para el dia.';
      
      EXECUTE FUNCTION fn_del_cal_total_acciones(v_dte_fecha_hoy, 
                                                 v_tmp_apli_interes_vivienda,
                                                 11)
         INTO v_si_estado,
              v_tmp_imp_patronal_infonavit_devolver;
      ...
      IF(v_si_estado <> 0)THEN
         -- [Solicitud en estatus de rechazada por falta valor de accion]
         EXECUTE PROCEDURE sp_dpe_inserta_rechazo( p_folio, 
                                                   03 ,
                                                   v_id_dpe_referencia, 
                                                   'NO' , 
                                                   22, 
                                                   'No se pudo obtener el valor de la accion'
                                                   );
         -- <status de error>
         LET v_b_registro_correcto = 1;         
         LET v_si_status_solicitud = v_si_rechazada;
         LET v_dpe_diagnostico     = 22;
      ELSE
         -- Indica que si se obtuvo el valor de las ivs
         -- [Verificar si existe saldo disponible]
         IF(v_d_saldo_disponible_aivs >0)THEN
            -- [Verificar si el saldo disponible cubre total a pagar]
            --TRACE("Camino f:12");
            IF(v_d_saldo_disponible_aivs >= v_tmp_apli_interes_vivienda)THEN
               -- Aplica pago de tipo total.
               LET v_dpe_diagnostico  = c_pago_total_nss; -- Pago por liquidar
               LET v_dpe_porcentaje_dev      = 100;
               
               -- Importe total a cubrir para el valor de las AIVS
               LET v_detalle_imp_viv_dev       = 0;
               
               -- Saldo a cubrir como totalidad
               LET v_detalle_avis_viv_dev      = v_tmp_apli_interes_vivienda;
               
               -- #[Tipo pago por liquidar total.]
               LET v_detalle_diagnostico       = c_pago_por_preliquidar_total;               
            ELSE
               -- Indica que si hay importe pero no se cubre el total
               --  solo una parcialidad.
               LET v_dpe_diagnostico  = c_pago_parcial_nss; -- pago parcial
               LET v_dpe_porcentaje_dev      = 1;
               
               -- Importe total a cubrir para el valor de las AIVS
               LET v_detalle_imp_viv_dev       = 0;
               
               -- Saldo a cubrir como parcialidad
               LET v_detalle_avis_viv_dev      = v_d_saldo_disponible_aivs;
               
               -- #[Tipo pago por liquidar parcial.]
               LET v_detalle_diagnostico       = c_pago_por_preliquidar_parcial;
               
            END IF
            
            -- Recupera datos de detalle
            LET v_detalle_id_dpe_referencia = v_id_dpe_referencia;
            LET v_detalle_folio             = p_folio;
            LET v_detalle_reg_patronal_imss = v_tmp_reg_patronal;
            LET v_detalle_periodo_pago      = v_tmp_periodo_pago;
            
            LET v_detalle_resul_op          = 0; -- desconocido
            LET v_detalle_diag_procesa      = 0; -- desconocido
            LET v_detalle_folio_respuesta   = 0; -- desconocido
            
            LET err_txt = 'Al generar importe en pesos del total de AIVs para el saldo disponible.';
            LET v_si_estado = 0;
            EXECUTE FUNCTION fn_del_cal_total_acciones(v_dte_fecha_hoy,
                                                       v_detalle_avis_viv_dev,
                                                       11)
               INTO v_si_estado,
                    v_detalle_imp_viv_dev;
            ...
            IF(v_si_estado <> 0)THEN
               ...
               EXECUTE PROCEDURE sp_dpe_inserta_rechazo( p_folio, 
                                                         ...
                                                         'NO' , 
                                                         23, 
                                                         'No se pudo obtener el valor de la accion'||v_detalle_imp_viv_dev
                                                         );
               -- <status de error>
               
               LET v_b_registro_correcto = 1;               
               LET v_si_status_solicitud = v_si_rechazada;
               LET v_dpe_diagnostico     = 23;
            END IF
            
            LET err_txt = 'Al insertar registro detalle:dpe_sol_trab_parcial.';
            -- Insertar registro de saldo a cubrir
            INSERT INTO dpe_sol_trab_parcial (id_dpe_referencia,
                                              folio,            
                                              reg_patronal_imss,
                                              periodo_pago,     
                                              imp_viv_dev,      
                                              ...
                                              diag_procesa,     
                                              folio_respuesta)
                   VALUES (v_detalle_id_dpe_referencia,
                           v_detalle_folio,            
                           v_detalle_reg_patronal_imss,
                           v_detalle_periodo_pago,     
                           v_detalle_imp_viv_dev,      
                           v_detalle_avis_viv_dev,     
                           v_detalle_diagnostico,      
                           v_detalle_resul_op,         
                           v_detalle_diag_procesa,
                           v_detalle_folio_respuesta);            
         END IF         
      END IF
...
--###########      
      ...
      IF(v_dpe_estado_solicitud = v_si_aceptada)THEN
         ...
         -- #[Pendiente definir el resultado de la marca             ]
         -- #[ para indicar si se realizó o no la marca correctamente]
      END IF
