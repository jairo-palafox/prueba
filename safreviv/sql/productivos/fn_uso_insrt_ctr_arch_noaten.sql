...
CREATE FUNCTION "safreviv".fn_uso_insrt_ctr_arch_noaten(p_c_nom_archivo CHAR(40),
                                             p_i_estado SMALLINT,
                                             p_v_usuario CHAR(20))
   ...
   DEFINE v_ax_tot_regs_grt    INTEGER; -- total de registros sin trámites de solicitud de marca 43 BIS
   DEFINE v_ax_tot_regs_uso    INTEGER; -- total de registros sin Trámites de Uso de Garantía 43 BIS
   ...
   --SET DEBUG FILE TO '/ds/safreviv_int/BD/usoCtrArch_noAten.trace';
   --TRACE ON;
   ...
   SELECT f_presentacion, cons_lote
     ...
     FROM safre_tmp:tmp_no_atend_enc_uso;
   ...
   IF p_i_estado = 10 THEN
      SELECT COUNT(*)
        INTO v_ax_tot_regs_aux
        FROM safre_tmp:tmp_no_atend_sum_uso;
   ...
   IF p_i_estado = 10 THEN
      SELECT num_reg_sin_marca, num_reg_sin_uso
        INTO v_ax_tot_regs_grt, v_ax_tot_regs_uso
        FROM safre_tmp:tmp_no_atend_sum_uso;

      -- se crea la sentencia sql que obtiene el numero registros insertados con tipo de transferencia 16
      SELECT COUNT(*)
        INTO v_ax_tot_regs_aux
        FROM safre_tmp:tmp_no_atend_det_uso
       WHERE tpo_transferencia = "16";

      -- se verifica el identificador de operacion
      IF v_ax_tot_regs_grt <> v_ax_tot_regs_aux THEN
      ...
      ELSE
         ...
         -- se verifica el identificador de operacion
         IF v_ax_tot_regs_uso <> v_ax_tot_regs_aux THEN
            --TRACE("ERROR: TOTAL EN SUMARIO NO IGUAL AL TOTAL DE REGISTROS");
            -- se marca como rechazado el archivo ya coincide el total del sumario con el insertado en detalle
            LET p_i_estado = 30;
            LET v_b_estatus_proc = 4;
