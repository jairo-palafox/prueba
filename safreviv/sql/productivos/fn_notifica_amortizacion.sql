...
CREATE FUNCTION "safreviv".fn_notifica_amortizacion(p_folio_proc_operativo  DECIMAL(9,0),
   ...
   EXECUTE IMMEDIATE v_query;
	...
	FOREACH 
		SELECT 
         ...
         --AND cta.ind_liquidacion IN (0,2,3,5)
		...
		IF (v_imp_ap_pat = 0 OR v_imp_ap_pat IS NULL) THEN
			IF (v_imp_am_cre <> 0 AND v_imp_am_cre IS NOT NULL) THEN
                LET v_query = 'INSERT INTO ' || v_nom_tabla || '(folio,nss,amortizacion,bimestre,ejercicio) VALUES ('
                                              ||p_folio_notificacion||',"'||v_nss||'","'||TRIM(TO_CHAR(v_imp_am_cre,"###,###,##&.&&"))||'","'||v_bimestre[5,6]||'","'||v_bimestre[1,4]||'")';
                EXECUTE IMMEDIATE v_query;
			END IF;
		END IF;
	END FOREACH;
