...
CREATE PROCEDURE "safreviv".sp_dwh_extractor_trans_acreditados()
RETURNING INTEGER, INTEGER, CHAR(200);
   ...
   CREATE TEMP TABLE tmp_mov_acr(f_liquida DATE ,
   ...
   LET v_query = "INSERT INTO tmp_mov_acr " ||
      "SELECT f_liquida, id_derechohabiente, "||
        ...
        " WHERE subcuenta IN (4,8) "||
          " AND movimiento IN (122,132,472,482) ";
   ...
   CREATE INDEX xtmp_mov_acr ON tmp_mov_acr (folio_liquida, id_derechohabiente);

   INSERT INTO safre_sdo@vivws_tcp:saci_transf_acreditados
        SELECT c.nss,
               a.num_credito,
               b.monto_pesos,
               b.monto_acciones,
         ...
         WHERE b.subcuenta = 4
           ...
           AND a.id_cre_acreditado = b.id_referencia
   ...
   INSERT INTO safre_sdo@vivws_tcp:glo_ctr_dwh
        SELECT TODAY,
               "saci_transf_acreditados",
               TODAY,
               ( SELECT COUNT(*) FROM safre_sdo@vivws_tcp:saci_transf_acreditados ),
               1
          FROM SYSTABLES
         WHERE tabid = 1;
   RETURN r_sql_err, r_isam_err, r_err_txt;
