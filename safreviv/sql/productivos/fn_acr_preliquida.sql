...
CREATE FUNCTION "safreviv".fn_acr_preliquida(p_folio_liq          DECIMAL(9,0),
...
RETURNING SMALLINT
   ...
   --SET DEBUG FILE TO '/ds/safreviv_int/BD/fn_acr_preliquida.trace';
   --TRACE ON;
   ...
   ELSE
      ...
      -- Se realiza la ejecución de la función que preliquida la subcuenta 55
      ...
      ELSE  ----Obtención saldo por subcuenta
         FOREACH
            SELECT m.subcuenta, c.prelacion, SUM(m.monto_acciones)
              ...
              --FROM safre_viv:cta_movimiento m, safre_viv:cat_subcuenta c
            ...
            IF v_subcuenta = 4 OR v_subcuenta = 8 THEN
               ...
               LET v_mvto_amort = 482;
            ELSE
               LET v_movimiento = 122;
               LET v_mvto_amort = 132;
            END IF
            ...
            ELSE
               ...
               IF v_aivs_am_subcta < 0 THEN   ---liquidación amortización
                  INSERT INTO cre_ta_preliquida
                  VALUES (v_f_liquida,
                          p_id_derechohabiente,
                          v_subcuenta,
              ...
              p_folio_acr );
{
      ...
      EXECUTE FUNCTION fn_cre_preliquida_fondo72(p_folio_liq,
                                                 p_id_cre_acreditado,
                                                 p_nss,
                                                 v_tpo_origina)
                                            INTO v_ax_status;
