...
CREATE FUNCTION "safreviv".fn_dae_integra_det_ajuste_manual(p_usuario    CHAR(20), 
                                                 p_pid            DECIMAL(9,0),
   ...
   DEFINE v_tmp_imp_amortizacion   CHAR(8) ;
   ...
   --SET DEBUG FILE TO "/ds/safreviv_int/BD/dae_integra_ajuste_manual.trace";
   --TRACE ON;
...
FOREACH
   SELECT num_credito  ,
          ...
          nss
   INTO   v_tmp_numero_credito  ,
          v_tmp_fecha_pago      ,
          v_tmp_periodo_pago    ,
          v_tmp_fecha_reg_pago  ,
          v_tmp_imp_amortizacion,
          v_tmp_nss
   FROM   safre_tmp:tmp_dae_preliquida
...
--########################################################                  
                  IF v_det_status_retiro = 1 OR v_det_status_retiro = 3 THEN 
                     FOREACH
                        ...
                        AND    fecha_pago    = v_tmp_fecha_pago
