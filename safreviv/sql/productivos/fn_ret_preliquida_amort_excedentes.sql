...
CREATE FUNCTION "safreviv".fn_ret_preliquida_amort_excedentes(p_folio DECIMAL(10,0),p_proceso_cod SMALLINT,
...
-- ret_voluntaria
DEFINE v_ret_am_excedente_id_solicitud       DECIMAL(9,0) ;
DEFINE v_ret_am_excedente_folio              DECIMAL(9,0) ;
DEFINE v_ret_am_excedente_id_derechohabiente DECIMAL(9,0) ;
DEFINE v_ret_am_excedente_nss                CHAR(11)     ;
DEFINE v_ret_am_excedente_f_solicitud        DATE         ;
DEFINE v_ret_am_excedente_tpo_retiro         SMALLINT     ;
DEFINE v_ret_am_excedente_total_aivs         DECIMAL(14,6);
DEFINE v_ret_am_excedente_total_importe      DECIMAL(14,2);
DEFINE v_ret_am_excedente_cod_actividad      SMALLINT     ;
DEFINE v_ret_am_excedente_cod_rechazo        SMALLINT     ;
DEFINE v_ret_am_excedente_estado_solicitud   SMALLINT     ;
...
DEFINE v_c_msj                          VARCHAR(200);
   ...
   ON EXCEPTION SET sql_err, isam_err, err_txt 
      ...
      RETURN v_si_resultado, isam_err, err_txt, v_ret_am_excedente_id_solicitud;
   END EXCEPTION
   ...
   LET v_ret_am_excedente_id_solicitud = NULL;
   ...
   LET v_movimiento = 1402; -- establecer movimiento para los retiros de amortizaciones excedentes
   ...
   SELECT
      ...
      estado_solicitud  
   INTO
      v_ret_am_excedente_id_solicitud      ,
      v_ret_am_excedente_folio             ,
      v_ret_am_excedente_id_derechohabiente,
      v_ret_am_excedente_nss               ,
      v_ret_am_excedente_f_solicitud       ,
      v_ret_am_excedente_tpo_retiro        ,
      v_ret_am_excedente_total_aivs        ,
      v_ret_am_excedente_total_importe     ,
      v_ret_am_excedente_cod_actividad     ,
      v_ret_am_excedente_cod_rechazo       ,
      v_ret_am_excedente_estado_solicitud  
   FROM ret_amort_excedente
   WHERE estado_solicitud = 15 -- integradas
           
      -- retiro por aportacion voluntaria
      --se concatena la cadena RETIRO
      LET ret_preliquida_origen = "RETIRO AMEXC";

      -- asignacion de valores generales al registro de la tabla de preliquidacion
      LET ret_preliquida_f_liquida          = TODAY; -- DATE                   ;
      LET ret_preliquida_id_derechohabiente = v_ret_am_excedente_id_derechohabiente;
      ...
      LET ret_preliquida_subcuenta          = 46; -- amortizaciones excedentes
      ...
      LET ret_preliquida_id_referencia      = v_ret_am_excedente_id_solicitud;
      LET ret_preliquida_monto_acciones     = v_ret_am_excedente_total_aivs * v_signo_movimiento; -- la paridad peso-AIV es 1 a 1
      LET ret_preliquida_monto_pesos        = v_ret_am_excedente_total_aivs * v_signo_movimiento * v_valor_fondo;
      ...
      SET    estado_solicitud = 50, -- preliquidada
             folio            = p_folio,
             total_importe    = ret_preliquida_monto_pesos
      WHERE  id_solicitud     = v_ret_am_excedente_id_solicitud;
      ...
      -- se actualiza el folio en la marca activa e historica
      UPDATE sfr_marca_activa
      SET    folio = p_folio
      WHERE  id_derechohabiente = v_ret_am_excedente_id_derechohabiente
      AND    n_referencia       = v_ret_am_excedente_id_solicitud
      AND    marca              = 810;
   ...
   RETURN v_si_resultado, isam_err, v_c_msj, v_ret_am_excedente_id_solicitud;
END FUNCTION
;
