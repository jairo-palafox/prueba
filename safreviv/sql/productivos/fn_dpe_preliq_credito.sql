...
CREATE FUNCTION "safreviv".fn_dpe_preliq_credito(p_folio       DECIMAL(10), 
...
DEFINE v_fondo_inversion_sief10   SMALLINT; -- Siefore 10 amortizacion 
...
DEFINE v_precio_fondo             DECIMAL(16,6);
    ...
    ON EXCEPTION IN (-206)
      ...
      RETURN v_i_resultado, v_i_registros_insertados, 0;
    END EXCEPTION
    
    --SET DEBUG FILE TO "/ds/safreviv_int/BD/trace.dpe.preliquida.INFONAVIT.txt";
    
    LET g_proceso_cod = 1003;
    LET g_opera_cod   = 3;
    LET v_i_resultado = 0;
    LET v_i_registros_insertados = 0;
    ...
    LET v_movimiento_cargo       = 372; -- Movimiento cargo DPE solo INFONAVIT
    LET v_movimiento_abono       = 121; -- Movimiento abono DPE solo INFONAVIT
      ...
      SELECT id_dpe_referencia,
       ...
       WHERE estado_solicitud = 1  -- solo solicitudes aceptadas
        ...
        IF v_tipo_trabajador = "I" THEN
           ...
           IF v_amortizacion_sol > 0 THEN
              LET v_preliq_f_liquida          = TODAY;
              LET v_preliq_id_derechohabiente = v_id_derechohabiente;
              LET v_preliq_subcuenta          = v_subcuenta_41;
              LET v_preliq_fondo_inversion    = v_fondo_inversion_sief10;
              LET v_preliq_movimiento         = v_movimiento_abono; -- Movimiento de abono
              LET v_preliq_folio_liquida      = p_folio;
              LET v_preliq_id_referencia      = v_id_dpe_referencia;
              LET v_preliq_monto_acciones     = v_amortizacion_sol; --abono
              LET v_preliq_monto_pesos        = v_amortizacion_sol;
              LET v_preliq_f_valor            = TODAY;
              LET v_preliq_f_registro         = TODAY;
              LET v_preliq_h_registro         = CURRENT HOUR TO SECOND;
              LET v_origen_preliquida         = v_periodo_pago||'-'||v_nrp;
              ...
              LET v_preliq_folio_liquida      = p_folio;
              LET v_preliq_id_referencia      = v_id_dpe_referencia;
              LET v_preliq_monto_acciones     = v_amortizacion_sol * -1; --cargo
              LET v_preliq_monto_pesos        = v_amortizacion_sol * -1;
              LET v_preliq_f_valor            = TODAY;
              LET v_preliq_f_registro         = TODAY;
              LET v_preliq_h_registro         = CURRENT HOUR TO SECOND;
              LET v_origen_preliquida         = v_periodo_pago||'-'||v_nrp;
           ...
           END IF
        END IF
        -- Se valida que sea derechohabiente solo infonavit
        IF v_tipo_trabajador = "S" THEN
           IF v_aportacion_sol > 0 THEN
              LET v_preliq_f_liquida          = TODAY;
              LET v_preliq_id_derechohabiente = v_id_derechohabiente;
           ...
           IF v_amortizacion_sol > 0 THEN
              LET v_preliq_f_liquida          = TODAY;
              LET v_preliq_id_derechohabiente = v_id_derechohabiente;
              LET v_preliq_subcuenta          = v_subcuenta_43;
              LET v_preliq_fondo_inversion    = v_fondo_inversion_sief10;
              LET v_preliq_movimiento         = v_movimiento_abono; -- Movimiento de abono
              LET v_preliq_folio_liquida      = p_folio;
              LET v_preliq_id_referencia      = v_id_dpe_referencia;
