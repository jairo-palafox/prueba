...
CREATE PROCEDURE "safreviv".sp_hps_valida_carga_recurrente()
...
DEFINE v_msg_sql    CHAR(254);
   ...
   ON EXCEPTION SET v_error_sql, v_error_isam, v_msg_sql
      ...
      IF(v_error_sql = -1213)THEN
         ...
         IF(v_val_nss IS NULL)THEN
            -- TRACE "0.2";
            INSERT INTO hps_tmp_inconsistencias_val(mensaje)
   ...
   --SET DEBUG FILE TO '/safreviv_int/BD/sp_hps_valida_carga_recurrente.-- TRACE';
   ...
   FOREACH SELECT TRIM(nss),
      ...
      IF(LENGTH(TRIM(v_nss)) > 0)THEN
         ...
         LET v_error_sql = 0;
         EXECUTE FUNCTION fn_mdt_valida_numerico(v_nss) INTO v_error_sql, v_msg_sql,v_val_nss;
         --LET v_val_nss = v_nss;
      ...
      ELSE
         ...
         LET v_error_sql = 0;
         EXECUTE FUNCTION fn_hps_valida_fecha(v_fec_ini_mandato) INTO v_error_sql, v_msg_sql, v_fec_ini_val;
         
         IF(v_error_sql <> 1)THEN
            -- TRACE "4.3";
            INSERT INTO hps_tmp_inconsistencias_val(mensaje)
      ...
      EXECUTE FUNCTION fn_hps_valida_fecha(v_fec_fin_mandato) INTO v_error_sql, v_msg_sql, v_fec_fin_val;
      ...
      ELSE
         ...
         LET v_cve_mandato_val = NULL;
         LET v_error_sql = 0;
         EXECUTE FUNCTION fn_mdt_valida_decimal(v_val_descuento) INTO v_error_sql, v_msg_sql,v_val_descuento_val;
         IF(v_error_sql <> 0)THEN
            -- TRACE "7.1";
            INSERT INTO hps_tmp_inconsistencias_val(mensaje)
      ...
      ELSE
         ...
         ELSE
            -- TRACE "8.2";
            LET v_error_sql = 0;
            EXECUTE FUNCTION fn_mdt_valida_numerico(v_cve_mandato) INTO v_error_sql, v_msg_sql,v_cve_mandato_val;
