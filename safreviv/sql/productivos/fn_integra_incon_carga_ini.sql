...
CREATE FUNCTION "safreviv".fn_integra_incon_carga_ini(p_d_folio DECIMAL(9,0),
   ...
   DEFINE tmp_fac_des_pesos        DECIMAL(8,0);
   ...
   DEFINE v_c_nom_archivo         CHAR(40); -- archivo de proceso (carga)
   DEFINE v_c_usuario             CHAR(20); -- usuario del proceso
   DEFINE r_ax_bandera            SMALLINT; -- retorno de la actualización de status
   -- control de excepciones 
   DEFINE v_error                 INTEGER; -- codigo de error SQL
   DEFINE v_error_isam            INTEGER; -- codigo de error ISAM
   DEFINE v_mensaje               VARCHAR(255); -- mensaje de error

   ON EXCEPTION SET v_error, v_error_isam, v_mensaje
      -- Devolvera el codigo de error cuando ocurra una excepción
      RETURN v_error, v_error_isam, v_mensaje;
   END EXCEPTION

   --SET DEBUG FILE TO '/ds/safreviv_int/BD/iniIntegInconCarIni.trace';
   --TRACE ON;
   ...
   LET v_error = 0;
   LET v_error_isam  = 0;
   LET v_mensaje = "El proceso de integracion de CI de recurrente finalizó correctamente";

   -- se obtiene el nombre del archivo y el usuario
   SELECT nom_archivo, usuario
     INTO v_c_nom_archivo, v_c_usuario
   ...
   -------------------------------------------------
   FOREACH
      -- se obtienen los datos de la tabla temporal del proceso de Inconsistente
      ...
      INTO tmp_nss,
           ...
           tmp_rel_laboral,
           tmp_fec_rel_laboral
      FROM safre_mig:tmp_cre_acreditado_carg_ini_incon
      ...
      IF v_ax_id_derechohabiente IS NOT NULL AND
         ...
         IF v_ax_tipo_trabajador = "S" THEN
         ...
         END IF
{
         ...
         LET cre_id_cre_acreditado  = safre_viv:seq_cre_acred.NEXTVAL;
         LET cre_id_cre_ctr_archivo = p_d_id_cre_ctr_arch;
         LET cre_folio_liquida      = 1;
      ...
      ELSE
         ...
         -- se inserta registro
         INSERT INTO safre_mig:tmp_cre_rch_acreditado_incon (
                     id_cre_ctr_archivo,
                     nss,
                     tpo_originacion,
                     tpo_credito,
   ...
   EXECUTE FUNCTION safre_mig:fn_act_edo_archivo(v_c_nom_archivo, p_d_folio, 2, v_c_usuario) INTO r_ax_bandera;
