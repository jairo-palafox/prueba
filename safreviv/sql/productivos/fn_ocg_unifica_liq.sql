...
CREATE FUNCTION "safreviv".fn_ocg_unifica_liq(p_id_unificador DECIMAL(9,0),p_id_unificado DECIMAL(9,0),p_folio INTEGER)
...
DEFINE v_alta                                DATE;
DEFINE v_tm_situacion                        SMALLINT;
DEFINE v_fm_situacion                        SMALLINT;
DEFINE v_liq_situacion                       SMALLINT;
DEFINE v_fm_sit                              SMALLINT;
DEFINE v_tm_sit                              SMALLINT;
DEFINE v_ug_sit                              SMALLINT;
DEFINE v_liq_sit                             SMALLINT;
DEFINE v_id_ocg_liq_unificado                DECIMAL(9,0);
DEFINE v_l_id_ocg_formalizacion              decimal(9,0); 
DEFINE v_l_id_ocg_tramite                    decimal(9,0);
DEFINE v_l_id_derechohabiente                decimal(9,0);
DEFINE v_l_cve_ent_financiera                smallint;
DEFINE v_l_num_ctr_int_ef                    char(18);
DEFINE v_l_bimestre_ap_subsec                char(6);
DEFINE v_l_importe_ap_subsec                 decimal(13,2);
DEFINE v_l_f_liberacion_gtia                 date;
DEFINE v_l_importe_devuelto                  decimal(13,2);
DEFINE v_l_id_causa_liquida                  smallint;
DEFINE v_l_f_deposito                        date;
DEFINE v_l_tpo_credito                       char(1);
DEFINE v_l_diagnostico                       char(2);
DEFINE v_l_estado                            smallint;
DEFINE v_l_situacion                         smallint;
DEFINE v_id_ocg_liq_unificador               DECIMAL(9,0);
DEFINE v_id_ocg_detalle_liq                  DECIMAL(9,0);
DEFINE v_id_ocg_uni_liq                      DECIMAL(9,0);
DEFINE v_id_ug_tramite_act                   DECIMAL(9,0);
DEFINE v_id_ocg_act                          DECIMAL(9,0);
...
-- se verifica si nss unificado es acreditado 43bis
   ...
   SELECT count(*)
    ...
    WHERE id_derechohabiente = v_id_dh_unificado
      AND situacion IN (140,150,155,160)
      AND diagnostico = 1;
...
-- se verifican datos de ocg_tramite del último registro aceptado y concrédito vigente
   ...
   FOREACH
      SELECT id_ocg_tramite,situacion
        INTO v_id_ocg_tramite_unificado,v_tm_situacion
        FROM ocg_tramite
       WHERE id_derechohabiente = v_id_dh_unificado
         AND situacion in (140,150,155,160)
         AND diagnostico = 1
...
-- inserta en ocg_fecha_mig
         ...
         LET v_tm_sit = v_tm_situacion + 200;

         LET v_id_ocg_uni_liq = seq_ocg_uni_liquidado.nextval;

         INSERT INTO ocg_uni_liquidado
              VALUES (v_id_ocg_uni_liq,
                      v_id_ocg_tramite_unificado,
                      v_id_ocg_tramite_unificador,
                      '1',
                      TODAY);

         UPDATE ocg_tramite
            SET situacion = v_tm_sit,
                estado    = v_tm_sit
          WHERE id_ocg_tramite = v_id_ocg_tramite_unificado;
...
-- se verifica si tiene formalización
   ...
   FOREACH 
      SELECT id_ocg_formalizacion,situacion
        INTO v_id_ocg_formalizacion_unificado,v_fm_situacion
...
--se debe recuperar el id_tramite del nuevo tramite que tega la formalizacion v_nvo_id_tramite
...
-- se insertan datos correspondientes
         ...
         SELECT id_ocg_actual
           INTO v_id_ocg_act
           FROM ocg_uni_liquidado
          WHERE id_ocg_anterior = v_f_id_tramite
            AND subproceso = 1;
...
-- se actualizan tablas
         ...
         LET v_fm_sit = v_fm_situacion + 200;

         LET v_id_ocg_uni_liq = seq_ocg_uni_liquidado.nextval;

         INSERT INTO ocg_uni_liquidado
              VALUES (v_id_ocg_uni_liq,
                      v_id_ocg_formalizacion_unificado,
                      v_id_ocg_formaliza_unificador,
                      '2',
                      TODAY);

         UPDATE ocg_formalizacion 
            SET situacion = v_fm_sit,
                estado    = v_fm_sit
          WHERE id_ocg_formalizacion = v_id_ocg_formalizacion_unificado;
...
--se recuperan consecutivos de secuencias
            ...
            SELECT id_ocg_actual
              INTO v_id_ocg_act
              FROM ocg_uni_liquidado
             WHERE id_ocg_anterior = v_u_id_ocg_tramite 
               AND subproceso = 1;

            LET v_id_ug_tramite_act = v_id_ocg_act;

            SELECT id_ocg_actual
              INTO v_id_ocg_act
              FROM ocg_uni_liquidado
             WHERE id_ocg_anterior = v_u_id_ocg_formalizacion 
               AND subproceso = 2;

            LET v_id_ocg_formaliza_unificador = v_id_ocg_act;
...
--se actualiza situacion en registro de unificado
         ...
         LET v_ug_sit = v_u_situacion + 200;
         ...
         INSERT INTO ocg_uni_liquidado
              VALUES (v_id_ocg_uni_liq,
                      ...
                      v_id_ocg_solic_ug_unificador,
                      '3',
                      TODAY);

            UPDATE ocg_solicitud_uso_garantia
               SET situacion = v_ug_sit,
                   estado    = v_ug_sit
...
-- se recuperan datos de crédito liquidado

   FOREACH
      SELECT id_ocg_liquidacion,situacion
        INTO v_id_ocg_liq_unificado,v_liq_situacion
        FROM ocg_liquidacion
       WHERE id_derechohabiente = v_id_dh_unificado
         AND situacion in (140,150,155,160)
         AND diagnostico = 1

-- si v_id_ocg_liq_unificado no es nulo, se recuperan datos de ocg_liquidacion
      IF v_id_ocg_liq_unificado IS NOT NULL THEN
         ...
         SELECT id_ocg_formalizacion,
                ...
                estado,
                situacion
           INTO v_l_id_ocg_formalizacion,
                v_l_id_ocg_tramite,
                v_l_cve_ent_financiera,
                v_l_num_ctr_int_ef,
                v_l_bimestre_ap_subsec,
                v_l_importe_ap_subsec,
                v_l_f_liberacion_gtia,
                v_l_importe_devuelto,
                v_l_id_causa_liquida,
                v_l_f_deposito,
                v_l_tpo_credito,
                v_l_diagnostico,
                v_l_estado,
                v_l_situacion
           FROM ocg_liquidacion
          WHERE id_ocg_liquidacion = v_id_ocg_liq_unificado;
         ...
         SELECT f_envio,
           ...
           INTO v_envio,
                ...
                v_alta
           FROM ocg_fecha_mig
          WHERE id_ocg_referencia = v_id_ocg_liq_unificado
            AND subproceso = 5;

         LET v_id_ocg_liq_unificador = seq_ocg_liquidacion.nextval;
         LET v_id_ocg_detalle_liq    = seq_ocg_detalle.nextval;

         INSERT INTO ocg_detalle
              VALUES(v_id_ocg_detalle_liq,
                     0,
                     v_id_dh_unificador,
                     1,
                     today,
            ...
            SELECT id_ocg_actual
              INTO v_id_ocg_act
              FROM ocg_uni_liquidado
             WHERE id_ocg_anterior = v_l_id_ocg_tramite
               AND subproceso = 1;

            LET v_l_id_ocg_tramite = v_id_ocg_act;

            SELECT id_ocg_actual
              INTO v_id_ocg_act
              FROM ocg_uni_liquidado
             WHERE id_ocg_anterior = v_l_id_ocg_formalizacion
               AND subproceso = 2;

            LET v_l_id_ocg_formalizacion = v_id_ocg_act;


         INSERT INTO ocg_liquidacion
              VALUES(v_id_ocg_liq_unificador,
                     v_id_ocg_detalle_liq,
                     v_l_id_ocg_formalizacion,
                     v_l_id_ocg_tramite,
                     v_id_dh_unificador,
                     v_l_cve_ent_financiera,
                     v_l_num_ctr_int_ef,
...
-- inserta en ocg_fecha_mig
         ...
         LET v_liq_sit = v_liq_situacion + 200;

         LET v_id_ocg_uni_liq = seq_ocg_uni_liquidado.nextval;

         INSERT INTO ocg_uni_liquidado
              VALUES (v_id_ocg_uni_liq,
                      v_id_ocg_liq_unificado,
                      v_id_ocg_liq_unificador,
                      '5',
                      TODAY);

         UPDATE ocg_liquidacion
            SET situacion = v_liq_sit,
                estado    = v_liq_sit
          WHERE id_ocg_liquidacion = v_id_ocg_liq_unificado;
