...
CREATE PROCEDURE "safreviv".sp_dwh_extractor_traspaso_ifv_afore()
RETURNING INTEGER, INTEGER, CHAR(200);
   ...
   CREATE TEMP TABLE tmp_mov_tia(f_liquida DATE ,
   ...
   LET v_query = "INSERT INTO tmp_mov_tia " ||
      "SELECT f_liquida, id_derechohabiente, "||
         ...
         " FROM cta_movimiento"||
        " WHERE subcuenta = 8 "||
          " AND movimiento IN (53,111,231,362,592) ";
   ...
   CREATE INDEX xtmp_mov_tia ON tmp_mov_tia (folio_liquida, id_derechohabiente);


   INSERT INTO safre_sdo@vivws_tcp:saci_traspaso_ifv_afore
        SELECT c.nss,
               "",
               "",
               "",
   ...
   INSERT INTO safre_sdo@vivws_tcp:glo_ctr_dwh
        SELECT TODAY,
               "saci_traspaso_ifv_afore",
               TODAY,
               ( SELECT COUNT(*) FROM safre_sdo@vivws_tcp:saci_traspaso_ifv_afore ),
               1
          FROM SYSTABLES
         WHERE tabid = 1;
END PROCEDURE
