...
create procedure "safreviv".fn_ret_tra_jud_cnt31(p_folio_liquida      DECIMAL(9,0),  --Folio de liquidación del proceso
...
DEFINE v_movimiento          SMALLINT;       --Tipo Movimiento
  ...
  --SET DEBUG FILE TO '/ds/safreviv_int/BD/fn_ret_tra_jud_cnt31.trace';
  ...
  --p_transaccion = 37 (Transacción Liquidación del proceso de Devolución del 
  --                    Saldo 97 por Trámite Judicial)
  --p_transaccion = 60 (Cancelación DAP's no cobrado)

  --Obtiene la suman del importe en pesos para la referencia a la cta contable
  --p_transaccion = 37 (Transacción Liquidación del proceso de Devolución del 
  --                    Saldo 97 por Trámite Judicial)
  IF p_transaccion = 37 THEN
