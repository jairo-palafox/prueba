...
CREATE FUNCTION "safreviv".fn_sep_integra_nrp_op29(p_folio           DECIMAL(9,0),
...
DEFINE v_pos_det03                    INTEGER      ;
   ...
   SET DEBUG FILE TO '/tmp/fn_sep_integra_nrp_op29.trace';
   --TRACE 'Inicia SPL ';
TRACE ON;
   ...
   --TRACE 'Inicia Insert en sep_cza_op29';
   ...
   --TRACE 'recuepera tmp_sep_det_02_op29';
   FOREACH SELECT tipo_registro      ,
      ...
      --TRACE 'v_t_invadido: '||v_m_det02_invadido;
      --TRACE 'v_m_id_derechohabiente: '||v_m_det02_id_derechohabiente_invadido;

      SELECT id_derechohabiente
        INTO v_m_det03_id_derechohabiente_asociado
        FROM safre_viv:afi_derechohabiente
       WHERE nss = v_m_det03_asociado;
      --TRACE 'v_t_det03_asociado: '||v_m_det03_asociado;
      --TRACE 'v_m_det03_id_derechohabiente: '||v_m_det03_id_derechohabiente_asociado;

      --TRACE 'Buscando op29 en operacion 28 (Det02) de nss: '||v_m_det02_id_derechohabiente_invadido;
              SELECT id_det_02_op28 , 
               ...
               WHERE id_derechohabiente_invadido = v_m_det02_id_derechohabiente_invadido
                 AND estado IN (25);
      --TRACE 'id_DET_02_OP28: '||v_m_det02_id_det_02_op28||' '||v_estado;
      --TRACE 'Buscando op29 en operacion 28 (Det03) de nss: '||v_m_det03_id_derechohabiente_asociado;
              SELECT NVL(COUNT(*),0)
                INTO v_cont_det03
                FROM sep_det_03_op28
               WHERE id_derechohabiente_asociado = v_m_det03_id_derechohabiente_asociado;
      --TRACE 'count(*) d03: '||v_cont_det03;

      IF (v_m_det02_id_det_02_op28 > 0 AND v_cont_det03 > 0) THEN
         --TRACE 'Parejas encontradas';
         LET v_total_aceptadas = v_total_aceptadas + 1;
         LET v_m_estado = 15;
      ELSE
         --TRACE 'Pareja no encontrada';
         LET v_total_rechazados = v_total_rechazados + 1;
         LET v_m_estado = 10;
      END IF;
      
      	 --TRACE 'Insertando movimientos Det02 y Det03 de OP29';
         ...
         --TRACE 'inserta en sep_det_02_op29'; 
         ...
         --TRACE 'Insert en sep_det_03_op29';
         ...
         END WHILE;
         --TRACE 'termina procesamiento de detalle';
      LET v_total_integrados = v_total_integrados + 1;
   END FOREACH;



   --TRACE 'Inicia Insert en sep_sum_op29';
   ...
   --TRACE 'actualiza glo_ctr_archivo';
