...
CREATE FUNCTION "safreviv".fn_cbd_marca_nss_inconsistentes()
      RETURNING DECIMAL(9,0),varchar(11),varchar(13),varchar(40),
                varchar(40),varchar(40),DECIMAL(22,6),DECIMAL(22,6);
--Funcion que marca cuentas con nss inconsistentes y retorna información de estas 
	...
	INSERT INTO tmp_nss_negativos
	SELECT id_derechohabiente,nss from afi_derechohabiente
		where  nss not matches '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]';
                                --and nss not matches '*-*';PRODINFXV-62
		
  SELECT tabla_saldo--busqueda de la tabla en servicio 
  INTO v_tabla
               ...
               CALL fn_marca_cuenta(v_id_derechohabiente,160,0,0,0,0,160,null,'OPSISSACI','703') RETURNING v_respuesta;
				IF v_respuesta=0 then 
					LET v_contador=v_contador+1;
                     ...
                     v_a_materno, v_nombre, v_s_92_AIVS, v_s_97_AIVS with resume;
				END IF
