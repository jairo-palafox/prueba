...
CREATE FUNCTION "safreviv".fn_insrt_dse_ctr_arch_rchdevsdos(p_c_nom_archivo CHAR(40),
                                                 p_d_pid DECIMAL(9,0),
                                                 p_v_usuario CHAR(20),
                                                 p_i_estado SMALLINT)
   RETURNING SMALLINT, INTEGER
   -- Registro de dse ctr archivo
   DEFINE v_ra_tpo_transferencia CHAR(2); -- operacion del proceso (campo registro)
   ...
   DEFINE v_ax_tpo_transf     CHAR(2); -- tipo de transferencia rechazos DSE
   DEFINE v_b_estatus_proc    SMALLINT; -- contiene el estatus a regresar segun el proceso

   --SET DEBUG FILE TO 'dbg_dseCtrArch_rechDSE.log';

   -- se inicializan variables
   LET v_ax_tpo_transf = "56";
   ...
   IF p_i_estado = 10 THEN
      ...
      SELECT UNIQUE nss
        ...
        FROM safre_tmp:tmp_rch_devolucion
         ...
         -- se buscar el nss en proceso en el catálogo de derechohabientes
         IF NOT EXISTS (
         SELECT nss
   ...
   IF p_i_estado = 10 THEN
      SELECT COUNT(*)
        INTO v_ax_cuenta_reg
        FROM safre_tmp:tmp_rch_devolucion_sum;
   ...
   LET v_ra_tpo_transferencia = v_ax_tpo_transf;
