...
CREATE FUNCTION "safreviv".fn_mig_ret_carga_inicial_transferencia( p_usuario_cod     CHAR(20)     ,
...
--TRACE "archivo: " || p_nombre_archivo;
   ...
   WHERE f_transferencia = p_fecha_valor -- para la fecha valor en turno
      ...
      LET v_pesos_aiv97 = ret_transf_aivs_viv97 * ret_cza_transferencia_precio_fondo;
   ...
   LET ret_cza_transferencia_f_valor_transferencia = p_fecha_valor; -- date,
   -- el precio del fondo se inserto en el foreach del detalle
   --LET ret_cza_transferencia_precio_fondo          = 0; -- decimal(14,6),
   LET ret_cza_transferencia_total_registros       = 0; -- integer,
   LET ret_cza_transferencia_total_importe         = v_sumario_importe_total; -- decimal(22,2),
   ...
   IF ( v_reg_det_rechazados > 0 OR v_b_rechazo_encabezado > 0 ) THEN
      ...
      INTO
         ...
         ret_transf_cod_rechazo           
      FROM
      safre_viv:ret_transferencia
      WHERE folio = p_folio
      
         -- se insertan en la tabla de rechazos
         -- se inserta el registro de rechazo
         INSERT INTO ret_transferencia_rch(
            id_derechohabiente    ,
            folio                 ,
      ...
      -- se rechaza el registro del encabezado
   ...
   UPDATE STATISTICS FOR TABLE safre_viv:ret_cza_transferencia;
