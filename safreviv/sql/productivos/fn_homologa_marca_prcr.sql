...
CREATE FUNCTION "safreviv".fn_homologa_marca_prcr(p_usuario CHAR(40),
                                       p_proceso SMALLINT,
                                       p_folio   DECIMAL(9,0))
   RETURNING SMALLINT;
            ...
            (v_tpo_originacion = 4)     THEN
{
...

   {
               ...
               IF v_marca_prc <> 232 THEN
                  IF v_marca_prc = 231 THEN
                     LET v_marca_p14 = 234;
                  END IF

                  IF v_marca_prc = 234 THEN
                     LET v_marca_p14 = 231;
                  END IF
...
--TRACE'Punto de control 2-1 : '||v_id_dh;
       ...
       IF ((v_tpo_originacion = 1) OR (v_tpo_originacion = 2) OR (v_tpo_originacion = 4) AND (bnd_reg_homologado = 1))
        OR
          ((v_tpo_originacion = 1) OR (v_tpo_originacion = 4) OR (bnd_reg_homologado = 1)) THEN
