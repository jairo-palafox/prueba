...
CREATE FUNCTION "safreviv".fn_uni_integra_resp_infonavit(p_folio_int_resp_sist_inf  DECIMAL(9,0),                         
                                              p_pid                      DECIMAL(9,0),
...
DEFINE v_resp_nss_unificado     CHAR(11);
...
END EXCEPTION 
   ...
   --SET DEBUG FILE TO '/ds/safreviv_int/BD/fn_integra_respuesta_INFONAVIT.trace';
   --TRACE ON;
...
LET err_txt        = '';
      ...
      SELECT fec_proceso
        INTO v_fec_proceso
        FROM safre_tmp:tmp_sum_res_oci; 
...
--TRACE 'ENTRA CONSULTA UNIFICADOrrrrr';      
         SELECT f_notificacion, 
          ...
          GROUP BY 1,2;
          --TRACE 'v_nss_unificador'||v_nss_unificador;
          --TRACE 'v_f_notificacion'||v_f_notificacion;
          --TRACE 'v_id_unificador'||v_id_unificador;
         ...
         --TRACE 'ENTRA CONSULTA UNIFICADO';
         --TRACE 'v_nss_unificado'||v_nss_unificado;  
         ...
         FOREACH
            ...
            --TRACE 'v_nss_unificado'||v_nss_unificado;  
            --TRACE 'v_id_unificado, '||v_id_unificado;
            
            IF v_nss_unificado IS NULL THEN
            ...
            SELECT COUNT(*)                                                        
             ...
             WHERE nss = v_nss_unificador
             AND   folio = p_folio_int_resp_sist_inf;                                                                               
   ...
   FOREACH 
      ...
      FROM   uni_resp_int_unificador
      WHERE  folio = p_folio_int_resp_sist_inf
      AND    estado = 1
      ...
      FOREACH
         ...
         SELECT nss
         INTO   v_resp_nss_unificado
         FROM   uni_inf_unificado
         WHERE  id_inf_unificado = v_resp_id_referencia_ado
         AND    id_unificador = v_resp_id_referencia;
         
         IF v_resp_nss <> v_resp_nss_unificado THEN
   ...
   FOREACH 
      ...
      WHERE  id_inf_unificador = v_env_id_referencia
      AND    estado_familia    = 1
   ...
   UPDATE bat_ctr_operacion 
      SET folio       = p_folio_int_resp_sist_inf,
