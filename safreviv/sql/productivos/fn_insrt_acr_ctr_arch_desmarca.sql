...
CREATE FUNCTION "safreviv".fn_insrt_acr_ctr_arch_desmarca(p_c_nom_archivo CHAR(40),
                                               p_i_estado SMALLINT,
                                               p_v_usuario CHAR(20))
   ...
   DEFINE v_ax_regs_det_desm   INTEGER; -- total de registros insertados como tipo 11 (Desmarca)
   DEFINE v_ax_regs_sum        INTEGER; -- contador de registros en sumario
   DEFINE v_ax_regs_tot_sum    INTEGER; -- numero total de registros registrados en sumario
   ...
   --SET DEBUG FILE TO '/ds/safreviv_int/BD/acrCtrArch_desmarca.trace';
   --TRACE ON;
   ...
   -- se crea la sentencia sql que obtiene el numero registros insertados como tipo 11
   SELECT COUNT(*)
     INTO v_ax_regs_11_det
     FROM safre_tmp:tmp_acr_desmarca;

   -- se agrupan los registros de liquidación de crédito en la tabla 11
   INSERT INTO safre_tmp:tmp_acr_desmarca SELECT * FROM safre_tmp:tmp_acr_desmarca_05;
   
   -- se agrupan los registros de rechazo en la tabla 02
   INSERT INTO safre_tmp:tmp_acr_desmarca_02 SELECT * FROM safre_tmp:tmp_acr_desmarca_03;
   INSERT INTO safre_tmp:tmp_acr_desmarca_02 SELECT * FROM safre_tmp:tmp_acr_desmarca_06;
   INSERT INTO safre_tmp:tmp_acr_desmarca_02 SELECT * FROM safre_tmp:tmp_acr_desmarca_07;
   INSERT INTO safre_tmp:tmp_acr_desmarca_02 SELECT * FROM safre_tmp:tmp_acr_desmarca_10;
   INSERT INTO safre_tmp:tmp_acr_desmarca_02 SELECT * FROM safre_tmp:tmp_acr_desmarca_13;

   -- se agrupan los registros de Recurrente Originación en la tabla 01
   INSERT INTO safre_tmp:tmp_acr_desmarca_01 SELECT * FROM safre_tmp:tmp_acr_desmarca_20;

   -- se agrupan los registros de Reactivación en la tabla 04
   INSERT INTO safre_tmp:tmp_acr_desmarca_04 SELECT * FROM safre_tmp:tmp_acr_desmarca_08;

   -- se crea la sentencia sql que obtiene el numero registros insertados en la tabla detalle (Originación)
   SELECT COUNT(*)
     INTO v_ax_regs_det_orig
     FROM safre_tmp:tmp_acr_desmarca_01;

   -- se crea la sentencia sql que obtiene el numero registros insertados en la tabla detalle (Sin originación)
   SELECT COUNT(*)
     INTO v_ax_regs_det_sinor
     FROM safre_tmp:tmp_acr_desmarca_02;

   -- se crea la sentencia sql que obtiene el numero registros insertados en la tabla detalle (Reactivación)
   SELECT COUNT(*)
     INTO v_ax_regs_det_react
     FROM safre_tmp:tmp_acr_desmarca_04;

   -- se crea la sentencia sql que obtiene el numero registros insertados en la tabla detalle (Desmarca)
   SELECT COUNT(*)
   ...
   SELECT COUNT(*)
     INTO v_ax_regs_sum
     FROM safre_tmp:tmp_acr_desmarca_sum;
   ...
   IF p_i_estado = 10 THEN
      ...
      IF v_ax_regs_det_desm = 0 AND v_ax_regs_det_orig = 0 AND v_ax_regs_det_react = 0 THEN
...
{
   -- Ya no se valida que el NSS exista en catalogo, en la integración se rechaza
   -- el registro para este caso. 23/11/2012 12:33:05 p.m.
   ...
   IF p_i_estado = 10 THEN
      ...
      SELECT UNIQUE nss
        ...
        FROM safre_tmp:tmp_acr_desmarca
         ...
         -- se buscar el nss obtenido en el catalogo
         IF NOT EXISTS (
         SELECT nss
