...
CREATE FUNCTION "safreviv".fn_agr_integra_orig_mf_recurr(v_rt_tpo_registro       CHAR(2),
                                              v_rt_nss                CHAR(11),
   ...
   --SET DEBUG FILE TO '/ds/safreviv_int/BD/agrIntegRecurrmf.trace';
   --SET DEBUG FILE TO '/home/selefp/homologa/agrIntegRecurr.trace';
   --TRACE ON;
   ...
   LET vcodResp                   = "01";
   LET v_ax_cre_acreditado        = 0;
   LET v_ax_edo_prcsr             = 0;
   ...
   IF v_ax_ins_hist = 1 THEN
      ...
      ELSE
         ...
         -- se desactiva la bandera
            LET v_ax_exist_numcred = 0; --no existe nci
         --END IF
      END IF
      ...
      IF v_ax_historico <> 1 THEN
         IF EXISTS (SELECT id_cre_acreditado
            ...
            DELETE
              FROM cre_saldo_deudor
             WHERE id_cre_acreditado = v_sdo_id_cre_acreditado
               AND movimiento        = v_ax_movimiento;
         END IF
      END IF
