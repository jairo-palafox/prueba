...
CREATE PROCEDURE "safreviv".fn_dis_transaccion12(p_fecha_01mm           DATE,          --Fecha de movimiento, proceso
...
RETURNING SMALLINT, SMALLINT, CHAR(70), DECIMAL(9,0), DECIMAL(9,0)
...
ON EXCEPTION
   SET sql_err, isam_err, error_info
       ...
       RETURN v_status, isam_err, error_info, v_derechohabiente_pag, p_folio_disp ;
END EXCEPTION

--SET DEBUG FILE TO '/ds/safreviv_int/dis/respaldo/PRODINF-639/fn_dis_transaccion12.TRACE';
--TRACE ON;
...
LET v_edo_num_credito      = 0; --Req SACI2018-175
  ...
  FOREACH
    ...
    FROM   cta_his_pagos pag,
           glo_valor_fondo fon,
           dis_liq_cred_vigente dlcv
    WHERE  pag.folio                = dlcv.folio_pago
    AND    pag.id_referencia        = dlcv.id_referencia
    AND    pag.id_derechohabiente   = dlcv.id_derechohabiente
    AND    pag.ind_liquidacion NOT IN (1,6)
    AND    fon.fondo                = 11
    AND    pag.f_pago               = fon.f_valuacion
    AND    dlcv.edo_liquida         = 0
    ...
    IF v_derechohabiente_pag = 4640460 THEN
       LET v_imp_ap_pat  = 0;
       LET v_aiv_ap_pat  = 0;
       LET v_monto_pesos = 0;
    END IF
  ...
  UPDATE STATISTICS FOR TABLE dis_liq_cred_vigente;
  ...
  EXECUTE PROCEDURE fn_dis_cnt19(p_folio_disp, TODAY, 19, p_proceso_cod, 0)
               INTO r_bnd_proceso_cnt;

  EXECUTE PROCEDURE fn_dis_cnt19(p_folio_disp, TODAY, 19, p_proceso_cod, 83)
               INTO r_bnd_proceso_cnt;

  EXECUTE PROCEDURE fn_revisa_reg_cnt(p_folio_disp, TODAY, 0)
               INTO r_bnd_proceso_cnt;

  --TRACE 'Finaliza fn_dis_transaccion con valor '||v_bnd_proceso;
  LET error_info = "  Liquidación finalizo correctamente";
  RETURN v_bnd_transaccion , v_status , error_info, '',p_folio_disp;
