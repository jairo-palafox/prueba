...
CREATE PROCEDURE "safreviv".sp_genera_pago277()
   ...
   DEFINE v_preliq_monto_acciones     DECIMAL(12,2);
   DEFINE v_preliq_monto_pesos        DECIMAL(12,2);
  ...
  -- TRACE "Inicia  FOREACH";
   ...
   FOREACH cur_his_pagos FOR
      ...
      FROM   cta_his_pagos a,
             ...
             safre_tmp:tmp_nss9141 tmp
      WHERE  a.folio = p_folio     -- El folio se captura arriba de este query
      and    c.folio = a.folio
      and    c.id_referencia = a.id_referencia
      AND    a.result_operacion = 1
      AND    a.id_derechohabiente = tmp.id_derecho
      AND    a.folio_sua          = tmp.folio_sua
      AND    a.periodo_pago       = tmp.periodo_pago
      AND    a.f_pago             = tmp.f_pago
      AND    a.nrp                = tmp.nrp
      AND    a.cve_ent_receptora  = tmp.cve_ent
      AND    a.imp_ap_pat         = tmp.imp_ap_pat
      AND    a.ind_liquidacion    = 4
      and    tmp.indicador = 9141
