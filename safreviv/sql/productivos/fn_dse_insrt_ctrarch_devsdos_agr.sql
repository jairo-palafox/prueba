...
CREATE FUNCTION "safreviv".fn_dse_insrt_ctrarch_devsdos_agr(p_c_nom_archivo CHAR(40),
                                                 p_d_pid         DECIMAL(9,0),
                                                 p_v_usuario     CHAR(20),
                                                 p_i_estado      SMALLINT)
   RETURNING SMALLINT, SMALLINT, INTEGER
   -- Registro de dse ctr archivo
   DEFINE v_ra_operacion      SMALLINT; -- operacion del proceso (campo registro)
   ...
   DEFINE v_cnt_nss           SMALLINT; -- contador de nss no catalogados
   ...
   --SET DEBUG FILE TO '/ds/safreviv_int/BD/fn_dse_insrt_ctrarch_devsdos_agr.trace';
   --TRACE ON;

   -- se asigna la operacion del proceso (43 - Devolucion De Saldos Anualidades)
   ...
   IF p_i_estado = 10 THEN
      SELECT COUNT(*)
        INTO v_ax_cuenta_reg
        FROM safre_tmp:tmp_dse_devol_sum_agr;
   ...
   IF p_i_estado = 10 THEN
      ...
      INSERT INTO safre_tmp:tmp_nss_no_catalogados_agr
      SELECT nss,
             num_aplic_interes97,
             num_aplic_interes92
        FROM safre_tmp:tmp_dse_devol_det_agr
