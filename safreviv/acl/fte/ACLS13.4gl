...
-- Modulo       => ACL                                                                    
-- Programa     => ACLS13
-- Objetivo     => Reporte cifras de rechazo de ACL CON cambio de nss en automatico al liquidar para MENÚ
-- Autor        => GERARDO ALFONSO VEGA PAREDES                                           
-- Fecha inicio => 20 de Junio de 2017                                                  
-----------------------------------------------------------------------------------------
...
--   LET g_nom_archivo = ARG_VAL(6)
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".ACLS13.log")

   -- se abre la ventana
   OPEN WINDOW w_consulta WITH FORM "ACLS131"
     ...
     ATTRIBUTES (UNBUFFERED)
      ...
      ON ACTION accept
         IF v_folio IS NOT NULL THEN
             ...
             FROM   glo_folio
             WHERE  proceso_cod = 103   -- aclaraciones sin cambio de NSS
             AND    folio = v_folio
            ...
            CALL fn_mensaje("Atención","Reporte generado","stop")
...
--===============================================================================
   ...
   WHERE  folio = p_folio
    ...
    LET v_v_nom_reporte = p_usuario_cod CLIPPED,"-ACLS13-",
                          g_pid USING "&&&&&", "-",
                          g_proceso_cod USING "&&&&&","-",
                          5 USING "&&&&&"
    ...
    CALL fgl_report_loadCurrentSettings(r_ruta_bin CLIPPED ||"/ACLS13.4rp") RETURNING v_existe
