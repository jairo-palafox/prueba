...
-- Modulo       => ACL                                                                    
-- Programa     => ACLS08                                                                 
-- Objetivo     => Reporte cifras de rechazo de ACL con cambio de nombre en automatico al liquidar
...
MAIN
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".ACLS08.log")
...
--   FROM   cta_his_pagos      --saci2018-67
   ...
   ELSE 
      ...
      CALL fn_display_proceso(0,"INICIO REPORTE RECHAZOS DISCCNOM")  
      ...
      CALL fn_actualiza_opera_fin(g_pid,g_proceso_cod,5) RETURNING r_resultado_opera
      LET v_mensaje = "El reporte de cifras de rechazo de disccnom ha finalizado correctamente"

      CALL fn_correo_proceso(g_pid,
                             g_proceso_cod,
                             4,
                             '', # Archivo adjunto
                             'Finalización de operación - '||v_proceso_desc CLIPPED||' - REPORTE RECHAZOS DISCCNOM',
                              v_mensaje
                             )
...
--===============================================================================
    ...
    LET v_v_nom_reporte = p_usuario_cod CLIPPED,"-ACLS08-",
                          g_pid USING "&&&&&", "-",
                          g_proceso_cod USING "&&&&&","-",
                          5 USING "&&&&&"
    ...
    CALL fgl_report_loadCurrentSettings(r_ruta_bin CLIPPED ||"/ACLS08.4rp") RETURNING v_existe
...
--             " FROM cta_his_pagos cta ",              --saci2018-67
    ...
    FOREACH cur_obtiene_causales INTO g_arr_causales_rechazo[v_contador_causales].*
        DISPLAY "VALORES:",g_arr_causales_rechazo[v_contador_causales].*
