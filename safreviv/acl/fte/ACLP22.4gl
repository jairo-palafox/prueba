...
#Módulo          => ACL                                                         #
#Programa        => ACLP22                                                      #
#Objetivo        => Programa de preliquidación de Aclaracion con cambio Nombre  #
...
DEFINE p_folio                     LIKE glo_folio.folio,
   ...
   IF(NUM_ARGS() > 0)THEN
      ...
      CALL STARTLOG(p_usuario_cod CLIPPED||".ACLP22.log")
      --Se imprimen los displays para el log
      CALL fn_display_proceso(0,"Preliquidación de aclaración con cambio de nombre")
         ...
         ELSE
            ...
            CALL fn_ejecuta_preliquidacion_conCambioNombre(p_folio,p_usuario_cod)
                             RETURNING r_resultado_opera
            # cambia a estado errone si no se ejecutó correctamente el SP
            IF(r_resultado_opera)THEN
               LET p_titulo = "Error de operación - Aclaraciones Con Cambio Nombre - Preliquidación"
               SELECT descripcion
                 INTO v_descripcion 
                 FROM cat_bat_parametro_salida
                WHERE cod_salida = r_resultado_opera
               ...
               CALL fn_error_opera(p_pid,p_proceso_cod,p_opera_cod_preliquidacion) 
                 RETURNING r_resultado_opera
            ELSE
               CALL fn_display_proceso(1,"Preliquidación de aclaración con cambio nombre")
               ...
               --se obtiene el codigo de programa
                  ...
                  LET p_titulo = "Finalización de operación - Aclaraciones Con Cambio Nombre - Preliquidación"
               END IF
            END IF
         END IF
...
{ ==========================================================================
Clave:  fn_ejecuta_preliquidacion_conCambioNombre
Nombre: fn_ejecuta_preliquidacion_conCambioNombre
Fecha creacion: 15 de Febrero de 2012
Autor: Hugo César Ramírez García
Narrativa del proceso que realiza:
 Esta función executa el store procedure que almacena la información 
 de la preliquidación para el módulo de "Aclaracion con cambio Nombre"
...
============================================================================}
FUNCTION fn_ejecuta_preliquidacion_conCambioNombre(p_folio,p_usuario)
DEFINE p_folio         LIKE glo_folio.folio,
       p_usuario       CHAR(20),
       v_sql_procedure STRING,
       v_error_sql     SMALLINT,
       v_error_isam    SMALLINT,
       v_mensaje_error CHAR(200),
       v_origen_archivo SMALLINT

   --WHENEVER ERROR CONTINUE
   --DISPLAY p_folio,p_usuario
   ...
   PREPARE prp_sqlPreliquidacionconCambioNombre FROM v_sql_procedure
   EXECUTE prp_sqlPreliquidacionconCambioNombre USING p_folio, p_usuario, v_origen_archivo
  INTO v_error_sql,v_error_isam,v_mensaje_error
   ...
   ELSE
      ...
      DISPLAY "Error en sp_preliquida_con_cambio_nss (ISAM):",v_error_isam,"\n"
