...
#Modulo       => ACL                                                                    #
#Programa     => ACLR16                                                                 #
#Objetivo     => Programa que realiza el reverso parcial(carga) para ACL Cambio Nombbre #
#Fecha inicio => Septiembre 11, 2012                                                    #
...
DEFINE p_usuario_cod      LIKE seg_usuario.usuario_cod
   ...
   -- se inicia el log
   CALL STARTLOG (p_usuario_cod CLIPPED|| ".ACLR16.log")
 {  
   -- se borran los datos de las tablas historicas
   DELETE FROM cta_his_pagos WHERE folio = p_d_folio
   DELETE FROM cta_pag_complemento WHERE folio = p_d_folio
   DELETE FROM acl_sum_sc_nss WHERE folio = p_d_folio

   -- se cambia el estatus del folio
   LET v_SqlQry = "UPDATE glo_folio SET status = -1 WHERE folio = ?"
   PREPARE sid_folio FROM v_SqlQry
   EXECUTE sid_folio USING p_d_folio
}

   -- se ejecuta el SP de reverso de integracion de LQINFO  
   LET v_SqlQry = "EXECUTE PROCEDURE sp_acl_ccnss_rev_integracion(?)"
   PREPARE sid_reverso FROM v_SqlQry
