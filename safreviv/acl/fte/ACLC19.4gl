...
-- Modulo        => ACL                                                                    
-- Programa      => ACLC19                                                                
-- Objetivo      => Consulta Detalle de rechazos por nss
-- Autor         => GERARDO ALFONSO VEGA PAREDES.
-- Fecha inicio  => 20 de Septiembre de 2018.
...
FUNCTION fn_consulta_registros(p_usuario_cod)
   ...
   DEFINE arr_registros DYNAMIC ARRAY OF RECORD
      ...
      nom_archivo           LIKE bat_ctr_proceso.nom_archivo,
      descripcion           CHAR(50)
   END RECORD
   
   DEFINE v_c_localiza_cod LIKE pag_localiza_trabajador.localiza_cod
   DEFINE v_arr_curr           INTEGER
       
   OPEN WINDOW w_consulta_registros WITH FORM "../../acl/bin/ACLC191"
   LET  v_ventana = UI.WINDOW.GETCURRENT()
   CALL v_ventana.SETTEXT("Consulta detalle de rechazos")
      ...
      INPUT v_nss FROM  ed_nss ATTRIBUTES(UNBUFFERED, WITHOUT DEFAULTS)
         ON ACTION ACCEPT
            IF v_nss IS NULL THEN
               ...
               NEXT FIELD ed_nss         	 
            ELSE
               LET v_query = " SELECT afi.id_derechohabiente ",
   ...
   IF NOT INT_FLAG THEN
      ...
      IF v_registros > 0 THEN
         ...
         LET v_query ="SELECT a.origen_archivo        ,",
                     ...
                     "AND    r.codigo_rechazo = t.codigo_rechazo ",
         ...
         FOREACH cur_registros INTO  cve_origen_archivo,
                                     ...
                                     arr_registros[v_indice].nom_archivo,
                                     arr_registros[v_indice].descripcion
             ...
             END IF
             IF arr_registros [v_indice].result_operacion CLIPPED = 3 THEN
                LET arr_registros [v_indice].result_operacion = "Rechazado"
             END IF             
...
FUNCTION fn_datos_complementarios(p_origen_archivo, arr_registros )
   ...
   OPEN WINDOW w_complementarios WITH FORM "../../acl/bin/ACLC192"
   LET  v_ventana = UI.WINDOW.GETCURRENT()
   ...
   IF INT_FLAG = 1 THEN  
      ...
      IF fgl_report_loadCurrentSettings("../../acl/bin/ACLC19.4rp") THEN 
      ...
      ELSE         
         DISPLAY "No fue posible generar el reporte. No se encuentra la plantilla ACLC19.4rp"
         EXIT PROGRAM 
      END IF   
