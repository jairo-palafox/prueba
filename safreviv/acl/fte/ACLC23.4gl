...
-- Modificación => Agregar porcentaje a todas las causales                                    --
...
MAIN
   ...
   DEFINE 
      ...
      v_folio_busqueda LIKE glo_folio.folio
      ...
      ON ACTION accept
         IF ( v_folio IS NOT NULL ) THEN
            ...
            IF ( v_folio_busqueda IS NULL ) THEN
               CALL fn_mensaje("Atención","El folio no corresponde con un proceso de Aclaraciones sin cambio de NSS","stop")
               CONTINUE INPUT
            END IF
...
DEFINE p_folio                 LIKE glo_folio.folio, -- folio de consulta
       v_r_desp         RECORD -- registro de consulta
   ...
   DEFINE v_cont_ace INTEGER  --saci2018-67-03
   DEFINE v_cont_ade INTEGER  --saci2018-67-03
   DEFINE v_cont_rch INTEGER  --saci2018-67-03
   DEFINE v_cont_esp INTEGER  --saci2018-67-03
   DEFINE v_cont_con INTEGER  --saci2018-67-03
...
--               "\n and    d.result_operacion <> 2                   ", 
               "\n and    d.result_operacion = 1                      ", 
--               "\n and    d.ind_liquidacion not in (4,-1)           ", --inc-1073065
   ...
   LET v_cont_ace = 0     --saci2018-67-03
   
   -- se transfieren los datos al arreglo de consulta
   FOREACH cur_consulta_aceptados INTO v_r_desp.*
      LET v_arr_despliegue[v_contador].* = v_r_desp.*
...
--      AND    d.result_operacion   <> 2
      ...
      AND    d.result_operacion   = v_r_desp.result_operacion
      AND    d.destino_ap_viv     = v_r_desp.destino
      AND    d.int_gen_pgo_ext    <> 0

      -- se incrementa el contador
      LET v_contador = v_contador + 1
      LET v_cont_total = v_cont_total + v_r_desp.num_registros   --prodinfxvi-141
      LET v_cont_ace = v_cont_ace + 1    --saci2018-67-03
   END FOREACH
...
--               " from   cta_his_pagos d,               ",     --saci2018-67
   ...
   -- se inicia el contador
   LET v_contador = 1
   LET v_cont_rch = 0   --saci2018-67-03
   
   -- se transfieren los datos al arreglo de consulta
   FOREACH cur_consulta_rechazados INTO v_r_desp.*
      LET v_arr_despliegue_rch[v_contador].* = v_r_desp.*
      
      -- se incrementa el contador
      LET v_contador = v_contador + 1
      LET v_cont_total = v_cont_total + v_r_desp.num_registros   --prodinfxvi-141
      LET v_cont_rch = v_cont_rch + 1   --saci2018-67-03
   END FOREACH
   ...
   -- se inicia el contador
   LET v_contador = 1
   LET v_cont_ade = 0   --saci2018-67-03
   
   -- se transfieren los datos al arreglo de consulta
   FOREACH cur_consulta_adelantados INTO v_r_desp.*
      LET v_arr_despliegue_adelantados[v_contador].* = v_r_desp.*
            
      -- se incrementa el contador
      LET v_contador = v_contador + 1
      LET v_cont_total = v_cont_total + v_r_desp.num_registros   --prodinfxvi-141
      LET v_cont_ade = v_cont_ade + 1   --saci2018-67-03
   END FOREACH
...
--==================================================================================  --saci2018-67-02
   ...
   -- se inicia el contador
   LET v_contador = 1
   LET v_cont_esp = 0   --saci2018-67-03
   ...
   FOREACH cur_consulta_especial INTO v_r_desp.*                  --saci2018-67-02
      ...
      LET v_contador = v_contador + 1
      LET v_cont_total = v_cont_total + v_r_desp.num_registros   --prodinfxvi-141
      LET v_cont_esp = v_cont_esp + 1   --saci2018-67-03
   END FOREACH
...
--==================================================================================  --saci2018-67-03
   ...
   -- se inicia el contador
   LET v_contador = 1
   LET v_cont_con = 0   --saci2018-67-03
   ...
   FOREACH cur_consulta_esp_conf INTO v_r_desp.*                  --saci2018-67-02
      ...
      LET v_contador = v_contador + 1
      LET v_cont_total = v_cont_total + v_r_desp.num_registros   --prodinfxvi-141
      LET v_cont_con = v_cont_con + 1   --saci2018-67-03
   END FOREACH
...
--==========================================================   --saci2018-67-03
   ...
   ATTRIBUTES ( UNBUFFERED)
      ...
      -- se despliegan los registros especiales aceptados
...
--         IF v_cont_ace >= 1 THEN    --saci2018-67-03         
            ...
            END FOR
--         END IF                      --saci2018-67-03

--         IF v_cont_ade >= 1 THEN     --saci2018-67-03        
...
--         IF v_cont_rch >= 1 THEN     --saci2018-67-03
...
--         IF v_cont_esp >=1 THEN      --saci2018-67-03
            ...
            FOR v_contador = 1 TO v_arr_despliegue_especial.getLength()    --saci2018-67-02 
               OUTPUT TO REPORT rpt_consulta_acl_scnss(p_folio, v_arr_despliegue_especial[v_contador].*, 4,v_cont_total)   --prodinfxvi-141 --saci2018-67-02
            END FOR
--         END IF                      --saci2018-67-03

--         IF v_cont_con >= 1 THEN     --saci2018-67-03
            -- se transfieren los datos de los registros epeciales confrimados  --saci2018-67-03
            FOR v_contador = 1 TO v_arr_despliegue_esp_conf.getLength()         --saci2018-67-03 
               OUTPUT TO REPORT rpt_consulta_acl_scnss(p_folio, v_arr_despliegue_esp_conf[v_contador].*, 5,v_cont_total)   --prodinfxvi-141 --saci2018-67-03
            END FOR
--         END IF                      --saci2018-67-03



{         IF v_cont_ace = 0 AND       --saci2018-67-03
            v_cont_ade = 0 AND
            v_cont_rch = 0 AND
            v_cont_esp = 0 AND
            v_cont_con = 0 THEN
            CALL fn_mensaje("Atención","No exiten datos para generar el reporte","stop")
            EXIT DIALOG
         END IF                      --saci2018-67-03
}
...
REPORT rpt_consulta_acl_scnss(v_folio, v_r_desp, v_tipo_registro,v_cont_total)  --prodinfxvi-141
DEFINE    v_r_desp          RECORD -- registro de consulta
             tpo_acl          VARCHAR(50)  ,   --prodinfxvi-141
             result_operacion VARCHAR(50)  ,
             num_registros    INTEGER      ,
             imp_ap_pat       DECIMAL(12,2),
...
FORMAT
   ...
   BEFORE GROUP OF v_r_desp.destino     --prodinfxvi-141
      ...
      IF ( v_tipo_registro = 1 ) THEN
         IF ( v_r_desp.destino = "1" ) THEN
            LET v_cadena_destino = "INFONAVIT"
         END IF

         IF ( v_r_desp.destino = "2" ) THEN
            LET v_cadena_destino = "AFORE"
         END IF
         IF ( v_r_desp.destino = "3" ) THEN
   ...
   AFTER GROUP OF v_r_desp.destino
   ...
   ON EVERY ROW
      ...
      LET v_num_registros   = v_num_registros   + v_r_desp.num_registros  
      ...
      -- acumulado por destino de la aportacion
      LET v_num_registros_destino   = v_num_registros_destino   + v_r_desp.num_registros  
      ...
      LET v_total_num_registros   = v_total_num_registros   + v_r_desp.num_registros  
   ...
   ON LAST ROW
      ...
      SELECT COUNT(*)
      INTO   v_contador
--      FROM   cta_his_pagos     --sacie2018-67
      ...
      ELSE
         ...
         LET v_nombre_archivo_rechazos = v_nombre_archivo[1,v_contador - 5] CLIPPED || "_" || v_folio_formato CLIPPED || "_rechazoACL.SINNSS" CLIPPED
      END IF
