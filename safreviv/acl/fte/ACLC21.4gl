...
#Modulo       => ACL                                                                    #
#Programa     => ACLC21                                                                 #
#Objetivo     => Consulta Totales de archivo de aclaraciones con cambio nombre          #
...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- clave del usuario
   ...
   OPEN WINDOW w_consulta_folio WITH FORM "ACLC211"
   ...
   INPUT v_folio WITHOUT DEFAULTS
      ...
      BEFORE INPUT
         ...
         SELECT a.*
         FROM   glo_folio a
         WHERE  a.proceso_cod = g_proceso_cod_acl_reg_pag_cambio_nombre -- registro pagos aclaratorio cambio nombre
         AND    a.status >= 0
...
DEFINE p_folio                  DECIMAL(9,0),
       vindice                  SMALLINT,
       reg_cifras               RECORD
         tot_ap                  INTEGER --LIKE acl_sum_cc_nss.tot_ap
        ,suma_ap_pat             DECIMAL(18,2) --LLIKE acl_sum_cc_nss.suma_ap_pat
        ,suma_am                 DECIMAL(18,2) --LIKE acl_sum_cc_nss.suma_am
        ,suma_aivs               DECIMAL(18,6) --LIKE acl_sum_cc_nss.suma_aivs
        ,suma_int_viv_pgo_ext    DECIMAL(18,2) --LIKE acl_sum_cc_nss.suma_int_viv_pgo_ext
        ,suma_aiv_pgo_ext        DECIMAL(18,6) --LIKE acl_sum_cc_nss.suma_aiv_pgo_ext
       END RECORD
   ...
   OPEN WINDOW w_consulta WITH FORM "ACLC212"
...
FUNCTION fn_reporte_carga_archivo(p_folio, reg_cifras, p_b_despliegue_pantalla)
   ...
   -- se consulta el nombre del archivo que se carga para despliegue en reporte
    SELECT nombre_archivo
     ...
     WHERE folio = p_folio
       AND proceso_cod = g_proceso_cod_acl_reg_pag_cambio_nombre

    --Se asigna la plantilla para generar el reporte
    IF fgl_report_loadCurrentSettings("rpt_carga_con_cambio_nombre.4rp") THEN 
        CALL fgl_report_selectDevice ("PDF")
                    
        LET v_ruta_reporte = v_ruta_listados CLIPPED,"/","carga_con_cambio_nombre"
    ...
    START REPORT rpt_carga_con_cambio_nombre TO XML HANDLER manejador_rpt
    -- Asigna el titulo del reporte
                
    OUTPUT TO REPORT rpt_carga_con_cambio_nombre(reg_cifras.*, p_folio, v_nombre_usuario, v_nombre_archivo)                                                                
    FINISH REPORT rpt_carga_con_cambio_nombre 
END FUNCTION

-- OBJETIVO: Emitir el reporte de liquidacion/preliquidacion
REPORT rpt_carga_con_cambio_nombre(reg_cifras, p_folio, p_nombre_usuario, p_nombre_archivo)
  DEFINE  p_folio DECIMAL(9,0)
  DEFINE reg_cifras RECORD
         tot_ap                  INTEGER --LIKE acl_sum_sc_nss.tot_ap
        ,suma_ap_pat             DECIMAL(18,2) --LIKE acl_sum_sc_nss.suma_ap_pat
