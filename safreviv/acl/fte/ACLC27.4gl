...
DEFINE p_tipo_ejecucion     SMALLINT                     -- forma como ejecutara el programa
   ...
   -- se abre la ventana
   OPEN WINDOW w_consulta WITH FORM "ACLC271"
     ...
     ATTRIBUTES (UNBUFFERED)
      ...
      ON ACTION accept
         IF ( v_folio IS NOT NULL ) THEN
             ...
             FROM   glo_folio
             WHERE  proceso_cod = g_proceso_cod_acl_registro_pagos -- aclaraciones en Aclara
             AND    folio = v_folio

            -- si no se encontro el folio, entonces no es un folio de ese proceso
            IF ( v_folio_busqueda IS NULL ) THEN
               CALL fn_mensaje("Atención","El folio no corresponde con un proceso de Aclaraciones en Aclara","stop")
              -- CONTINUE INPUT
            END IF

            -- se consultan los datos del folio dado
            CALL f_consulta_acl_enaclara(v_folio)
         ELSE
...
{ ======================================================================
Clave: 
Nombre: f_consulta_acl_enaclara
Fecha creacion: Junio 20, 2012
Autor: Ivan Vega
Narrativa del proceso que realiza:
Realiza una consulta sobre los datos de Aclaraciones en aclara
...
FUNCTION f_consulta_acl_enaclara(p_folio)
...
--               "\n        sum(d.aiv_gen_pgo_ext) aiv_int             ",
               ...
               "\n and    e.estado_pago = 90                         ", -- regisssin cambio desta
   ...
   FOREACH cur_consulta_actualizados INTO v_r_despliegue.*
      ...
      -- se obtienen los aivs de pagos extemporaneos de los registros que si tienen pesos       
      SELECT NVL(SUM(d.aiv_gen_pgo_ext),0)
      INTO   v_arr_despliegue_actualizados[v_contador].aiv_gen_pgo_ext
      FROM   cta_his_pagos       d,                     
             pag_tpo_archivo     a,                     
             pag_ctr_pago        e                      
      WHERE  d.folio = p_folio
      ...
      AND    d.id_referencia = e.id_referencia          
      AND    d.id_derechohabiente = e.id_derechohabiente
      AND    e.estado_pago = 90                         
      AND    d.tpo_aclaracion not in (13,17)            
   ...
   LET v_sql = "\n select d.tpo_aclaracion       acl,                ",
               ...
               "\n and    d.id_referencia = e.id_referencia          ",               
               "\n and    d.id_derechohabiente = e.id_derechohabiente", 
               "\n and    e.estado_pago = 30                         ", -- adelantados
               "\n and    d.tpo_aclaracion in (13,17)                ", -- aclaraciones 13 y 17
               "\n group  by 1,2                                     ",
               "\n order  by 1,2;                                    "
   ...
   OPEN WINDOW w_resultados WITH FORM "ACLC272"
   ...
   ATTRIBUTES ( UNBUFFERED)
      ...
      ON ACTION reporte        
         ...
         IF ( fgl_report_loadCurrentSettings("ACLC27.4rp") ) THEN
            LET v_handler = fgl_report_commitCurrentSettings()      -- commit the file settings
         ELSE
            CALL fn_mensaje("Error","No se encuentra la plantilla ACLC27.4rp. No se puede emitir el reporte","stop")
            CONTINUE DIALOG
         END IF
      
         -- se inicia la emision del reporte
         START REPORT rpt_consulta_acl_enaclara TO XML HANDLER v_handler
         
         -- se transfieren los datos
         FOR v_contador = 1 TO v_arr_despliegue_actualizados.getLength()
            -- registros actualizados
            OUTPUT TO REPORT rpt_consulta_acl_enaclara(p_folio, v_arr_despliegue_actualizados[v_contador].*, 1)
         END FOR

         -- se transfieren los datos de los registros adelantados
         FOR v_contador = 1 TO v_arr_despliegue_adelantados.getLength()
            -- registros adelantados
            OUTPUT TO REPORT rpt_consulta_acl_enaclara(p_folio, v_arr_despliegue_adelantados[v_contador].*, 3)
         END FOR
         
         -- se finaliza el reporte
         FINISH REPORT rpt_consulta_acl_enaclara
...
{ ======================================================================
Clave: 
Nombre: rpt_consulta_acl_enaclara
Fecha creacion: Junio 20, 2012
Autor: Ivan Vega
Narrativa del proceso que realiza:
Genera el reporte de cifras de control de salida de aclaratorio en Aclara
...
REPORT rpt_consulta_acl_enaclara(v_folio, v_r_despliegue, v_tipo_registro)
...
FORMAT
   ...
   BEFORE GROUP OF v_tipo_registro
      ...
      CASE v_tipo_registro
         WHEN 1 -- aceptado
            LET v_leyenda_registro = "Registros Actualizados (que cambiaron de causal)"

         WHEN 3 -- adelantado
            LET v_leyenda_registro = "Registros Adelantados por cambio de causal (13 y 17)"
      END CASE
      ...
      LET v_aiv_gen_pgo_ext = v_aiv_gen_pgo_ext + v_r_despliegue.aiv_gen_pgo_ext
{
   ON LAST ROW
      ...
      ELSE
         ...
         -- NombreArchivoOriginal + _FOLIO_rechazoACL.ENACLARA"
         LET v_folio_formato = v_folio USING "&&&&&&&&&"
         
         LET v_contador = LENGTH(v_nombre_archivo CLIPPED)
         
         LET v_nombre_archivo_rechazos = v_nombre_archivo[1,v_contador - 5] || "_" || v_folio_formato || "_rechazoACL.ENACLARA"
      END IF
