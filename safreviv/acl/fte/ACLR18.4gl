...
#Modulo       => ACL                                                                    #
#Programa     => ACLR06                                                                 #
#Objetivo     => Programa que realiza el reverso de liquidación de Aclaraciones con     #
#                cambio                                                                 #
...
DEFINE p_usuario_cod   LIKE seg_usuario.usuario_cod
  ...
  --     v_opera_cod_liquidacion LIKE cat_operacion.opera_cod
   ...
   CALL STARTLOG (p_usuario_cod CLIPPED|| ".ACLR18.log")
      ...
      CALL fn_reverso_reg_cnt(p_i_folio) RETURNING   r_bandera          
{
      ...
      LET v_s_sql = "UPDATE glo_folio SET status = 1 WHERE folio = ", p_i_folio

      EXECUTE IMMEDIATE v_s_sql
 }

      -- se ejecuta el SP de reverso
      LET v_s_sql = " EXECUTE PROCEDURE sp_acl_ccnss_rev_liquidacion(?,?)"
...
{       
   ...
   LET v_proceso_cod   = g_proceso_cod_acl_reg_pag_cambio -- Aclaraciones con cambio
