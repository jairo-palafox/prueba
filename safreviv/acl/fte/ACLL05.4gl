...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod,
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED||".ACLL05.log")
...
Narrativa del proceso que realiza:
 Esta función carga un archivo del proceso de "Aclaraciones enaclara"
...
DEFINE p_usuario_cod     LIKE seg_usuario.usuario_cod, #Clave de usuario
   ...
   LET g_proceso_cod           = g_proceso_cod_acl_registro_pagos # Aclaraciones enaclara
   --LET g_opera_cod_carga       = 1  # Carga archivos Aclaraciones enaclara
   --LET g_opera_cod_integracion = 2  # integración

   CALL fn_rutas('acl') RETURNING v_ruta_ejecutable,v_ruta_vacia 
   
   --OPEN WINDOW w_sin_cambio WITH FORM v_ruta_ejecutable CLIPPED||"/ACLL051"
      ...
      IF(r_resultado_opera = 0)THEN
        ...
        DISPLAY "@ v_cadena_opera/g_opera_cod_integracion: ", v_cadena_opera 
         ...
         CALL fn_carga_archivo(g_pid, g_proceso_cod, g_opera_cod_carga, p_tpo_ejecucion,
                               "ACLL05",v_comando, p_usuario_cod, TRUE) RETURNING r_bnd_carga
         # Si se realizó la carga se continua con el proceso
         IF NOT(r_bnd_carga)THEN
            CALL fn_mensaje("ACLARACION ENACLARA","Se ha cancelado la carga de información","about")
         END IF
      ELSE
