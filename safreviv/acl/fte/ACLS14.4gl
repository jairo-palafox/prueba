...
-- Modulo       => ACL                                                                    
-- Programa     => ACLS14                                                                 
-- Objetivo     => Reporte cifras de rechazo de ACL con cambio en nombre en automatico al liquidar para MENÚ
...
--   LET g_nom_archivo = ARG_VAL(6)
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".ACLS14.log")

   -- se abre la ventana
   OPEN WINDOW w_consulta WITH FORM "ACLS141"
     ...
     ATTRIBUTES (UNBUFFERED)
      ...
      ON ACTION accept
         IF v_folio IS NOT NULL THEN
             ...
             FROM   glo_folio
             WHERE  proceso_cod = 107   -- aclaraciones con cambio de Nombre
             AND    folio = v_folio
...
--===============================================================================
   ...
   WHERE  folio = p_folio
    ...
    LET v_v_nom_reporte = p_usuario_cod CLIPPED,"-ACLS14-",
                          g_pid USING "&&&&&", "-",
                          g_proceso_cod USING "&&&&&","-",
                          5 USING "&&&&&"
    ...
    CALL fgl_report_loadCurrentSettings(r_ruta_bin CLIPPED ||"/ACLS14.4rp") RETURNING v_existe
