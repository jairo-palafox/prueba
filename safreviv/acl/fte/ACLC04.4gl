...
#Modulo       => ACL                                                                    #
#Programa     => ACLC01                                                                 #
#Objetivo     => Consulta Totales de archivo de aclaraciones con cambio                 #
...
DEFINE p_usuario_cod            LIKE seg_usuario.usuario_cod, -- clave del usuario
   ...
   OPEN WINDOW w_consulta_folio WITH FORM "ACLC041"
   ...
   INPUT v_folio WITHOUT DEFAULTS
      ...
      BEFORE INPUT
         ...
         SELECT a.*
         FROM   glo_folio a
         WHERE  a.proceso_cod = g_proceso_cod_acl_reg_pag_cambio --Proceso Aclaraciones con cambio
         AND    a.status >= 0
...
FUNCTION fn_consulta_archivo(p_folio)
   ...
   DEFINE
      ...
      reg_cifras RECORD
         --tpo_registro            LIKE acl_sum_sc_nss.tpo_registro
   ...
   OPEN WINDOW w_consulta WITH FORM "ACLC042"
...
FUNCTION fn_reporte_carga_archivo(p_folio, reg_cifras, p_b_despliegue_pantalla)
   ...
   -- se consulta el nombre del archivo que se carga para despliegue en reporte
    ...
    IF fgl_report_loadCurrentSettings("rpt_carga_con_cambios.4rp") THEN 
        CALL fgl_report_selectDevice ("PDF")
                    
        LET v_ruta_reporte = v_ruta_listados CLIPPED,"/","carga_con_cambios"
    ...
    START REPORT rpt_carga_con_cambios TO XML HANDLER manejador_rpt
    -- Asigna el titulo del reporte
                
    OUTPUT TO REPORT rpt_carga_con_cambios(reg_cifras.*, p_folio, v_nombre_usuario, v_nombre_archivo)                                                                
    FINISH REPORT rpt_carga_con_cambios 
END FUNCTION

-- OBJETIVO: Emitir el reporte de liquidacion/preliquidacion
REPORT rpt_carga_con_cambios(reg_cifras, p_folio, p_nombre_usuario, p_nombre_archivo)
