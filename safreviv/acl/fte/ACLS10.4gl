...
-- Modulo       => ACL                                                                   
-- Programa     => ACLS10                                                                
-- Objetivo     => Reporte Cifras rechazos aclaratorio DISCCNSS
...
MAIN
   ...
   -- se abre la ventana
   OPEN WINDOW w_consulta WITH FORM "ACLS101"
     ...
     ATTRIBUTES (UNBUFFERED)
      ...
      ON ACTION accept
         IF ( v_folio IS NOT NULL ) THEN
             ...
             FROM   glo_folio
             WHERE  proceso_cod = g_proceso_cod_acl_reg_pag_cambio -- aclaraciones sin cambio de NSS
             AND    folio = v_folio
...
--===============================================================================
    ...
    LET v_v_nom_reporte = g_usuario_cod CLIPPED,"-ACLS10-",
                          TODAY USING 'YYYYMMDD','DISCCNSS'

    CALL fn_rutas("acl") RETURNING r_ruta_bin, r_ruta_listados

    -- Ejecución de archivo de rechazo 
    LET p_pid = 0
    LET p_proceso_cod = 103
    LET g_opera_cod = 0
    LET p_folio = v_folio
    ...
    LET v_s_qry = " fglrun ",r_ruta_bin CLIPPED,"/ACLS03 ",
                            p_usuario, " ",
                            p_pid, " ",
                            p_proceso_cod, " ",
                            p_opera_cod, " ",
    ...
    CALL fgl_report_loadCurrentSettings(r_ruta_bin CLIPPED ||"/ACLS10.4rp") RETURNING v_existe
