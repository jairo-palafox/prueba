...
-- Modulo       => ACL                                                                    
-- Programa     => ACLS06                                                                 
-- Objetivo     => Reporte cifras de rechazo de ACL sin cambio de nss en automatico al liquidar
-- Autor        => GERARDO ALFONSO VEGA PAREDES                                           
-- Fecha inicio => 7 de Diciembre de 2015                                                  
---------------------------------------------------------------------------------------------
...
MAIN
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".ACLS06.log")
   ...
   ELSE 
      ...
      CALL fn_display_proceso(0,"INICIO REPORTE RECHAZOS DISSCNSS")  
      ...
      CALL fn_actualiza_opera_fin(g_pid,g_proceso_cod,5) RETURNING r_resultado_opera
      LET v_mensaje = "El reporte de cifras de rechazo de disscnss ha finalizado correctamente"

      CALL fn_correo_proceso(g_pid,
                             g_proceso_cod,
                             5,
                             '', # Archivo adjunto
                             'Finalización de operación - '||v_proceso_desc CLIPPED||' - REPORTE RECHAZOS DISSCNSS',
                              v_mensaje
                             )
...
--===============================================================================
    ...
    LET v_v_nom_reporte = p_usuario_cod CLIPPED,"-ACLS06-",
                          g_pid USING "&&&&&", "-",
                          g_proceso_cod USING "&&&&&","-",
                          5 USING "&&&&&"
    ...
    CALL fgl_report_loadCurrentSettings(r_ruta_bin CLIPPED ||"/ACLS06.4rp") RETURNING v_existe
