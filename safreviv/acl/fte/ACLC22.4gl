...
DEFINE p_tipo_ejecucion     SMALLINT                     -- forma como ejecutara el programa
   ...
   -- se abre la ventana
   OPEN WINDOW w_consulta WITH FORM "ACLC221"
     ...
     ATTRIBUTES (UNBUFFERED)
      ...
      ON ACTION accept
         IF ( v_folio IS NOT NULL ) THEN
             ...
             FROM   glo_folio
             WHERE  proceso_cod = g_proceso_cod_acl_reg_pag_cambio -- aclaraciones con cambio de NSS
             AND    folio = v_folio

            -- si no se encontro el folio, entonces no es un folio de ese proceso
            IF ( v_folio_busqueda IS NULL ) THEN
               CALL fn_mensaje("Atención","El folio no corresponde con un proceso de Aclaraciones con Cambio de NSS","stop")
               --CONTINUE INPUT
            END IF
...
DEFINE p_folio                 LIKE glo_folio.folio, -- folio de consulta
       ...
       v_contador   INTEGER,  --prodinfxvi-141
...
--               " from   cta_his_pagos d,                ",     --saci2018-67
               ...
               " where  d.folio = ", p_folio, "          ",
               " and    a.archivo_cod = d.origen_archivo ",
               " and    d.result_operacion in (2,3)      ",                   -- rechazados --26jun2017 se agregó 3
...
--               "\n and    d.result_operacion <> 2                     ",    --inc-1073065 (Se unifica criterio con ACLC23
...
--==========================================================  --saci2018-67-02
   ...
   -- se transfieren los datos al arreglo de consulta
   FOREACH cur_consulta_especial INTO v_r_desp.*                  --sacie2018-67-02
...
--==========================================================   --saci2018-67-03
   ...
   OPEN WINDOW w_resultados WITH FORM "ACLC222"
   ...
   ATTRIBUTES ( UNBUFFERED)
      ...
      ON ACTION reporte        
         ...
         IF ( fgl_report_loadCurrentSettings("ACLC22.4rp") ) THEN
            LET v_handler = fgl_report_commitCurrentSettings()      -- commit the file settings
         ELSE
            CALL fn_mensaje("Error","No se encuentra la plantilla ACLC22.4rp. No se puede emitir el reporte","stop")
            CONTINUE DIALOG
         END IF
      
         -- se inicia la emision del reporte
         START REPORT rpt_consulta_acl_ccnss TO XML HANDLER v_handler
         
         -- se transfieren los datos
         FOR v_contador = 1 TO v_arr_despliegue.getLength()
            -- registros aceptados
            OUTPUT TO REPORT rpt_consulta_acl_ccnss(p_folio, v_arr_despliegue[v_contador].*, 1,v_cont_total)  --prodinfxvi-141
         END FOR

         -- se transfieren los datos de los registros adelantados
         FOR v_contador = 1 TO v_arr_despliegue_adelantados.getLength()
            -- registros adelantados
            OUTPUT TO REPORT rpt_consulta_acl_ccnss(p_folio, v_arr_despliegue_adelantados[v_contador].*, 3,v_cont_total)  --prodinfxvi-141
         END FOR
         
         -- se transfieren los datos de los registros rechazados
         FOR v_contador = 1 TO v_arr_despliegue_rch.getLength()
            -- registros rechazados
            OUTPUT TO REPORT rpt_consulta_acl_ccnss(p_folio, v_arr_despliegue_rch[v_contador].*, 2,v_cont_total)  --prodinfxvi-141
         END FOR
         ...
         FOR v_contador = 1 TO v_arr_despliegue_especial.getLength()    --saci2018-67-02 
            OUTPUT TO REPORT rpt_consulta_acl_ccnss(p_folio, v_arr_despliegue_especial[v_contador].*, 4,v_cont_total)   --prodinfxvi-141 --saci2018-67-02
         END FOR

         FOR v_contador = 1 TO v_arr_despliegue_esp_conf.getLength()         --saci2018-67-03
            OUTPUT TO REPORT rpt_consulta_acl_ccnss(p_folio, v_arr_despliegue_esp_conf[v_contador].*, 5,v_cont_total)   --prodinfxvi-141 --saci2018-67-03
         END FOR

         -- se finaliza el reporte
         FINISH REPORT rpt_consulta_acl_ccnss
...
REPORT rpt_consulta_acl_ccnss(v_folio, v_r_desp, v_tipo_registro,v_cont_total) --prodinfxvi-141
...
--      FROM   cta_his_pagos     --sacie2018-67
      ...
      ELSE
         ...
         -- NombreArchivoOriginal + _FOLIO_rechazoACL.CONNSS
         LET v_folio_formato = v_folio USING "&&&&&&&&&"
         
         LET v_contador = LENGTH(v_nombre_archivo CLIPPED)
         
         LET v_nombre_archivo_rechazos = v_nombre_archivo[1,v_contador - 5] CLIPPED || "_" || v_folio_formato CLIPPED || "_rechazoACL.CONNSS" CLIPPED
      END IF
