...
#Modulo       => ACL                                                                    #
#Programa     => ACLR04                                                                 #
#Objetivo     => Programa que realiza el reverso integracion CCNSS                      #
...
DEFINE p_usuario_cod      LIKE seg_usuario.usuario_cod
   ...
   -- se inicia el log
   CALL STARTLOG (p_usuario_cod CLIPPED|| ".ACLR04.log")
{   
   ...
   LET v_ls_SqlQry = "UPDATE glo_folio SET status = -1 WHERE folio = ?"
   PREPARE sid_folio FROM v_ls_SqlQry
   EXECUTE sid_folio USING p_d_folio
}
   -- se ejecuta el SP de reverso de integracion de LQINFO  
   LET v_ls_SqlQry = "EXECUTE PROCEDURE sp_acl_ccnss_rev_integracion(?)"
