...
#                del detalle de la devolucion por errores de operacion                  #
...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED||".DEOL03.log")
   ...
   WHERE
    proceso_cod = g_proceso_cod
     ...
     DISPLAY "El pid recuperado es >",  g_pid, "<"
   ...
   --CALL fn_deo_ejecuta_integracion(p_usuario_cod)
...
DEFINE p_usuario_cod LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
   ...
   OPEN WINDOW w_inte_devol WITH FORM "DEOL031"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION integrar
         ...
         CALL fn_ventana_confirma("Registro de Avance de Pago",
              ...
              " de Devoluciones al Infonavit?",
              "quest") RETURNING v_respuesta

        IF v_respuesta = 1 THEN
           CALL fn_deo_ejecuta_integracion(p_usuario_cod, l_v_arch_proceso)
           EXIT DIALOG
        END IF
...
======================================================================
Clave: 
Nombre: fn_deo_ejecuta_integracion
...
FUNCTION fn_deo_ejecuta_integracion(p_usuario_cod, v_nombre_archivo)
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
       ...
       ## Se prepara una prevalidación antes de realizar la integracion
       LET v_s_sql = "EXECUTE FUNCTION fn_deo_prevalida_op98(?)"            
       -- se prepara la ejecucion del stored procedure para la integracion
       PREPARE sid_preintegradeo FROM v_s_sql
       -- se ejecuta el stored procedure
       EXECUTE sid_preintegradeo USING p_usuario_cod INTO v_i_resultado, v_si_error_isam, v_mensaje_error

       DISPLAY "1:",v_i_resultado,"|",v_si_error_isam,"|",v_mensaje_error,"|"
       
       IF(v_i_resultado <> 0)THEN
          CASE(v_i_resultado)
             WHEN 1
                 CALL fn_mensaje("Atención",
                     "Montos Depositados por Afore no Capturados","stop")
             WHEN 2
                CALL fn_mensaje("Atención",
                     "Montos Depositados por Afore no conciden con el archivo","stop")
             WHEN 3
                CALL fn_mensaje("Atención",
                     "Archivo de Devolución de recursos no capturado","stop")
             WHEN 4
                CALL fn_mensaje("Atención",
                     "La fecha de devolución en archivo no coincide con la fecha de devolución capturada","stop")                 
             WHEN 0
                DISPLAY "Comparación de datos de saldos vs archivo válidas"
             OTHERWISE
                CALL fn_mensaje("Error",
                                v_mensaje_error,"stop")

          END CASE
          RETURN
       END IF

       -- se verifica si se puede ejecutar la operacion
       CALL fn_valida_operacion(g_pid,g_proceso_cod,g_opera_cod)
       RETURNING v_i_resultado 

       DISPLAY "2:",v_i_resultado
       
       IF ( v_i_resultado = 0 ) THEN
       
          CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,g_opera_cod,v_folio,"DEOP01",
                              "",p_usuario_cod) RETURNING v_i_resultado
          IF ( v_i_resultado = 0 ) THEN
             LET v_s_comando = " nohup time fglrun ",g_reg_modulo.ruta_exp CLIPPED,"/DEOP01 ",                           
                               p_usuario_cod, " ",
                               g_pid  , " " ,
                               g_proceso_cod , " " ,
                               g_opera_cod ," ",
                               0 ," '", -- se manda folio cero, aun no existe
                               v_nombre_archivo ,"' ",                            
...
END FUNCTION -- fn_deo_ejecuta_integracion
