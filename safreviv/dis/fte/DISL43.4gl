...
#Modulo           => DIS                                                       #
#Programa         => DISL43                                                    #
#Objetivo         => Programa Lanzador Extractor Avance Abierto                #
#                    y Crédito Liquidado.                                      #
#                                                                              #
...
MAIN
  ...
  CALL STARTLOG (g_usuario CLIPPED|| ".DISL43.log")
  ...
  OPEN WINDOW w1 WITH FORM "DISL43"
    DIALOG ATTRIBUTES(UNBUFFERED) 
      INPUT BY NAME v_folio_dis
        BEFORE INPUT
        ...
        ON ACTION ACCEPT 
           LET v_bnd_existe_info = 0
           CALL fn_verificar_info_ava(v_folio_dis) RETURNING v_bnd_existe_info
           ...
           IF v_bnd_existe_info >= 1 THEN
              ...
              IF (fn_valida_operacion(g_pid, g_proceso_cod, 1) = 0 ) THEN
                 ...
                 --Validaciones 
                 LET l_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/DISS42.42r ",
                                 v_folio_dis," ",
                                 --v_cb_destino," ",
                                 g_pid," ",
                                 g_opera_cod," ",
                 ...
                 LET v_mensaje = "Se ha enviado el extractor de avance créditos liquidados: ",
                                 g_pid CLIPPED,
                                 ".\nPuede revisar el avance del proceso en el monitor de ejecución de procesos"

                 CALL fn_mensaje("Extractor de Avance Créditos Liquidados", v_mensaje, "information")
           ...
           ELSE
              CALL fn_mensaje("Aviso","No existe información de avance abierto con créditos liquidados.","stop") 
              EXIT PROGRAM
           END IF  
      END INPUT
...
FUNCTION fn_verificar_info_ava(p_folio_dis) 
...
DEFINE v_folio_reg_pag       DECIMAL(9,0)
  ...
  IF p_folio_dis IS NOT NULL THEN
     LET g_sql_txt = g_sql_txt, " \n AND avp.folio  = ", p_folio_dis
  END IF
