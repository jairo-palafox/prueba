###############################################################################
#VERSION                    => 1.0.0                                           #
#FECHA ULTIMA MODIFICACION  => 23/08/2016                                      #
#------------------------------------------------------------------------------#
...
#MODULO           => DIS                                                       #
#PROGRAMA         => DISE26                                                    #
#OBJETIVO         => Ejecuta la función de la Provisión del Pago               #
#                                                                              #
...
MAIN
  ...
  CALL STARTLOG(g_usuario CLIPPED||".DISE26.log")
  ...
  DISPLAY "Inicia Validaciones para provisionar el pago: " --,TIME
  CALL fn_pre_autorizar()

END MAIN

FUNCTION fn_pre_autorizar()
  DEFINE 
  ...
  LET g_proceso_cod = 3915 --Codigo del Proceso de Provisión del Pago
  ...
  IF bnd_continuar THEN
     ...
     LET g_proceso_cod = 3915 -- Provisionar Pago
     LET g_opera_cod   = 1    -- liquidación        
     LET r_nom_archivo = ""

     CALL fn_act_edo_pre_auto() --PROVISIONAR PAGO
     ...
     {WHENEVER ERROR CONTINUE  
       ...
       IF v_bnd_confirmacion = 0 THEN      
          WHENEVER ERROR CONTINUE         
             ...
             LET bnd_continuar = 1
       ELSE
          DISPLAY "Error en la provisión del pago: ",v_bnd_confirmacion
       ...
       IF r_bandera = 0 THEN 
          ...
          CALL fn_correo_proceso(p_pid,
                                 ...
                                 "Provisión Pago",
                                 "\n ID Proceso   : "||p_pid||
                                 "\n Proceso      : Provisión Pago"||
                                 "\n Operacion    : Provisión Pago"||
                                 "\n Fecha Inicio : "||TODAY||
                                 "\n Fecha Fin    : "||TODAY)
       ELSE
          CALL fn_error_opera(p_pid, g_proceso_cod, g_opera_cod)
...
FUNCTION fn_genera_reporte_fact_apo_sub(p_folio_pago,
  ...
  LET v_nom_archivo       = "provision_pago_"         --nombre de archivo
   
  LET v_ddmmaaaa          = TODAY USING "ddmmyyyy" --fecha del archivo sin separadores
  LET v_busca_nom_archivo = "provision_pago_" || v_ddmmaaaa --Concatena nombre a buscar
  ...
  IF v_indice1 >= 1 THEN
     ...
     IF fgl_report_loadCurrentSettings(v_ruta_ejecutable CLIPPED ||"/DISE261.4rp") THEN
   ...
   ELSE
      DISPLAY "No existen registros para generar el reporte de Provisión de Pago."
   END IF
...
FUNCTION fn_obtiene_nombre_archivo(p_ruta_envio_ocg, p_busca_nom_archivo)
  ...
  WHILE ch.read([fn])
    LET v_consecutivo     = fn[24,26] --Posición del consecutivo dentro de la cadena
  ...
  LET v_nom_archivo_sal = "provision_pago_"          --nombre de archivo
  LET v_ddmmaaaa        = TODAY USING "ddmmyyyy"    --Fecha del archivo sin separadores
...
FUNCTION fn_verifica_fact_rech(p_folio_factura,
  ...
  LET v_ls_query = ""
  LET v_ls_query = "\n SELECT COUNT(*) ",
                   "\n FROM   dic_ctr_factura_aps fac, ",
                   "\n        dis_ctr_edo_actura_aps edo  ",
...
FUNCTION fn_act_edo_pre_auto()
 DEFINE arr_act_apo                DYNAMIC ARRAY OF RECORD
    v_id_dis_interface_hs          DECIMAL(9,0),
    v_id_derechohabiente           DECIMAL(9,0)
  END RECORD
  
  DEFINE v_id_dis_interface_hs     DECIMAL(9,0),
         v_id_derechohabiente      DECIMAL(9,0),
  ...
  ----- Busqueda de Totales
  LET v_ls_query = ""
  LET v_ls_query = "\n SELECT au.id_dis_interface_ef, ",
                   "\n        au.id_derechohabiente   ",
                   "\n FROM   tmp_dis_pre_auto au     ",
                   "\n WHERE  au.folio_factura = ", g_folio_factura,
                   "\n AND    au.tpo_credito   = ", g_tipo_credito

  --DISPLAY "v_ls_query: -",v_ls_query,"-"

  PREPARE pr_pre_auto FROM v_ls_query
  DECLARE cur_pre_auto CURSOR FOR pr_pre_auto
  
  LET v_indice2     = 1
  
  FOREACH cur_pre_auto INTO arr_act_apo[v_indice2].v_id_dis_interface_hs,
                            arr_act_apo[v_indice2].v_id_derechohabiente

    UPDATE dis_ctr_aps_tns
    SET    estado              = 55
    WHERE  id_dis_interface_ef = arr_act_apo[v_indice2].v_id_dis_interface_hs 
    AND    id_derechohabiente  = arr_act_apo[v_indice2].v_id_derechohabiente
    AND    folio_factura       = g_folio_factura
    AND    tpo_credito         = g_tipo_credito
