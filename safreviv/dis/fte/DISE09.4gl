...
MAIN
  ...
  LET p_cve_proceso_cnt = 44  --Cancelación por Depuración (Avances de pago)
  LET p_transaccion_cnt = 62  --SE DEBERÁ REGISTRAR EL IMPORTE DEL AVANCE
  ...
  IF r_bnd_proceso_cnt = 1 THEN
     ...
     IF v_cuenta_contable > 0 THEN  
        DISPLAY "El registro contable de la cancelación por depuración (Avances de pago) se realizó exitosamente."
     ELSE 
  ...
  CALL fn_correo_proceso(l_pid,
                         ...
                         g_opera_cod,
                         'adjunto?',
                         'Integración de Cancelación por Depuración (Avances de Pago)',
                         'ID Proceso   : '||l_pid||
...
--Genera el reporte de los datos que fueron procesados en la Cancelación por Depuración (Avances de Pago)
FUNCTION fn_genera_reporte_can()
  ...
  IF fgl_report_loadCurrentSettings(v_ruta_ejecutable CLIPPED ||"/DISE091.4rp") THEN
