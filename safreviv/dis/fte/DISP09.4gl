...
#Modulo           => DIS                                                       #
#Programa         => DISP09                                                    #
#Objetivo         => Programa que lanza el proceso de Facturación del módulo   #
#                    OCG                                                       #
...
MAIN
  --Datos de entrada
  DEFINE 
    v_folio_transaccion      DECIMAL(9,0),  --Folio de transacción
      ...
      INPUT BY NAME v_folio_transaccion, 
        ...
        ON ACTION ACCEPT
           ...
           {IF v_f_transaccion_ini > TODAY THEN 
              CALL fn_mensaje("ATENCIÓN",
                              "La Fecha de Transacción Inicial no puede ser mayor al Día de Hoy.",
                              "about")
           ...
           IF v_f_transaccion_fin > TODAY THEN 
              CALL fn_mensaje("ATENCIÓN",
                              "La Fecha de Transacción Final no puede ser mayor al Día de Hoy.",
                              "about")
           ...
           --PARA EL PROGRAMA DE FACTURAR SI SE NECESITA ESTA EJECUCIÓN
                ...
                ON ACTION facturar
                   ...
                   IF v_bnd_confirma = 1 THEN
                      ...
                      CALL fn_ventana_confirma("FACTURAR", 
                                               "¿Está seguro que desea facturar los registros seleccionados?", 
                                               "quest")
                      ...
                      IF v_respuesta = 1 THEN
                         ...
                         IF (fn_valida_operacion(p_pid, v_proceso_cod, 1) = 0 ) THEN
                            ...
                            --LA EJECUCIÓN DE LA ESTA FUNCIÓN SE COLOCA EN EL PROGRAMA DISE25                            
                            {WHENEVER ERROR CONTINUE  
                            ...
                            IF v_bnd_confirmacion = 0 THEN      
                                ...
                                LET v_mensaje = "Se ha enviado la facturación: ",
                                                p_pid CLIPPED,
                   ...
                   ELSE --v_bnd_confirma = 0
                      LET v_mensaje = "Aún no ha seleccionado ningún registro para facturar."
                      CALL fn_mensaje("Facturación", v_mensaje, "information")   
           ...
           ELSE
              {IF (v_folio_transaccion IS NOT NULL) THEN
                 ...
                 IF v_estado = 0 THEN
                    CALL fn_mensaje("ATENCIÓN",
                                    "El Folio de Transacción no ha sido Confirmado.",
                                    "about")
                    CALL ui.interface.refresh()
                 END IF
