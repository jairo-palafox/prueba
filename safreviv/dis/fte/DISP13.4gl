###############################################################################
#VERSION                    => 1.0.1                                           #
#FECHA ULTIMA MODIFICACION  => 04/10/2018                                      #
################################################################################
################################################################################
...
MAIN 
  ...
  CALL STARTLOG(p_usuario CLIPPED||".DISP13.log")
...
#OBJETIVO: Genera el proceso de Preliquidacion de Cancelación Parcial Avances de Pago
FUNCTION fn_dis_preliquida()
  ...
  LET p_proceso_cod = 939 --Codigo del Proceso de Cancelación Parcial Avances de Pago (4)(939)       
  LET p_opera_cod   = 3   --Codigo de la operacion de preliquidacion (3)
  ...
  DISPLAY "PROCESO CANC PARC AV PAG: ", p_proceso_cod_reg_pago
  --PRELIQUIDACION 
  --CANCELACIÓN PARCIAL AVANCES DE PAGO
  IF p_proceso_cod_reg_pago = 939 THEN
     --Ejecuta función para la preliquidación de cancelación parcial av pag
     WHENEVER ERROR CONTINUE
       PREPARE prp_fn_transaccion
       FROM    "EXECUTE FUNCTION fn_dis_preliq_can_par_ap(?,?,?,?,?,?,?,?)"
     ...
     DISPLAY "Función Preliq Canc Parc Av Pag",r_bnd_preliquidacion
     ...
     IF r_bnd_preliquidacion <> 0 THEN
        ...
        DISPLAY "Error en la transacción Preliq Canc Parc Av Pag",v_status_err," ",v_desc_err
        EXIT PROGRAM
     END IF
  END IF --NORMAL
  ...
  IF r_bnd_preliquidacion = 0 THEN      
     ...
     IF v_bnd_preliq_ok = 1 THEN
        ...
        CALL fn_reporte_liquidacion(p_folio_disp,"dis_canc_pav_preliquida",p_usuario,p_pid,
     ...
     END IF
  ELSE
     DISPLAY "Error en la preliquidación de cancelación parcial avances de pago",r_bnd_preliquidacion
     CALL fn_error_opera(p_pid,p_proceso_cod,p_opera_cod)
     RETURNING r_bnd_opera_err
     EXIT PROGRAM
  END IF
  ...
  IF bnd_continuar THEN
     ...
     IF r_bandera = 0 THEN 
        ...
        CALL fn_correo_proceso(p_pid,
                               ...
                               "Preliquidación de Cancelación Parcial Avances de Pago",
                               "\n ID Proceso   : "||p_pid||
                               "\n Proceso      : Cancelación Parcial Avances de Pago"||
                               "\n Operacion    : Preliquidación de cancelación parcial avances de pago"||
                               "\n Fecha Inicio : "||TODAY||
                               "\n Fecha Fin    : "||TODAY)
     END IF
  END IF      
...
--------------------------------------
  ...
  LET v_QryTxt = "\n SELECT dp.f_liquida, ",
                 ...
                 "\n FROM   dis_canc_pav_preliquida dp",
  ...
  IF fgl_report_loadCurrentSettings(v_ruta_ejecutable CLIPPED ||"/DISP071.4rp") THEN
...
#          dis_ap_preliquida y dis_interface_ef
FUNCTION fn_borra_crea_tablas()

  WHENEVER ERROR CONTINUE;
    DROP TABLE dis_canc_pav_preliquida;
  WHENEVER ERROR STOP;
...
#         encontro información relacionada en las tablas de dis_ap_preliquida 
FUNCTION fn_valida_preliquidacion(v_p_pid, v_p_proceso_cod, v_p_opera_cod)
  ...
  IF v_bnd_preliquidacion <= 0 THEN    
     ...
     DISPLAY ""
     DISPLAY "No se encontraron registros a preliquidar con el folio de cancelación parcial avances de pago seleccionado"
     ...
     LET v_p_proceso_cod = 939 -- cancelación parcial avances de pago
     LET v_p_opera_cod   = 4   -- liquidación
     ...
     IF ( v_bandera = 0 ) THEN
        ...
        DISPLAY ""
        DISPLAY "No se encontraron registros a liquidar con el folio de cancelación parcial avances de pago seleccionado"
     END IF
