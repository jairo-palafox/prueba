################################################################################
#Version                    => 1.0.0                                           #
#Fecha ultima modificacion  => 19/08/2012                                      #
################################################################################
################################################################################
...
#                     negativas por avance cuando la aportación o la           #
#                     amortización son mayores a $2 (Abono para HS)            #
...
MAIN 
  ...
  AND    av.monto_dif_apo*(-1)     > 2 --Se multiplica por -1 para hacer positivas las diferencias negativas
  --Si está nula la devuelve a cero
  IF v_cuenta_apo IS NULL THEN  
     LET v_cuenta_apo = 0 
  END IF 
  ...
  WHERE  folio_dis                 = v_folio
  AND    av.id_dis_det_avance_pago = di.id_dis_det_avance_pago
  AND    di.edo_compensa_amo       = 2
  AND    av.monto_dif_amo*(-1)     > 2 --Se multiplica por -1 para hacer positivas las diferencias negativas
  --Si está nula la devuelve a cero
  IF v_cuenta_amo IS NULL THEN 
     LET v_cuenta_amo = 0 
  END IF

  LET v_cuenta_apo = v_cuenta_apo + v_cuenta_amo 
   
  IF v_cuenta_apo = 0 OR v_cuenta_apo IS NULL THEN 
     DISPLAY "No se generó la interface de Abono para HS (>$2) por falta de información"
     EXIT PROGRAM 
  END IF 
  ...
  LET v_nom_archivo       = "/esp_abono_avance_hs_"
  LET v_ddmmaaaa          = TODAY USING "ddmmyyyy"
  LET v_busca_nom_archivo = "esp_abono_avance_hs_" || v_ddmmaaaa
  ...
  WHENEVER ERROR CONTINUE;
    DROP TABLE tmp_dis_interface_dn_esp;
    ...
    AND    av.id_dis_det_avance_pago    = di.id_dis_det_avance_pago 
    AND   ((di.edo_compensa_apo IN (2) AND av.monto_dif_apo*(-1) > 2)
    OR     (di.edo_compensa_amo IN (2) AND av.monto_dif_amo*(-1) > 2))
    INTO TEMP tmp_dis_interface_dn_esp

    UPDATE STATISTICS FOR TABLE tmp_dis_interface_dn_esp
   
    DECLARE cur_consulta CURSOR FOR 
    SELECT *
    FROM   tmp_dis_interface_dn_esp
    FOREACH cur_consulta INTO v_folio,
      ...
      IF v_edo_compensa_apo = 2 AND v_importe_apo2 > 2 THEN 
      ...
      IF v_edo_compensa_amo = 2 AND v_importe_amo2 > 2 THEN 
         CALL v_ch_arch_salida.writeLine(v_tipo_registro2||
                                         v_nss||
                                         c_numero_credito||
                                         v_periodo||
  ...
  DISPLAY "Se ha generado el archivo de Abono a HS (>2) correctamente: ", v_nom_archivo
...
FUNCTION fn_genera_copia_interface_abono(p_archivo_envio, p_ruta_destino)
  ...
  LET v_archivo_copia = "esp_abono_avance_hs"
  ...
  DISPLAY "Se ha realizado la copia de la interface de Abono a HS (>$2): ", v_archivo_copia

  DISPLAY "\n\nEjecutando scripts de transferencia de archivos (Abono a HS >$2)"
  LET v_cmd = " sh /opt/Interpel/Scripts/abono_avance_hs.sh"
  --LET v_cmd = "sh ", p_ruta_destino CLIPPED, "/abono_avance_hs.sh"
  RUN v_cmd
