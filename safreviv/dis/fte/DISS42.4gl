################################################################################
#Version                    => 1.0.0                                           #
#Fecha ultima modificacion  => 19/08/2015                                      #
################################################################################
################################################################################
...
#                     Avance Abierto con Crédito Liquidado.                    #
...
MAIN
   ...
   LET g_tot_ava_liq  = ARG_VAL(5)
   LET p_usuario      = ARG_VAL(6)
   
   CALL STARTLOG(p_usuario CLIPPED||".DISS42.log")
...
FUNCTION fn_genera_interface_avance_cred_liq()
  ...
  WHENEVER ERROR CONTINUE 
    DISPLAY "INICIA fn_dis_extractor_avance_cred_liq: ", CURRENT HOUR TO SECOND 
    PREPARE ps_sp_dis_avance_cred_liq FROM "EXECUTE PROCEDURE fn_dis_extractor_avance_cred_liq(?)"
    EXECUTE ps_sp_dis_avance_cred_liq USING p_folio_dis
                                      INTO r_bnd, v_status_err, v_desc_err
    DISPLAY "TERMINA fn_dis_extractor_avance_cred_liq: ", CURRENT HOUR TO SECOND 
  WHENEVER ERROR STOP 

   CALL fn_extractor_avance_cred_liq()
...
FUNCTION fn_extractor_avance_cred_liq()
  DEFINE 
   ...
   DEFINE v_arr_avance_cred_liq DYNAMIC ARRAY OF RECORD
   ...
   LET v_desc_destino = "AVANCE ABIERTO CRÉDITO LIQUIDADO"
   LET v_aaaammdd  = TODAY USING "yyyymmdd"
   

   
   {LET g_qrytxt = " SELECT acl.* ",
                  "\n FROM tmp_avance_cred_liq acl",
                  "\n ORDER BY acl.nss, acl.nrp, acl.periodo_pago "
   ...
   PREPARE ps_dis_extractor_avance_liq FROM g_qrytxt
   DECLARE cur_dis_extractor_avance_liq CURSOR FOR ps_dis_extractor_avance_liq}
 ...
 --TITULOS
   LET v_nom_archivo   = "/dis_ext_avance_cred_liq_", v_aaaammdd,"_"
   ...
   SELECT UNIQUE("FOLIO LIQUIDA"), 
          ...
          "APORTACIÓN (PAGO)",
          "AMORTIZACIÓN (PAGO)",
          "FECHA LIQUIDACIÓN CRÉDITO" 
   FROM tmp_avance_cred_liq acl
  

  --DETALLE
   LET v_nom_archivo_det   = "/dis_ext_avance_cred_liq_det", v_aaaammdd,"_"
   LET v_nom_archivo_det   = v_nom_archivo_det CLIPPED ||p_folio_dis CLIPPED||"."|| v_modulo_cod
   LET v_ruta_nomarch_det  = v_ruta_envio_dis CLIPPED || v_nom_archivo_det

   --DISPLAY "v_nom_archivo_det : ", v_nom_archivo_det
   ...
   --TOTALES
   LET v_nom_archivo_tot   = "/dis_ext_avance_cred_liq_tot"
   LET v_nom_archivo_tot   = v_nom_archivo_tot CLIPPED ||p_folio_dis CLIPPED||"."|| v_modulo_cod
 ...
 --Imprime encabezado del archivo
  ...
  -- LET v_encabezado = " DESTINO:  ",v_desc_destino
   ...
   PREPARE ps_totales_avance_cred_liq FROM g_qrytxt
   EXECUTE ps_totales_avance_cred_liq INTO v_tot_registros,
                                           v_sum_aportacion,
                                           v_sum_amortizacion,
                                           v_sum_imp_ap_pat,
                                           v_sum_imp_am_cre
   ...
   {LET i = 1
   FOREACH cur_dis_extractor_avance_liq INTO v_arr_avance_cred_liq[i].folio_liquida, 
                                             v_arr_avance_cred_liq[i].nss,
                                             v_arr_avance_cred_liq[i].periodo_pago,      
                                             v_arr_avance_cred_liq[i].num_credito,
                                             v_arr_avance_cred_liq[i].nrp,                
                                             v_arr_avance_cred_liq[i].monto_apo,       
                                             v_arr_avance_cred_liq[i].monto_amo, 
                                             v_arr_avance_cred_liq[i].f_liquida, 
                                             v_arr_avance_cred_liq[i].folio_pago,
                                             v_arr_avance_cred_liq[i].folio_sua,          
                                             v_arr_avance_cred_liq[i].f_pago, 
                                             v_arr_avance_cred_liq[i].periodo_pag,      
                                             v_arr_avance_cred_liq[i].imp_ap_pat,     
                                             v_arr_avance_cred_liq[i].imp_am_cre      
      
      LET v_detalle = v_arr_avance_cred_liq[i].folio_liquida, "|",
                      v_arr_avance_cred_liq[i].nss USING "&&&&&&&&&&&", "|", 
                      v_arr_avance_cred_liq[i].periodo_pago USING "&&&&&&", "|", 
                      v_arr_avance_cred_liq[i].num_credito , "|",
                      v_arr_avance_cred_liq[i].nrp, "|",                 
                      (v_arr_avance_cred_liq[i].monto_apo * 100) USING "&&&&&&&&&&&&", "|",          
                      (v_arr_avance_cred_liq[i].monto_amo * 100) USING "&&&&&&&&&&&&", "|", 
                      v_arr_avance_cred_liq[i].f_liquida USING "yyyymmdd", "|", 
                      v_arr_avance_cred_liq[i].folio_pago, "|",        
                      v_arr_avance_cred_liq[i].folio_sua USING "&&&&&&", "|",          
                      v_arr_avance_cred_liq[i].f_pago USING "yyyymmdd", "|", 
                      v_arr_avance_cred_liq[i].periodo_pagoUSING "&&&&&&", "|",      
                      (v_arr_avance_cred_liq[i].imp_ap_pat * 100) USING "&&&&&&&&&&&&", "|",          
                      (v_arr_avance_cred_liq[i].imp_am_cre * 100) USING "&&&&&&&&&&&&" 
                                  
      CALL v_ch_arch_salida.write([v_detalle])

      
      LET v_tot_registros    = v_tot_registros    + 1
      --LET v_sum_aivs         = v_sum_aivs         + v_arr_avance_cred_liq[i].aivs
      LET v_sum_aportacion   = v_sum_aportacion   + v_arr_avance_cred_liq[i].monto_apo
      LET v_sum_amortizacion = v_sum_amortizacion + v_arr_avance_cred_liq[i].monto_amo

      LET v_sum_imp_ap_pat   = v_sum_imp_ap_pat + v_arr_avance_cred_liq[i].imp_ap_pat
      LET v_sum_imp_am_cre   = v_sum_imp_am_cre + v_arr_avance_cred_liq[i].imp_am_cre 
      
      LET i = i + 1
   END FOREACH

   LET i = i - 1
   CALL v_arr_avance_cred_liq.deleteElement(v_arr_avance_cred_liq.getLength())
   ...
   IF p_folio_dis IS NULL OR p_folio_dis = 0 THEN
      LET p_folio_dis = ""
   END IF
   ...
   DISPLAY "Registros con Avance Cred Liq  : ", g_tot_ava_liq
   DISPLAY "Registros después Validaciones : ", v_tot_registros
   DISPLAY "Suma Monto Aportación Avance   : ", v_sum_aportacion
   --DISPLAY "Suma Monto AIVS                : ", v_sum_aivs
   DISPLAY "Suma Monto Amortización Avance : ", v_sum_amortizacion
