...
#Modulo       => DIS                                                           #
#Programa     => DISR12                                                        #
#Objetivo     => Programa que ejecuta el reverso de los casos por excepción a  #
#                dispersar.                                                    #
...
MAIN
  ...
  DEFINE r_arr_caso_excep    DYNAMIC ARRAY OF RECORD
  ...
  OPEN WINDOW vtn_casos_exc WITH FORM "DISR121"
    DIALOG ATTRIBUTES(UNBUFFERED) 
      INPUT v_folio, v_fecha FROM f_folio, f_fecha
        BEFORE INPUT
      ...
      DISPLAY ARRAY r_arr_caso_excep TO svr_detalle.* 
      END DISPLAY
      ...
      ON ACTION reverso
         CALL fn_ventana_confirma("Casos Excepción a Dispersar", 
         ...
         IF r_respuesta = 1 THEN
            ...
            IF v_bnd_val_rev = 0 THEN
               LET v_QryTxt = " nohup time fglrun ",v_seg_modulo.ruta_bin CLIPPED,"/DISR121.42r ",
               ...
               CALL fn_mensaje("Atención",
                               "Se ha enviado el reverso de los Casos de Excepción a Dispersar.\n"||
...
FUNCTION fn_obtiene_informacion(v_folio, v_fecha)
  DEFINE 
    ...
    r_arr_caso_excep         DYNAMIC ARRAY OF RECORD
    v_periodo_pago           CHAR(6),
    v_aportacion             DECIMAL(12,2),
    v_amortizacion           DECIMAL(12,2),
    v_total_cuentas          DECIMAL(9,0) 
  ...
  ELSE
    LET v_sql_txt= "\n SELECT cm.periodo_pago, ",
                   "\n        sum(cm.aportacion),",
                   "\n        sum(cm.amortizacion), count(*)",
                   "\n FROM   dis_caso_excepcion cm",
                   "\n WHERE  folio = ", v_folio
    ...
    FOREACH cur_casos_excep INTO r_arr_caso_excep[v_indice].v_periodo_pago,
                                 r_arr_caso_excep[v_indice].v_aportacion,
                                 r_arr_caso_excep[v_indice].v_amortizacion,
                                 r_arr_caso_excep[v_indice].v_total_cuentas
      LET v_tot_aportacion    = v_tot_aportacion    + r_arr_caso_excep[v_indice].v_aportacion 
      LET v_tot_amortizacion  = v_tot_amortizacion  + r_arr_caso_excep[v_indice].v_amortizacion
      LET v_tot_total_cuentas = v_tot_total_cuentas + r_arr_caso_excep[v_indice].v_total_cuentas
      LET v_indice            = v_indice            + 1
    END FOREACH

    CALL r_arr_caso_excep.deleteElement(v_indice)

    IF v_tot_total_cuentas = 0 THEN
       SELECT UNIQUE a.f_registro
       INTO   v_fecha
       FROM   dis_caso_excepcion a
       WHERE  a.folio = v_folio
    END IF
  END IF
  
  RETURN r_arr_caso_excep,
         v_tot_aportacion,
         v_tot_amortizacion,
         v_tot_total_cuentas,
         v_fecha,
