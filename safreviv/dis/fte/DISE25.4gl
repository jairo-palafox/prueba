...
#MODULO           => DIS                                                       #
#PROGRAMA         => DISE25                                                    #
#OBJETIVO         => Ejecuta la función de Facturación de los créditos OCG     #
#                                                                              #
...
GLOBALS
  ...
  DEFINE g_folio_transaccion       DECIMAL(9,0)  --Folio de transacción
  DEFINE g_f_transaccion_ini       DATE          --Fecha de transacción inicial
  DEFINE g_f_transaccion_fin       DATE          --Fecha de transacción final 
...
MAIN
  ...
  LET g_f_transaccion_ini        = ARG_VAL(3) --Codigo de operación
  LET g_f_transaccion_fin        = ARG_VAL(4) --Codigo de operación
  LET g_tipo_credito             = ARG_VAL(5) --Codigo de operación
  ...
  DISPLAY "Inicia Validaciones para la facturación: " --,TIME
...
FUNCTION fn_facturar_apo_sub()
  ...
  LET g_proceso_cod = 3905 --Codigo del Proceso de Facturación
  ...
  IF bnd_continuar THEN
     ...
     IF v_status = 0 THEN                                            
        ...
        DISPLAY "La facturación se ha realizado exitosamente."
     ELSE                      
        LET v_bnd_confirmacion = 1
        DISPLAY "Ocurrió un error inesperado."
     END IF
     ...
     ELSE
        DISPLAY "Error en la facturación: ",v_bnd_confirmacion
...
FUNCTION fn_genera_reporte_fact_apo_sub(p_folio_factura,
  ...
  RETURNING v_ruta_ejecutable, v_ruta_listados
   ...
   ELSE
      DISPLAY "No existen registros para generar el reporte de Facturación."
   END IF
...
FUNCTION fn_genera_archivo_fact_rech(p_folio_trans,
  ...
  LET v_titulos = "FOLIO TRANSACCIÓN |FECHA TRANSACCIÓN |NSS |NOMBRE |PERIODO PAGO |FECHA PAGO |FOLIO SUA |NRP |AIVS |APORTACIÓN |TIPO CRÉDITO |ESTADO |FECHA ARCHIVO |INTERFACE "
  ...
  CALL fn_mensaje("Información","Se ha generado el archivo de No Acreditados (Facturar) \n en la ruta "||v_ruta_nomarch,"information")
