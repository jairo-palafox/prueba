...
#Modulo           => DIS                                                       #
#Programa         => DISC35                                                    #
#Objetivo         => Programa de consulta de Devoluciones de Aportaciones      #
...
GLOBALS
  ...
  --Datos de detalle
  DEFINE a_devoluciones      DYNAMIC ARRAY OF RECORD    
...
MAIN
  ...
  CALL STARTLOG (g_usuario CLIPPED||".DISC35.log")
      ...
      INPUT BY NAME v_folio_transaccion, 
        ...
        ON ACTION ACCEPT
           ...
           IF v_folio_transaccion IS NULL AND 
              ...
              v_nss               IS NULL AND v_tipo_credito IS NULL THEN 
           ...
           IF v_f_transaccion_ini IS NOT NULL AND 
              v_f_transaccion_fin IS NULL     THEN 
              CALL fn_mensaje("ATENCIÓN", 
           ...
           IF v_indice1 > 0 THEN  
              ...
              DISPLAY ARRAY a_devoluciones TO rec_ent_fin_bloq.*
              ATTRIBUTES (ACCEPT=FALSE, CANCEL=FALSE)
                BEFORE DISPLAY
                  ...
                  ON ACTION archivo
                     CALL fn_genera_archivo(v_folio_transaccion, 
                                            v_f_transaccion_ini, 
                                            v_f_transaccion_fin, 
                                            v_tipo_credito,
                                            v_nss)
...
FUNCTION fn_consultar(p_folio_transaccion, p_f_transaccion_ini, 
                      p_f_transaccion_fin, p_tipo_credito, p_nss)
  DEFINE 
  ...
  LET g_sql_txt = "\n SELECT H.id_dis_interface_ef, ",
                  ...
                  "\n AND    H.concepto          IN (108,408,508,808)    "
  ...
  LET g_sql_txt = g_sql_txt, "\n INTO TEMP tmp_afi_disc35 "

  PREPARE prep_tmp_afi FROM g_sql_txt
  EXECUTE prep_tmp_afi

  UPDATE STATISTICS FOR TABLE tmp_afi_disc35
  ...
  LET g_sql_txt = "\n SELECT dc.cve_ent_financiera,          ",
                  ...
                  --"\n WHERE  dc.id_derechohabiente = af.id_derechohabiente ",
                  "\n WHERE  dc.id_dis_interface_ef = af.id_dis_interface_ef ",
                  ...
                  "\n AND    dc.concepto           IN (108,408,508,808)     "
  ...
  LET g_sql_txt = g_sql_txt,"\n GROUP BY 1,2,3,4,5,6,7 ",
                            "\n ORDER BY dc.cve_ent_financiera, dc.concepto, dc.estado"
  DISPLAY "g_sql_txt: -",g_sql_txt,"-"

  PREPARE ps_devol FROM g_sql_txt
  DECLARE cur_devol CURSOR FOR ps_devol
  
  LET v_indice1         = 1
  LET v_tot_registros   = 0.0   
  LET v_tot_monto_pesos = 0.0

  CALL a_devoluciones.clear()
  FOREACH cur_devol INTO v_entidad_financiera,
                         a_devoluciones[v_indice1].entidad_financiera,    
                         a_devoluciones[v_indice1].cuenta_bancaria,
                         v_concepto,
                         a_devoluciones[v_indice1].concepto,
                         v_estado,
                         a_devoluciones[v_indice1].estado,
                         a_devoluciones[v_indice1].tot_registros,
                         a_devoluciones[v_indice1].monto

    LET a_devoluciones[v_indice1].entidad_financiera = v_entidad_financiera USING "&&&",' - ',a_devoluciones[v_indice1].entidad_financiera CLIPPED  
    LET a_devoluciones[v_indice1].concepto           = v_concepto CLIPPED,' - ',a_devoluciones[v_indice1].concepto CLIPPED
    LET a_devoluciones[v_indice1].estado             = v_estado CLIPPED,' - ',a_devoluciones[v_indice1].estado CLIPPED                        
      
    LET v_tot_registros   = v_tot_registros   + a_devoluciones[v_indice1].tot_registros
    LET v_tot_monto_pesos = v_tot_monto_pesos + a_devoluciones[v_indice1].monto
    LET v_indice1         = v_indice1         + 1
  END FOREACH
  
  CALL a_devoluciones.deleteElement(v_indice1)
  LET v_indice1    = v_indice1 - 1  
...
FUNCTION fn_genera_archivo(p_folio_transaccion, p_f_transaccion_ini, 
                           p_f_transaccion_fin, p_tipo_credito, p_nss)
  DEFINE 
    p_folio_transaccion      DECIMAL(9,0), --Folio de transacción
    p_f_transaccion_ini      DATE,         --Fecha de transacción inicial
  ...
  LET v_nom_archivo   = "/fact_devoluciones_", v_hora
  ...
  LET g_sql_txt = g_sql_txt,"\n ORDER BY dc.cve_ent_financiera, dc.concepto, dc.estado, dc.folio_transaccion, dc.f_transaccion DESC, ad.nss, dc.periodo_pago DESC, dc.f_pago DESC "
  ...
  LET v_encabezado = "NSS |NOMBRE| PERIODO PAGO |FECHA PAGO |FOLIO SUA |NRP |AIVS |APORTACIÓN |ENTIDAD FINANCIERA |CONCEPTO PAGO |TRANSACCIÓN |FOLIO TRANSACCIÓN |FECHA TRANSACCIÓN |STATUS |FECHA ARCHIVO |INTERFACE "
  CALL v_ch_arch_salida.write([v_encabezado])
  ...
  FOREACH cur_sl_inf_arc INTO arr_info_apo[v_indice].v_nss,
                              arr_info_apo[v_indice].v_nombre_completo,
    ...
    LET v_detalle = arr_info_apo[v_indice].v_nss, "|",
                    arr_info_apo[v_indice].v_nombre_completo CLIPPED, "|",
                    arr_info_apo[v_indice].v_periodo_pago, "|",
  ...
  CALL fn_mensaje("Información","Se ha generado el archivo de Devoluciones \n en la ruta "||v_ruta_nomarch,"information") 
END FUNCTION 
...
REPORT rep_ent_fin_bloq(p_folio_transaccion, 
  ...
  DEFINE v_t_registros       DECIMAL(9,0)  
  
  DEFINE 
    v_fecha_consulta         DATE, --Fecha de proceso 
    v_tot_reporte            DECIMAL(9,0), 
