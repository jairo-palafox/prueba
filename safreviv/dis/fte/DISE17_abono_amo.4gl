################################################################################
#Version                    => 1.0.0                                           #
#Fecha ultima modificacion  => 20/02/2015                                      #
################################################################################
################################################################################
...
MAIN
  ...
  --Borra y crea tablas dis_preliquida, dis_interface_hs y dis_interface_ef
  ...
  IF r_bnd_preliquidacion = 0 THEN
      ...
      SELECT f_liquida,
             ...
             origen
      FROM   dis_preliquida
      WHERE  folio_liquida = v_folio_liquida_dis
...
FUNCTION fn_borra_crea_tablas()
   ...
   EXECUTE IMMEDIATE "SET INDEXES FOR dis_interface_hs DISABLED;"
   EXECUTE IMMEDIATE "SET INDEXES FOR dis_compensa_avance DISABLED;"
   --EXECUTE IMMEDIATE "SET INDEXES FOR dae_det_solicitud DISABLED;"
   --EXECUTE IMMEDIATE "SET INDEXES FOR dis_cta_ind_pau DISABLED;"
   WHENEVER ERROR STOP;
