...
#Modulo       => DIS                                                           #
#Programa     => DISR13                                                        #
#Objetivo     => Programa que ejecuta el reverso de los creditos ceros.        #
#                                                                              #
#Fecha inicio => 05/03/2015                                                    #
################################################################################
DATABASE safre_viv
GLOBALS
  DEFINE 
...
MAIN
  ...
  DEFINE v_arr_cred_cero     DYNAMIC ARRAY OF RECORD
  ...
  CALL STARTLOG(p_usuario_cod CLIPPED||".DISR13.log")
  ...
  OPEN WINDOW w_rev_creditos_cero WITH FORM "DISR131"
    DIALOG ATTRIBUTES(UNBUFFERED) 
      ...
      DISPLAY ARRAY v_arr_cred_cero TO svr_detalle.* 
      END DISPLAY
      ...
      ON ACTION ACCEPT
         ...
         ELSE
            ...
            CALL fn_mensaje("Reverso Créditos Ceros",
                            "Información no existe ", "about")
            LET v_folio = NULL 
            LET v_fecha = NULL 
            NEXT FIELD f_folio
      ...
      ON ACTION reverso
         CALL fn_ventana_confirma("Créditos Ceros", 
         ...
         IF r_respuesta = 1 THEN
            ...
            IF v_bnd_val_rev = 0 THEN
               LET v_QryTxt = " nohup time fglrun ",v_seg_modulo.ruta_bin CLIPPED,"/DISR131.42r ",
               ...
               CALL fn_mensaje("Atención",
                               "Se ha enviado el reverso de los Créditos Ceros.\n"||
...
FUNCTION fn_obtiene_informacion(v_folio, v_fecha)
  DEFINE v_folio             DECIMAL(9,0), --Folio
         ...
         v_arr_cred_cero     DYNAMIC ARRAY OF RECORD
            v_periodo_pago   CHAR(6),
            v_aportacion     DECIMAL(12,2),
            v_amortizacion   DECIMAL(12,2),
            v_aivs           DECIMAL(22,2),
  ...
  --ELSE
    LET v_sql_txt= "\n SELECT dcc.periodo_pago, ",
                   "\n        sum(dli.aportacion),",
                   "\n        sum(dli.amortizacion),", 
                   "\n        sum(dli.aivs),",
                   "\n        count(*)",
    ...
    FOREACH cur_casos_excep INTO v_arr_cred_cero[v_indice].v_periodo_pago,
                                 v_arr_cred_cero[v_indice].v_aportacion,
                                 v_arr_cred_cero[v_indice].v_amortizacion,
                                 v_arr_cred_cero[v_indice].v_aivs,
                                 v_arr_cred_cero[v_indice].v_total_cuentas
                                 
      LET v_tot_aportacion    = v_tot_aportacion    + v_arr_cred_cero[v_indice].v_aportacion 
      LET v_tot_amortizacion  = v_tot_amortizacion  + v_arr_cred_cero[v_indice].v_amortizacion
      LET v_tot_aivs          = v_tot_aivs          + v_arr_cred_cero[v_indice].v_aivs
      LET v_tot_total_cuentas = v_tot_total_cuentas + v_arr_cred_cero[v_indice].v_total_cuentas
      LET v_indice            = v_indice            + 1
    END FOREACH

    CALL v_arr_cred_cero.deleteElement(v_indice)
