###############################################################################
#VERSION                    => 1.0.1                                           #
#FECHA ULTIMA MODIFICACION  => 06/01/2020                                      #
################################################################################
################################################################################
...
MAIN 
  ...
  CALL STARTLOG(p_usuario CLIPPED||".DISP07.log")
...
FUNCTION fn_dis_preliquida()
  ...
  LET p_proceso_cod = 932 --Codigo del Proceso de Aportaciones Subscuentes Sin Adelanto (4)(932)       
  ...
  DISPLAY "PROCESO APO SUBS: ", p_proceso_cod_reg_pago
  --PRELIQUIDACION 
  -- APORTACIONES SUBSECUENTES
  IF p_proceso_cod_reg_pago = 932 THEN
     --Ejecuta función para la preliquidación de dispersión
     WHENEVER ERROR CONTINUE
       PREPARE prp_fn_transaccion
       FROM    "EXECUTE FUNCTION fn_dis_transaccion14(?,?,?,?,?,?,?,?)"
     ...
     DISPLAY "Función Transaccion 14", r_bnd_preliquidacion
     ...
     IF r_bnd_preliquidacion <> 0 THEN
        ...
        DISPLAY "Error en la transacción 14",v_status_err," ",v_desc_err
        EXIT PROGRAM
     END IF
  END IF -- NORMAL
  ...
  IF r_bnd_preliquidacion = 0 THEN      
     ...
     IF v_bnd_preliq_ok = 1 THEN
        ...
        CALL fn_reporte_liquidacion(p_folio_disp,"dis_ap_preliquida",p_usuario,p_pid,
  ...
  IF bnd_continuar THEN
     ...
     IF r_bandera = 0 THEN 
        ...
        CALL fn_correo_proceso(p_pid,
                               ...
                               "Preliquidación de Aportaciones Subsecuentes",
                               "\n ID Proceso   : "||p_pid||
                               "\n Proceso      : Aportaciones Subsecuentes"||
                               "\n Operacion    : Preliquidación de aportaciones subsecuentes"||
                               "\n Fecha Inicio : "||TODAY||
                               "\n Fecha Fin    : "||TODAY)
     END IF
  END IF      
...
FUNCTION fn_borra_crea_tablas()
  WHENEVER ERROR CONTINUE;
    DROP TABLE dis_ap_preliquida;
    EXECUTE IMMEDIATE "SET INDEXES FOR dis_interface_ef DISABLED;"
    EXECUTE IMMEDIATE "SET INDEXES FOR dis_ctr_aps_tns DISABLED;"
  WHENEVER ERROR STOP;
END FUNCTION
...
FUNCTION fn_valida_preliquidacion(v_p_pid, v_p_proceso_cod, v_p_opera_cod)
  ...
  IF v_bnd_preliquidacion <= 0 THEN    
     ...
     DISPLAY ""
     DISPLAY "No se encontraron registros a preliquidar con el folio de aportaciones subsecuentes seleccionado"

     CALL fn_correo_proceso(v_p_pid,
                            v_p_proceso_cod,
                            v_p_opera_cod,
                            "",
                            "Preliquidación de Aportaciones Subseceuntes",
                            "\n ID Proceso   : "||v_p_pid||
                            "\n Proceso      : Aportaciones Subsecuentes de Pagos"||
                            "\n Operacion    : Preliquidación de aportaciones subsecuentes"||
                            "\n Fecha Inicio : "||TODAY||
                            "\n Fecha Fin    : "||TODAY)
     ...
     LET v_p_proceso_cod = 932 -- aportaciones subsecuentes
     LET v_p_opera_cod   = 4   -- liquidacion
     ...
     IF ( v_bandera = 0 ) THEN
        ...
        DISPLAY ""
        DISPLAY "No se encontraron registros a liquidar con el folio de aportaciones subsecuentes seleccionado"
     END IF
