...
MAIN
  ...
  DEFINE
    ...
    v_f_valor                DATE,
    v_f_registro             DATE,
    v_h_registro             DATETIME HOUR TO SECOND,
    v_origen                 CHAR(20),
  ...
  IF v_cuenta_hs = 0 OR v_cuenta_hs IS NULL THEN
     DISPLAY "No se generó la interface de Hipotecaria Social (Créditos MTC's Con coincidencias) por falta de información"
     EXIT PROGRAM 
  END IF 
   
  {WHENEVER ERROR CONTINUE;
    DROP TABLE tmp_dis_interface_mtcc;
  WHENEVER ERROR STOP}
  ...
  AND    A.nss               IN (SELECT B.nss
                                 FROM   dis_interface_mtc B)
  INTO TEMP tmp_dis_interface_mtcc
 
  UPDATE STATISTICS FOR TABLE tmp_dis_interface_mtcc}
  ...
  LET v_nom_archivo       = "/pag_hs_mtc_" --nombre de archivo
  #LET v_nom_archivo       = "/pag_hs" --nombre de archivo
  LET v_ddmmaaaa          = TODAY USING "ddmmyyyy"  --Fecha del archivo sin separadores
  LET v_busca_nom_archivo = "pag_hs_mtc_" || v_ddmmaaaa  --Concatena nombre a buscar
  ...
  LET v_cons = "EXECUTE FUNCTION fn_marca_cuenta(?,?,?,?,?,?,?,?,?,?)"
  PREPARE prp_marca FROM v_cons CLIPPED
   
  DECLARE cur_consulta_folio_interface_hs CURSOR FOR
  {SELECT *
  FROM   tmp_dis_interface_mtcc}
  SELECT a.nss,
         a.num_crd_ifv,
         a.periodo_pago,
         a.f_pago,
  ...
  FOREACH cur_consulta_folio_interface_hs INTO v_arr_tipo2.nss,
    ...
    --MARCA CUENTA 450 Aclaración-Amortización No Dispersada
    EXECUTE prp_marca USING v_id_derechohabiente,
                             '450',
                             v_arr_tipo2.folio_sua,
                             v_folio_liquida,
                             '0',
    ...
    UPDATE dis_creditos_mtc
    SET    cta_marcada   = v_cod_rechazo
    WHERE  nss           = v_arr_tipo2.nss
    AND    folio_sua     = v_arr_tipo2.folio_sua
    AND    periodo_pago  = v_arr_tipo2.periodo_pago
    AND    nrp           = v_arr_tipo2.nrp
    AND    folio_liquida = v_folio_liquida
    AND    num_crd_ifv   = v_arr_tipo2.num_credito
    ...
    WHERE  cta.folio_liquida      = v_folio_liquida
  ...
  UPDATE glo_folio
  SET    status = 2
  WHERE  folio  = v_folio_ajuste
  
  UPDATE glo_folio
  SET    status = 2
  WHERE  folio  = v_folio_n_dis
  ...
  DISPLAY "Se ha generado la interface de Pago Real HS (Créditos MTC con coincidencias) correctamente: ",v_nom_archivo
...
FUNCTION fn_genera_copia_interface_real(p_archivo_envio,p_ruta_destino)
  ...
  LET v_archivo_copia = "pag_hs_mtc"
  ...
  --DISPLAY "v_comando_dos ",v_comando_dos 
  RUN v_comando_dos
  DISPLAY "     Se ha realizado la copia de la interface de Pago Real HS (Créditos MTC con coincidencias): ",v_archivo_copia
