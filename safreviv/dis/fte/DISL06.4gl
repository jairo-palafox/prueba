################################################################################
#Version                    => 1.0.1                                           #
#Fecha ultima modificacion  => 30/04/2012                                      #
################################################################################
################################################################################
...
DEFINE v_tipo_proceso    SMALLINT, -- Forma como ejecutara el programa 
   ...
   CALL STARTLOG (v_usurio CLIPPED|| ".DISL06.log")
...
DEFINE l_arr_arch_pend    DYNAMIC ARRAY OF VARCHAR(100), -- Archivos pendientes 
   ...
   CLOSE WINDOW SCREEN 
   OPEN WINDOW w_inte_acred WITH FORM "DISL061"
     ...
     {ON ACTION deshacer
      ...
      ON ACTION integrar
         ...
         IF r_bandera = 0 THEN  
            ...
            CALL fn_actualiza_opera_ini(v_max_pid,g_proceso_cod, g_opera_cod,g_folio,"DISE05",l_v_arch_proceso,v_usurio)
            RETURNING r_b_valida
                                                
            IF r_b_valida = 0 THEN
            
               LET l_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/DISE05.42r ",
                                v_usurio," ",
                                v_max_pid," ",
                                g_proceso_cod," ",
                                g_opera_cod," ",
