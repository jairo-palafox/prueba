...
#Modulo           => DIS                                                       #
#Programa         => DISC23                                                    #
#Objetivo         => Programa de consulta de Facturación por Concepto de Pago  # 
           ...
           ELSE
             IF (v_folio_liquida IS NOT NULL) THEN
                ...
                ELSE
                  ...
                  IF v_estado = 60 THEN
                     CALL fn_mensaje("ATENCIÓN",
                                     "El Folio de Operación está en Trámite de Pago.",
                                     "about")
                     CALL ui.interface.refresh()
                  ELSE
                    SELECT UNIQUE dc.estado
                    ...
                    IF v_estado = 55 THEN
                       CALL fn_mensaje("ATENCIÓN",
                                       "El Folio de Operación ya fue Provisionado.",
                                       "about")
                       CALL ui.interface.refresh()
                    ELSE
                      SELECT UNIQUE dc.estado
...
FUNCTION fn_genera_archivo(v_folio_liquida, v_f_liquida_ini, v_f_liquida_fin)
  ...
  --Imprime encabezado del archivo
  LET v_titulos = "FOLIO OPERACIÓN |FECHA OPERACIÓN |NSS |NOMBRE |PERIODO PAGO |FECHA PAGO |FOLIO SUA |NRP |AIVS |APORTACIÓN |CONCEPTO PAGO |FECHA ARCHIVO |INTERFACE "
  ...
  CALL fn_mensaje("Información","Se ha generado el archivo de Consulta de Facturación por Concepto de Pago \n en la ruta "||v_ruta_nomarch,"information") 
END FUNCTION