...
#                     Social - Pago Real para los ajustes operativos de los    #
#                     créditos Mejora Tu Casa (MTC's)                          #
...
MAIN
  ...
  WHERE  folio_ajuste = v_folio_liquida
  IF v_cuenta_hs  = 0    OR 
     v_cuenta_hs IS NULL THEN
     DISPLAY "No se generó la interface de Hipotecaria Social por falta de información"
     EXIT PROGRAM 
  END IF 
   
  WHENEVER ERROR CONTINUE;
    DROP TABLE tmp_ajuste_op_mtc;
  WHENEVER ERROR STOP
  ...
  SELECT H.nss,
         ...
         H.cxc_calculada,
         H.folio_sua
  FROM   dis_ajuste_op_mtc H
  WHERE  H.folio_ajuste = v_folio_liquida
  INTO TEMP tmp_ajuste_op_mtc
 
  UPDATE STATISTICS FOR TABLE tmp_ajuste_op_mtc
  ...
  LET c_tot_apo_4                   = (v_arr_tipo4.v_tot_apo_4 * 100) 
                                      USING "&&&&&&&&&&&&&&&&&&&&&&"
  LET v_arr_tipo4.v_tot_amo_4       = v_arr_tipo3.v_total_amortizacion
  LET c_tot_amo_4                   = (v_arr_tipo4.v_tot_amo_4 * 100) 
                                      USING "&&&&&&&&&&&&&&&&&&&&&&"
  LET v_arr_tipo4.v_tot_gen_4       = v_arr_tipo3.v_total_general
  LET c_tot_gen_4                   = (v_arr_tipo4.v_tot_gen_4 * 100) 
                                      USING "&&&&&&&&&&&&&&&&&&&&&&"
...
#          relacionandolo con folio_sua de dis_ajuste_op_mtc
