...
#Modulo           => DIS                                                       #
#Programa         => DISP10                                                    #
#Objetivo         => Programa que lanza el proceso de Provisión del Pago del   #
#                    módulo OCG                                                #
...
MAIN
  ...
  LET bnd_consulta  = 0
  LET p_proceso_cod = 3915        
  LET p_opera_cod   = 1

  CALL STARTLOG (g_usuario CLIPPED||".DISP10.log")
  ...
  IF f_existe_proceso_operacion_ejecutando(3915, 1) THEN
     MENU "No se puede ejecutar" ATTRIBUTES ( STYLE="dialog", 
     COMMENT="Provisión del Pago ejecutándose,\ningrese a esta opción cuando finalice.",
     IMAGE="information" )
  ...
  OPEN WINDOW w1 WITH FORM "DISP101"
          ...
          AFTER FIELD v_folio_factura
                  ...
                  ATTRIBUTES (APPEND ROW = FALSE, INSERT ROW = FALSE, DELETE ROW = FALSE, UNBUFFERED, CANCEL = FALSE, ACCEPT = FALSE) 
                    BEFORE INPUT
                      ...
                      AFTER INPUT
                        ...
                        ON ACTION Provisionar
                           LET v_bnd_confirma = 0
                           CALL fn_verificar_apo_sele() RETURNING v_bnd_confirma

                           IF v_bnd_confirma = 1 THEN
                              LET v_respuesta = 0
                              CALL fn_ventana_confirma("PROVISIONAR PAGO", 
                                                       "¿Está seguro que desea provisionar el pago de los registros seleccionados?", 
                                                       "quest")
                                 ...
                                 IF (fn_valida_operacion(p_pid, v_proceso_cod, 1) = 0 ) THEN
                                    ...
                                    IF v_bnd_confirmacion = 0 THEN      
                                       WHENEVER ERROR CONTINUE         
                                         ...
                                         --Validaciones 
                                         LET l_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/DISE26.42r ",
                                                         v_folio_disp," ",
                                                         v_folio_factura," ",
                                                         TODAY, " ",
                                                         TODAY," ", 
                                         ...
                                         LET v_mensaje = "Se ha enviado la provisión del pago: ",
                                                         p_pid CLIPPED,
                                                         ".\nPuede revisar el avance del proceso en el monitor de ejecución de procesos."

                                         CALL fn_mensaje("Provisión Pago", v_mensaje, "information")                               
                                    ELSE
                           ...
                           ELSE --v_bnd_confirma = 0
                              LET v_mensaje = "Aún no ha seleccionado ningún registro para provisionar el pago."
                              CALL fn_mensaje("Provisión Pago", v_mensaje, "information")   
           ...
           ELSE
              {IF (v_folio_factura IS NOT NULL) THEN
                 ...
                 IF v_estado = 0 THEN
                    CALL fn_mensaje("ATENCIÓN",
                                    "El Folio Factura no ha sido Publicado.",
                                    "about")
                    CALL ui.interface.refresh()
                 END IF
              ELSE}              
...
FUNCTION fn_llen_tmp_fac_apo_sub(p_folio_factura, p_tipo_credito, p_ent_financiera, 
  ...
  LET g_sql_txt = " INSERT INTO tmp_dis_pre_auto VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) "
...
FUNCTION fn_borra_crea_tmp_apo_sub()
  WHENEVER ERROR CONTINUE;
    DROP TABLE tmp_dis_pre_auto;
    CREATE TABLE tmp_dis_pre_auto (id_dis_interface_ef DECIMAL(9,0),
                                   id_derechohabiente  DECIMAL(9,0),
                                   folio_sua           DECIMAL(6,0),
                                   periodo_pago        CHAR(6),
                                   f_pago              DATE,
    ...
    --DISPLAY "Se borró y se creó la tabla tmp_dis_pre_auto"  
END FUNCTION