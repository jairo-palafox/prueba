################################################################################
#Version                    => 1.1.0                                           #
#Fecha ultima modificacion  => 07/02/2018                                      #
################################################################################
################################################################################
  ...
  IF r_edo_rech = 10 THEN
     WHENEVER ERROR CONTINUE 
        ...
        PREPARE prp_sp_apo_sub
           FROM "EXECUTE PROCEDURE safre_viv:sp_dis_apo_subs1_ea(?,?)"
     ...
     WHENEVER ERROR CONTINUE
        ...
        PREPARE prp_sp_apo_sub4
           FROM "EXECUTE PROCEDURE safre_viv:sp_dis_apo_subs4_ea(?,?)"
     ...
     WHENEVER ERROR CONTINUE 
        ...
        PREPARE prp_sp_apo_sub1
           FROM "EXECUTE PROCEDURE safre_viv:sp_dis_apo_subs2_ea(?,?)"
...
FUNCTION fn_genera_reporte_aportaciones(p_folio)
  ...
  DEFINE 
    ...
    v_sum_f_transf           DATE,          --Fech transferencia
  ...
  FROM   dis_ctr_ap_subsecuente_ea
  WHERE  folio = p_folio
  ...
  IF fgl_report_loadCurrentSettings(v_ruta_ejecutable CLIPPED ||"/DISE271.4rp") THEN
