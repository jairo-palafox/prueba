...
#Modulo           => DIS                                                       #
#Programa         => DISC26                                                    #
#Objetivo         => Programa de consulta de Facturación por NSS               #
...
FUNCTION fn_consultar(p_nss, p_f_liquida_ini, p_f_liquida_fin)
  ...
  FOREACH cur_apo_sub_concepto INTO v_nss,
    ...
    --Asiganación de FECHA DE APLICACIÓN de acuerdo al estado
    ------------------------------------------------------------
    -- v_estado = 10 (REGISTRADO)          <----> Fecha de Liquidación
    -- v_estado >= 20 <= 29 (CONFIRMADO)   <----> Fecha de Transacción
    -- v_estado >= 30 <= 39 (FACTUTRADO)   <----> Fecha de Facturación
    ------------------------------------------------------------
    CASE 
      WHEN v_estado = 10 OR --REGISTRADO     --> Fecha Liquidación
           v_estado = 40 OR --EF BLOQUEADA   --> Fecha Liquidación 
           v_estado = 50    --PAGO DUPLICADO --> Fecha Liquidación
        LET a_datos_apo_sub[v_indice].f_aplicacion = v_f_liquida
        
      WHEN v_estado >= 20 AND v_estado <= 29 --CONFIRMADO --> Fecha de Transacción
        LET a_datos_apo_sub[v_indice].f_aplicacion = v_f_transaccion
            
      WHEN v_estado >= 30 AND v_estado <= 39 --FACTURADO --> Fecha de Facturación
...
FUNCTION fn_genera_archivo_apo_sub_nss(v_nss, v_f_liquida_ini, v_f_liquida_fin)
  ...
  IF v_f_liquida_ini IS NOT NULL AND v_f_liquida_fin IS NOT NULL THEN 
     ...
     LET v_encabezado = "FECHA DE OPERACIÓN INICIAL: ",v_f_liquida_ini USING "dd-mm-yyyy"
     CALL v_ch_arch_salida.write([v_encabezado])
      
     LET v_encabezado = "FECHA DE OPERACIÓN FINAL: ",v_f_liquida_fin USING "dd-mm-yyyy"
     CALL v_ch_arch_salida.write([v_encabezado])
  END IF
  ...
  LET v_titulos = "FECHA OPERACIÓN |PERIODO PAGO |FECHA PAGO |FOLIO SUA |NRP |AIVS |APORTACIÓN |ENTIDAD FINANCIERA |NÚMERO CONTROL INTERNO |CONCEPTO PAGO |TRANSACCIÓN |STATUS |FECHA ARCHIVO |INTERFACE "
  ...
  FOREACH cur_arch_apo_sub_nss INTO v_f_liquida,
    ...
    CASE 
      ...
      WHEN v_estado = 55 OR --PROVISIONADO --> Fecha Facturación
           v_estado = 60 OR --PUBLICADO    --> Fecha Facturación
           v_estado = 70    --PAGADO       --> Fecha Facturación
        LET arr_info_apo[v_indice].f_aplicacion = v_f_factura
  ...
  CALL fn_mensaje("Información","Se ha generado el archivo de Consulta Facturación por NSS \n en la ruta "||v_ruta_nomarch,"information") 
END FUNCTION