################################################################################
#Version                    => 1.0.0                                           #
#Fecha ultima modificacion  => 05/03/2015                                      #
################################################################################
################################################################################
...
#Modulo       => DIS                                                           #
#Programa     => DISR131                                                       #
#Objetivo     => Programa de Reverso de los creditos ceros.                    #
#                                                                              #
...
MAIN
  #Si se ha recibido parámetros se continua    
   IF (NUM_ARGS() > 0)THEN
      ...
      CALL STARTLOG(g_usuario_cod CLIPPED||".DISR131.log")
      CALL fn_reverso()
   END IF
END MAIN
...
FUNCTION fn_reverso()
   ...
   DEFINE r_bnd_reverso_cred_cero   SMALLINT, 
   ...
   --Reversa la Liquidacion de Créditos Ceros
   CALL fn_reverso_liq() RETURNING bnd_reverso_liq_ok

   DISPLAY ""
   DISPLAY "____________________________________________________________________"
   DISPLAY "============================================================"
   DISPLAY " REVERSO DE LA PRELIQUIDACION DE CRÉDITOS CEROS                 "
   DISPLAY ""

   IF bnd_reverso_liq_ok = 0 THEN
      -- Ejecuta el SP que realiza el reverso de los casos de excepción a dispersar
      PREPARE prp_reversa_cred_cero
      FROM "EXECUTE PROCEDURE safre_viv:sp_dis_rev_cred_cero(?)"
      --EXECUTE prp_reversa_cred_cero USING g_folio
      
      DISPLAY ""
      DISPLAY ""
      DISPLAY ("##############################################################")
      DISPLAY "Inicio del Reverso de Preliquidación de Créditos Ceros"
      DISPLAY "Folio:"   ,g_folio
      
      EXECUTE prp_reversa_cred_cero INTO r_bnd_reverso_cred_cero, v_status_err, v_desc_err
                                    USING g_folio

      DISPLAY  r_bnd_reverso_cred_cero, v_status_err, v_desc_err

      PREPARE prp_reversa_cred_cero_preliq
      FROM "EXECUTE PROCEDURE safre_viv:sp_dis_rev_compensa_avance(?)"

      EXECUTE prp_reversa_cred_cero_preliq USING g_folio

      
      DISPLAY "Fin de Reverso Preliquidación de Créditos Ceros. "
      DISPLAY ""
      ...
      DISPLAY ""
      DISPLAY "Reverso de la Preliquidación Creditos Ceros finalizado. "
      DISPLAY ""
      DISPLAY ("##############################################################")

   ELSE
      DISPLAY "____________________________________________________________"
      DISPLAY "\nNo se pudo realizar el Reverso de la Preliquidación Creditos Ceros"
   END IF

END FUNCTION


FUNCTION fn_reverso_liq()
   ...
   DISPLAY ""
   DISPLAY ("##############################################################")
   DISPLAY "Inicio del Reverso de Liquidación de Créditos Ceros"
   DISPLAY "Folio:"   ,g_folio

   CALL fn_reverso_liquidacion(g_folio) RETURNING r_sql_code
   ...
   IF r_sql_code = 0 THEN
      ...
      DISPLAY "Reverso de Liquidación de Dispersión Créditos Ceros Finalizado"
      DISPLAY ""
      DISPLAY ("##############################################################")
   ELSE
      DISPLAY "============================================================"
