...
#Modulo       => DIS                                                           #
#Programa     => DISR03                                                        #
#Objetivo     => Programa que ejecuta el reverso de la liquiidacion de la      #
#                dispersion                                                    #
...
DEFINE seg_modulo_bat        RECORD LIKE seg_modulo.*
  ...
  OPEN WINDOW vtn_prel_disp WITH FORM "DISR031"
    DIALOG ATTRIBUTES(UNBUFFERED) 
      ...
      ON ACTION reverso
         ...
         IF r_respuesta = 1 THEN
            ...
            --CALL fn_valida_poliza_cnt(v_folio_liquida, g_proceso_cod)
            --LET v_QryTxt = "EXECUTE PROCEDURE fn_reverso_reg_cnt(?,?)"
            --
            --PREPARE prp_reverso_reg_cnt FROM v_QryTxt
            --EXECUTE prp_reverso_reg_cnt USING v_folio_liquida,g_proceso_cod INTO r_bandera
            --DISPLAY "r_bandera del contable ",r_bandera
            ...
            --IF r_bandera = 0 THEN        
                 --Función que elimina el registro en la tabla cta_movimiento
                 CALL fn_reverso_liquidacion(v_folio_liquida) RETURNING r_sql_code

                 IF r_sql_code = 0 THEN 
                    --CALL fn_muestra_inc_operacion(r_sql_code)

                    --Función para actualizar edos y fechas de tablas control de procesos
                 ...
                 ELSE
                    ...
                    IF r_sql_code = 2 THEN 
                       CALL fn_mensaje("ERROR","La fecha de emisión es diferente al día de hoy","error")
                    END IF
                 ...
                 LET v_QryTxt = " nohup time fglrun ",v_seg_modulo.ruta_bin CLIPPED,"/DISR031.42r ",
...
DEFINE 
  ...
  IF v_existe_liquida_movto > 0 THEN
     ...
     --DISPLAY "El folio regresado -- ",v_folio_reg
...
DEFINE 
  ...
  RETURN v_arr_ctamovto,v_tot_monto_pesos,
         v_tot_monto_acciones,v_tot_total_cuentas
...
DEFINE 
  ...
  RETURN v_arr_mandatos, v_indice,
         v_tot_monto_pesos, v_tot_total_cuentas
