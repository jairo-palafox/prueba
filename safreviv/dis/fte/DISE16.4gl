################################################################################
#Version                    => 1.1.0                                           #
#Fecha ultima modificacion  => 26/01/2015                                      #
################################################################################
################################################################################
...
#Modulo            => DIS                                                      #
#Programa          => DISE16                                                   #
#Objetivo          => Programa para integrar el archivo de dispersión cartera  #                   
...
MAIN
  ...
  END IF
   ...
   --Borra y crea tablas tmp_dis_cons_hs
   CALL fn_borra_crea_tablas_tmp()

  --Validaciones de Negocio
  WHENEVER ERROR CONTINUE 
    PREPARE ps_sp_dis_hs FROM "EXECUTE PROCEDURE sp_dis_consulta_hs(?)"
    EXECUTE ps_sp_dis_hs USING g_folio
  ...
  ELSE
     ...
     LET v_s_qry  = "fglrun ", r_c_ruta_bin CLIPPED, "/DISS34.42r ", g_folio
     RUN v_s_qry
...
FUNCTION fn_genera_reporte_pagos(p_folio)
  ...
  DEFINE arr_dis_cartera        DYNAMIC ARRAY OF RECORD 
            v_estado       SMALLINT,
  ...
  SELECT COUNT(dis_cart1.nss) --, 
         --SUM(dis_cart1.monto_apo/100),
         --SUM(dis_cart1.monto_amo/100)
  INTO   v_tot_nss --,
         --v_sum_ava_apo,
         --v_sum_ava_amo
  FROM   safre_tmp:tmp_ext_dis_cart1 dis_cart1

{
  ...
  SELECT COUNT(dis_hs.nss),
         --SUM(dis_hs.ava_aportacion),
         --SUM(dis_hs.ava_amortizacion),
         --SUM(dis_hs.pag_aivs),
         SUM(dis_hs.monto_apo),
         SUM(dis_hs.monto_amo) --,
         --SUM(dis_hs.ava_apo_pendiente),
         --SUM(dis_hs.ava_amo_pendiente)
  INTO   v_tot_nss1,
         -- v_sum_ava_apo_dis,
         --v_sum_ava_amo_dis,
         -- v_sum_pag_aiv,
         ...
         --v_sum_pen_apo,
         --v_sum_pen_amo
  FROM   safre_viv:tmp_dis_cons_hs dis_hs
  ...
  DISPLAY "\n ############## INTEGRACIÓN ARCHIVO CONSULTA DISPERSIÓN CARTERA ##############"
  ...
  DISPLAY " Total del monto de aportación (Pagos)      : ", v_cantidad CLIPPED USING "###,###,###,##&.&&"

  LET v_cantidad = v_sum_pag_amo
  DISPLAY " Total del monto de amortización (Pagos)    : ", v_cantidad CLIPPED USING "###,###,###,##&.&&"
  ...
  LET g_qrytxt = " SELECT estado, " , ASCII 10,
                   " CASE estado ", ASCII 10,
                       " WHEN 0 THEN 'NSS NO EXISTE EN BD' ", ASCII 10,
                       " WHEN 1 THEN 'NSS DISP A CARTERA' ", ASCII 10,
                       " WHEN 2 THEN 'NSS DEV SALDOS' ", ASCII 10,
                       " WHEN 3 THEN 'NSS SIN DISPERSIÓN' ", ASCII 10,
                    " END AS desc_estado, ", ASCII 10,
                      "  COUNT(estado) ", ASCII 10,
                " FROM tmp_dis_cons_hs ", ASCII 10,
  ...
  IF fgl_report_loadCurrentSettings(v_ruta_ejecutable CLIPPED ||"/DISE161.4rp") THEN
     ...
     START REPORT rep_dis_cartera TO XML HANDLER manejador_rpt
       DECLARE cur_ava_montos CURSOR FOR prp_consulta_detalle   
       FOREACH cur_ava_montos INTO arr_dis_cartera[v_indice_1].v_estado,
                                   arr_dis_cartera[v_indice_1].v_desc_estado,
                                   arr_dis_cartera[v_indice_1].v_tot_estado
         LET v_indice_1 = v_indice_1  + 1
                  
       END FOREACH

       CALL arr_dis_cartera.deleteElement(v_indice_1)         
       ...
       FOR v_ind_rpt = 1 TO v_indice_1
           OUTPUT TO REPORT rep_dis_cartera(g_folio,
                                              v_usuario,
                                              v_fec_proc,
                                              arr_dis_cartera[v_ind_rpt].*,
                                              r_edo_archivo,
                                              v_tot_nss,
                                              v_tot_nss1,
                                              v_nom_archivo)
       END FOR 
     FINISH REPORT rep_dis_cartera
  ELSE
     DISPLAY "no funciono"
     EXIT PROGRAM
  END IF
...
REPORT rep_dis_cartera(v_rfolio,
                         v_rusuario,
                         v_rfec_proc,
                         rec_dis_cartera,
                         r_redo_archivo,
  ...
  FORMAT
    ...
    ON EVERY ROW
       PRINTX rec_dis_cartera.v_estado
       PRINTX rec_dis_cartera.v_desc_estado
       PRINTX rec_dis_cartera.v_tot_estado
...
FUNCTION fn_obtiene_nombre_archivo(p_ruta_envio_dis, p_busca_nom_archivo)
  ...
  LET v_nom_archivo_sal = "dis_cons_cartera_sal_" --nombre de archivo
  LET v_ddmmaaaa        = TODAY USING "ddmmyyyy"  --Fecha del archivo sin separadores
