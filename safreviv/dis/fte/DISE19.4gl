...
#Modulo            => DIS                                                      #
#Programa          => DISE19                                                   #
#Objetivo          => Programa para integrar el archivo de dispersión          # 
...
--Objetivo: Funcion que realiza la carga de tablas hitoricas de números de crédito
MAIN
  DEFINE 
    r_b_valida               SMALLINT,
  ...
  END IF
   ...
   --Borra y crea tablas tmp_dis_x_nc1
   CALL fn_borra_crea_tablas_tmp()

  --Validaciones de Negocio
  WHENEVER ERROR CONTINUE 
    PREPARE ps_sp_dis_nc  FROM "EXECUTE PROCEDURE sp_dis_cons_nss_nc(0,1)"
    EXECUTE ps_sp_dis_nc  INTO r_bnd, v_status_err, v_desc_err
  WHENEVER ERROR STOP 
  ...
  ELSE
     ...
     LET v_s_qry  = "fglrun ", r_c_ruta_bin CLIPPED, "/DISS37.42r ", g_folio, 0, 1
     RUN v_s_qry
...
FUNCTION fn_genera_reporte(p_folio)
  ...
  DEFINE
    v_tot_nc                DECIMAL(9,0), --Total NSS
    v_tot_nc1               DECIMAL(9,0)  --Total Pagos
  ...
  DEFINE arr_dis_nc        DYNAMIC ARRAY OF RECORD 
            v_estado       SMALLINT,
  ...
  LET v_nom_archivo       = "/dis_cons_nc" --nombre de archivo
  LET v_ddmmaaaa          = TODAY USING "ddmmyyyy"  --Fecha del archivo sin separadores
  LET v_busca_nom_archivo = "dis_cons_nc" || v_ddmmaaaa  --Concatena nombre a buscar
  ...
  DISPLAY "\n ############## INTEGRACIÓN ARCHIVO CONSULTA DISPERSIÓN POR NÚMERO DE CRÉDITO ##############"

  LET v_cantidad = v_tot_nc
  DISPLAY " Total de Números de Crédito              : ", v_cantidad CLIPPED USING "###,###,###,##&"

  DISPLAY " Nombre del archivo (Integrado)             : ", l_arch_proceso CLIPPED 

  DISPLAY "\n ############## ARCHIVO DISPERSIÓN POR NÚMERO DE CREDITO SALIDA ##############"

  LET v_cantidad = v_tot_nc1
  ...
  IF fgl_report_loadCurrentSettings(v_ruta_ejecutable CLIPPED ||"/DISE191.4rp") THEN
     ...
     START REPORT rep_dis_nc TO XML HANDLER manejador_rpt 
       DECLARE cur_ava_montos CURSOR FOR prp_consulta_detalle   
       FOREACH cur_ava_montos INTO arr_dis_nc[v_indice_1].v_estado,
                                   arr_dis_nc[v_indice_1].v_desc_estado,
                                   arr_dis_nc[v_indice_1].v_tot_estado
         LET v_indice_1 = v_indice_1  + 1
                  
       END FOREACH

       CALL arr_dis_nc.deleteElement(v_indice_1)         
       ...
       FOR v_ind_rpt = 1 TO v_indice_1
           OUTPUT TO REPORT rep_dis_nc(g_folio,
                                              v_usuario,
                                              v_fec_proc,
                                              arr_dis_nc[v_ind_rpt].*,
                                              r_edo_archivo,
                                              v_tot_nc,
                                              v_tot_nc1,
                                              v_nom_archivo)
       END FOR 
     FINISH REPORT rep_dis_nc
  ELSE
     DISPLAY "no funciono"
     EXIT PROGRAM
  END IF
...
REPORT rep_dis_nc(v_rfolio,
  ...
  FORMAT
    ...
    ON EVERY ROW
       PRINTX rec_dis_nc.v_estado
       PRINTX rec_dis_nc.v_desc_estado
       PRINTX rec_dis_nc.v_tot_estado
...
FUNCTION fn_obtiene_nombre_archivo(p_ruta_envio_dis, p_busca_nom_archivo)
  ...
  LET v_nom_archivo_sal = "dis_cons_nc" --nombre de archivo
  LET v_ddmmaaaa        = TODAY USING "ddmmyyyy"  --Fecha del archivo sin separadores
