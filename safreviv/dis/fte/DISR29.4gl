################################################################################
#Version                    => 1.1.0                                           #
#Fecha ultima modificacion  => 25/04/2019                                      #
################################################################################
################################################################################
...
#Fecha inicio => 25/04/2019                                                    #
################################################################################
DATABASE safre_viv
GLOBALS
  DEFINE arr_det_act         DYNAMIC ARRAY OF RECORD
    r_estado                 CHAR(25),
    r_causal                 CHAR(65),
    r_total                  DECIMAL(12,0)
  END RECORD
...
MAIN
  ...
  OPEN WINDOW vtn_reverso_aiad WITH FORM "DISR291"   
    DIALOG ATTRIBUTES(UNBUFFERED) 
      INPUT v_folio_carga
      FROM  f_folio_carga_aiad
      ...
      DISPLAY ARRAY arr_det_act TO scr_detalle.* 
      END DISPLAY
      
      ON ACTION ACCEPT            
         --Valida que el folio capturado exista en la tabla de histórico
         CALL fn_valida_folio_aiad(v_folio_carga)
      ...
      ON ACTION reverso
         --LET v_folio_carga = arr_det_act[arr_curr()].r_folio_det_ctr
         ...
         --Por solicitud del usuario, se requiere reversar folios viejos
         LET r_bandera = 0
         ...
         IF r_bandera = 0 THEN   
            ...
            IF r_bandera = 1 THEN
               ...
               LET l_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,
                               "/DISR291.42r ",p_usuario_cod," ",p_pid," ",
...
DISPLAY "l_comando: ", l_comando
               LET v_s_mensaje = "Se ha enviado el Reverso de la Carga del Archivo de Actualización Indicador Adelanto Liquidación DIS con el pid: ",
                   p_pid CLIPPED,
...
FUNCTION fn_consulta_cifras(p_folio)
  ...
  FOREACH cur_det_crt_cpav INTO v_estado,
                                v_causal,
                                arr_det_act[v_indice_ctr].r_total
                                
    IF v_estado = 1 THEN
       LET arr_det_act[v_indice_ctr].r_estado = v_estado, " - RECHAZADO" 
    ELSE
       LET arr_det_act[v_indice_ctr].r_estado = v_estado, " - ACEPTADO"
    END IF

    SELECT a.desc_causal
    INTO   v_desc_causal
    FROM   cat_causal_rch_aia_dis a
    WHERE  a.causal = v_causal

    IF v_desc_causal IS NOT NULL OR
       v_desc_causal <> ''       THEN
       LET arr_det_act[v_indice_ctr].r_causal = v_causal, " - ", v_desc_causal CLIPPED
    ELSE
       LET arr_det_act[v_indice_ctr].r_causal = v_causal, " - SIN DESCRIPCIÓN"
    END IF

    LET v_indice_ctr = v_indice_ctr + 1
  END FOREACH 

  CALL arr_det_act.deleteElement(v_indice_ctr)
