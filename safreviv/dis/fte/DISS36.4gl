################################################################################
#Version                    => 1.0.0                                           #
#Fecha ultima modificacion  => 30/04/2015                                      #
################################################################################
################################################################################
...
MAIN
  ...
  IF v_cuenta_hs = 0 OR v_cuenta_hs IS NULL THEN
     DISPLAY "No se generó la interface de Portabilidad por falta de información"
     EXIT PROGRAM 
  END IF 
   
  WHENEVER ERROR CONTINUE;
    DROP TABLE tmp_dis_interface_prt;
  WHENEVER ERROR STOP
  ...
  SELECT A.nss,
         ...
         H.f_pago,H.nrp,
         H.imp_ap_pat,
         0 AS imp_am_cre, --H.imp_am_cre,
         H.folio_sua
  FROM   dis_interface_prt H, afi_derechohabiente A
  WHERE  H.folio_liquida      = v_folio_liquida
  AND    H.id_derechohabiente = A.id_derechohabiente
  INTO TEMP tmp_dis_interface_prt
 
  UPDATE STATISTICS FOR TABLE tmp_dis_interface_prt
  ...
  LET v_nom_archivo = "/pag_hs_prt_" --nombre de archivo
   
  LET v_ddmmaaaa          = TODAY USING "ddmmyyyy"  --Fecha del archivo sin separadores
  LET v_busca_nom_archivo = "pag_hs_prt_" || v_ddmmaaaa --Concatena nombre a buscar
  ...
  LET v_arr_tipo2.entidad_recau  = "370" --Portabilidad
  LET v_arr_tipo2.indica_seg     = " "
  LET v_arr_tipo2.clave_rch      = "          "
  LET v_dia_habil                = 6 --Para que traiga el séptimo día hábil
   
  DECLARE cur_consulta_folio_interface_prt CURSOR FOR
  SELECT  *
  FROM    tmp_dis_interface_prt
  FOREACH cur_consulta_folio_interface_prt INTO v_arr_tipo2.nss,
  ...
  CALL v_ch_arch_salida.close()
   ...
   DISPLAY  " Total Aportación    : "||v_arr_tipo3.v_total_aportacion
   DISPLAY  " Total Amortización  : "||v_arr_tipo3.v_total_amortizacion
   DISPLAY  " Total General       : "||v_arr_tipo3.v_total_general
   DISPLAY  " Fecha Inicio : "||TODAY
   DISPLAY  " Fecha Fin    : "||TODAY
   DISPLAY  ""

   DISPLAY " Se ha generado la interface de Pago Real HS PRT correctamente: ", v_nom_archivo
...
FUNCTION fn_crea_nombre_archivo(p_ruta_envio_dis, p_busca_nom_archivo)
  ...
  DEFINE fn                  CHAR(25)  --Almacena el nombre completo del nombre del archivo en el servidor con su extensión
  ...
  WHILE ch.read([fn])
    LET v_consecutivo = fn[20,22]  --Posición del consecutivo dentro de la cadena
  END WHILE
...
FUNCTION fn_genera_copia_interface_real(p_archivo_envio, p_ruta_destino)
  ...
  LET v_archivo_copia = "pag_hs_prt"
  ...
  DISPLAY " Se ha realizado la copia de la interface de Pago Real HS PRT: ", v_archivo_copia

  {DISPLAY "\n\nEjecutando scripts de transferencia de archivos (Pago Real HS PRT)"
  LET v_cmd = " sh /opt/Interpel/Scripts/pag_hs_prt.sh"
  --LET v_cmd = "sh ", p_ruta_destino CLIPPED, "/pag_hs_prt.sh"
  RUN v_cmd}
