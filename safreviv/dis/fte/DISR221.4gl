...
MAIN
   ...
   IF (NUM_ARGS() > 0)THEN
      ...
      CALL STARTLOG(g_usuario_cod CLIPPED||".DISR221.log")
      CALL fn_reverso()
   END IF
END MAIN
...
FUNCTION fn_reverso()
   ...
   DISPLAY ""
   DISPLAY ("##############################################################")
   DISPLAY "Inicio del Reverso de Liquidación de Aportaciones Subsecuentes Sin Conciliar "
   ...
   IF r_sql_code = 0 THEN
      ...
      DISPLAY "Reverso de Liquidación de Aportaciones Subsecuentes Sin Conciliar Finalizado"
      DISPLAY ""
      DISPLAY ("##############################################################")
   ELSE
      DISPLAY ("##############################################################")
