################################################################################
#Version                    => 1.0.0                                           #
#Fecha ultima modificacion  => 30/01/2013                                      #
################################################################################
################################################################################
...
#                     Social - Pago Real pero con el número de crédito correcto#
#Fecha inicio      => 30/01/2013                                               #
################################################################################
DATABASE
     safre_viv
...
DEFINE v_arr_tipo4 RECORD  --Record para tipo de registro cuatro
   ...
   LET v_folio_liquida = ARG_VAL(1) -- Valor del folio de liquidación
   ...
   LET v_nom_archivo = "/esp_pag_hs_" --nombre de archivo
   
   LET v_ddmmaaaa = TODAY USING "ddmmyyyy"  --Fecha del archivo sin separadores
   LET v_busca_nom_archivo = "esp_pag_hs_" || v_ddmmaaaa  --Concatena nombre a buscar
   ...
   DECLARE cur_consulta_folio_interface_hs_esp CURSOR FOR
   
      SELECT H.nss,
             H.num_credito_cor, --Se trae el número de crédito correcto 
             H.periodo_pago, 
             H.f_pago,H.nrp,
             H.imp_ap_pat,
             H.imp_am_cre,
             H.folio_sua
        FROM dis_numero_credito_hs H
       WHERE H.folio_liquida      = v_folio_liquida

   FOREACH cur_consulta_folio_interface_hs_esp INTO 
                                                v_arr_tipo2.nss,
   ...
   DISPLAY "Se ha generado la interface Especial de Pago Real HS correctamente: ",v_nom_archivo
   ...
   CALL fn_genera_copia_interface_real_esp(v_ruta_nomarch,v_ruta_envio_dis)
...
FUNCTION fn_genera_copia_interface_real_esp(p_archivo_envio,p_ruta_destino)
    ...
    LET v_archivo_copia = "esp_pag_hs"
    ...
    --DISPLAY "v_comando_dos ",v_comando_dos 
    RUN v_comando_dos
    DISPLAY "     Se ha realizado la copia de la interface Especial de Pago Real HS: ",v_archivo_copia
