################################################################################
#Version                    => 1.0.0                                           #
#Fecha ultima modificacion  => 29/09/2018                                      #
################################################################################
################################################################################
...
#Modulo            => DIS                                                      #
#Programa          => DISL68                                                   #
#Objetivo          => Programa lanzador para la integración del archivo de     #
...
DEFINE 
  ...
  -- se abre la ventana para elegir los archivos a integrar
  OPEN WINDOW w_inte_acred WITH FORM "DISL681"
    DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION CANCEL
         CALL fn_mensaje("Aviso",
                         "Se ha cancelado la integración.",
                         "stop")
         EXIT DIALOG
      ...
      ON ACTION ACCEPT
         ...
         FOR l_i_iter = 1 TO l_i_num_arch
             ...
             LET l_s_qryTxt = "\n SELECT sum(aportacion), sum(amortizacion), count(*)",
                              "\n FROM   safre_tmp:tmp_dis_can_parc_ava0"
                             
             PREPARE pre_sql_total_registros_archivo FROM l_s_qryTxt
             EXECUTE pre_sql_total_registros_archivo INTO l_i_tot_apo, l_i_tot_amo, l_i_tot_reg
             ...
             LET l_arr_integrados[l_i_iter].tot_apo     = l_i_tot_apo/100
             LET l_arr_integrados[l_i_iter].tot_amo     = l_i_tot_amo/100
      ...
      ON ACTION integrar
         ...
         IF r_bandera = 0 THEN
            CALL fn_actualiza_opera_ini(v_max_pid,g_proceso_cod,g_opera_cod,g_folio,"DISE29",l_v_arch_proceso,v_usuario)
            RETURNING r_b_valida
                                                
            IF r_b_valida = 0 THEN --Actualiza Operación Inicial
               LET l_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,
                               "/DISE29.42r ",v_usuario," ",v_max_pid," ",
               ...
               LET v_s_mensaje = "Se ha enviado la integración de la Cancelación Parcial de Avances de Pago con PID: ",
                                 v_max_pid CLIPPED,
