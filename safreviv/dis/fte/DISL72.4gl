################################################################################
#Version                    => 1.0.0                                           #
#Fecha ultima modificacion  => 22/04/2019                                      #
################################################################################
################################################################################
...
DEFINE 
  ...
  OPEN WINDOW w_inte_acred WITH FORM "DISL721"
    DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION ACCEPT
         ...
         FOR l_i_iter = 1 TO l_i_num_arch
             ...
             LET l_s_qryTxt = "\n SELECT count(*)",
                              "\n FROM   safre_tmp:tmp_dis_act_ind_ald0"
      ...
      ON ACTION integrar
         ...
         IF r_bandera = 0 THEN
            CALL fn_actualiza_opera_ini(v_max_pid,g_proceso_cod,g_opera_cod,g_folio,"DISE30",l_v_arch_proceso,v_usuario)
            RETURNING r_b_valida
                                                
            IF r_b_valida = 0 THEN --Actualiza Operación Inicial
               LET l_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,
                               "/DISE30.42r ",v_usuario," ",v_max_pid," ",
               ...
               LET v_s_mensaje = "Se ha enviado la integración de la Actualización del Indicador de Adelanto de Liquidación del Proceso de Dispersión de Pagos con PID: ",
                                 v_max_pid CLIPPED,
