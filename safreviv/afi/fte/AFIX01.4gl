...
MAIN
   ...
   CALL STARTLOG ("AFIX01.log")

   IF g_archivo IS NULL THEN
      DISPLAY "EJECUTAR EL PROGRAMA: fglrun ASIX01 nombre_archivo"
   ELSE
      CALL fn_crea_tabla()
...
FUNCTION fn_genera_device()
   ...
   LET v_str_archivo = g_archivo_ruta CLIPPED||"/"||g_usuario CLIPPED||
                       "."||g_archivo_nombre CLIPPED||".device"
   LET v_canal = base.Channel.create()
   CALL v_canal.openFile(v_str_archivo,"w")
...
DEFINE p_modulo_cod     CHAR(3),
   ...
   ELSE
      LET v_comando = "nohup fglrun AFIX02 ", g_usuario, " ",
                      p_modulo_cod, " ",
                      g_archivo_nombre ," 1>AFIX02.sal 2>AFIX02.log  &"
      DISPLAY ""
      DISPLAY "SE EJECUTA POR NOHUP LA CARGA DEL ARCHIVO."
      DISPLAY "VERFICAR LOS ARCHIVOS DE SALIDA AFIX02.sal y AFIX02.log"
      RUN v_comando
   END IF
END FUNCTION
