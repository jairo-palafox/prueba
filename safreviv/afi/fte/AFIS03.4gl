...
MAIN
    ...
    LET v_nom_archivo = v_ruta_envio CLIPPED ,"/His_cons_rojas",TODAY USING "DDMMYYYY",".txt"    
      CALL fn_genera_reporte()
      CALL fn_mensaje("Mensaje","Reporte y Archivo creados correctamente, puede visualizar el archivo de texto en \n"||v_nom_archivo,"information")
      EXIT INPUT
   ON ACTION CANCEL
      EXIT INPUT 
   END INPUT 
...
DISPLAY v_ruta_reporte
   ...
   IF NOT v_excepcion THEN
      ...
      LET v_query_reporte = 'SELECT  a.usuario,
                                     ...
                                     a.h_consulta,
                                     "NO"
                              FROM   afi_log_consulta a,
                                     afi_derechohabiente b,
                                     seg_usuario c,
                                     afi_nss_rojo r,
                                     cat_categoria_roja cat
                             WHERE   a.usuario = c.usuario_cod
                               AND   r.nss = a.nss
                               AND   r.categoria = cat.categoria
                               AND   a.nss = b.nss'
          ...
          SELECT COUNT (*)
           ...
           WHERE f_consulta <= f_final
            ...
            IF v_cuenta <> 0 THEN 
              ...
              UNLOAD TO v_nom_archivo v_consulta_reporte
           ELSE 
