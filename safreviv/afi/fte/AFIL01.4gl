...
#                del detalle de movimientos afiliatorios                                #
...
MAIN
   ...
   LET g_proceso_cod = g_proceso_cod_afi_movimientos_imss -- movimientos afiliatorios
   LET g_opera_cod   = g_opera_cod_afi_movimientos_integracion_imss -- integracion
...
#fdid---------------------------------------------
   ...
   LET l_s_qryTxt = "SELECT nombre_archivo \n",
                    ...
                    " WHERE proceso_cod = ", g_proceso_cod,"\n",
                    "   AND opera_cod   = ", g_opera_cod_afi_movimientos_carga_imss, "\n",--,g_opera_cod que tiene la carga
   ...
   OPEN WINDOW w_inte_devol WITH FORM "AFIL011"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION ACCEPT
         ...
         FOR l_i_iter = 1 TO l_i_num_arch
            ...
            SELECT COUNT(*)
            INTO l_arr_integrados[l_i_iter].tot_riss
      ...
      ON ACTION integrar
         ...
         CALL fn_ventana_confirma("Afiliación",
              "¿Desea ejecutar el proceso de Integración de Movimientos Afiliatorios?",
...
Ivan Vega    23julio2013              Si el proceso 703-Generacion de archivo para precalificacion esta en
                                      ejecucion, entonces no se puede ejecutar la integracion de movimientos
                                      afiliatorios

Registro de modificaciones:
Autor                  Fecha            Descrip. cambio
Mauro Muñiz Caballero  10junio2014    Se da de alta el tipo de movimiento 21 "Inscripción al RISS"
======================================================================
}
...
FUNCTION fn_ret_ejecuta_integracion_afiliacion(p_usuario_cod, v_archivo)
   ...
   -- si se está ejecutando el proceso, entonces no se puede integrar AFI
   ...
   ELSE
      -- se verifica que se pueda ejecutar el proceso
      ...
      IF ( v_i_resultado = 0 ) THEN
         ...
         IF ( r_resultado_opera = 0 ) THEN
            LET v_s_comando = " nohup time fglrun ",g_reg_modulo.ruta_exp CLIPPED,"/AFIP01 ",
            ...
            CALL fn_mensaje("Atención",
                            "Se ha enviado la integración.\nPuede revisar el resultado en el monitor de ejecución de procesos",
                            "information")
         ELSE
      ...
      ELSE
         -- se indica la razon por la que no se puede iniciar el proceso
         CALL fn_recupera_inconsis_opera(v_i_resultado) RETURNING v_mensaje
      ...
      END IF
   END IF -- valida que no esté ejecutando Precalificación
