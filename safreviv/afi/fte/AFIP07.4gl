...
#                de recurrente de teléfono de derechohabientes                          #
...
MAIN
   ...
   DEFINE v_nss_error            VARCHAR(11) -- nss que pudo haber generado un error
   ...
   DEFINE v_r_afi_telefono_rch RECORD LIKE afi_telefono_rch.* -- registro de la tabla de rechazos
   ...
   LET v_s_sql = "EXECUTE FUNCTION fn_afi_integra_telefono(?,?,?,?,?)"
   ...
   LET p_mensaje = "ID Proceso   : ", g_pid, "\n", 
                   "Proceso      : AFILIACION - ALTA DE TELEFONO\n",
   ...
   IF ( v_i_resultado = 0 )THEN
      ...
      IF ( fgl_report_loadCurrentSettings("../../afi/bin/AFIP07.4rp") ) THEN  -- if  the file loaded OK
         -- sin preview
         ...
         INITIALIZE v_r_afi_telefono_rch.* TO NULL
         LET v_r_afi_telefono_rch.cod_rechazo = -99
         ...
         OUTPUT TO REPORT rpt_cifras_control(p_folio          ,
                                             ...
                                             v_regs_aceptados ,
                                             v_regs_rechazados,
                                             v_r_afi_telefono_rch.*)

          -- se leen los registros rechazados
          DECLARE cur_rechazos CURSOR FOR
          SELECT *
            FROM afi_telefono_rch
           WHERE folio_lote = p_folio

          FOREACH cur_rechazos INTO v_r_afi_telefono_rch.*
             OUTPUT TO REPORT rpt_cifras_control(p_folio          ,
                                                 p_usuario_cod    ,
                                                 v_regs_totales   ,
                                                 v_regs_aceptados ,
         ...
         -- =====================================================================================
      ELSE
         DISPLAY "No se puede leer la plantilla del reporte.\nNo se generó el reporte."
      END IF
   ELSE
      -- se complementa el mensaje
   ...
   LET p_titulo = "Finalización de operación - ALTA DE TELÉFONO - INTEGRACIÓN"
...
============================================================================}
   ...
   DEFINE p_regs_rechazados            INTEGER -- numero de movimientos rechazados
   DEFINE p_r_afi_telefono_rch         RECORD LIKE afi_telefono_rch.* -- registro de la tabla de rechazos
   ...
   FORMAT
      ...
      BEFORE GROUP OF p_r_afi_telefono_rch.cod_rechazo
         -- se obtiene la descripcion del rechazo
         SELECT des_rechazo
           INTO v_desc_rechazo
           FROM afi_cat_rch
          WHERE cod_rechazo = p_r_afi_telefono_rch.cod_rechazo

         -- se envia la descripcion del tipo de rechazo
         PRINTX p_r_afi_telefono_rch.cod_rechazo, v_desc_rechazo

      ON EVERY ROW
         PRINTX p_r_afi_telefono_rch.*
