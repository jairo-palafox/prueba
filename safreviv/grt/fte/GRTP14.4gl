...
#Modulo            =>GRT                                                #
#Programa          =>GRTP14                                             #
#Objetivo          =>Programa que inserta el registro en la tabla de    #
#                    control correspondiente a Devolución de Solic.     #
#                    para el módulo de Uso de Anualidad 43 bis          #
...
DEFINE p_v_usuario      LIKE seg_usuario.usuario, -- nombre del usuario
       ...
       p_i_lote         SMALLINT, -- lote del archivo en proceso
       p_dt_f_lote      DATE -- fecha del lote
...
MAIN
   ...
   LET p_i_lote         = ARG_VAL(7)
   LET p_dt_f_lote      = ARG_VAL(8)  

   -- se crea el archivo log
   CALL STARTLOG(p_v_usuario CLIPPED|| ".GRTP14.log")

   DISPLAY "=INICIA GRTP14="
   ...
   IF v_i_estado_cod = 4 THEN
      --DISPLAY "ESTADO VALIDO EN GLO CTR ARCHIVO"
      LET v_i_estado = 10
   ELSE
   ...
   -- ===========================================================================
   -- ===========================================================================
   -- NO SE MARCA EL INICIO DE LA OPERACION PORQUE ESO LO HIZO LA CARGA, SOLO
   -- SE ACTUALIZARA LA HORA Y FECHA DE FIN EN CUANTO EL ANALISIS DE LA VALIDACION
   -- CULMINE CON LA EJECUCION DEL SIGUIENTE STORED PROCEDURE

   -- se crea la sentencia que ejecuta el procedure que realiza la actualizacion
   -- de la tabla de control de archivos
   LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_uso_insrt_crectrarch_devoluc(?,?,?)"
    
   PREPARE prp_insrt_cre_hist_deud FROM v_s_qryTxt
   EXECUTE prp_insrt_cre_hist_deud USING p_v_arch_proceso, v_i_estado, p_v_usuario
