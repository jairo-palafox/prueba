...
#Modifico            =>Mauricio Sanchez, EFP                         #
#Fecha inicio        =>16 Febrero 2012                               #
#Fecha modificacion  =>2 de Mayo 2012                                #
######################################################################
...
DEFINE g_pid               LIKE bat_ctr_proceso.pid, --  ID del proceso
       ...
       g_tpo_transferencia LIKE cre_uso_garantia.tpo_transferencia --tipo de transferencia 
END GLOBALS
...
DEFINE p_tipo_ejecucion SMALLINT, -- forma como ejecutara el programa
   ...
   LET g_tpo_transferencia = "16" --Solicitud de saldo en garantia
   ...
   CALL fn_consulta_historico_grt() 
END MAIN

## ventana que muestra las opciones para consultas de acreditados ##
FUNCTION fn_consulta_historico_grt() 
   DEFINE v_op_consulta CHAR(4)

   OPEN WINDOW w_op_consulta WITH FORM "GRTC011"
      INPUT v_op_consulta FROM opciones ATTRIBUTE (UNBUFFERED)
         ON ACTION ACCEPT
            ...
            IF v_op_consulta = "opt2" THEN         
              --CALL fn_mensaje("Aviso","Llamado a la función: fn_consulta_estado","Information")
              CALL fn_consulta_estado()
            END IF
            IF v_op_consulta = "opt3" THEN         
              --CALL fn_mensaje("Aviso","Llamado a la función: fn_consulta_historica","Information")
         ...
         IF v_cta_nombre > 1 THEN 
            ...
            ELSE     
               SELECT COUNT(*) 
                ...
                OUTER cre_uso_garantia g
                WHERE a.id_derechohabiente = v_id_derechohabiente
                  AND g.id_derechohabiente = a.id_derechohabiente
...
DEFINE v_s_condicion  STRING,
  ...
  { IF estado IS NULL THEN 
      LET v_s_condicion = v_s_condicion, " AND estado IN (10"
   END IF 
      ...
      BEFORE CONSTRUCT 
          ...
          CALL cb.clear()
              LET v_sqlqry = " SELECT diagnostico, edo_procesar ",
                             ...
                             "  WHERE edo_procesar not in('10','05','30','55','60','120','210') ",
...
FUNCTION fn_consulta_archivo()
   ...
   OPEN WINDOW w_op_archivo WITH FORM "GRTC019"
   ...
   INPUT v_id_cre_ctr_archivo FROM cmb_archivo ATTRIBUTE (UNBUFFERED) 
      ...
      BEFORE INPUT
         LET v_id_proceso = g_id_proceso_grt --Solicitud de saldo en garantia 43 bis
...
FUNCTION fn_despliega_archivo(p_id_cre_ctr_archivo, p_nom_archivo)
   ...
   OPEN WINDOW w_des_archivo WITH FORM "GRTC015"
      ...
      DISPLAY ARRAY v_ar_archivo TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("GRTC104.4rp") THEN 
               ...
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-GRTC014-","00000","-","00000","-","00000" 
...
FUNCTION fn_obt_derechohabiente(p_s_condicion)
   ...
   OPEN WINDOW w_list_derechohabiente WITH FORM "GRTC018" 
...
FUNCTION fn_despliega_derechohab(p_id_derechohabiente)
  ...
  OPEN WINDOW w_des_derechohab WITH FORM "GRTC013" 
     LET  w = ui.Window.getCurrent()
     LET  f_w = w.getForm()  
     ...
     DIALOG ATTRIBUTES (FIELD ORDER FORM, UNBUFFERED)     
        DISPLAY ARRAY v_ar_his_transferencia TO tabla_trans.*
           BEFORE DISPLAY               
              LET v_lsqry = " SELECT f_proceso,estado,edo_procesar,diagnostico\n",
                            ...
                            "          FROM cre_acreditado\n",
                            "         WHERE id_derechohabiente = ",p_id_derechohabiente,")\n",
                            "    AND tpo_transferencia = '",g_tpo_transferencia CLIPPED,"'\n",
                            "  GROUP BY 1,2,3,4"
        ...
        ON ACTION reporte
           ...
           IF fgl_report_loadCurrentSettings("GRTC101.4rp") THEN 
              ...
              LET v_nom_reporte = p_usuario_cod CLIPPED || "-GRTC011-","00000","-","00000","-","00000" 
              CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
...
FUNCTION fn_despliega_estado(p_s_condicion)
   ...
   OPEN WINDOW w_des_estado WITH FORM "GRTC015"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm() 
      LET v_sqlqry = " SELECT estado,edo_procesar",
                     "   FROM cre_acreditado", 
      ...
      FOREACH cur_edos INTO v_estado, v_edo_procesar
         ...
         ELSE 
            ...
            LET v_rec_estado.total_cuentas = v_tot_ctas
            ...
            LET v_rec_estado.total_cuentas = v_rec_estado.total_cuentas + v_tot_ctas
            LET v_rec_estado.estado = v_estado_aux
            LET v_rec_estado.edo_procesar = v_edo_procesar_aux
      ...
      DISPLAY ARRAY v_ar_estado TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("GRTC102.4rp") THEN 
               ...
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-GRTC012-","00000","-","00000","-","00000" 
               CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
...
FUNCTION fn_despliega_historica(p_s_condicion)
   ...
   OPEN WINDOW w_des_historica WITH FORM "GRTC017"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm() 
      ...
      DISPLAY ARRAY v_ar_historica TO tabla_historica.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("GRTC103.4rp") THEN 
               ...
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-GRTC013-","00000","-","00000","-","00000" 
               CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
