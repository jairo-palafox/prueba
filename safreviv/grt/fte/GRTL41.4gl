...
#                    conciliación de DSE para el módulo de         #
#                    Créditos en Garantia 43 bis                   #
...
MAIN
   ...
   DEFINE p_v_nom_prog        VARCHAR(30), -- nombre del programa
          ...
          v_cmb_operacion     CHAR(02),
          v_i_cuenta_regis    INTEGER, -- combos
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".GRTL41.log")
   ...
   LET v_i_proceso_cod = g_proc_cod_grt_concilia_dse -- conciliación dev sdo exc
   ...
   OPEN WINDOW w_genera_conciliacion_dev WITH FORM "GRTL411"
   INPUT v_cmb_operacion, v_d_folio WITHOUT DEFAULTS FROM cmb_opera, folio ATTRIBUTES(UNBUFFERED)
      ...
      ON ACTION ACCEPT
         ...
         -- se valida si fue posible inicializar el proceso
         IF r_b_valida <> 0 THEN
            -- en caso de error se muestra un mensaje a usuario y no continua
            CALL fn_muestra_inc_operacion(r_b_valida)
         ...
         CASE v_cmb_operacion
            WHEN "01" -- Rechazo de saldos excedentes
               LET v_s_comando = v_s_comando,"GRTP26 "

            WHEN "02" -- Confirmacion de devolucion de saldos
               LET v_s_comando = v_s_comando,"GRTP27 "
...
DEFINE v_v_nombre_combo VARCHAR(20),   -- nombre del combobox
   ...
   # Opciones del combo
   CALL cb.addItem("01","19 - Rechazo de saldos")
   CALL cb.addItem("02","19 - Confirmación de saldos")
END FUNCTION
