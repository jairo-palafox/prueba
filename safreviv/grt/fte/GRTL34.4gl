...
#                     archivo de entrada Rechazo DSE para el módulo #
...
# Objetivo: Programa que permite la seleccion del archivo Rechazo Devolución
...
MAIN
   ...
   DEFINE v_r_dse_ctr_arch          RECORD LIKE dse_ctr_archivo.* -- registro del control de archivo
   ...
   DEFINE v_c_tpo_transf            LIKE dse_ctr_archivo.tpo_transferencia -- tipo de transferencia
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".GRTL34.log")
   ...
   LET v_i_opera_cod     = 2 -- integra rechazos devolución sdo exc grt
   LET v_i_opera_cod_ant = 1 -- Valida Rechazo Saldos
   LET v_c_tpo_transf    = "96" -- Créditos en Garantía 43 bis
   LET v_d_folio         = 0
   ...
   OPEN WINDOW w_integra_arch_rechdse WITH FORM "GRTL321"
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         ON ACTION ACCEPT
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               -- se asigna el nombre del archivo en la variable parámetro 
               ...
               LET v_s_qryTxt = " SELECT *\n",
                ...
                IF v_r_dse_ctr_arch.nom_archivo IS NULL THEN
                   CALL fn_mensaje("Integración Rechazo Saldos","El archivo está marcado como Rechazado, no se puede continuar\ncon la integración del archivo","stop")
                   CONTINUE DIALOG
                END IF

                -- se asigna la información del archivo integrado
            ...
            -- si se muestra la información del archivo a integrar se habilita el botón de integrar
         ...
         ON ACTION integrar
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               IF r_b_valida = 0 THEN
                  ...
                  LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/GRTE13 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
   ...
   CLOSE WINDOW w_integra_arch_rechdse
...
FUNCTION f_valida_arch_integra()
   ...
   SELECT COUNT(*)
     INTO v_i_cont_regs
     FROM safre_tmp:tmp_dse_rech_det_grt c
    WHERE c.nss NOT IN(
       SELECT t.nss
         FROM safre_tmp:tmp_ctr_devolucion_grt t
         INNER JOIN safre_tmp:tmp_dse_rech_det_grt s
         ON t.nss = s.nss)
