...
MAIN
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".GRTC104.log")
...
FUNCTION fn_consulta_archivos()
   ...
   OPEN WINDOW w_arch WITH FORM "GRTC1041"
...
FUNCTION fn_consulta_detalle(p_nom_arch)
   ...
   END RECORD 
   DEFINE v_id_grt_ctr_archivo DECIMAL(9,0)

   SELECT id_grt_ctr_Archivo 
     INTO v_id_grt_ctr_archivo
     FROM grt_ctr_archivo 
   ...
   OPEN WINDOW w_det WITH FORM "GRTC1042"
      DISPLAY BY NAME p_nom_arch
      DISPLAY BY NAME v_rec_det_arch.*
      MENU 
         ON ACTION detalle 
           CALL fn_det_arch(v_id_grt_ctr_archivo)
         ON ACTION CANCEL 
            EXIT MENU 
      END MENU 
   CLOSE WINDOW w_det
...
FUNCTION fn_det_arch(p_id_arch)
   ...
   LET v_qry = "SELECT c.estado,                                  
                  ...
                  FROM grt_tramite a,                             
                       grt_detalle b,                             
                       grt_ctr_archivo c                          
                 WHERE b.id_grt_ctr_archivo = ",p_id_arch,"                 
                   AND c.id_grt_ctr_archivo = b.id_grt_ctr_archivo
   ...
   OPEN WINDOW w_lalito WITH FORM "GRTC1043"
