...
#                    Confirmación DSE para el módulo de Créditos   #
#                    en Garantia 43 bis                            #
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".GRTE14.log")

   DISPLAY "=INICIA GRTE14="
   ...
   LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_dse_integra_confdse_grt(?,?,?,?)"
   ...
   DISPLAY "TOTAL RECHAZADOS: ",v_r_dse_ctr_arch.tot_rechazados
{
   ...
   LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/GRTP27 ",
                                           p_v_usuario, " ",
                                           p_d_pid, " ",
                                           p_i_proceso_cod, " ",
                                           p_i_opera_cod, " ",
   ...
   RUN v_s_comando
{
   ...
   LET v_v_nom_reporte = p_v_usuario CLIPPED, "-GRTL36-", p_d_pid USING "&&&&&", "-", p_i_proceso_cod USING "&&&&&", "-", p_i_opera_cod USING "&&&&&"
   ...
   IF NOT r_b_existe_err THEN
      ...
      LET v_s_titulo_correo = "Proceso: RECEPCIÓN CONFIRMACIÓN SALDOS EXC GRT"
      ...
      LET v_s_mens_correo =  "ID Proceso   : ",p_d_pid,"\n",
                             "Proceso      : RECEPCIÓN CONFIRMACIÓN SALDOS EXC GRT\n",
...
FUNCTION f_genera_rpt_IntegraConfDSE(p_c_ruta_listados, p_v_nom_reporte)
   ...
   IF fgl_report_loadCurrentSettings("GRTE141.4rp") THEN
...
FUNCTION fn_crea_tmp_conf_devol_saldo()
   ...
   DROP TABLE tmp_deudor_conf_devol_saldo_grt

   -- al encontrar un error detiene el programa
   WHENEVER ERROR STOP

   CREATE TABLE tmp_deudor_conf_devol_saldo_grt(id_dse_devolucion  DECIMAL(9,0),
