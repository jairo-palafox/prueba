...
#                     de Solicitudes para el módulo de Uso de Garantía   #
...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".GRTE10.log")

   DISPLAY "=INICIA GRTE10="
   ...
   LET v_s_qryTxt = " SELECT f_presentacion, cons_lote\n",
                    "   FROM safre_tmp:tmp_devoluc_enc_uso"
   ...
   DISPLAY " INTEGRA DEVOLUCIONES"
    -- se crea la sentencia que ejecuta el procedure que realiza la integracion
    -- de devoluciones
    LET v_s_qryTxt = "EXECUTE FUNCTION fn_uso_integra_devoluc(?,?,?,?)"
   ...
   LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/GRTP15 ",
                                           p_v_usuario, " ",
                                           p_d_pid, " ",
                                           p_i_proceso_cod, " ",
                                           p_i_opera_cod, " ",
...
FUNCTION fn_crea_tmp_devoluc()
   ...
   DROP TABLE tmp_deudor_devoluc_grt

   WHENEVER ERROR STOP

   CREATE TABLE tmp_deudor_devoluc_grt(id_cre_acreditado DECIMAL(9,0),
