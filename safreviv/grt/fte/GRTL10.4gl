...
#                    para el módulo de Solic. de Saldo en Garantía #
...
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".GRTL10.log")
   ...
   LET v_d_folio = 0
   LET v_c_programa_cod = "GRTL10" -- nombre del programa
         ...
         ON ACTION integrar
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               IF r_b_valida = 0 THEN
                  ...
                  LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/GRTE05 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
...
FUNCTION f_valida_arch_integra()
   DEFINE v_si_tpo_originac LIKE cre_acreditado.tpo_originacion, -- tipo de originación
          ...
          v_c_tpo_transf    CHAR(2), -- tipo de transferencia
          v_c_per_pago      CHAR(6), -- periodo de pago
   ...
   LET v_i_status = 1 -- se asume que el registro será valido para Integrar
   ...
   FOREACH cur_tmp_nss INTO v_c_nss_tmp, v_c_tpo_transf, v_c_per_pago
      --DISPLAY "nss: ",v_c_nss_tmp, v_c_tpo_transf
      ...
      --DISPLAY "id: ",v_d_id_derechohab

      -- se inicializa el contador de registros
      LET v_i_cont_regs = 0

      -- verifica si el registro corresponde a Solicitud de Saldo en Garantía
      IF v_c_tpo_transf = "16" THEN
         ...
         END FOREACH
      ELSE
         -- verifica si el registro corresponde a Uso de Garantía
         IF v_c_tpo_transf = "18" THEN
            -- se busca si el id derechohabiente está en estatus requerido
            LET v_s_qryTxt = " SELECT estado\n",
