...
MAIN
   ...
   LET v_i_proceso_cod  = g_proc_cod_grt_uso_liquida -- liquidación uso garantía 43bis
   ...
   IF v_cuenta_accion = 0 THEN
      LET v_s_mensaje = "No es posible realizar la preliquidación, ya que no hay precio de aiv"
   ...
   IF v_cta_reg = 0 THEN
      LET v_s_mensaje = "No es posible realizar la preliquidación, ya que no hay registros para preliquidar"
      CALL fn_mensaje("Aviso",v_s_mensaje,"stop")
      EXIT PROGRAM
   END IF    
   ...
   IF r_b_valida = 0 THEN
      -- se crea el comando que ejecuta el módulo que reliza la inserción del registro de control
...
#Objetivo: Función que llena el árbol de los montos agrupados por subcuenta y movimiento
FUNCTION fn_llena_arbol_montos_pre(p_folio, v_tabla)
   ...
   -- se asigna el primer registro del árbol
   LET arr_arbol_pre[1].subcuenta      = ""
   LET arr_arbol_pre[1].subcuenta_desc = "IMPORTE USO DE GARANTÍA"
   ...
   -- se asigna el segundo registro del árbol
   ...
   -- se obtiene la subcuenta y la descripción
...
#Objetivo: Función que obtiene la descripción del proceso y la operación
FUNCTION fn_despliega_desc_pre(p_proceso_cod, p_opera_cod)
   ...
   -- se busca la descripción del proceso
   SELECT proceso_desc
     INTO v_proceso_desc
     FROM cat_proceso
    WHERE proceso_cod = p_proceso_cod

   -- se busca la descripción de la operación
   SELECT opera_desc
     INTO v_opera_desc
     FROM cat_operacion
    WHERE proceso_cod = p_proceso_cod
