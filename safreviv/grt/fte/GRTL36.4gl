...
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".GRTL36.log")
   ...
   LET v_i_opera_cod = 2 -- integra confirmación devolución sdo exc grt
   LET v_i_opera_cod_ant = 1 -- Valida Rechazo Saldos
   LET v_c_tpo_transf = '99' -- Confirmación DSE GRT
   LET v_d_folio = 0
   LET v_c_programa_cod = "GRTL36"
   ...
   OPEN WINDOW w_integra_arch_confdse WITH FORM "GRTL321"
         ...
         ON ACTION integrar
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               IF r_b_valida = 0 THEN
                  ...
                  LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/GRTE14 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
...
FUNCTION f_valida_arch_integra()
   ...
   SELECT COUNT(*)
     INTO v_i_cont_regs
     FROM safre_tmp:tmp_dse_confirm_det_grt c
    WHERE c.nss NOT IN(
       SELECT t.nss
         FROM safre_tmp:tmp_ctr_devolucion_grt t
         INNER JOIN safre_tmp:tmp_dse_confirm_det_grt s
         ON t.nss = s.nss)
