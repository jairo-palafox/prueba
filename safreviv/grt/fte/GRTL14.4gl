...
#                    proceso a reversar: Archivos Entrada o        #
#                    Archivos Salida para el módulo de Solicitud   #
#                    de Saldo en Garantía 43 bis                   #
...
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".GRTL14.log")
   ...
   LET m_i_proceso_cod = g_proc_cod_grt_reverso -- reverso solicitud saldo garantía 43bis
   LET m_i_opera_cod = 1 -- reverso solicitud saldo garantía 43bis
   LET m_c_programa_cod = "GRTL14"
   LET m_i_tpo_originacion = 2 -- Créditos en Garantía 43 bis
   LET v_d_pid = 0
   LET v_d_folio = 0
   ...
   OPEN WINDOW w_reverso WITH FORM "GRTL141"
   INPUT v_c_op_arch_ent, v_c_op_arch_sal WITHOUT DEFAULTS
    FROM gentrada, gsalida ATTRIBUTES(UNBUFFERED)
      ON CHANGE gentrada
      ...
      ON ACTION ACCEPT
         ...
         IF v_c_op_arch_ent IS NULL AND v_c_op_arch_sal IS NULL THEN
...
FUNCTION fn_reversa_arch_entrada(p_c_op_arch_ent)
   DEFINE p_c_op_arch_ent       VARCHAR(5), -- contiene la opcion a reversar de Archivos entrada
          ...
          v_fecha_lote          DATE, --LIKE cre_ctr_archivo.f_lote, -- feche de lote
          v_mes_fecha_aux       VARCHAR(2), --LIKE cre_ctr_archivo.f_lote, -- feche de lote
   ...
   OPEN WINDOW w_arch_entrada WITH FORM "GRTL142"
   ...
   IF v_i_indice_arr = 1 THEN
      ...
      ELSE
         ...
         LET v_s_qryTxt = " SELECT COUNT(*)\n",
                          ...
                          "  WHERE folio_liquida = ",v_d_folio_arch
      ...
      ON ACTION ACCEPT
         ...
         CASE p_c_op_arch_ent
            WHEN "opt1" -- Recurrente
               CALL fn_valida_recurrente(v_id_cre_ctr_archivo, v_d_folio_arch) RETURNING v_b_continua
         ...
         IF r_b_valida = 0 THEN
            ...
            IF r_b_valida = 0 THEN
               ...
               LET v_s_comando = " nohup time fglrun ",m_c_ruta_bin CLIPPED,"/GRTR01 ",
                                                       p_v_usuario, " ",
                                                       v_d_pid, " ",
                                                       m_i_proceso_cod, " ",
                                                       m_i_opera_cod, " ",
...
FUNCTION fn_valida_ext_archivo(p_c_op_arch_ent, p_c_nom_archivo)
   ...
   CASE p_c_op_arch_ent
      WHEN "opt1" -- Recurrente
         LET v_i_proceso_cod = g_proc_cod_grt_recurrente -- recepción recurrente crédito gtía 43bis
      ...
      WHEN "opt2" -- Rechazo de Saldos
         LET v_i_proceso_cod = g_proc_cod_grt_rech_saldos -- recepción rechazo de saldos 43bis
      ...
      WHEN "opt4" -- Solicitudes Devueltas
         LET v_i_proceso_cod = g_proc_cod_grt_devol_solic -- recepción devolucón solicitudes 43bis
      ...
      WHEN "opt5" -- Solicitudes no Atendidas
         LET v_i_proceso_cod = g_proc_cod_grt_no_atendidas	-- recepción solicitudes no atendidas 43bis
...
FUNCTION fn_reversa_arch_salida()
   ...
   OPEN WINDOW w_arch_salida WITH FORM "GRTL143"
      -- se deplega el nombre del proceso a reversar en el grupo
      LET v_ui_ventana = ui.Window.getCurrent()
      LET v_ui_forma = v_ui_ventana.getForm()
      CALL v_ui_forma.setElementText("grevent",v_nom_grupo)
            ...
            ON ACTION ACCEPT
               ...
               IF r_b_valida = 0 THEN
                  ...
                  IF r_b_valida = 0 THEN
                     ...
                     LET v_s_comando = " nohup time fglrun ",m_c_ruta_bin CLIPPED,"/GRTR02 ",
                                                             p_v_usuario, " ",
                                                             v_d_pid, " ",
                                                             m_i_proceso_cod, " ",
                                                             m_i_opera_cod, " ",
...
FUNCTION fn_valida_recurrente(p_id_cre_ctr_archivo, p_d_folio_arch)
   DEFINE p_id_cre_ctr_archivo  LIKE cre_ctr_archivo.id_cre_ctr_archivo,
          p_d_folio_arch        LIKE cre_ctr_archivo.folio_archivo, -- folio del archivo
          v_i_proceso_cod       LIKE cat_proceso.proceso_cod, -- código del proceso
          v_i_opera_cod         LIKE cat_operacion.opera_cod, -- código de operación
          v_si_status           LIKE glo_folio.status, -- estatus del folio
   ...
   LET v_i_proceso_cod = g_proc_cod_grt_liquidacion
   LET v_i_opera_cod = 1 -- preliquidación fortalecmiento crédito GTÍA
   ...
   -- se validca si existe información en la tabla maestro
   ...
   -- se crea la sentencia sql que obtiene el estado del folio
   LET v_s_qryTxt = " SELECT status\n",
                    "   FROM glo_folio\n",
                    "  WHERE folio = ",p_d_folio_arch,"\n",
                    "    AND opera_cod = ",v_i_opera_cod,"\n",
                    "    AND proceso_cod = ",v_i_proceso_cod

   PREPARE prp_status_folio FROM v_s_qryTxt
   EXECUTE prp_status_folio INTO v_si_status

   IF v_si_status = 2 THEN
      CALL fn_mensaje("Reverso","No es posible reversar el proceso, debido a que el archivo ya fue Liquidado","stop")
