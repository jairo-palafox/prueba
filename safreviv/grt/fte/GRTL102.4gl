...
#Modulo            => GRT                                                   #
#Programa          => GRTL102                                               #
#Objetivo          => Programa que ejecuta la integración del archivo       #
#                     Recepción entidades financieras 43 Bis                #
#Autor             => Héctor Fabián Jiménez Lara                            #
#Fecha inicio      => 29 de Septiembre del 2015                             #
#############################################################################
IMPORT os
...
MAIN
   ...
   END RECORD
   DEFINE v_r_grt_ctr_arch    RECORD LIKE grt_ctr_archivo.*   -- registro del control de archivo
   ...
   DEFINE v_drag_index        INTEGER                         -- índice del drag
   DEFINE v_drop_index        INTEGER                         -- índice del drop
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".GRTL102.log")
   ...
   LET v_i_proceso_cod   = 3901          -- recepción entidades financieras
   LET v_i_opera_cod     = 2             -- integra archivo recurrente
   LET v_i_opera_cod_ant = 1             -- Valida Rechazo Saldos
   LET v_i_operacion     = 16            -- operación del proceso
   LET v_si_id_proceso   = 3901          -- recepción entidades financieras
   LET v_d_folio         = 0
   LET v_c_programa_cod  = "GRTL102"     -- nombre del programa
   ...
   EXECUTE prp_slc_rutas_mod INTO v_c_ruta_bin, v_c_ruta_rescate, v_c_ruta_listados
   ...
   OPEN WINDOW w_integra_arch_recurr WITH FORM "GRTL1021"
         ...
         ON ACTION ACCEPT
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               LET v_s_qryTxt = " SELECT *\n",
                ...
                EXECUTE prp_info_archivo INTO v_r_grt_ctr_arch.*

                -- se valida la información del archivo consultado
                IF v_r_grt_ctr_arch.id_grt_ctr_archivo IS NULL THEN
                ...
                LET v_arr_integrados[v_i_iter].nom_archivo        = v_r_grt_ctr_arch.nom_archivo
                LET v_arr_integrados[v_i_iter].fecha_trans        = v_r_grt_ctr_arch.f_lote
                LET v_arr_integrados[v_i_iter].lote               = 1 ---v_r_grt_ctr_arch.
                LET v_arr_integrados[v_i_iter].fecha_proceso      = v_r_grt_ctr_arch.f_proceso
                LET v_arr_integrados[v_i_iter].tot_registros      = v_r_grt_ctr_arch.tot_registros
                LET v_arr_integrados[v_i_iter].id_cre_ctr_archivo = v_r_grt_ctr_arch.id_grt_ctr_archivo
                LET v_arr_integrados[v_i_iter].folio              = v_r_grt_ctr_arch.folio_archivo
            END FOR
         ...
         ON ACTION integrar
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               IF r_b_valida = 0 THEN
                  ...
                  --LET v_s_comando = " nohup time fglrun ",v_c_ruta_bin CLIPPED,"/GRTP102 ",
                  LET v_s_comando = " nohup time fglrun ",v_c_ruta_bin CLIPPED,"/GRTE102 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
