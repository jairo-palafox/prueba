...
--GLOBALS "GRTW101.inc" -- saldo preliminar PROCESAR
...
MAIN
   ...
   CALL STARTLOG(p_usuario CLIPPED|| ".GRTP101.log")

   DISPLAY "=INICIA GRTP101="
   ...
   LET v_s_qry = "EXECUTE FUNCTION fn_grt_valida_subpr1_tramite(?,?)"

   PREPARE prp_exe_fn_proceso FROM v_s_qry
   EXECUTE prp_exe_fn_proceso USING v_id_grt_ctr_arch,p_usuario
                               INTO v_sp_error
...
--      EXIT PROGRAM
   ...
   FOREACH cur_cons_edo_pr INTO v_rec_edo_pr.id_derechohabiente,
                                v_rec_edo_pr.curp,
                                v_rec_edo_pr.id_grt_detalle,
                                v_rec_edo_pr.id_grt_tramite
      ...
      ELSE 
         ...
         LET v_s_qry = " INSERT INTO grt_inconsistencia
                              VALUES(?,?,?,?) "

         PREPARE prp_ins_incons FROM v_s_qry
         EXECUTE prp_ins_incons USING v_rec_edo_pr.id_grt_tramite,
                                      v_subproceso,
                                      v_inconsistencia,
                                      v_f_inconsistencia

         -- Se actualiza  la situación de grt_tramite a rechazado
         LET v_s_qry = " UPDATE grt_tramite
                            SET diagnostico = 2
                          WHERE id_grt_tramite = ? "

         PREPARE prp_upd_tram FROM v_s_qry
         EXECUTE prp_upd_tram USING v_rec_edo_pr.id_grt_tramite
      END IF 
   END FOREACH
   ...
   LET v_s_qry = " SELECT COUNT(*)
                     FROM grt_tramite a, 
                          grt_detalle b 
                    WHERE diagnostico = 1
                      AND a.id_grt_detalle = b.id_grt_detalle
                      AND b.id_grt_ctr_archivo = ? "

   PREPARE prp_tot_acept FROM v_s_qry
   EXECUTE prp_tot_acept INTO v_cnt_aceptados USING v_id_grt_ctr_arch
   ...
   EXECUTE prp_tot_rech INTO v_cnt_rechazados  USING v_id_grt_ctr_arch
