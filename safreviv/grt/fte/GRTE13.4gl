...
#                     DSE del módulo de Créditos Garantía 43 bis     #
...
MAIN
   ...
   DEFINE v_s_titulo_correo         STRING -- contiene el título del correo
   ...
   DEFINE r_b_existe_err            BOOLEAN -- indica si ocurrió un error durante un subproceso
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".GRTE13.log")

   DISPLAY "=INICIA GRTE13="
   ...
   LET v_s_qryTxt = "EXECUTE FUNCTION safre_viv:fn_dse_integra_rchdse_grt(?,?,?,?)"
   ...
   DISPLAY "TOTAL RECHAZADOS: ",v_r_dse_ctr_arch.tot_rechazados
{
   ...
   LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/GRTP26 ",
                                           p_v_usuario, " ",
                                           p_d_pid, " ",
                                           p_i_proceso_cod, " ",
                                           p_i_opera_cod, " ",
...

{
   ...
   LET v_v_nom_reporte = p_v_usuario CLIPPED, "-GRTL34-", p_d_pid USING "&&&&&", "-", p_i_proceso_cod USING "&&&&&", "-", p_i_opera_cod USING "&&&&&"
   ...
   IF NOT r_b_existe_err THEN
      ...
      LET v_s_titulo_correo = "Proceso: RECEPCIÓN RECHAZO DEVOL SDOS EXC GRT"
      ...
      LET v_s_mens_correo =  "ID Proceso   : ",p_d_pid,"\n",
                             "Proceso      : RECEPCIÓN RECHAZO DEVOL SDOS EXC GRT\n",
                             "Operacion    : INTEGRA RECHAZOS DEVOLUCIÓN SDO EXC GRT\n",
                             "Fecha Inicio : ",TODAY,"\n",
                             "Fecha Fin    : ",TODAY
...
FUNCTION f_genera_rpt_IntegraRechDSE(p_c_ruta_listados, p_v_nom_reporte)
   ...
   IF fgl_report_loadCurrentSettings("GRTE131.4rp") THEN
...
FUNCTION fn_crea_tmp_rech_devol_saldo()
   ...
   DROP TABLE tmp_deudor_rech_devol_saldo_grt

   -- al encontrar un error detiene el programa
   WHENEVER ERROR STOP

   CREATE TABLE tmp_deudor_rech_devol_saldo_grt(id_dse_devolucion  DECIMAL(9,0),
