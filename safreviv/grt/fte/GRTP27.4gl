...
#Modulo            =>GRT                                                   #
#Programa          =>GRTP27                                                #
#Objetivo          =>Programa que realiza la conciliación de la informacion#
#                    de confirmacion de devolucion de saldos               #
...
MAIN
   DEFINE p_v_usuario           LIKE seg_usuario.usuario, -- nombre del usuario
          ...
          v_v_ruta_reporte      VARCHAR(200), -- nombre del reporte
          ...
          v_folio_archivo       LIKE glo_folio.folio, -- folio del archivo
          v_d_pid_arch          LIKE bat_ctr_operacion.pid, -- pid del proceso
          v_c_programa_cod      LIKE cat_operacion.programa_cod, -- nombre del programa origen
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".GRTP27.log")

   DISPLAY "=INICIA GRTP27="
   DISPLAY " USUARIO       : ",p_v_usuario
   DISPLAY " PID           : ",p_d_pid
   DISPLAY " FOLIO         : ",p_d_folio USING "#########&"
   DISPLAY " ARCHIVO       : ",p_v_arch_proceso
   DISPLAY " CONCILIACIÓN DE CONFIRMACIÓN DE DEV. DE SDOS. EXC. GRT"
   ...
   LET v_s_qryTxt = " SELECT UNIQUE folio_archivo\n",
                    " FROM safre_tmp:tmp_deudor_conf_devol_saldo_grt"
   ...
   IF fgl_report_loadCurrentSettings("GRTP271.4rp") THEN  -- if  the file loaded OK
   ...
   LET v_d_pid_arch = fn_max_pid(g_proc_cod_grt_conf_dse, 2)
   ...
   LET v_s_qryTxt = " SELECT *\n",
                    ...
                    "    AND proceso_cod = ",g_proc_cod_grt_conf_dse,"\n",
                    "    AND opera_cod = 2\n",
   ...
   LET v_s_qryTxt = " SELECT NVL(SUM(num_aplic_interes97 / 1000000),0),\n",
                    "        NVL(SUM(saldo_vivienda97 / 100),0)\n",
                    "   FROM safre_tmp:tmp_dse_confirm_det_grt\n"
   PREPARE con_pagCredTradAfore97_Rech FROM v_s_qryTxt
   EXECUTE con_pagCredTradAfore97_Rech INTO v_r_pago_cred_trad_afo.aivs97_afore, v_r_pago_cred_trad_afo.pesos97_afore
   ...
   LET v_s_titulo_correo = "Proceso: CONCILIACIÓN DE CONFIRMACIÓN DEVOLUCIÓN DE SALDOS EXCEDENTES CRÉDITOS EN GARANTÍA 43BIS"
...
REPORT reporte_Confirmacion_ds(p_r_registro, p_v_usuario, p_edit_folio,p_c_subcuenta_desc)
