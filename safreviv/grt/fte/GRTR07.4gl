...
MAIN
   ...
   CALL STARTLOG(p_v_usuario CLIPPED|| ".GRTR07.log")

   DISPLAY "=INICIA GRTR07="
   ...
   CASE p_c_op_arch_ent
      WHEN "opt5" -- Agrupación
         LET v_si_proceso_cod_rev = g_proc_cod_grt_agrupacion_dse -- agrupación registros con devolución grt
         LET v_si_opera_cod_rev   = 1 -- agrupa registros con devolución grt
         CALL fn_reverso_agrupacion_dsegrt(p_d_folio) RETURNING r_b_valida

      WHEN "opt6" -- Preliquidación
         LET v_si_proceso_cod_rev = g_proc_cod_grt_liquida_dse -- liquidación devolución saldos exc grt
         LET v_si_opera_cod_rev   = 1 -- preliquida devolución de saldos exc grt
         CALL fn_reverso_preliquida_dsegrt(p_d_folio) RETURNING r_b_valida

      WHEN "opt7" -- Liquidación
         LET v_si_proceso_cod_rev = g_proc_cod_grt_liquida_dse -- liquidación devolución saldos exc grt
         LET v_si_opera_cod_rev   = 2 -- liquidación solicitud de saldo en garant
         CALL fn_reverso_liquida_dsegrt(p_d_folio) RETURNING r_b_valida
   END CASE
...
FUNCTION fn_reverso_agrupacion_dsegrt(p_d_folio)
   DEFINE p_d_folio             LIKE glo_ctr_archivo.folio, -- numero de folio
...
FUNCTION fn_reverso_preliquida_dsegrt(p_d_folio)
...
FUNCTION fn_reverso_liquida_dsegrt(p_d_folio)
   DEFINE p_d_folio        LIKE glo_ctr_archivo.folio, -- numero de folio
