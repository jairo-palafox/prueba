...
#Modulo            => GRT                                                     #
#Programa          => GRTC03                                                  #
#Objetivo          => CONSULTA DE PRELIQUIDACION DE GRT                       #
...
DEFINE p_tipo_ejecucion SMALLINT, -- forma como ejecutara el programa
   ...
   CALL STARTLOG(g_usuario CLIPPED|| ".GRTC03.log")
   ...
   LET g_proceso_cod = g_proc_cod_grt_uso_liquida -- liquidación uso garantia 
   LET g_opera_cod   = 1 -- preliquida uso garantia 

   -- se invoca la funcion de consulta de preliquidacion   
   CALL fn_consulta_preliquida_grt(g_usuario, g_proceso_cod, g_opera_cod)
END MAIN

#Objetivo funcion que relaiza la consulta de preliquidacion
FUNCTION fn_consulta_preliquida_grt(p_usuario, p_proceso_cod, p_opera_cod)
   ...
   SELECT nombre_tabla
     INTO v_tabla_preliq
     FROM cat_preliquida
    WHERE proceso_cod = g_proc_cod_grt_uso_liquida -- proceso cod del proceso de preliquidación uso garantia
      AND opera_cod   = 2 -- opera cod del proceso de preliquidación

    DISPLAY "v_tabla_preliq ",v_tabla_preliq    
   CALL fn_consulta_folios_preliq_grt(p_usuario, p_proceso_cod, p_opera_cod, v_tabla_preliq)
END FUNCTION

FUNCTION fn_consulta_folios_preliq_grt(p_usuario, p_proceso_cod, p_opera_cod, v_tabla)
   DEFINE p_usuario       CHAR(20)
   DEFINE p_proceso_cod   SMALLINT
   DEFINE p_opera_cod     SMALLINT
   ...
   OPEN WINDOW vent_folios WITH FORM "GRTC031"
   ...
   IF NOT INT_FLAG THEN       
      IF fn_valida_folio_preliq_grt(v_condicion,v_tabla, p_proceso_cod, p_opera_cod) THEN
         CALL fn_folios_consulta_preliq_grt(v_tabla, p_proceso_cod, p_opera_cod, p_usuario)
      ELSE  
         CALL fn_mensaje("Consulta",
...
FUNCTION fn_despliega_desc_preliq_grt(p_proceso_cod, p_opera_cod)
   DEFINE p_proceso_cod       SMALLINT
   DEFINE p_opera_cod         SMALLINT
...
FUNCTION fn_valida_folio_preliq_grt(v_condicion, v_tabla, p_proceso_cod, p_opera_cod)
   DEFINE v_condicion        STRING
...
FUNCTION fn_folios_consulta_preliq_grt(v_tabla, p_proceso_cod, p_opera_cod, p_usuario_cod)
   DEFINE v_tabla            STRING
   DEFINE p_proceso_cod      SMALLINT
   DEFINE p_opera_cod        SMALLINT
   ...
   CALL fn_rutas("grt") RETURNING r_c_ruta_bin, r_c_ruta_listados
{
   ...
   EXECUTE prp_slc_rutasModulo INTO r_c_ruta_bin, v_c_ruta_rescate, r_c_ruta_listados
}
   LET w = ui.Window.getCurrent() -- asigna a "w" la ventana activa
   LET f = w.getForm()
   LET f_w = w.getForm()    
   ...
   DIALOG ATTRIBUTES(UNBUFFERED)
      ...
      ON ACTION reporte
         ...
         -- se crea el comando que ejecuta el modulo que genera el reporte de preliquidacion
