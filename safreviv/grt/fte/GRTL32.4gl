...
#                    archivo de entrada Devolución de Saldos       #
#                    Excedentes para el módulo de Créditos en      #
#                    Garantía 43 bis                               #
...
# Objetivo: Programa que permite la seleccion de archivos Devolución de saldos para su correspondiente integracion
MAIN
   DEFINE p_v_usuario       LIKE seg_usuario.usuario,-- usuario firmado al sistema
          ...
          v_arr_integrados  DYNAMIC ARRAY OF RECORD -- arreglo que contiene los archivos integrados
             ...
             fecha_trans    DATE, -- fecha de transferencia
             lote           INTEGER, -- lote
             fecha_proceso  DATE, -- fecha del lote
             tot_registros  INTEGER -- total de registros insertados
          END RECORD,
          ...
          v_i_operacion     LIKE dse_ctr_archivo.tpo_transferencia, -- operacion del proceso
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".GRTL32.log")
   ...
   LET v_i_opera_cod = 2 -- integra devolución saldos
   LET v_i_opera_cod_ant = 1 -- Valida Rechazo Saldos
   LET v_i_operacion = 19 -- operacion del proceso
   LET v_d_folio = 0
   LET v_c_programa_cod = "GRTL32"
   ...
   OPEN WINDOW w_integra_arch_dse WITH FORM "GRTL321"
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         BEFORE DIALOG
            -- se ocultan el boton de Integrar
         ...
         ON ACTION ACCEPT
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               LET v_s_qryTxt = " SELECT *\n",
                                ...
                                "  WHERE tpo_transferencia = ",v_i_operacion,"\n",
                ...
                IF v_r_dse_ctr_arch.nom_archivo IS NULL THEN
                   CALL fn_mensaje("Devoluciones Saldo","El archivo está marcado como Rechazado, no se puede continuar\ncon la integración del archivo","stop")
                   CONTINUE DIALOG
                END IF
         ...
         ON ACTION integrar
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               IF r_b_valida = 0 THEN
                  ...
                  LET v_s_comando = " nohup time fglrun ",r_c_ruta_bin CLIPPED,"/GRTE12 ",
                                    p_v_usuario, " ",
                                    v_d_pid, " ",
                                    v_i_proceso_cod, " ",
                                    v_i_opera_cod, " ",
         ...
         ON ACTION cancelar
            EXIT DIALOG
      END DIALOG
   CLOSE WINDOW w_integra_arch_dse
END MAIN
