...
#                    de creditos en garantia 43 bis                #
#Autor             =>Mauricio Sanchez, EFP                         #
#Fecha inicio      =>22 JUNIO 2012                                 #
####################################################################
...
DEFINE p_tipo_ejecucion SMALLINT, -- forma como ejecutara el programa
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".GRTC08.log")
...
DEFINE v_s_condicion        STRING,
   ...
   OPEN WINDOW w_op_derechohab WITH FORM "GRTC082"   
...
FUNCTION fn_consulta_archivo()
   ...
   OPEN WINDOW w_op_archivo WITH FORM "GRTC085"
...
FUNCTION fn_despliega_archivo(p_id_cre_ctr_archivo, p_nom_archivo)
   ...
   OPEN WINDOW w_des_archivo WITH FORM "GRTC086"
      ...
      DISPLAY ARRAY v_ar_archivo TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("GRTC081.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-GRTC082-","00000","-","00000","-","00000" 
...
FUNCTION fn_consulta_fecha()
   ...
   OPEN WINDOW w_op_fecha WITH FORM "GRTC087"
   ...
   OPEN WINDOW w_des_archivo WITH FORM "GRTC086"
      ...
      DISPLAY ARRAY v_ar_archivo TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("GRTC081.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-GRTC083-","00000","-","00000","-","00000" 
               CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
...
FUNCTION fn_consulta_tipo_rechazo()
   ...
   OPEN WINDOW w_op_archivo WITH FORM "GRTC088"
...
FUNCTION fn_despliega_rechazo(v_estado)
   ...
   OPEN WINDOW w_des_rechazo WITH FORM "GRTC086"
      LET  w = ui.Window.getCurrent()
      LET  f_w = w.getForm() 
      LET v_sqlqry = " SELECT b.desc_estado, c.originacion_desc,\n",
                     "        SUM(a.sdo_deudor), d.f_proceso, d.nom_archivo\n",
      ...
      DISPLAY ARRAY v_ar_archivo TO tabla_estado.* ATTRIBUTES (UNBUFFERED)
         ...
         ON ACTION reporte
            ...
            IF fgl_report_loadCurrentSettings("GRTC081.4rp") THEN 
               CALL fgl_report_selectDevice("PDF") 
               LET v_nom_reporte = p_usuario_cod CLIPPED || "-GRTC084-","00000","-","00000","-","00000" 
               CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
...
FUNCTION fn_obt_derechohabiente(p_s_condicion)
   ...
   OPEN WINDOW w_list_derechohabiente WITH FORM "GRTC084" 
...
FUNCTION fn_despliega_por_derechohab(v_nss)
  ...
  LET v_indice_aux = v_indice - 1

  OPEN WINDOW w_des_derechohab WITH FORM "GRTC083" 
     LET  w = ui.Window.getCurrent()
     LET  f_w = w.getForm()   
     ...
     DISPLAY ARRAY v_arr_nss_rechazados TO tbl_nss_rechazados.*
        ...
        ON ACTION reporte
           ...
           # Se indica que el reporte usara la plantilla creada
           IF fgl_report_loadCurrentSettings("GRTC082.4rp") THEN 
              CALL fgl_report_selectDevice("PDF") 
              LET v_nom_reporte = p_usuario_cod CLIPPED || "-GRTC082-","00000","-","00000","-","00000" 
              CALL fgl_report_setOutputFileName(r_ruta_listados CLIPPED||"/"||v_nom_reporte CLIPPED)
              ...
              FOR v_indice_rep = 1 TO v_indice_aux
...
DEFINE p_rec_archivo        RECORD 
   ...
   FIRST PAGE HEADER
      LET v_titulo_reporte = "CONSULTA DE RECHAZOS POR ARCHIVO GRT" 
      LET v_fecha_reporte = TODAY      
      PRINTX v_fecha_reporte USING "dd-mm-yyyy"             
      PRINTX p_usuario_cod 
...
DEFINE p_rec_archivo        RECORD 
   ...
   FIRST PAGE HEADER
      LET v_titulo_reporte = "CONSULTA DE RECHAZOS POR FECHA GRT" 
      LET v_fecha_reporte = TODAY      
      PRINTX v_fecha_reporte USING "dd-mm-yyyy"             
      PRINTX p_usuario_cod 
      PRINTX p_rec_archivo.nom_archivo  
...
DEFINE p_rec_archivo        RECORD 
   ...
   FIRST PAGE HEADER
      LET v_titulo_reporte = "CONSULTA DE POR TIPO DE RECHAZO GRT" 
      LET v_fecha_reporte = TODAY      
      PRINTX v_fecha_reporte USING "dd-mm-yyyy"             
      PRINTX p_usuario_cod 
      PRINTX p_rec_archivo.nom_archivo  
