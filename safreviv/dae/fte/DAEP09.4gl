...
DEFINE p_pid                      LIKE bat_ctr_operacion.pid, -- PID del proceso
   ...
   LET v_s_sql = "EXECUTE FUNCTION fn_dae_integra_det_ajuste_ind(?, ?, ?, ?, ?) "
   ...
   CASE
      WHEN (v_i_resultado = 0)
         ...
         IF ( v_si_solicitudes_totales > 0 ) THEN
            CALL fn_actualiza_opera_fin(g_pid,g_proceso_cod,g_opera_cod)
                 RETURNING r_bnd_fin_oper

            --Si no hay registros ACEPTADOS finaliza la operación  
            IF v_total_aceptadas = 0 THEN
               CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,3,0,"DAEL32","",p_usuario)
                    RETURNING r_bnd_ini_oper
               CALL fn_actualiza_opera_fin(g_pid,g_proceso_cod,3)
                    RETURNING r_bnd_fin_oper
               CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,4,0,"DAEL33","",p_usuario)
                    RETURNING r_bnd_ini_oper
         ...
         ELSE
            ...
            DISPLAY "  Total de solicitudes integradas : ", v_si_solicitudes_totales
      ...
      WHEN (v_i_resultado < 0)
          ...
          DISPLAY "  Integración realizada pero con errores"
