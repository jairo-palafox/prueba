--==============================================================================
-- Version: 1.0.0
-- Fecha ultima modificacion: 19/10/2013
--==============================================================================
...
END RECORD
   ...
   OPEN WINDOW w_folio_preliquida WITH FORM "DAEL130"
...
{           --Se quita regla folio-nombre archivo y se cambia a folio - dictamen  
         ...
         IF(v_i_conArch<1)THEN
            CALL fn_mensaje("Atención",
                 "No existen registros dictaminados pendientes de preliquidar","info")
            EXIT INPUT
         END IF
      ...
      ON ACTION ACCEPT
         ...
         --Recupera el folio del lote
         SELECT folio
         INTO   v_folio_lote 
         FROM   dae_det_solicitud 
         WHERE  folio_dictamen = v_folio
         GROUP BY 1
         
DISPLAY v_folio_lote, p_usuario_cod, v_folio
         -- se invoca la ejecucion del stored procedure
         CALL fn_dae_ejecuta_preliquidacion(v_folio_lote, p_usuario_cod, v_folio)
         EXIT INPUT
...
DEFINE p_usuario_cod     LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
   ...
   -- Inicio operacion.
      ...
      IF (r_bnd_fin_oper = 0) THEN
         LET v_s_comando = " nohup time fglrun ",g_reg_modulo.ruta_exp CLIPPED,"/DAEP05 ",
                             p_usuario_cod, " ",
                             g_pid  , " " ,
                             g_proceso_cod , " " ,
                             g_opera_cod ," ",
      ...
      END IF         
   ELSE
      CALL fn_recupera_inconsis_opera(v_si_codigo) RETURNING v_mensaje
      CALL fn_mensaje("Atención", v_mensaje.trim(),"information") 
   END IF
