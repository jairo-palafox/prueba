...
DEFINE p_pid            LIKE bat_ctr_operacion.pid, -- PID del proceso
   ...
   LET p_folio_dictamen = ARG_VAL(5)
   ...
   LET g_opera_cod   = p_opera_cod   -- genera archivo procesar
      ...
      CALL fn_archivo_salida(p_usuario_cod, p_d_inicial, p_d_final, p_folio_dictamen)
...
FUNCTION fn_archivo_salida(p_usuario_cod, v_d_inicial, v_d_final, p_folio_dictamen)
...
-- se obtienen la ruta envio del modulo
   ...
   LET v_busca_archivo = "DIC_"||v_d_hoy||"_"
   ...
   LET v_nom_archivo_axway =  "dic_" ||v_d_hoy ||"_"||v_reg_dia||".sdae"
   ...
   DISPLAY "Folio Dictamen   : ", v_folio_dictamen
      ...
      DECLARE cur_dae_det_solicitud CURSOR FOR 
            SELECT a.id_dae_referencia ,
             ...
             WHERE a.folio_dictamen = v_folio_dictamen 
   ...
   ELSE
     --Actualiza registros a archivo generado
     UPDATE dae_det_solicitud 
     SET    estado = 5
     WHERE  folio_dictamen = v_folio_dictamen;
   
     LET v_ruta_mensaje = "El archivo se creo satisfactoriamente dentro de la siguiente ruta: \n" || v_v_ruta_nomarch , "\n"
     --CALL fn_mensaje ("Atención", v_ruta_mensaje , "about")
     DISPLAY v_ruta_mensaje 
     
      --Convierte archivo a DOS
      LET v_v_nom_archivo = v_busca_archivo||v_reg_dia CLIPPED||".sdae"
      LET v_convierte_archivo = "\n unix2dos "||" "||v_c_ruta_env_dpe CLIPPED||" "||v_v_nom_archivo, "\n"
      RUN v_convierte_archivo 

      DISPLAY "\n Convierte archivo a DOS:", v_convierte_archivo, "\n"

      -- Genera copia a archivo fijo
      LET v_nombre_destino = "DIC_1.sdae"
      ...
      LET v_ejecuta_sh = "\n sh /opt/Interpel/Scripts/DIC_1.sh"
      RUN v_ejecuta_sh

      DISPLAY "\n Se ejecutó la transaferencia a cartera"
      DISPLAY " -------------------------------- "
----
      LET v_copia_archivo = "\n   cp "||v_c_ruta_env_dpe CLIPPED||"/"||v_v_nom_archivo CLIPPED|| " " ||v_c_ruta_env_dpe CLIPPED||"/"||v_nom_archivo_axway
      RUN v_copia_archivo 

      DISPLAY "\n Archivo en directorio transfer para PROCESAR:", v_copia_archivo
      DISPLAY "\n El Script de ejecucion para el envio del archivo es > sh /opt/Interpel/Scripts/DEVOLUCIONES/SACI.sh <"
      LET v_ejecuta_sh = " sh /opt/Interpel/Scripts/DEVOLUCIONES/SACI.sh"
      RUN v_ejecuta_sh
----      
   END IF
...
#Objetivo: genera el número consecutivo por día parapwd el archivo de salida
