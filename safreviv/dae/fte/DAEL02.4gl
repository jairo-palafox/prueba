...
#Modulo       => DAE                                                           #
#Programa     => DAEL17                                                        #
#Objetivo     => Programa lanzador de integración de Ajuste Amortizaciones     #
#                Excedentes                                                    #
...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
   ...
   LET g_opera_cod   = g_opera_cod_dae_integracion --2
...
DEFINE p_usuario_cod LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
       ...
       l_arr_integrados   DYNAMIC ARRAY OF RECORD -- Detalle archivos a integrar
            ...
            tot_valida_montos INTEGER,
            tot_val_montos_menor INTEGER
   ...
   OPEN WINDOW w_inte_devol WITH FORM "DAEL020"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      DISPLAY ARRAY l_arr_arch_int TO archdestino.*
         ...
         ON DROP(l_dnd)
            ...
            SELECT monto_valida
            INTO   v_ed_monto_pesos
            FROM   dae_captura_monto;

            DISPLAY v_ed_monto_pesos TO ed_monto_pesos       
            
            CALL DIALOG.setActionHidden("accept",0)
            --CALL DIALOG.setActionHidden("valida_monto",0)
      ...
      ON ACTION ACCEPT
         ...
         FOR l_i_iter = 1 TO l_i_num_arch
            ...
            IF v_ed_monto_pesos <> 0 THEN
               LET v_s_qry = "\n SELECT COUNT(*)",
                             "\n FROM safre_tmp:tmp_det_dev_amort_excedentes",
                             "\n WHERE importe_amort/100 >= ", v_ed_monto_pesos
              DISPLAY v_s_qry
               PREPARE prp_valida_montos FROM v_s_qry
               EXECUTE prp_valida_montos INTO v_tot_valida_montos

               LET v_tot_valida_montos_menor = l_i_tot_reg - v_tot_valida_montos
            END IF 
            
            -- se asigna el total de solicitudes
            LET l_arr_integrados[l_i_iter].total_solictudes  = l_i_tot_reg
            LET l_arr_integrados[l_i_iter].tot_valida_montos = v_tot_valida_montos
            LET l_arr_integrados[l_i_iter].tot_val_montos_menor =  v_tot_valida_montos_menor
         END FOR
      ...
      ON ACTION integrar
        ...
        IF v_respuesta = 1 THEN
           CALL fn_dae_ejecuta_integracion(p_usuario_cod, l_v_arch_proceso, v_ed_monto_pesos)
           EXIT DIALOG
        END IF
...
#OBJETIVO: Ejecutar la preliquidación de devolución de amortizaciones excedentes
FUNCTION fn_dae_ejecuta_integracion(p_usuario_cod, p_nombre_archivo, p_monto_valida)
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
       ...
       IF ( v_i_resultado = 0 ) THEN
       	  ...
       	  --Inicio operacion.
         CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,g_opera_cod,0,"DAEL02",
                                 "",p_usuario_cod)
           RETURNING r_bnd_fin_oper
           IF (r_bnd_fin_oper = 0) THEN
           	  
              LET v_s_comando = " nohup time fglrun ",g_reg_modulo.ruta_exp CLIPPED,"/DAEP01 ",
                                p_usuario_cod, " ",
                                g_pid  , " " ,
                                g_proceso_cod , " " ,
                                g_opera_cod ," ",
