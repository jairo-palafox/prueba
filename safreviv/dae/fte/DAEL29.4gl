...
DEFINE v_condicion        STRING,
   ...
   CLOSE WINDOW SCREEN
   OPEN WINDOW w_captura WITH FORM "DAEL290.4fd" 
     ...
     IF v_condicion = " 1=1" THEN 
        ...
        IF v_valida_operacion = 0 THEN 
           CALL fn_ventana_confirma("Atención",
                                    "Se ejecutará la extracción de todos los registros pagados.\n ¿Desea continuar?",
                                     "about")
...
END RECORD
   ...
   LET v_QryTxt13 = "\n SELECT b.id_derechohabiente,", 
      ...
      IF p_condicion <> " 1=1" THEN
         LET v_QryTxt13 = v_QryTxt13 , "\n AND ", p_condicion  
      END IF

      PREPARE prp_detalles_13 FROM v_QryTxt13
      DECLARE cur_detalles_13 CURSOR FOR prp_detalles_13 

      --Cunsulta periodo 2013
      FOREACH cur_detalles_13 INTO arr_detalles[i].v_id_derechohabiente,
                                   arr_detalles[i].v_num_credito,
                                   arr_detalles[i].v_fecha_pago,
                                   arr_detalles[i].v_periodo_pago,
                                   arr_detalles[i].v_fec_reg_pag,
   ...
   LET v_QryTxt14 = "\n SELECT b.id_derechohabiente,", 
      ...
      IF p_condicion <> " 1=1" THEN
         LET v_QryTxt14 = v_QryTxt14 , "\n AND ", p_condicion  
      END IF
DISPLAY v_QryTxt14
      PREPARE prp_detalles_14 FROM v_QryTxt
      DECLARE cur_detalles_14 CURSOR FOR prp_detalles_14 
      --Cunsulta periodo 2014
      FOREACH cur_detalles_14 INTO arr_detalles[i].v_id_derechohabiente,
                                   arr_detalles[i].v_num_credito,
                                   arr_detalles[i].v_fecha_pago,
                                   arr_detalles[i].v_periodo_pago,
                                   arr_detalles[i].v_fec_reg_pag,
   ...
   LET v_QryTxt15 = "\n SELECT b.id_derechohabiente,", 
      ...
      IF p_condicion <> " 1=1" THEN
         LET v_QryTxt15 = v_QryTxt15, "\n AND ", p_condicion  
      END IF

      PREPARE prp_detalles_15 FROM v_QryTxt
      DECLARE cur_detalles_15 CURSOR FOR prp_detalles_15 

      FOREACH cur_detalles_15 INTO arr_detalles[i].v_id_derechohabiente,
                                   arr_detalles[i].v_num_credito,
                                   arr_detalles[i].v_fecha_pago,
                                   arr_detalles[i].v_periodo_pago,
                                   arr_detalles[i].v_fec_reg_pag,
   ...
   LET v_QryTxt = "\n SELECT b.id_derechohabiente,", 
      ...
      PREPARE prp_detalles_16 FROM v_QryTxt
      DECLARE cur_detalles_16 CURSOR FOR prp_detalles_16

      FOREACH cur_detalles_16 INTO arr_detalles[i].v_id_derechohabiente,
                                   arr_detalles[i].v_num_credito,
                                   arr_detalles[i].v_fecha_pago,
                                   arr_detalles[i].v_periodo_pago,
                                   arr_detalles[i].v_fec_reg_pag,
...
DEFINE p_usuario_cod      LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
   ...
   CALL fn_valida_operacion(g_pid,g_proceso_cod,g_opera_cod)
   RETURNING v_valida_operacion
DISPLAY "v_valida_operacion", v_valida_operacion
   IF v_valida_operacion = 0 THEN
      ...
      --Inicia proceso
      CALL fn_inicializa_proceso(g_pid,g_proceso_cod,g_opera_cod,0,"DAEL29","",p_usuario_cod) 
      ...
      IF ( v_inicia_proceso = 0)THEN
         ...
         IF ( v_valida_operacion = 0 ) THEN
            ...
            RETURNING v_folio_archivo 
      	    ...
      	    -- Inicio operacion.
            CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,g_opera_cod,v_folio_archivo ,"DAEL29","",p_usuario_cod)
            RETURNING r_bnd_fin_oper
         
            IF (r_bnd_fin_oper = 0) THEN
      
               LET v_s_comando = " nohup time fglrun ",g_seg_modulo.ruta_exp CLIPPED,"/DAES05 ",
