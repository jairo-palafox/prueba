...
DEFINE p_usuario_cod       LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
       ...
       vQryCta             STRING,
       v_existe_referencia DECIMAL(9,0)
...
END RECORD 
   ...
   FOREACH cur_tab_mvto INTO v_tabla_cta_mvto
      ...
      LET vQryCta = "\n SELECT  a.id_derechohabiente, a.f_liquida, a.monto_acciones ",
                    "\n FROM ", v_tabla_cta_mvto, " a", 
                    "\n INNER JOIN ret_amort_excedente b ",
                    "\n         ON a.folio_liquida = b.folio ",
                    "\n        AND a.id_referencia = b.id_solicitud ",
                    "\n        AND a.id_derechohabiente = b.id_derechohabiente ",
                    "\n        AND b.estado_solicitud IN (71,72,74,75) ",
                    "\n WHERE  a.subcuenta = 46 ",
                    "\n AND    a.movimiento = 1402 "
      ...
      FOREACH cur_cta_mov INTO arr_detalles[j].v_id_derechohabiente,
                               arr_detalles[j].v_f_liquida,
                               arr_detalles[j].v_aivs_liquida
         --DISPLAY  arr_detalles[i].v_id_derechohabiente,arr_detalles[i].v_f_liquida
         LET v_QryTxt12 = "\n SELECT a.id_dae_referencia, ", 
                     ...
                     "\n WHERE  a.id_derechohabiente = ", arr_detalles[j].v_id_derechohabiente,
                     "\n AND    a.folio_liquida IS NOT NULL ",
                     "\n AND    a.status_retiro = 4",
                     "\n AND    a.folio_ajuste IS NULL ",
                     "\n AND    a.fecha_liquida <= '", arr_detalles[j].v_f_liquida, "'"
         ...
         --Cunsulta periodo 2012 
         FOREACH cur_detalles_12 INTO arr_detalles[i].v_id_dae_referencia,
                                      arr_detalles[i].v_id_derechohabiente,
            ...
            IF (arr_detalles[i].v_monto_aivs IS NULL) AND arr_detalles[i].v_fecha_pago < "07/01/1997" THEN
               LET arr_detalles[i].v_monto_aivs = (arr_detalles[i].v_imp_amortizacion / 0.50348)
            END IF

            IF arr_detalles[j].v_f_liquida IS NOT NULL THEN 
               LET v_fecha_retiro = arr_detalles[j].v_f_liquida;

               SELECT precio_fondo
               INTO   v_precio_fondo
               FROM   glo_valor_fondo
               WHERE  f_valuacion = v_fecha_retiro
               AND    fondo = 11; 
            ...
            INTO   v_existe_referencia
            FROM   safre_tmp:dae_tmp_det_extractor_pagado
            WHERE  id_dae_referencia = arr_detalles[i].v_id_dae_referencia

            IF v_existe_referencia IS NULL THEN
               INSERT INTO safre_tmp:dae_tmp_det_extractor_pagado
               VALUES (arr_detalles[i].v_id_dae_referencia,
                       arr_detalles[i].v_id_derechohabiente,
                       arr_detalles[i].v_num_credito,
                       arr_detalles[i].v_fecha_pago,
                       arr_detalles[i].v_periodo_pago,
               ...
               LET v_existe_referencia = NULL;
               LET v_precio_fondo = NULL;
               --LET v_fecha_retiro = NULL;
               INITIALIZE arr_detalles[i].v_id_dae_referencia,
                             arr_detalles[i].v_num_credito,
                             arr_detalles[i].v_fecha_pago,
                             arr_detalles[i].v_periodo_pago,
                             arr_detalles[i].v_fec_reg_pag,
         ...
         END FOREACH
         LET j = j+1;
         INITIALIZE arr_detalles[j].v_id_derechohabiente,
                    arr_detalles[j].v_f_liquida TO NULL 
      END FOREACH
   END FOREACH
...
--------------------------------------------------------------------------------
      ...
      DISPLAY "   cta_movimiento"
      FOREACH cur_cta_mov_ INTO arr_detalles[j].v_id_derechohabiente,
                                arr_detalles[j].v_f_liquida,
                                arr_detalles[j].v_aivs_liquida
         ...
         --Consulta  AÑO ACTUAL
         FOREACH cur_detalles INTO    arr_detalles[i].v_id_dae_referencia,
                                      arr_detalles[i].v_id_derechohabiente,
                                      arr_detalles[i].v_num_credito,
                                      arr_detalles[i].v_fecha_pago,
                                      arr_detalles[i].v_periodo_pago,
...
"\n     fecha_retiro        ",
      ...
      DECLARE cur_totales CURSOR FOR prp_totales
         ...
         FOREACH cur_totales
           ...
           IF v_monto_rendimiento   IS NOT NULL THEN
