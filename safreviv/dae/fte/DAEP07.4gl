...
-- Fecha ultima modificacion: 05/Nov/2014 Se cambian los mensajes de finalización
--==============================================================================
...
#Modulo       => DAE                                                           #
#Programa     => DAEP07                                                        #
#Objetivo     => Programa lanzado  de integración de Ajuste Amortizaciones     #
#                Excedentes                                                    #
...
DEFINE p_pid                      LIKE bat_ctr_operacion.pid, -- PID del proceso
   ...
   LET v_s_sql = "EXECUTE FUNCTION fn_dae_integra_det_ajuste(?, ?, ?, ?, ?) "
         ...
         IF ( v_si_correcto_integra = 0 AND v_si_solicitudes_totales > 0 ) THEN
            CALL fn_actualiza_opera_fin(g_pid,g_proceso_cod,g_opera_cod)
             ...
             IF v_total_aceptadas = 0 THEN
                CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,3,0,"DAEL18","",p_usuario)
                RETURNING r_bnd_ini_oper
                CALL fn_actualiza_opera_fin(g_pid,g_proceso_cod,3)
                RETURNING r_bnd_fin_oper
                CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,4,0,"DAEL19","",p_usuario)
                RETURNING r_bnd_ini_oper
