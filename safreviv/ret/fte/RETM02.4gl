################################################################################
# Version: 1.0.0                                                               #
# Fecha ultima modificacion: 16/07/2013                                        #
################################################################################
################################################################################
...
MAIN
   ...
   SELECT MAX(pid)
    ...
    WHERE proceso_cod = PROC_CUENTAS_CLABE
   ...
   CALL fn_drgandrop_cta_clabe()
   
END MAIN


#PENDIENTE
FUNCTION fn_drgandrop_cta_clabe()
   ...
   LET l_s_qryTxt = "SELECT nombre_archivo ",
                    ...
                    "AND proceso_cod = ", PROC_CUENTAS_CLABE, " ",
                    "AND estado = 1"
   ...
   OPEN WINDOW w_inte_ret WITH FORM "RETM021"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION integrar
         ...
         FOR l_i_iter = 1 TO l_i_num_arch
            ...
            SELECT count(*) 
            INTO l_arr_integrados[l_i_iter].t_registros
            FROM safre_tmp:tmp_ret_cta_clabe

            SELECT f_actualiza
            INTO l_arr_integrados[l_i_iter].f_proceso
            FROM glo_ctr_archivo
            where opera_cod = OP_VALIDA_ARCHIVO
            and proceso_cod = PROC_CUENTAS_CLABE
            and estado = EDO_ARCHIVO_VALIDADO
      ...
      ON ACTION ACCEPT
         ...
         CALL fn_ventana_confirma("Atención",
                    "¿Desea ejecutar el proceso de Integracion del archivo de cuentas CLABE?",
...
FUNCTION fn_ejecuta_integracion(p_nombre_archivo, v_ruta_ejecutable, v_ruta_listados)
   ...
   CALL fn_valida_operacion(v_pid,PROC_CUENTAS_CLABE,OP_INTEGRA_ARCHIVO) RETURNING v_resultado
   IF v_resultado = 0 THEN
      LET v_folio = 1
      
      # Inicia operación
      CALL fn_actualiza_opera_ini(v_pid,PROC_CUENTAS_CLABE,OP_INTEGRA_ARCHIVO,v_folio,"RETMP02",
                            p_nombre_archivo,p_usuario) RETURNING v_resultado
      
      LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/RETMP02.42r ",
                                       p_usuario," ",
                                       v_pid," ",
                                       PROC_CUENTAS_CLABE," ",
                                       OP_INTEGRA_ARCHIVO," ",
