#############################################################################
#Módulo          => RET                                                     #        
#Programa        => RETE348.4gl                                             #
#Objetivo        => Programa carga de archivo notificacion TRAN_BP_VAL      #
...
DEFINE p_nom_archivo       STRING,
   ...
   LET v_cadena_registros = p_usuario CLIPPED, ".", "RETE348.log"
   ...
   DISPLAY "Generando tabla temporal tmp_ret_tran_bp_val"

   -- cambio a base de datos temporal
   DATABASE safre_tmp

   -- se regenera la tabla temporal
   LET v_sql = "\nDROP TABLE IF EXISTS tmp_ret_tran_bp_val"
   ...
   LET v_sql = "\n CREATE TABLE tmp_ret_tran_bp_val(   ", 
               "\n  nss                    CHAR(11),   ",
               "\n  tpo_credito            CHAR(10),   ",
               "\n  estatus_credito        CHAR(09),   ",
               "\n  marcado_procesar       CHAR(19),   ",
   ...
   WHILE ( v_ch_archivo.read([v_registro_archivo.*]) )
       ...
       --IF (v_reg_archivo >= 6) AND (v_registro_archivo.nss[1,1] <> '-' ) THEN 
           ...
           INSERT INTO tmp_ret_tran_bp_val values (
