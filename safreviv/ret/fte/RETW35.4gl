...
#                     SE INCLUYE LA SUBCUENTA 12, SUMA DE VIV92 Y VIV97       #
...
DEFINE v_resultado       INTEGER, -- recibe el resultado de la ejecucion del servicio 
  ...
  LET v_ruta_log = v_ruta_ejecutable CLIPPED, "/RETW35."
  ...
  CALL ERRORLOG("Iniciando servidor de Consulta de Saldo de Retiro Generico Subcuenta 12 Version 1.2 ...")
...
FUNCTION fn_crea_servicio_consulta_saldo_retiro_generico(p_generar_WSDL)
  ...
  TRY
    -- =============================
    -- se crea el servicio
    LET v_webservice = com.WebService.CreateWebService("consultaSaldoRetiroGenericoSubcta12", v_service_NameSpace)
    ...
    --CALL v_webservice.publishOperation(op, "urn:http://10.90.8.199:7783/consultaSaldoRetiroGenericoSubcta12/fn_ret_saldos_subcuenta") -- se crea con el nombre de operacion fn_ret_saldos_subcuenta
...
DEFINE v_indice     SMALLINT, -- indice de la subcuenta
       ...
       v_tot_aivs           DECIMAL(24,6), -- Suma de aivs 92 y 97 
       v_tot_pesos          DECIMAL(22,2), -- Suma de pesos 92 y 97
       v_tabla_consulta     VARCHAR(40), -- tabla de donde se consultara el saldo
       v_f_saldo            DATE, -- fecha de calculo del saldo0
       v_f_saldo_tot        DATE, -- fecha del saldo total de vivienda
   ...
   IF v_id_derechohabiente IS NULL THEN 
       ...
       AND    sm.marca = 151
       AND    af.nss = v_nss  -- Se implemento esta busqueda por los que estan inhabilitados por unificacion  requerimiento PRODINF-935
   END IF 
         ...
         FOREACH cur_subcuenta INTO v_subcuenta
            ...
            IF ( SQLCA.SQLCODE = 0 ) THEN
               IF v_aivs > 0 OR v_pesos > 0 THEN 
                   ...
                   LET ws_ret_cons_saldos_subcuenta_out.saldo_x_subcuenta[v_indice].monto_pesos = v_pesos
                   IF v_subcuenta = 4 OR v_subcuenta = 8 OR v_subcuenta = 47 THEN
                       LET v_tot_aivs = v_tot_aivs + v_aivs 
                       LET v_tot_pesos = v_tot_pesos + v_pesos
                       LET v_f_saldo_tot = v_f_saldo
                   END IF 
                   LET v_indice = v_indice + 1
               END IF 
            END IF
         END FOREACH
         IF v_tot_aivs > 0 OR v_tot_pesos > 0 THEN 
         ---- Esta subcuenta no se encuentra en el catalogo de subcuentas, solo identifica la sumatoria de las subcuentas de vivienda 92 y 97
             LET ws_ret_cons_saldos_subcuenta_out.saldo_x_subcuenta[v_indice].subcuenta   = 12 -- Suma de subcuentas 4 y 8
             LET ws_ret_cons_saldos_subcuenta_out.saldo_x_subcuenta[v_indice].f_valuacion = v_f_saldo_tot USING "yyyymmdd"
             LET ws_ret_cons_saldos_subcuenta_out.saldo_x_subcuenta[v_indice].monto_aivs  = v_tot_aivs
             LET ws_ret_cons_saldos_subcuenta_out.saldo_x_subcuenta[v_indice].monto_pesos = v_tot_pesos
         END IF 
      END IF        
   END IF
