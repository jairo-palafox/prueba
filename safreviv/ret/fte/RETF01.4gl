...
#Modulo       => RET                                                                    #
#Programa     => RETF01                                                                 #
#Objetivo     => Programa para capturar los montos 
...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
   ...
   LET g_proceso_cod = g_proceso_cod_ret_disposicion -- retiro por disposicion de recursos

   -- se invoca la captura de saldos para devolucion por errores de operacion
   CALL fn_captura_saldos_notificados_ret_disposicion(p_usuario_cod)

END MAIN

{ ======================================================================
Clave: RETF01
Nombre: fn_captura_saldos_notificados_ret_disposicion
...
Ivan Vega      22 Feb 2013           - Se permite una tolerancia de hasta 1 peso de diferencia
                                       entre el monto capturado y monto notificado
======================================================================
}
FUNCTION fn_captura_saldos_notificados_ret_disposicion(p_usuario_cod)
DEFINE p_usuario_cod           LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
       --r_ret_mto_notificado    RECORD LIKE ret_mto_notificado.*, -- registro de monto notificado
   ...
   OPEN WINDOW w_captura_montos WITH FORM "RETF011"
   ...
   LET v_continuar = TRUE
     ...
     ATTRIBUTES ( UNBUFFERED )
        ...
        ON ACTION ACCEPT
           ...
           -- se verifica si la diferencia es mayor a la tolerancia del retiro
           IF ( v_diferencia > g_tolerancia_max_disposicion ) THEN
              CALL fn_mensaje("Atención", "El monto capturado no coincide con el monto recibido en archivo","stop")
              CONTINUE INPUT
           END IF
           
           {
           IF ( v_total_importe_archivo <> r_ret_mto_notificado.total_importe ) THEN
              CALL fn_mensaje("Atención", "El monto capturado no coincide con el monto recibido en archivo","stop")
              CONTINUE INPUT
           END IF
           }
        ...
        ON ACTION notifica_error
           ...
           ATTRIBUTES ( STYLE="dialog", COMMENT = p_mensaje )
              COMMAND "Aceptar"
                 ...
                 LET p_titulo = "Informe de inconsistencia en montos notificados RETIROS POR DISPOSICION DE RECURSOS"
                 ...
                 CALL fn_correo_proceso(g_pid, g_proceso_cod_ret_disposicion, g_opera_cod_ret_disp_integracion,
...
DEFINE  p_usuario_cod        LIKE seg_usuario.usuario_cod -- clave del usuario firmado
   ...
   --IF ( fn_valida_reverso(g_pid,g_proceso_cod,g_opera_cod) = 0 ) THEN
      ...
      LET v_s_comando = " nohup time fglrun ",g_reg_modulo.ruta_exp CLIPPED,"/RETR25 ",
