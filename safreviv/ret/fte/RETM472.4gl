...
#                 las tablas ret_solicitud_generico y ret_ley73_generico.      #
#Fecha inicio  => 23 de Julio, 2015.                                           #
#Requerimiento => SACI2018-140                                                 #
################################################################################
DATABASE safre_viv
...
MAIN
    ...
    CALL STARTLOG (p_usuario CLIPPED||".RETM472.log")

    CLOSE WINDOW SCREEN
    
    --Se llama a la funcion que desplegara la ventana para introducir los 
    --parametros de la consulta
    OPEN WINDOW actualiza WITH FORM "RETM4721"
               ...
               LET v_estado_indicador = 0
                ...
                ON ACTION actualizar
                    ...
                    IF v_datos_der_hab.estado_solicitud =  8 OR 
                       ...
                       LET v_intentos_desmarca = 0

                       WHILE v_continua = 0
                          CALL fn_desmarca_procesar(v_datos_der_hab.nss, p_usuario, v_datos_der_hab.id_solicitud) RETURNING v_diagnostico,v_estado_cuenta,v_estado_indicador
                          IF (v_diagnostico = 101 AND v_estado_cuenta = 101) OR 
                              v_estado_indicador = 5 THEN 
                              LET v_continua = 1
                          END IF 
                          IF v_estado_indicador = 4 THEN 
                              IF v_diagnostico = 127 THEN 
                                 LET v_intentos_desmarca = v_intentos_desmarca + 1
                                 SLEEP 60
                                 IF v_intentos_desmarca = 3 THEN 
                                    LET v_continua = 1
                                 END IF 
                              ELSE 
                                 LET v_continua = 1
...
FUNCTION fn_busca_der_hab(p_nss,p_caso_adai,p_id_solicitud)
    ...
    LET v_query = "SELECT a.nombre_af, a.ap_paterno_af, a.ap_materno_af,
                          ...
                          '3 - Ley 73', r.nss, c.descripcion, r.f_solicitud,
...
#Regresa un estado de   0 - Sin proceso
#                       1 - Con solicitud
#                       2 - Sin solicitud de desmarca
FUNCTION fn_desmarca_procesar(p_nss, p_usuario_cod, p_id_solicitud)
    ...
    DEFINE r_estado_indicador   SMALLINT
    DEFINE v_estado_cuenta      SMALLINT
    ...
    DEFINE v_aivs_viv92     LIKE ret_ley73_generico.aivs_viv92
    DEFINE v_aivs_viv97     LIKE ret_ley73_generico.aivs_viv97
    DEFINE v_pesos_viv92    LIKE ret_ley73_generico.importe_viv92
    DEFINE v_pesos_viv97    LIKE ret_ley73_generico.importe_viv97
    DEFINE v_diagnostico    SMALLINT
    ...
    LET v_aivs_viv92       = 0 -- AIV'S en vivienda 92
    LET v_aivs_viv97       = 0 -- AIV'S en vivienda 97
    LET v_pesos_viv92      = 0 -- Pesos en vivienda 92
    LET v_pesos_viv97      = 0 -- Pesos en vivienda 97
    LET v_diagnostico      = 0 -- Respuesta de WS
    LET v_estado_cuenta    = 0 -- Respuesta de WS
    LET r_estado_indicador = 0 -- Estado de la operacion
    LET v_indicador_marca  = 2 -- Desmarca
    LET v_id_solicitud     = p_id_solicitud

    CALL recuperaDatos(v_id_solicitud) RETURNING v_caso_crm

   -- Se realiza la consulta a PROCESAR
   CALL fn_consulta_saldo_vivienda_afore(p_nss, 60) -- Se llama con 60 para desmarca
   ...
   IF v_diagnostico = "101" THEN
      ...
      IF v_estado_cuenta = "101" THEN
          LET r_estado_indicador = 5 -- Desmarcado
      ELSE
          LET r_estado_indicador = 4 -- No desmarcado
      END IF
   ELSE
      LET r_estado_indicador = 4 -- No desmarcado
   END IF

   DISPLAY "NSS:   ",p_nss,". Diagnostico: ",v_diagnostico,
          ". Estado Cuenta:   ",v_estado_cuenta

   -- Se guardan los datos en la bitacora
   CALL fn_guarda_consulta_ws_vent_afore(p_nss,
                                        v_indicador_marca,
                                        r_estado_indicador,
                                        TODAY,
   ...
   RETURN v_diagnostico, v_estado_cuenta,r_estado_indicador
...
DEFINE v_cod_rechazo_return SMALLINT 
   ...
   OPEN WINDOW w_codigos_rechazo WITH FORM "RETM4722"
