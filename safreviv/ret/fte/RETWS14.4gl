...
#                     RETIRO DE LA DEVOLUCIÓN AUTOMÁTICA DEL SSV A TRAVÉS DE  #
#                     LA FIEL                                                 #
...
DEFINE ws_ret_generico_solicitud_in RECORD
         ...
         medio_entrega    SMALLINT, -- Sello generado por la consulta biometrica (aplica para medio_entrega = 2  Devolucion automatica)
...
DEFINE v_resultado       INTEGER, -- recibe el resultado de la ejecucion del servicio 
  ...
  IF num_args() = 2 AND arg_val(1) = "-W" THEN
      ...
      CALL fn_crea_servicio_dev_automatica_ley73(TRUE)
      EXIT PROGRAM
  ELSE 
  ...
  CALL fn_crea_servicio_dev_automatica_ley73(FALSE)
...
======================================================================
Clave: 
Nombre: fn_crea_servicio_dev_automatica_ley73
Fecha creacion: Noviembre 30, 2017
Autor: Ricardo Pérez, EFP
Narrativa del proceso que realiza:
Genera el servicio web de retiro de ley 73 de devolución automática
...
FUNCTION fn_crea_servicio_dev_automatica_ley73(p_generar_WSDL)
  ...
  TRY
    -- =============================
    -- se crea el servicio
    LET v_webservice = com.WebService.CreateWebService("retiroCreaSolicitudDevAutomaticaLey73", v_service_NameSpace)
  
    -- =============================
    -- Publicacion de las funciones
    
    -- fn_retiro 
    LET op = com.WebOperation.CreateDOCStyle("fn_ret_sol_dev_automatica_ley73","fn_ret_sol_dev_automatica_ley73",ws_ret_generico_solicitud_in,ws_ret_generico_solicitud_out)
    --LET op = com.WebOperation.CreateDOCStyle("fn_retiro","fn_retiro",ret_retiro_fondo,ret_respuesta)
    CALL v_webservice.publishOperation(op, "fn_ret_sol_dev_automatica_ley73")
...
======================================================================
Clave: 
Nombre: fn_ret_sol_dev_automatica_ley73
Fecha creacion: Noviembre 30, 2017
Autor: Ivan Vega, EFP
Narrativa del proceso que realiza:
...
FUNCTION fn_ret_sol_dev_automatica_ley73()
DEFINE v_indice_retiro       SMALLINT,
   ...
   ELSE
      ...
      IF ws_ret_generico_solicitud_in.grupo <> 1 AND ws_ret_generico_solicitud_in.medio_entrega <> 2 THEN 
...
--               EXIT FOR
            END IF   
            IF ws_ret_generico_solicitud_in.grupo = 1 AND ws_ret_generico_solicitud_in.medio_entrega = 2 AND v_indice_beneficiario = 1 THEN 
               EXIT FOR
            END IF 
         END FOR
         ...
         IF ( NOT v_existe_beneficiario OR NOT v_cta_clabe_correcta ) THEN
            ...
            ELSE
               ...
               CALL fn_respuesta_ws(v_nss, v_rfc, 3,
                                  gi_solicitud_rechazada, gi_cuenta_clabe_duplicada, 0, 0, NULL)               
            END IF
         ELSE            
   ...
   IF ( p_estado_solicitud = gi_solicitud_aceptada ) THEN      
      ...
      IF ws_ret_generico_solicitud_in.medio_entrega = 2 AND 
         ...
         LET v_cadena = p_id_solicitud USING "##########", ws_ret_generico_solicitud_in.nss, 
                        v_curp, v_fecha_hora, v_ape_paterno CLIPPED, v_ape_materno CLIPPED, 
                        v_nombre CLIPPED, p_pesos_viv92 + p_pesos_viv97 USING "<,<<<,<<&.&&"
         ...
         CALL fn_genera_reporte(ws_ret_generico_solicitud_in.nss,v_curp,
                                v_ape_paterno, v_ape_materno, v_nombre, 
                                v_fecha_hora,p_id_solicitud,p_pesos_viv92,p_pesos_viv97,v_c_sha, 
                                ws_ret_generico_solicitud_in.caso_adai)
         CALL fn_load_pdf(v_ruta_pdf, v_archivo_pdf, ws_ret_generico_solicitud_in.caso_adai)--Se crea, se envia y se borra el reporte.pdf
...
PRIVATE  FUNCTION fn_genera_reporte(p_nss, p_curp, p_paterno, p_materno, p_nombre,p_fecha_hora,
                                    p_id_solicitud,p_pesos_viv92,p_pesos_viv97,p_sello,
                                    p_caso)
   ...
   IF fgl_report_loadCurrentSettings(v_reporte) THEN
      ...
      IF reporte IS NOT NULL THEN
         START REPORT pdf_acuse TO XML HANDLER reporte
            OUTPUT TO REPORT pdf_acuse(p_nss, p_curp, p_paterno, p_materno, p_nombre,p_fecha_hora,
                                       p_id_solicitud,p_pesos_viv92,p_pesos_viv97,v_sello_funcionario, v_aviso,
                                       p_caso)
...
REPORT pdf_acuse(p_nss, p_curp, p_paterno, p_materno, p_nombre,p_fecha_hora,
                 p_id_solicitud,p_pesos_viv92,p_pesos_viv97,p_sello, p_aviso, p_caso) 
    DEFINE p_nss             LIKE afi_derechohabiente.nss
    DEFINE p_curp            LIKE afi_derechohabiente.curp
    DEFINE p_paterno         CHAR(40)     -- Apellido paterno
    DEFINE p_materno         CHAR(40)     -- Apellido materno
   ...
   FIRST PAGE HEADER
      ...
      PRINTX p_nss
      PRINTX p_curp
      LET v_nombre = p_paterno CLIPPED, " ", p_materno CLIPPED, " ", p_nombre CLIPPED
      PRINTX v_nombre
      LET v_tramite = 'Devolución del saldo de la subcuenta de vivienda'
      PRINTX v_tramite
      LET v_grupo = "Uno (pensionados posteriores al 13 de enero de 2012)"
      PRINTX v_grupo
      PRINTX p_fecha_hora
      PRINTX p_id_solicitud
      PRINTX p_caso
      LET v_medioSolicitud = "Internet"
      PRINTX v_medioSolicitud
      LET v_pesosViv92 = p_pesos_viv92 USING "$$$,$$$,$$&.&&"
      PRINTX v_pesosViv92
      LET v_pesosViv97 = p_pesos_viv97 USING "$$$,$$$,$$&.&&"
      PRINTX v_pesosViv97
      LET v_pesosTotal = (p_pesos_viv92 + p_pesos_viv97) USING "$$$,$$$,$$&.&&"  
      PRINTX v_pesosTotal
      PRINTX p_sello
      PRINTX p_aviso
