...
MAIN
   ...
   LET v_nss                = ARG_VAL(1)
   LET v_ventanilla         = ARG_VAL(2)
   LET v_rechazo            = ARG_VAL(3)
   LET v_afore              = ARG_VAL(4)
   LET v_fecha_not_inicio   = ARG_VAL(5)
   LET v_fecha_not_fin      = ARG_VAL(6)
   LET v_fecha_pago_inicio  = ARG_VAL(7)
   LET v_fecha_pago_fin     = ARG_VAL(8)
   ...
   IF v_nss IS NULL AND 
      ...
      DISPLAY "Debe al menos ingresar valor en un filtro para realizar la consulta."
   ELSE 
      DISPLAY v_nss
      DISPLAY v_ventanilla
      DISPLAY v_rechazo
      DISPLAY v_afore
      DISPLAY v_fecha_not_inicio
      DISPLAY v_fecha_not_fin
      DISPLAY v_fecha_pago_inicio
      DISPLAY v_fecha_pago_fin
...
DISPLAY v_sql
   ...
   ELSE
      DISPLAY "No se encontro ningún resultado."
   END IF
END FUNCTION
...
FUNCTION fn_genera_archivo(p_detalle_solicitud)
   ...
   -- se deja el archivo en la ruta donde esta el programa
   LET v_ruta_arch = v_archivo
   ...
   DISPLAY "Ruta de archivo de salida: "||v_texto||"/"||v_archivo
