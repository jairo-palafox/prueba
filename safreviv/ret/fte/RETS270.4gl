...
DEFINE p_folio        LIKE glo_folio.folio,
       ...
       r_det_arch_fico RECORD
       	...
       	v_monto            CHAR(12),
       	--v_banco            CHAR(4), -- no se usa en DAP
       	v_fec_pago         CHAR(10),
        ...
        --v_beneficiario     CHAR(60),-- no se usa en DAP
       	v_cve_pais         CHAR(3),
       	v_estado           CHAR(3),
       	v_rfc              CHAR(16),
        ...
        --v_cta_clabe        CHAR(18) -- no se usa en DAP
   ...
   DISPLAY " Generando archivo de salida para creación de cuentas por pagar a SAP-FICO pago por DAP"
   ...
   -- SACI_DAPFAAAAMMDD.TXT
   -- SACI_DAPFAAAAMMDD.KEY
   LET v_nom_archivo = "SACI_DAPSFA", TODAY USING "yyyymmdd"
   ...
   DISPLAY "~~~~~~~~~~~"
   DISPLAY "Archivo para pago por DAP generado: ", v_v_ruta_nomarch
   ...
   -- =========================================================================================
   -- =========================================================================================
   -- LAS UNICAS SOLICITUDES QUE SE PAGAN POR DAP SON DE FONDO DE AHORRO
...
--             "\n AND afi.rfc                = rs.rfc                 "
   ...
   FOREACH cur_ret_genericof72 INTO v_id_derechohabiente         ,
      ...
      -- la referencia es el id_solicitud
      LET r_det_arch_fico.v_referencia = v_id_solicitud
      ...
      -- Se pidio usar el nombre de CRM
      ...
      -- se pasa a mayusculas el nombre del acreedor
...
{-- DAP no tiene beneficiario en el archivo
      -- Formatea el nombre
      LET v_s_nom_beneficiario = v_nom_beneficiario                            
      LET v_s_ap_beneficiario  = v_ap_beneficiario
      LET v_s_am_beneficiario  = v_am_beneficiario
      ...
      -- el concepto de pago DAP Fondo de ahorro es 0F
      LET r_det_arch_fico.v_concepto = "  17"
      ...
      LET v_s_detalle = r_det_arch_fico.v_control        ,
                        ...
                        --r_det_arch_fico.v_banco          , -- no se usa en DAP
                        ...
                        --r_det_arch_fico.v_beneficiario   , -- no se usa en DAP
                        ...
                        --r_det_arch_fico.v_cta_clabe      -- no se usa en DAP
   ...
   LET v_comando = "cp ",v_v_ruta_nomarch," ",v_c_ruta_env_acr CLIPPED,"/SACI_DAPSFA.TXT"
   ...
   LET v_comando = "shasum ", v_c_ruta_env_acr CLIPPED, "/SACI_DAPSFA.TXT > ", v_c_ruta_env_acr CLIPPED, "/SACI_DAPSFA.KEY"
   DISPLAY "\n\n ==== Generando hash del archivo SACI_DAPSFA.TXT ==="
   DISPLAY v_comando
   RUN v_comando 
   DISPLAY "Finalizado..."
   ...
   LET v_ejecuta_sh = "sh /safreviv_int/ret/envio/SACI_DAPSFA.sh"
   RUN v_ejecuta_sh

   --LET v_ejecuta_sh = " sh /opt/Interpel/Scripts/SACI_TRANSF_KEY.sh"
   LET v_ejecuta_sh = "sh /safreviv_int/ret/envio/SACI_DAPSFA_KEY.sh"
   RUN v_ejecuta_sh
