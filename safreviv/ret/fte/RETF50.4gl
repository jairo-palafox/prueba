...
#Modulo       => RET                                                                    #
#Programa     => RETF50                                                                 #
#Objetivo     => Programa para capturar los montos notificados de retiros tipo N        #
...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
   ...
   CALL fn_captura_saldos_notificados_ret_tipoN(p_usuario_cod)

END MAIN

{ ======================================================================
Clave: 
Nombre: fn_captura_saldos_notificados_ret_tipoN
...
Ivan Vega      25 Feb 2013           - Se permite una tolerancia de hasta 1 peso de diferencia
                                       entre el monto capturado y monto notificado
======================================================================
}
FUNCTION fn_captura_saldos_notificados_ret_tipoN(p_usuario_cod)
DEFINE p_usuario_cod                  LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
   ...
   OPEN WINDOW w_captura_montos WITH FORM "RETF501"
   ...
   LET v_continuar = TRUE
     ...
     ATTRIBUTES ( UNBUFFERED )
        ...
        ON ACTION ACCEPT
           ...
           SELECT SUM(total_viv92)
           INTO   v_total_importe_archivo
           FROM   ret_cza_tipo_n
           WHERE  folio = r_ret_mto_notificado.folio
           ...
           DISPLAY "En archivo: ", v_total_importe_archivo
           DISPLAY "Capturado: ", r_ret_mto_notificado.total_importe  
           ...
           IF ( v_diferencia > g_tolerancia_max_tipo_n ) THEN           
           ...
           INTO   v_f_operacion_procesar_archivo
           FROM   ret_cza_tipo_n
           WHERE  folio = r_ret_mto_notificado.folio
        ...
        ON ACTION notifica_error
           ...
           ATTRIBUTES ( STYLE="dialog", COMMENT = p_mensaje )
              COMMAND "Aceptar"
                 ...
                 LET p_titulo = "Informe de inconsistencia en montos notificados RETIROS TIPO N"
                 ...
                 CALL fn_correo_proceso(g_pid, g_proceso_cod_ret_tipo_N, g_opera_cod_ret_tipoN_integracion,
...
DEFINE  p_usuario_cod        LIKE seg_usuario.usuario_cod -- clave del usuario firmado
   ...
   LET g_opera_cod   = g_opera_cod_ret_tipoN_integracion -- integracion
   ...
   --IF ( fn_valida_reverso(g_pid,g_proceso_cod,g_opera_cod) = 0 ) THEN
      ...
      LET v_s_comando = " nohup time fglrun ",g_reg_modulo.ruta_exp CLIPPED,"/RETR65 ",
