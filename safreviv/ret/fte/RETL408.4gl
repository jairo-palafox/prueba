...
#Modulo       => RET                                                                    #
#Programa     => RETL408                                                                #
#Objetivo     => Invoca la carga de la marca de embargos con nuevo layout               #
...
MAIN
   ...
   LET g_proceso_cod = g_proceso_marca_embargo_todas -- carga de marca de embargos
   LET g_opera_cod   = g_opera_marca_embargo_todas_carga -- carga de archivo
   ...
   IF ( v_rest_valida = 0 ) THEN
      ...
      CALL fn_carga_archivo_ret(g_pid, g_proceso_cod, g_opera_cod, 2, "RETL408",
...
DEFINE v_layout          LIKE cat_operacion.layout_cod,
   ...
   CALL STARTLOG(p_usuario CLIPPED||".RETL408.log")
      ...
      DISPLAY "inicio de before input"
         ...
         ON ACTION ACCEPT           
            ...
            IF ( fn_verifica_permisos_archivo(g_ruta_rescate ,v_nom_archivo) ) THEN
               ...
               --  Inicia operación
               CALL fn_actualiza_opera_ini(p_pid,p_proceso,p_operacion,c_folio,"RETL408",
              ...
              WHERE  modulo_cod = 'ret'
               ...
               -- Construye comando
               LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/RETE408.42r "
                                              ,p_usuario," "
                                              ,v_aux_pid," "
                                              ,v_aux_proc," "
                                              ,v_aux_opera
