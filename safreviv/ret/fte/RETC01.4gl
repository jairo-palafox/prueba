...
#Modulo       => RET                                                                    #
#Programa     => RETC01                                                                 #
#Objetivo     => Consulta de datos rechazados de retiros por disposicion de             #
#                recursos                                                               #
...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
   ...
   LET g_proceso_cod = g_proceso_cod_ret_disposicion -- retiros por disposicion
   ...
   -- consulta datos rechazados de retiros por disposicion de recursos
   CALL fn_consulta_rechazo_ret_disposicion(p_usuario_cod)

END MAIN

{ ======================================================================
Clave: RETC01
Nombre: fn_consulta_rechazo_ret_disposicion
Fecha creacion: Abril 12, 2012
Autor: Ivan Vega, EFP
Narrativa del proceso que realiza:
Consulta los datos rechazados en integracion de retiros por
...
FUNCTION fn_consulta_rechazo_ret_disposicion(p_usuario_cod)
DEFINE p_usuario_cod            LIKE seg_usuario.usuario_cod, -- clave del usuario
   ...
   OPEN WINDOW w_consulta_rechazo_ret_disposicion WITH FORM "RETC01"
   ...
   ATTRIBUTES (UNBUFFERED)
      ...
      BEFORE INPUT
         ...
         DECLARE cur_folios CURSOR FOR
           SELECT DISTINCT g.*
            ...
            WHERE g.proceso_cod = g_proceso_cod -- retiros por disposicion de recursos
              AND ( g.folio IN (
                   SELECT DISTINCT a.folio FROM ret_disposicion_rch a
                   )
                   OR g.folio IN (
                   SELECT DISTINCT b.folio FROM ret_cza_disposicion_rch b
                   )
                  )
         ...
         FOREACH cur_folios INTO v_r_glo_ctr_archivo.*
            ...
            -- se agrega el elemento al combo de folios
      ...
      ON ACTION ACCEPT
         IF ( v_folio IS NULL ) THEN
            CALL fn_mensaje("Atención","Debe elegir un folio","info")
         ...
         -- se cuentan los errores por folio
         SELECT COUNT(*)
           INTO v_si_indice
           FROM ret_cza_disposicion_rch 
          WHERE folio =  v_d_folio
         
         IF ( v_si_indice <= 0 ) THEN
            -- Se muestran errores detalle de archivo
            -- Se envian los parametros de consulta
            CALL fn_muestra_consulta_rechazo_detalle(v_folio                 ,
                                                     v_dt_fecha_carga_inicial,
                                                     v_dt_fecha_carga_final)
         ELSE
            -- Se muestran errores de sumario o encabezado de archivo
            CALL fn_muestra_rechazo_sumario_encabzado(v_folio,
                                                      v_dt_fecha_carga_inicial,
                                                      v_dt_fecha_carga_final)
         END IF
   ...
   CLOSE WINDOW w_consulta_rechazo_ret_disposicion
...
FUNCTION fn_muestra_consulta_rechazo_detalle(p_folio                 ,
...
DEFINE p_folio                  STRING, -- folio
   ...
   LET v_s_sql =  "SELECT 0, b.id_ret_matriz_derecho, a.tpo_retiro, 999, SUM(b.aivs_viv92),"
                 ,"\n SUM(b.aivs_viv97), 999, 999,"
                 ,"\n 999, 999, b.f_solicitud, b.folio"
                 ,"\n FROM ret_matriz_derecho a, ret_disposicion_rch b"
   ...
   DISPLAY "Consulta de detalle:", v_s_sql
...
DEFINE p_dt_fecha_carga_inicial LIKE glo_ctr_archivo.f_actualiza,
   ...
   LET v_s_sql = v_s_sql || " proceso_cod = 16\n"
   ## FJNM
   ...
   FOREACH cur_folioscarga INTO v_folio
      DISPLAY "Al menos encontre un dato-",v_folio,"-"
...
DEFINE p_s_claves_afores       STRING -- codigo de afore
   ...
   LET v_s_qry =
       ...
       ,"\n      ,l.campo_desc"
       ,"\n FROM (safre_viv:deo_cat_rechazo r LEFT OUTER JOIN safre_viv:cat_campo l"
       ,"\n   ON r.layout_cod = l.layout_cod"
       ,"\n  AND r.registro   = l.registro"
       ,"\n  AND r.campo_cod  = l.campo_cod) RIGHT OUTER JOIN safre_viv:deo_det_op98_rch i"
...
======================================================================
Clave: DEOC04
Nombre: fn_muestra_rechazo_sumario_encabzado
...
FUNCTION fn_muestra_rechazo_sumario_encabzado(
...
DEFINE  p_folio                CHAR(10)
   ...
   LET v_s_qry =
       ...
       ,"\n   ON r.layout_cod = l.layout_cod"
       ,"\n  AND r.registro   = l.registro"
       ,"\n  AND r.campo_cod  = l.campo_cod) RIGHT OUTER JOIN safre_viv:deo_cza_op98_rch b"
