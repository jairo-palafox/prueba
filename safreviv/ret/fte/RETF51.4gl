...
#                retiro Tipo N para rechazar casos especificos                          #
...
DEFINE p_usuario_cod        LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
       ...
       v_consec_cuenta      LIKE afi_decreto.consec_cuenta, -- consecutivo cuenta de decreto
       v_nombre             LIKE ret_tipo_n.nombre, -- nombre completo del trabajador
       ...
       v_id_decreto         LIKE afi_decreto.id_decreto, -- ID DECRETO
   ...
   AND   b.opera_cod   = g_opera_cod_ret_tipoN_preliquidacion -- preliquidacion
   ...
   IF ( g_pid IS NULL ) THEN
      LET v_cadena = "No existe un proceso de Retiros Tipo N ejecutándose en este momento,\n",
   ...
   OPEN WINDOW w_valida_manual WITH FORM "RETF511"
   ...
   -- se captura el consecutivo cuenta de la solicitud que se desea rechazar
   INPUT BY NAME v_consec_cuenta
   WITHOUT DEFAULTS
   ATTRIBUTES (UNBUFFERED)

      ON ACTION ACCEPT
         IF ( v_consec_cuenta IS NULL ) THEN
            CALL fn_mensaje("Atención","Es necesario capturar un Núm. de Identificación Único","stop")
            CONTINUE INPUT
         END IF
         ...
         AND    a.consec_cuenta = v_consec_cuenta
         AND    b.cod_rechazo   = 0 -- solo aceptadas
         
         -- Si no se encontro solicitud
         IF ( v_id_solicitud IS NULL ) THEN
            CALL fn_mensaje("Atención","No se encontró una solicitud con el Núm. de Identificación Único dado","stop")
            CONTINUE INPUT
         END IF  
