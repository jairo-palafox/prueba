...
MAIN
   ...
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".RETS250.log")
...
DEFINE p_folio        LIKE glo_folio.folio,
   ...
   IF ( v_conteo < 1 ) THEN
      ...
      LET p_mensaje = "ID Proceso  : ", g_pid, "\n", 
                      "Proceso      : RETIRO PARCIAL GENERICO\n",
   ...
   ELSE -- se ejecuta la generación de archivo
      ...
      LET v_nom_archivo = "SACI_PARCIAL", TODAY USING "yyyymmdd"
      ...
      DISPLAY "~~~~~~~~~~~"
      DISPLAY "Archivo de solicitudes de retiro parcialidades: ", v_v_ruta_nomarch
      ...
      LET v_sql="\n SELECT rs.id_derechohabiente       ,                     ",
                ...
                "\n WHERE  rcp.estado_solicitud      = 60                    ",
                "\n AND    rs.modalidad_retiro       = 11                    ", -- modalidades de retiro que no son fondo de ahorro
                "\n AND    rb.id_solicitud           = rs.id_solicitud       ",
                "\n AND    rcp.id_solicitud          = rb.id_solicitud       ",
                "\n AND    rps.id_solicitud          = rcp.id_solicitud      ",
                "\n AND    rps.consec_beneficiario   = rb.consec_beneficiario",
                "\n AND    rs.id_derechohabiente     = afi.id_derechohabiente",
                "\n AND    MONTH(rcp.f_estimada_pago) = MONTH(TODAY)         ",
                "\n AND    YEAR(rcp.f_estimada_pago)  = YEAR(TODAY)          "
      ...
      FOREACH cur_ret_generico INTO v_id_derechohabiente       ,
         ...
         SELECT num_parcialidades
         INTO   v_num_parcialidades
         FROM   ret_parcial_generico p
         ...
         -- el concepto de parcialidades aun no se define
         LET r_det_arch_fico.v_concepto = "  18"
         ...
         CASE v_modalidad_retiro
            ...
            -- =====================================================================================
            -- =====================================================================================
            --                 CONCEPTOS DE FICO SEGUN MODALIDAD DE RETIRO
            -- =====================================================================================
            ...
            WHEN 2 -- fondo de ahorro
               ...
               SELECT SUM(saldo_viv72 + tanto_adicional)
               INTO   v_monto_pesos
               FROM   ret_fondo_ahorro_generico
               WHERE  id_solicitud = v_id_solicitud
               
               -- el concepto de amortizaciones excedentes es 17. Alineado a la derecha
      ...
      LET v_comando = "cp ",v_v_ruta_nomarch," ",v_c_ruta_env_acr CLIPPED,"/SACI_PARCIAL.TXT"
      ...
      LET v_comando = "shasum ", v_c_ruta_env_acr CLIPPED, "/SACI_PARCIAL.TXT > ", v_c_ruta_env_acr CLIPPED, "/SACI_PARCIAL.KEY"
      ...
      LET v_ejecuta_sh = "sh /safreviv_int/ret/envio/SACI_TRANSF.sh"
      RUN v_ejecuta_sh

      --LET v_ejecuta_sh = " sh /opt/Interpel/Scripts/SACI_TRANSF_KEY.sh"
      LET v_ejecuta_sh = "sh /safreviv_int/ret/envio/SACI_TRANSF_KEY.sh"
      RUN v_ejecuta_sh
