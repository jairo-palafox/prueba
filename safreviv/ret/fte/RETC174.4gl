...
#MODULO       => RET                                                                    #
#PROGRAMA     => RETC174                                                                #
#OBJETIVO     => Consulta de retiros por disposicion de Recursos PMG                    #
...
por disposicion de recursos PMG
...
DEFINE cbx_tipo_retiro      ui.ComboBox, -- combo de tipo de retiro
       ...
       ar_ret_rechazo          RECORD LIKE cat_rch_ret_procesar.*, -- registro con los codigos de rechazo
   ...
   OPEN WINDOW w_consulta WITH FORM "RETC1741"
   ...
   ATTRIBUTES ( UNBUFFERED )
      ...
      BEFORE INPUT
         ...
         SELECT a.folio
         FROM   glo_folio a
         WHERE  a.proceso_cod = g_proceso_cod_ret_disposicion_pmg -- PMG
         AND    a.status >= 0
         ORDER BY folio DESC
...
{         
         ...
         SELECT *
         FROM   cat_rch_ret_procesar  -- se integra esta tabla para considerar los rechazos segun PROCESAR
         WHERE  cod_rechazo <> 0
         ...
         FOREACH cur_codrechazo INTO ar_ret_rechazo.*
            LET v_cadena = ar_ret_rechazo.cod_rechazo, " - ", ar_ret_rechazo.desc_corta
      ...
      ON ACTION accept
         ...
         IF ( v_fecha_inicio IS NULL OR v_fecha_fin IS NULL ) AND v_folio IS NULL  THEN
         ...
         IF ( v_etapa = 1 ) THEN
            ...
            CALL fn_consulta_disposicion_solicitud(v_tipo_retiro  ,
                                           v_etapa        ,
                                           v_nss          ,
                                           v_folio        ,
                                           v_estado       ,
         ...
         ELSE       
            ...
            CALL fn_consulta_disposicion_preliquidacion_liquidacion(v_tipo_retiro  ,
                                                            v_etapa        ,
                                                            v_nss          ,
                                                            v_folio        ,
                                                            v_estado       ,
...
======================================================================
Clave: 
Nombre: fn_consulta_disposicion_solicitud
Fecha creacion: junio 10, 2013
Autor: Ivan Vega, EFP
Narrativa del proceso que realiza:
...
FUNCTION fn_consulta_disposicion_solicitud(v_tipo_retiro, v_etapa, v_nss, v_folio, v_estado, v_cod_rechazo,
...
DEFINE v_tipo_retiro        LIKE ret_tipo_retiro.tpo_retiro,
   ...
   LET v_sql = "\n SELECT                 ",
               ...
               "\n AND a.folio = b.folio ",
               "\n AND b.proceso_cod = ", g_proceso_cod_ret_disposicion_pmg, -- PMG
   ...
   LET v_sql = "\n SELECT                 ",
               ...
               "\n      ret_tipo_retiro e ",
               "\n WHERE 1 = 1            ",
               "\n AND a.tipo_retiro = e.tpo_retiro ",
               "\n AND a.folio = b.folio ",
               "\n AND b.proceso_cod = ", g_proceso_cod_ret_disposicion_pmg, -- PMG
               "\n AND b.status >= 0 " -- folios integrados a liquidados
   ...
   -- si se capturaron fechas
   IF v_fecha_inicio IS NOT NULL AND v_fecha_fin IS NOT NULL THEN 
       LET v_sql = v_sql || " AND a.f_solicitud BETWEEN '", v_fecha_inicio, "' AND '", v_fecha_fin, "'"
   END IF 
   ...
   -- tipo de retiro especifico
   IF ( v_tipo_retiro IS NOT NULL ) THEN
      LET v_sql = v_sql || "\n AND a.tipo_retiro = '", v_tipo_retiro, "'"
   END IF
...
{   
   ...
   OPEN WINDOW w_consulta1 WITH FORM "RETC1742"
...
{   
   ...
   OPEN WINDOW w_consultadetallada WITH FORM "RETC1743"
...
======================================================================
Clave: 
Nombre: fn_consulta_disposicion_preliquidacion_liquidacion
Fecha creacion: junio 10, 2013
Autor: Ivan Vega, EFP
Narrativa del proceso que realiza:
Realiza la consulta de los datos de retiro PMG de acuerdo con los parametros
...
FUNCTION fn_consulta_disposicion_preliquidacion_liquidacion(v_tipo_retiro, v_etapa, v_nss, v_folio, v_estado, v_cod_rechazo,
                                  v_fecha_inicio, v_fecha_fin)
DEFINE v_tipo_retiro        LIKE ret_tipo_retiro.tpo_retiro,
       v_etapa              SMALLINT, -- 1 Solicitud, 2 Preliquidacion, 3 Liquidacion
       v_nss                LIKE afi_derechohabiente.nss, 
   ...
   LET v_sql = v_sql, "\n afi_derechohabiente b,",
                      ...
                      "\n AND   c.proceso_cod = ", g_proceso_cod_ret_disposicion_pmg, -- Disposicion PMG
   ...
   OPEN WINDOW w_consultapreliq WITH FORM "RETC1744"
