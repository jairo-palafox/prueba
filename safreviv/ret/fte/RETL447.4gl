...
#Modulo            => RET                                                       #
#Programa          => RETL447                                                   #
#Objetivo          => Programa lanzador para integrar archivos de respuesta FICO#
#                     de las notificaciones de pago de las Excepciones de la    #
#                     Devolución del SSV                                        #
...
DEFINE v_tipo_proceso    SMALLINT,   -- Forma como ejecutara el programa 
   ...
   -- se asignan los codigos de proceso
   LET g_proceso_cod   = g_proceso_excep_devol_ssv_resp_fico -- respuesta de FICO pago por DAP
   LET g_opera_cod     = g_opera_resp_excep_devol_ssv_integra -- integracion
   LET g_opera_cod_ant = g_opera_resp_excep_devol_ssv_carga -- carga
   ...
   -- se invoca la integracion del archivo
   CALL fn_integra_resp_fico_dap()
   
END MAIN
#Objetico: Función que permite seleccionar y arrastrar el archivo a integrar
FUNCTION fn_integra_resp_fico_dap()
DEFINE l_arr_arch_pend    DYNAMIC ARRAY OF VARCHAR(100), -- Archivos pendientes 
   ...
   OPEN WINDOW w_inte_fico WITH FORM "RETL4471"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION integrar
         ...
         IF r_bandera = 0 THEN  
            ...
            CALL fn_actualiza_opera_ini (v_max_pid,g_proceso_cod,g_opera_cod,g_folio,"RETP447",l_v_arch_proceso,v_usuario)
