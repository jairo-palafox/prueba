#############################################################################
#Módulo          => RET                                                     #        
#Programa        => RETE320.4gl                                             #
#Objetivo        => Programa carga de archivo notificacion DAPS vencidos    #
#Fecha Inicio    => 27 MARZO 2014                                           #
#############################################################################
DATABASE safre_viv
GLOBALS 
...
DEFINE p_nom_archivo       STRING,
   ...
   LET v_cadena_registros = p_usuario CLIPPED, ".", "RETE320.log"
   ...
   DISPLAY "Generando tabla temporal tmp_ret_not_dap_vencido"

   -- cambio a base de datos temporal
   DATABASE safre_tmp

   -- se regenera la tabla temporal
   LET v_sql = "\nDROP TABLE IF EXISTS tmp_ret_not_dap_vencido"
   ...
   LET v_sql = "\nCREATE TABLE tmp_ret_not_dap_vencido (", 
               "\n id_registro            CHAR(1),   ",
               "\n num_docto              CHAR(10),  ",
               "\n posicion               CHAR(3),   ",
               "\n anio                   CHAR(4),   ",
   ...
   WHILE ( v_ch_archivo.read([v_registro_archivo.*]) )
       ...
       IF v_reg_archivo > 6 THEN 
           DISPLAY "Registro leido (REFERENCIA): ", v_registro_archivo.referencia
         
           -- se inserta un registro
           INSERT INTO tmp_ret_not_dap_vencido values (v_registro_archivo.*)
   ...
   IF ( v_reg_archivo = (v_reg_aceptados + 6)) THEN
