...
DEFINE l_s_qryTxt                      STRING, -- guarda una sentencia SQL a ejecutar
   ...
   LET v_fecha_reg       = TODAY
      ...
      CALL fn_display_proceso(0, "INTEGRACION DE RESPUESTA DE FICO")
   ...
   LET  v_hash_es_correcto = TRUE
      ...
      IF ( v_hash_es_correcto ) THEN
         ...
         PREPARE prp_carga_disp
            FROM "EXECUTE FUNCTION sp_integra_fico(?,?,?,?)"
         ...
         IF ( v_error < 0 ) THEN
            DISPLAY "Error al ejecutar el SP de integración de respuesta de FICO"     
         ...
         ELSE
            ...
            LET p_mensaje = "El proceso de integración de la respuesta de FICO ha finalizado correctamente."
            DISPLAY "\n", p_mensaje
                                 
            -- se crea el titulo del mensaje
            LET p_titulo = "Finalización de operación - INTEGRACIÓN ARCHIVO FICO"     
         END IF
      ELSE
         -- EL HASH NO SE VALIDO CORRECTAMENTE. EL PROCESO SE MARCA EN ERROR
         LET p_mensaje = "El archivo ", v_nom_archivo, " de respuesta de FICO no cumple con la validación del archivo KEY. No se procesará el archivo. Es necesario regenerar y reenviar el archivo."
         DISPLAY p_mensaje
      ...
      -- se envia la cadena que indica el fin de la etapa
      CALL fn_display_proceso(1, "INTEGRACION DE RESPUESTA DE FICO")
