...
#                del detalle de retiro de fondo ahorro por traspasos                    #
...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
   ...
   LET g_proceso_cod = g_proceso_cod_ret_fondo_ahorro_trasp -- retiro de fondo ahorro traspasos
   LET g_opera_cod   = g_opera_fondo_ahorro_trasp_integra -- integracion
...
DEFINE p_usuario_cod LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
   ...
   LET l_s_qryTxt = "SELECT nombre_archivo"
                    ...
                    ,"\n   AND opera_cod   = ", g_opera_fondo_ahorro_trasp_carga --,g_opera_cod que tiene la carga 
   ...
   LET l_i_indice = 0
   -- Se inicializa la variable de la cantidad de registros a procesar
   ...
   OPEN WINDOW w_inte_devol WITH FORM "RETL4601"
   DIALOG ATTRIBUTE(UNBUFFERED)
      ...
      ON ACTION integrar
         ...
         IF ( v_respuesta = 1 ) THEN
            ...
            FROM   safre_tmp:tmp_ret_det_fa_traspaso
            WHERE  LENGTH(TRIM(contenido)) = 122
            AND    LENGTH(TRIM(SUBSTR(contenido,4,13)))   = 13  --- RFC
            AND    LENGTH(TRIM(SUBSTR(contenido,66,11)))  = 11  --- NSS
            AND    LENGTH(TRIM(SUBSTR(contenido,81,13)))  = 13  --- credito
            AND    LENGTH(TRIM(SUBSTR(contenido,101,10))) = 10  --- fecha
            AND    LENGTH(TRIM(SUBSTR(contenido,111,20))) > 2   --- Importe
            
            IF v_regs_a_procesar > 0 THEN 
               CALL fn_ret_ejecuta_integracion_fondo_ahorro(p_usuario_cod, l_v_arch_proceso)
            ELSE 
               CALL fn_ventana_confirma("Retiros",
               "El archivo no contiene registros que cumplan los criterios para procesarlo\n"||
               "¿Desea reversar la carga para procesar otro archivo?",
               "quest") RETURNING v_respuesta
               IF v_respuesta = 1 THEN 
                  ...
                  WHERE  pid = g_pid
                  AND    opera_cod = 1;
                  DELETE FROM glo_ctr_archivo   WHERE nombre_archivo = l_v_arch_proceso
               END IF 
...
======================================================================
Clave: 
Nombre: fn_ret_ejecuta_integracion_fondo_ahorro
Fecha creacion: Noviembre 22, 2017
Autor: Ricardo Pérez, EFP
Narrativa del proceso que realiza:
Ejecuta la integracion de retiros de traspaso fondo ahorro
para un folio dado
...
FUNCTION fn_ret_ejecuta_integracion_fondo_ahorro(p_usuario_cod, v_archivo)
DEFINE p_usuario_cod      LIKE seg_usuario.usuario_cod, -- usuario que ejecuta el programa
   ...
   --DISPLAY  "fn_valida_operacion",g_pid,g_proceso_cod,g_opera_cod
   CALL fn_valida_operacion(g_pid,g_proceso_cod,g_opera_cod) RETURNING v_i_resultado
   --DISPLAY "v_i_resultado", v_i_resultado 
   IF ( v_i_resultado = 0 ) THEN
       -- Inicio operacion.
      CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,g_opera_cod,v_folio,
                                "RETL460",v_archivo,p_usuario_cod) RETURNING r_resultado_opera
              
      IF( r_resultado_opera = 0 ) THEN
         LET v_s_comando = " nohup time fglrun ",g_reg_modulo.ruta_exp CLIPPED,"/RETP460 ",
   ...
   ELSE
      ...
      CALL fn_mensaje("Atención", v_mensaje|| v_i_resultado, "stop")
