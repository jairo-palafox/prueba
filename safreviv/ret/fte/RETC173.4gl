...
#Modulo       => RET                                                                    #
#Programa     => RETC173                                                                #
#Objetivo     => Consulta sdos insuficientes en solicitudes de retiro por PMG           #
#Fecha inicio => Octubre 25, 2012                                                       # 
#########################################################################################
...
Fecha creacion: Octubre 25, 2012
Registro de modificaciones:
Descrip: CONSULTA SALDOS INSUFICIENTES EN SOLICITUDES DE RETIRO POR PMG
==============================================================================
}
...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- clave del usuario
   ...
   OPEN WINDOW w_consulta_saldos WITH FORM "RETC1731"
   LET  v_ventana = UI.WINDOW.GETCURRENT()
   CALL v_ventana.SETTEXT("Consulta Saldos Insuficientes Pensión Mínima Garantizada (PMG)")
   ...
   INPUT v_folio, v_afore, v_nss, v_variacion, v_f_inicial, v_f_final 
      ...
      BEFORE INPUT 
         ...
         SELECT a.*
         FROM   glo_folio a
         WHERE  a.proceso_cod = g_proceso_cod_ret_disposicion_pmg
         AND    a.status >= 1
         ORDER BY folio DESC
      ...
      ON ACTION ACCEPT
         ...
         LET v_query_agrupador = "\n SELECT DISTINCT b.folio                         ,",
                                 ...
                                 "\n AND gf.proceso_cod = ", g_proceso_cod_ret_disposicion_pmg,
                                 "\n AND gf.status >= 0                               "
         ...
         OPEN WINDOW w_detalle_consulta_saldos WITH FORM "RETC1732"
         ...
         ATTRIBUTES ( UNBUFFERED )
            ...
            DISPLAY ARRAY v_arr_agrupador TO tbl_grupo_afore.*
               BEFORE ROW
                  ...
                  arr_reg_sds.precio_fondo
                     ...
                     LET arr_reg_sds.aivs97_dif        = arr_reg_sds.aivs97_sol       - arr_reg_sds.aivs97_sdo     
                     LET arr_reg_sds.aivs97_dif_pesos  = arr_reg_sds.aivs97_sol_pesos - arr_reg_sds.aivs97_sdo_pesos
            ...
            ON ACTION Exportar
                ...
                LET v_nom_archivo = "PMG_", p_usuario_cod CLIPPED, "_", TODAY USING "yyyymmdd"
            ...
            ON ACTION reporte                      
               ...
               IF ( fgl_report_loadCurrentSettings("../../ret/bin/RETC173.4rp") ) THEN 
               ...
               ELSE         
                  CALL fn_mensaje("Atención","No fue posible generar el reporte. No se encuentra la plantilla RETC173.4rp", "stop")
                  CONTINUE DIALOG
               END IF   
