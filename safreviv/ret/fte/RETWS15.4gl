...
DEFINE ws_ret_generico_solicitud_in RECORD
         ...
         arr_beneficiario DYNAMIC ARRAY OF RECORD
              ...
              numAcuerdo         CHAR(15),
              descJuez           CHAR(100),
              nombreFacultado    CHAR(40),
              puestoFacultado    CHAR(50),
              fechaEjecucion     CHAR(10),
              procedeJuicio      CHAR(40)
         END RECORD
       END RECORD,
...
DEFINE v_resultado       INTEGER, -- recibe el resultado de la ejecucion del servicio 
  ...
  LET v_ruta_log = v_ruta_ejecutable CLIPPED, "/RETWS15."
  ...
  IF FGL_GETENV("RETWS15LOG") THEN
     CALL STARTLOG(FGL_GETENV("RETWS15LOG"))
     DISPLAY "Ruta del log creada del servidor: " || FGL_GETENV("RETWS15LOG")
  ELSE
     DISPLAY "Ruta del log creada del servidor: ", v_ruta_log
  ...
  IF num_args() = 2 AND arg_val(1) = "-W" THEN
      ...
      CALL fn_crea_servicio_4_grupos_ley73(TRUE)
      EXIT PROGRAM
  ELSE 
  ...
  CALL fn_crea_servicio_4_grupos_ley73(FALSE)
...
======================================================================
Clave: 
Nombre: fn_crea_servicio_4_grupos_ley73
...
FUNCTION fn_crea_servicio_4_grupos_ley73(p_generar_WSDL)
  ...
  TRY
    -- =============================
    -- se crea el servicio
    LET v_webservice = com.WebService.CreateWebService("retiroCreaSolicitud4GruposLey73", v_service_NameSpace)
  
    -- =============================
    -- Publicacion de las funciones
    
    -- fn_retiro 
    LET op = com.WebOperation.CreateDOCStyle("fn_4_grupos_solicitud_ley73","fn_4_grupos_solicitud_ley73",ws_ret_generico_solicitud_in,ws_ret_generico_solicitud_out)
    --LET op = com.WebOperation.CreateDOCStyle("fn_retiro","fn_retiro",ret_retiro_fondo,ret_respuesta)
    CALL v_webservice.publishOperation(op, "fn_4_grupos_solicitud_ley73")
...
======================================================================
Clave: 
Nombre: fn_4_grupos_solicitud_ley73
Fecha creacion: Noviembre 30, 2017
Autor: Ivan Vega, EFP
Narrativa del proceso que realiza:
...
FUNCTION fn_4_grupos_solicitud_ley73()
DEFINE v_indice_retiro       SMALLINT,
       ...
       v_error_beneficiarios SMALLINT ,
       v_clabe_duplicada     SMALLINT 
   ...
   IF ( v_cta_clabe_correcta = TRUE ) THEN
      ...
      ELSE   
         ...
         IF ws_ret_generico_solicitud_in.grupo = 1 AND 
            ws_ret_generico_solicitud_in.medio_entrega = 1 THEN
         ...
         FOR v_indice_beneficiario = 1 TO ws_ret_generico_solicitud_in.arr_beneficiario.getLength()
            ...
            IF ( ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].tipo_beneficiario   IS NULL OR
               ...
               ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].entidad_federativa  IS NULL OR 
               ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].entidad_federativa < 1 OR 
               ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].entidad_federativa > 32 ) THEN
               LET v_error_beneficiarios = 0
               IF ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].tipo_beneficiario   IS NULL THEN 
                  LET v_error_beneficiarios = gi_benef_sin_tipo_beneficiario
               END IF 
               IF ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].rfc IS NULL THEN 
                  LET v_error_beneficiarios = gi_benef_sin_rfc
               END IF
               --IF ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].email IS NULL THEN 
               --   LET v_error_beneficiarios = gi_benef_sin_email
               --END IF 
               IF ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].nombre IS NULL THEN 
                  LET v_error_beneficiarios = gi_benef_sin_nombre
               END IF 
               IF ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].ap_paterno IS NULL THEN 
                  LET v_error_beneficiarios = gi_benef_sin_paterno
               END IF 
               IF (ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].entidad_federativa IS NULL OR 
                   ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].entidad_federativa < 1 OR 
                   ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].entidad_federativa > 32 ) THEN 
                  LET v_error_beneficiarios = gi_benef_sin_entidad_federativa
               END IF 
            ...
            ELSE
               ...
               --   EXIT FOR
               --END IF
               -- Se valida la CLABE para que no exista para otra solicitud con diferente NSS
               --CALL fn_valida_solicitud_clabe(v_nss, 
               --                               ws_ret_generico_solicitud_in.caso_adai, 
               --                               ws_ret_generico_solicitud_in.arr_beneficiario[v_indice_beneficiario].clabe_bancaria) 
               --     RETURNING v_clabe_duplicada
               --IF v_clabe_duplicada > 0 THEN  
               --   LET v_cta_clabe_correcta = FALSE
               --END IF 
               --EXIT FOR
            END IF   
            IF ws_ret_generico_solicitud_in.grupo = 1 AND ws_ret_generico_solicitud_in.medio_entrega = 1 AND v_indice_beneficiario = 1 THEN 
               EXIT FOR
            END IF 
         END FOR
         ...
         IF ( NOT v_existe_beneficiario OR NOT v_cta_clabe_correcta ) THEN
            ...
            IF ( NOT v_existe_beneficiario ) THEN
               CALL fn_respuesta_ws(v_nss, v_rfc, 3,
                                  gi_solicitud_rechazada, v_error_beneficiarios, 0, 0, NULL)
            ELSE
               -- se rechaza por cuenta clabe duplicada
               CALL fn_respuesta_ws(v_nss, v_rfc, 3,
...
--         CALL ERRORLOG("c_sha prueba>"||v_c_sha||"<")
      ...
      FOR v_conteo = 1 TO ws_ret_generico_solicitud_in.arr_beneficiario.getLength()
         ...
         IF ws_ret_generico_solicitud_in.grupo = 4 AND 
            --(v_subgrupo = 104 OR v_subgrupo = 124) THEN 
            (v_subgrupo = 104) THEN 
         ...
         IF ws_ret_generico_solicitud_in.grupo = 1 AND
            ...
            IF (ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].porcentaje = 0 OR 
               ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].porcentaje IS NULL ) THEN
               LET ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].porcentaje = 100
            END IF 
         END IF 
         CALL fn_registra_beneficiario_retiro_generico(
            ...
            ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].porcentaje,
            --100,
            v_total_aivs * (ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].porcentaje/100),
            v_total_pesos * (ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].porcentaje/100),
         ...
         IF ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].tipo_beneficiario <> 1 THEN 
            CALL fn_registra_beneficiario_retiro_generico_juridico(p_id_solicitud,
               ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].juicio,
               ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].numAcuerdo,
               ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].descJuez,
               ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].nombreFacultado,
               ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].puestoFacultado,
               ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].fechaEjecucion,
               ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].procedeJuicio,
               ws_ret_generico_solicitud_in.arr_beneficiario[v_conteo].tipo_beneficiario)
         END IF 
      END FOR      
   ELSE
      ...
      IF ws_ret_generico_solicitud_in.grupo = 1 THEN 
         -- Se desmarca en Procesar
         CALL fn_consulta_saldo_vivienda_afore(p_nss, 60)  -- Desmarca en Procesar
              RETURNING v_diagnostico, v_estatus, p_aivs_viv92, p_pesos_viv92, p_aivs_viv97, p_pesos_viv97, p_rechazo
         IF v_diagnostico = 127 THEN 
             CALL fn_guarda_consulta_ws_vent_afore(p_nss, 2, 4, TODAY, CURRENT HOUR TO SECOND, v_diagnostico, v_estatus,
                                                   p_aivs_viv92, p_aivs_viv97, 'OPSISSACI', p_id_solicitud, ws_ret_generico_solicitud_in.caso_adai, 1) -- Se deberá reenviar la solicitud de marca 
         ELSE 
