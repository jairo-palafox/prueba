-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
-- Módulo          => RET                                                     -- 
-- Programa        => RETGE03.4gl                                             -- 
-- Objetivo        => Programa general de carga de archivo                    -- 
-- Fecha Inicio    => 25 Noviembre 2015                                       -- 
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
IMPORT os
DATABASE safre_viv
...
CONSTANT c_folio = 0
   ...
   CALL STARTLOG(p_usuario CLIPPED||".RETGE03.log")
      ...
      INPUT BY NAME v_nom_archivo ATTRIBUTES (UNBUFFERED, ACCEPT = FALSE, CANCEL = FALSE)
         ...
         ON ACTION ACCEPT
            ...
            IF ( fn_verifica_permisos_archivo(g_ruta_rescate ,v_nom_archivo) ) THEN
               ...
               --  Inicia operación
               CALL fn_actualiza_opera_ini(p_pid,p_proceso,p_operacion,c_folio,"RETGE03",
               ...
               -- Construye comando
               LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/RETGE04.42r "
                                              ,p_usuario," "
                                              ,v_aux_pid," "
                                              ,v_aux_proc," "
                                              ,v_aux_opera
...
DEFINE p_ruta_rescate      STRING,--LIKE seg_modulo.ruta_rescate,
   ...
   --DISPLAY " *******************registro entro" ,v_reg_layout.registro --ERV 
      ...
      CASE v_reg_layout.tipo_carga
         ...
         WHEN 3   
            DISPLAY  "Entra a funcion fn_load_archivo_tabla"         
      ...
      DISPLAY "SE ENCONTRARON EN LA TABLA ", v_reg_procesados
...
DEFINE p_ruta_rescate   STRING,--LIKE seg_modulo.ruta_rescate,
   ...
   CALL  v_canal.setDelimiter(NULL)
   -- Indica la tabla y campos a insrtar y las posiciones de cada campo
   
   WHENEVER ERROR STOP 
   WHILE  v_canal.read([v_tmp_campos.*])

      IF LENGTH(v_tmp_campos.nss) != 11 OR  v_tmp_campos.nss IS NULL THEN
          DISPLAY "No se integrara el NSS: ",v_tmp_campos.nss
      ELSE
          INSERT INTO safre_tmp:tmp_ret_multinss VALUES (v_tmp_campos.*)
      END IF
