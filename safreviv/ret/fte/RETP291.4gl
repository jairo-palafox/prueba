...
#                     retiros genéricos fondo ahorro 72                        #
#Autor             =>                                                          #
...
DEFINE p_pid              LIKE bat_ctr_operacion.pid,         # PID del proceso
   ...
   LET p_titulo = "Preliquidación restitución retiro genérico fondo ahorro 72"

   DISPLAY p_mensaje
   LET v_estado_solicitud          = gi_estado_restitucion_ret_generico
   LET v_estado_preliq_restitucion = gi_estado_preliq_restitucion_ret_generico
                      
   LET v_consulta = "EXECUTE FUNCTION fn_restitucion_retiros_genericos_72(?,?,?,?,?,?,?)"

   # se prepara la ejecucion del stored procedure para la preliquidacion
   PREPARE prp_restitucion_ret_generico72 FROM v_consulta
   EXECUTE prp_restitucion_ret_generico72 USING g_folio,
                                                g_proceso_cod,
                                                g_opera_cod,
                                                p_usuario_cod,
                                                g_pid,
