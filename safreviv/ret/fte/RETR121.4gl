...
DEFINE  p_s_titulo       STRING   -- titulo de la ventana       
   ...
   SELECT s.ruta_bin, s.ruta_rescate, s.ruta_listados
    ...
    DISPLAY "Realizando reverso de preliquidación..."
     
    CALL fn_reverso_ret_viv72_preliquidacion(p_folio, p_proceso_cod, p_opera_cod) RETURNING v_b_rev_pre

    -- si el reverso se realizo correctamente
    IF ( v_b_rev_pre = 0 ) THEN
       -- se reversa la operacion
       CALL fn_reversa_operacion(p_pid,p_proceso_cod,p_opera_cod)
       RETURNING v_b_rev_pre

       IF ( v_b_rev_pre <> 0 ) THEN
          DISPLAY "Ocurrió un error al reversar la operación de preliquidación."
       ELSE
          DISPLAY "Operación lista para volver a ejecutarse."
       END IF
...
DEFINE p_folio       DECIMAL(9,0),
   ...
   PREPARE prp_reversa_preli FROM "EXECUTE FUNCTION fn_reverso_ret_viv72_preliquidacion_cont( ?, ?, ? )"
