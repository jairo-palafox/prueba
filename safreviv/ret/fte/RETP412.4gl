...
#Modulo        => RET                                                          #
#Programa      => RETP412                                                      #
#Ojetivo       => Proceso para realizar la marca/desmarcar de solicitudes      #
#                 a PROCESAR.                                                  #
...
MAIN
   ...
   -- Log del proceso
   CALL STARTLOG(p_usuario_cod CLIPPED||".RETP412.log")
   ...
   --Encabezado para el archivo de monitoreo
   DISPLAY "Terminó el Batch de Marcas a Procesar"
   DISPLAY "*******************************************************************"
   DISPLAY " PROCESO            : ",v_proceso_desc
   DISPLAY " OPERACIÓN          : ",v_opera_desc
   DISPLAY " FECHA              : ",TODAY USING 'dd-mm-yyyy'
...
--                 " AND   a.estado_indicador = 1 "
   ...
   LET v_sql =   " SELECT DISTINCT a.id_solicitud, a.nss, a.estado_indicador, a.origen, b.caso_adai ",
                 ...
                 " AND   c.estado_indicador = 1 "

                 
   LET v_solicitudes_procesadas = 0

   PREPARE prp_solicitudes_marca_pendientes FROM v_sql
   DECLARE cur_solicitudes_marca_pendientes CURSOR FOR prp_solicitudes_marca_pendientes

   FOREACH cur_solicitudes_marca_pendientes INTO reg_marca_procesar_maestra.id_solicitud,
...
--      AND diagnostico = 127
      ...
      --Mando llamar funcionalidad de Marca a Procesar
      CALL fn_consulta_saldo_vivienda_afore(reg_marca_procesar_maestra.nss, 44)
      ...
      IF (v_diagnostico = 127) THEN
         ...
         CALL fn_guarda_consulta_ws_vent_afore(reg_marca_procesar_maestra.nss, 1, 1, TODAY, CURRENT HOUR TO SECOND, v_diagnostico, v_estatus,
      ...
      IF (v_diagnostico = 101 AND v_estatus <> 101) THEN
         CALL fn_guarda_consulta_ws_vent_afore(reg_marca_procesar_maestra.nss, 1, 2, TODAY, CURRENT HOUR TO SECOND, v_diagnostico, v_estatus,
      ...
      IF (v_diagnostico = 101 AND v_estatus = 101 AND v_saldo_total <= 0) THEN
         CALL fn_guarda_consulta_ws_vent_afore(reg_marca_procesar_maestra.nss, 1, 2, TODAY, CURRENT HOUR TO SECOND, v_diagnostico, v_estatus,
                                               v_aivs_viv92, v_aivs_viv97, 'OPSISSACI', reg_marca_procesar_maestra.id_solicitud, v_caso_crm, 2) -- La solicitud se marca, pero tiene saldo cero
      END IF
   ...
   DISPLAY "Se mandaron marcar a Procesar ", v_solicitudes_procesadas, " solicitudes correctamente."
