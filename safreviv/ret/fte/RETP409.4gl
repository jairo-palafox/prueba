...
#                de las marcas de embargo para el nuevo layout.                         #
#Fecha inicio => Dciembre 2015                                                          #
#########################################################################################
DATABASE safre_viv
GLOBALS "RETG01.4gl"
GLOBALS
...
DEFINE p_pid                  LIKE bat_ctr_operacion.pid, -- PID del proceso
   ...
   LET g_pid         = p_pid      
   LET g_proceso_cod = p_proceso_cod -- marcas embargo (todas)
   ...
   LET v_s_sql = "EXECUTE FUNCTION fn_ret_integra_marca_embargo_todas(?,?,?,?,?)"
   ...
   IF ( v_i_resultado = 0 )THEN
      ...
      -- Registros Encontrados Mas de una ocasión
      LET v_contador = 0;
      SELECT COUNT(*)
        INTO v_contador
        FROM ret_marca_embargo_todas
       WHERE folio = p_folio
         AND estado_solicitud = 100
         AND cod_rechazo = 52; 
       LET p_mensaje = p_mensaje || "Registros Encontrados mas de una Ocasión: ", v_contador, "\n"
   ...
   LET p_titulo = "Finalización de operación - MARCA DE EMBARGOS - INTEGRACION"
