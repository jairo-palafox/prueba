#############################################################################
#Módulo          => RET                                                     #        
#Programa        => RETE340.4gl                                             #
#Objetivo        => Programa carga de archivo notificacion DISP_BP_VAL      #
...
DEFINE p_nom_archivo       STRING,
       ...
       v_registro_archivo  RECORD    -- registro del archivo	  
	        ...
	        nss                  CHAR(11),
            tpo_credito          CHAR(10),
            estatus_credito      CHAR(09),
            marcado_procesar     CHAR(19),
            desmarcado_procesar  CHAR(22)
      END RECORD,			
   ...
   LET v_cadena_registros = p_usuario CLIPPED, ".", "RETE340.log"
   ...
   DISPLAY "Generando tabla temporal tmp_ret_disp_bp_val"

   -- cambio a base de datos temporal
   DATABASE safre_tmp

   -- se regenera la tabla temporal
   LET v_sql = "\nDROP TABLE IF EXISTS tmp_ret_disp_bp_val"
   ...
   LET v_sql = "\n CREATE TABLE tmp_ret_disp_bp_val (  ", 
               "\n  nss                    CHAR(11),   ",
               "\n  tpo_credito            CHAR(10),   ",
               "\n  estatus_credito        CHAR(09),   ",
               "\n  marcado_procesar       CHAR(19),   ",
   ...
   WHILE ( v_ch_archivo.read([v_registro_archivo.*]) )
       ...
       --DISPLAY "Extracto >" || v_registro_archivo.nss[2,2] || "<"
       
       IF v_registro_archivo.nss[2,2] <> '.' AND v_registro_archivo.nss[2,2] <> '-' AND 
          v_registro_archivo.nss[2,2] <> 'C' THEN     
       --IF (v_reg_archivo >= 6) AND (v_registro_archivo.nss[1,1] <> '-' ) THEN 1
           LET v_reg_archivo = v_reg_archivo + 1
           -- se inserta un registro
           INSERT INTO tmp_ret_disp_bp_val values (
                  v_registro_archivo.nss,
                  v_registro_archivo.tpo_credito,
                  v_registro_archivo.estatus_credito,
                  v_registro_archivo.marcado_procesar,
                  v_registro_archivo.desmarcado_procesar)
