...
#MODULO       => RET                                                           #
#PROGRAMA     => RETC397                                                       #
#OBJETIVO     => Consulta especiales de solicitudes de retiro generico         #
#Fecha inicio => 13 Noviembre 2015                                             #
#Modificacion =>                                                               #
################################################################################
...
GLOBALS 
   DEFINE 
      g_arr_solicitudes_folio DYNAMIC ARRAY OF RECORD -- arreglo que contiene los folios elegidos para consultar
             elegir                 SMALLINT,
...
Clave: 
Nombre: main
Fecha creacion: Noviembre 13, 2015
...
DEFINE cbx_modalidad_retiro ui.ComboBox,     -- combo de modalidad de retiro
       ...
       cbx_grupo            ui.ComboBox,     -- Combo para el grupo (Ley 73)
       cbx_ventanilla       ui.ComboBox,     -- Combo para la ventanilla (Ley 73, Grupo 1)
       cbx_clave_afore      ui.ComboBox,     -- Combo para la clave de Afore (Ley 73, Grupo 1)
       ...
       v_fecha_pago_inicio  DATE, -- Fecha de inicio de pago
       v_fecha_pago_fin     DATE, -- Fecha de fin de pago
       ...
       v_grupo              SMALLINT,
       v_ventanilla         SMALLINT,
       v_clave_afore        SMALLINT,
   ...
   CALL STARTLOG (p_usuario_cod CLIPPED|| ".RETC397.log")
   
   CLOSE WINDOW SCREEN
   
   -- se abre la ventana de consulta
   OPEN WINDOW w_consulta WITH FORM "RETC3971"
   ...
   ATTRIBUTES ( UNBUFFERED )
      ...
      BEFORE INPUT
         ...
         -- se llena el combo de causal
             ...
             CALL v_formulario.setElementHidden("lblcausal", TRUE) --Oculta la Sección de Detalles
             CALL v_formulario.setElementHidden("formonly.v_causal_retiro", 1) --Oculta la Sección de Detalles

         -- se llena el combo de grupo
         
             LET cbx_grupo = ui.ComboBox.forName("formonly.v_grupo")
             CALL cbx_grupo.clear()
             CALL cbx_grupo.addItem(1,"1 - GRUPO 1")
             CALL cbx_grupo.addItem(2,"2 - GRUPO 2")
             CALL cbx_grupo.addItem(3,"3 - GRUPO 3")
             CALL cbx_grupo.addItem(4,"4 - GRUPO 4")
             CALL cbx_grupo.addItem(NULL,"Todos")
             
             -- se inicia con todos los grupos
             LET v_grupo = NULL
             CALL v_formulario.setElementHidden("lblgrupo", TRUE) --Oculta la Sección de Grupos
             CALL v_formulario.setElementHidden("formonly.v_grupo", 1) --Oculta la Sección de Grupos

         -- se llena el combo de ventanillas
         
             LET cbx_ventanilla = ui.ComboBox.forName("formonly.v_ventanilla")
             CALL cbx_ventanilla.clear()
             CALL cbx_ventanilla.addItem(101,"101 - INFONAVIT")
             CALL cbx_ventanilla.addItem(201,"201 - AFORE")
             CALL cbx_ventanilla.addItem(NULL,"Ambas")
             
             -- se inicia con ambas ventanillas
             LET v_ventanilla = NULL
             CALL v_formulario.setElementHidden("lblventanilla", TRUE) --Oculta la Sección de Grupos
             CALL v_formulario.setElementHidden("formonly.v_ventanilla", 1) --Oculta la Sección de Grupos

         -- se llena el combo de clave de Afores
             
             CALL fn_llena_combo_clave_afore(v_ventanilla)
             RETURNING cbx_clave_afore,v_clave_afore
             CALL v_formulario.setElementHidden("lblclaveafore", TRUE) --Oculta la Sección de Grupos
             CALL v_formulario.setElementHidden("formonly.v_clave_afore", 1) --Oculta la Sección de Grupos
...

{
      ...
      ON CHANGE v_modalidad_retiro
         CASE v_modalidad_retiro
            WHEN 2
               ...
               CALL v_formulario.setElementHidden("lblgrupo", TRUE) --Oculta la Sección de Grupos
               CALL v_formulario.setElementHidden("formonly.v_grupo", 1) --Oculta la Sección de Grupos
               CALL v_formulario.setElementHidden("lblventanilla", TRUE) --Oculta la etiqueta de Ventanilla
               CALL v_formulario.setElementHidden("formonly.v_ventanilla", 1) --Oculta el combo de Ventanilla
               CALL v_formulario.setElementHidden("lblclaveafore", TRUE) --Oculta la etiqueta de Clave de Afore
               CALL v_formulario.setElementHidden("formonly.v_clave_afore", 1) --Oculta el combo de Clave de Afore
               LET v_clave_afore = NULL
               LET v_ventanilla = NULL
               LET v_grupo = NULL
            ...
            WHEN 3
               ...
               CALL v_formulario.setElementHidden("lblgrupo", FALSE) --Muestra la etiqueta de Sección de Grupos
               CALL v_formulario.setElementHidden("formonly.v_grupo", 0) --Muestra la Sección de Grupos
               LET v_causal_retiro = NULL
               LET v_causal_retiro_trab = NULL
               --NEXT FIELD v_grupo
            OTHERWISE
      ...
      ON CHANGE v_grupo
         CASE v_grupo
            WHEN 1
               CALL v_formulario.setElementHidden("lblventanilla", FALSE) --Muestra la etiqueta de Ventanilla
               CALL v_formulario.setElementHidden("formonly.v_ventanilla", 0) --Muestra la Sección de Ventanilla
               --NEXT FIELD v_ventanilla
            OTHERWISE
               CALL v_formulario.setElementHidden("lblventanilla", TRUE) --Oculta la etiqueta de Ventanilla
               CALL v_formulario.setElementHidden("formonly.v_ventanilla", 1) --Oculta el combo de Ventanilla
               CALL v_formulario.setElementHidden("lblclaveafore", TRUE) --Oculta la etiqueta de Clave de Afore
      ...
      AFTER FIELD v_grupo
         CASE v_grupo
            ...
            OTHERWISE
               ...
               LET v_ventanilla = NULL
         END CASE
      ON CHANGE v_ventanilla
         CALL v_formulario.setElementHidden("lblclaveafore", v_ventanilla IS NULL) --Oculta la etiqueta de Clave de Afore
         CALL v_formulario.setElementHidden("formonly.v_clave_afore", v_ventanilla IS NULL) --Oculta el combo de Clave de Afore
         IF v_ventanilla IS NULL THEN
            LET v_clave_afore = NULL
         ELSE
            CALL fn_llena_combo_clave_afore(v_ventanilla)
            RETURNING cbx_clave_afore,v_clave_afore
         END IF
      AFTER FIELD v_ventanilla
         CALL v_formulario.setElementHidden("lblclaveafore", v_ventanilla IS NULL) --Oculta la etiqueta de Clave de Afore
         CALL v_formulario.setElementHidden("formonly.v_clave_afore", v_ventanilla IS NULL) --Oculta el combo de Clave de Afore
         IF v_ventanilla IS NULL THEN
            LET v_clave_afore = NULL
      ...
      ON ACTION accept
      {
         ...
         ELSE
            ...
            END IF
         END IF
         -- deben venir ambas fechas de pago
         IF ( v_fecha_pago_inicio IS NULL OR v_fecha_pago_fin IS NULL ) THEN
            CALL fn_mensaje("Atención","Debe capturar ambas fechas de pago","stop")
            CONTINUE INPUT
         ELSE
            -- si estan las dos, la fecha inicio no puede ser posterior a la fecha fin
            IF ( v_fecha_pago_inicio > v_fecha_pago_fin ) THEN
               CALL fn_mensaje("Atención","La fecha de inicio de pago no puede ser posterior a la fecha de término de pago","stop")
               CONTINUE INPUT
            ELSE
               -- ninguna de las fechas puede ser posterior a la fecha actual
               IF ( v_fecha_pago_inicio > TODAY OR v_fecha_pago_fin > TODAY ) THEN
                  CALL fn_mensaje("Atención","La fecha de consulta de inicio o fin de pago no pueden ser posteriores a la fecha actual","stop")
                  CONTINUE INPUT
               END IF
            END IF
         END IF
...
FUNCTION fn_llena_combo_clave_afore(v_ventanilla)
   DEFINE
      cbx_clave_afore         ui.ComboBox,     -- Combo para la clave de Afore (Ley 73, Grupo 1)
      v_ventanilla            SMALLINT,
      v_clave_afore           SMALLINT,
      v_r_cat_afore           RECORD LIKE cat_afore.*,
      v_cadena                STRING
   
   LET cbx_clave_afore = ui.ComboBox.forName("formonly.v_clave_afore")
   CALL cbx_clave_afore.clear()
   CASE v_ventanilla
      WHEN 101 --INFONAVIT
         DECLARE  cur_infonavit CURSOR FOR
         SELECT   afore_cod,afore_desc
         FROM     cat_afore
         WHERE    afore_cod IN (530,534,538,544,550,552,556,562,564,568,578)
         ORDER BY afore_cod
         FOREACH cur_infonavit INTO v_r_cat_afore.afore_cod, v_r_cat_afore.afore_desc
            -- Se agregan las afores
            LET v_cadena = v_r_cat_afore.afore_cod || " - " || v_r_cat_afore.afore_desc
            CALL cbx_clave_afore.addItem(v_r_cat_afore.afore_cod, v_cadena)
         END FOREACH
         -- se agrega el valor nulo
         CALL cbx_clave_afore.addItem(NULL, "Todas")
         CALL cbx_clave_afore.addItem(0, "Blanco")
      WHEN 201 --AFORE
         DECLARE  cur_afore CURSOR FOR
         SELECT   afore_cod,afore_desc
         FROM     cat_afore
         WHERE    afore_cod IN (550,552,556,562,564,568,578)
         ORDER BY afore_cod
         FOREACH cur_infonavit INTO v_r_cat_afore.afore_cod, v_r_cat_afore.afore_desc
            -- Se agregan las afores
            LET v_cadena = v_r_cat_afore.afore_cod || " - " || v_r_cat_afore.afore_desc
   ...
   -- se inicia con todas las modalidades
   LET v_clave_afore = NULL
   RETURN cbx_clave_afore,v_clave_afore
END FUNCTION
...
Luis Prieto   20151117         Se agrega consulta consulta por grupo, ventanilla y clave de afore,
                               fecha de inicio de pago y fecha final de pago
======================================================================
}
FUNCTION fn_consulta_solicitud_generico(v_modalidad_retiro, v_causal_retiro, v_nss, v_rfc, v_caso_adai,
                                        ...
                                        v_grupo, v_ventanilla, v_clave_afore,
...
DEFINE v_modalidad_retiro   LIKE ret_modalidad_retiro.modalidad_retiro,
       ...
       v_fecha_pago_inicio  DATE, -- Fecha de inicio de Pago
       v_fecha_pago_fin     DATE, -- Fecha de fin de Pago
       v_grupo              SMALLINT, -- Clave del grupo
       v_ventanilla         SMALLINT, -- Clave de ventanilla
       v_clave_afore        SMALLINT, -- Clave de afore
...
LET v_sql = "\n SELECT                                                                    ",
            ...
            "\n                          WHERE rsp_estatus = 2                            ",
            "\n                          AND rsp_f_pago BETWEEN '",fn_fecha_reg(v_fecha_pago_inicio),"' ",     
            "\n                          AND '",fn_fecha_reg(v_fecha_pago_fin),"') "
   ...
   IF ( v_grupo IS NOT NULL ) THEN
      LET v_sql = v_sql || "\n AND a.id_solicitud in (select id_solicitud from ret_ley73_generico ",
                           "\n                        where gpo_ley73 = ", v_grupo, 
                           "\n                        and f_solicitud BETWEEN '", v_fecha_inicio, "' AND '", v_fecha_fin, "')"
   END IF

   IF ( v_ventanilla IS NOT NULL ) THEN
      LET v_sql = v_sql || "\n AND a.grupo_ventanilla = ", v_ventanilla
   END IF

   IF ( v_clave_afore IS NOT NULL ) THEN
      IF ( v_clave_afore = 0 ) THEN
         LET v_sql = v_sql || "\n AND a.id_solicitud in (select id_solicitud from ret_ws_sol_retiro_vent_afore ",
                                 "\n                        where cve_afore IS NULL)"
      ELSE
         LET v_sql = v_sql || "\n AND a.id_solicitud in (select id_solicitud from ret_ws_sol_retiro_vent_afore ",
                              "\n                        where cve_afore = ", v_clave_afore, ")"
      END IF
   END IF
   ...
   OPEN WINDOW w_consulta1 WITH FORM "RETC3972"
...
DEFINE v_tipo_retiro         LIKE ret_tipo_retiro.tpo_retiro,
       ...
       v_r_beneficiario     RECORD --arreglo para los beneficiarios de la solicitud
         ...
         aivs2_92            LIKE ret_beneficiario_generico.aivs               ,
         aivs2_97            LIKE ret_beneficiario_generico.aivs               ,
         pesos2_92           LIKE ret_beneficiario_generico.importe            ,
         pesos2_97           LIKE ret_beneficiario_generico.importe            ,
         rfc_benef           CHAR(13)                                          ,
         curp_benef          CHAR(18)
       END RECORD,          
   ...
   LET v_sql = "\n SELECT                                                                   ",
               ...
               "\n ''                                                                       ,",
               "\n ''                                                                       ,",
               "\n CASE rl73.gpo_ley73 WHEN 1 THEN 'GRUPO 1' WHEN 2 THEN 'GRUPO 2'           ",
               "\n WHEN 3 THEN 'GRUPO 3' WHEN 4 THEN 'GRUPO 4' END AS gpo_ley73             ,",
               "\n rwsva.cve_afore                                                          ,",
               "\n rwsva.curp                                                               ,",
               "\n afid.curp                                                                ,",
               "\n a.grupo_ventanilla                                                        ",
               ...
               "\n LEFT OUTER JOIN ret_ley73_generico rl73                                   ",
               "\n ON a.id_solicitud = rl73.id_solicitud                                     ",
               "\n LEFT OUTER JOIN ret_ws_sol_retiro_vent_afore rwsva                        ",
               "\n ON a.id_solicitud = rwsva.id_solicitud                                    ",
               "\n LEFT OUTER JOIN ret_ws_consulta_pago_fico rwscpf                          ",
               "\n ON a.id_solicitud = rwscpf.id_solicitud                                   ",
               "\n LEFT OUTER JOIN afi_derechohabiente afid                                  ",
               "\n ON a.id_derechohabiente = afid.id_derechohabiente,                        ",
               "\n ret_modalidad_retiro      e,                                              ",
               "\n ret_estado_solicitud      es,                                             ",
               "\n ret_rechazo_generico       cr                                             ",    
               "\n WHERE a.f_solicitud BETWEEN '", v_fecha_inicio, "' AND '", v_fecha_fin, "'",
   ...
   FOREACH cur_solicitudesdet INTO v_r_despliegue.*
      ...
      LET v_arr_despliegue[v_indice].gpo_ley73        = v_r_despliegue.gpo_ley73
      LET v_arr_despliegue[v_indice].cve_afore        = v_r_despliegue.cve_afore
      LET v_arr_despliegue[v_indice].rsp_f_pago       = fn_fecha(v_r_despliegue.rsp_f_pago)
      ...
      AND v_r_despliegue.grupo_ventanilla = 101 THEN
         ...
         LET v_arr_despliegue_det[v_indice].curp            = v_r_despliegue.curp
      ELSE
         --Cualquier otro toma del maestro de afiliados de INFONAVIT
         LET v_arr_despliegue_det[v_indice].curp            = v_r_despliegue.curp_afi
      END IF
      ...
      WHEN 10
         ...
         INTO v_arr_despliegue[v_indice].aivs,v_arr_despliegue[v_indice].pesos,
              v_arr_despliegue[v_indice].monto,v_arr_despliegue[v_indice].tanto_adicional,
              v_arr_despliegue[v_indice].aivs_92,v_arr_despliegue[v_indice].pesos_92,
              v_arr_despliegue[v_indice].aivs_97,v_arr_despliegue[v_indice].pesos_97
         FROM ret_voluntaria
...
{      
   ...
   OPEN WINDOW w_consultadetallada WITH FORM "RETC3973"
   ...
   DISPLAY ARRAY v_arr_despliegue TO tbl_despliegue.*
      ...
      LET v_curp_det          = v_arr_despliegue_det[ARR_CURR( )].curp

      -- despliega la informacion de detalle
      DISPLAY BY NAME v_num_registros, v_total_pesos,v_total_aivs
        ,v_rfc_det,v_curp_det,v_folio_det , v_folio_restitucion,v_nombre
      ...
      FOREACH cur_benefiriariosdet INTO v_r_beneficiario.*
         ...
         LET v_arr_beneficiario[v_indice].aivs2_92  = v_arr_despliegue[ARR_CURR( )].aivs_92  * (v_arr_beneficiario[v_indice].porcentaje  / 100)
         LET v_arr_beneficiario[v_indice].pesos2_92 = v_arr_despliegue[ARR_CURR( )].pesos_92 * (v_arr_beneficiario[v_indice].porcentaje  / 100)
         LET v_arr_beneficiario[v_indice].aivs2_97  = v_arr_despliegue[ARR_CURR( )].aivs_97  * (v_arr_beneficiario[v_indice].porcentaje  / 100)
         LET v_arr_beneficiario[v_indice].pesos2_97 = v_arr_despliegue[ARR_CURR( )].pesos_97 * (v_arr_beneficiario[v_indice].porcentaje  / 100)
         --
         SELECT rfc_beneficiario,curp_beneficiario
         INTO v_arr_beneficiario[v_indice].rfc_benef, v_arr_beneficiario[v_indice].curp_benef
         FROM ret_ws_sol_retiro_vent_afore
         WHERE id_solicitud = v_id_solicitud
         AND ind_beneficiario = v_arr_beneficiario[v_indice].consec_beneficiario
         --
         LET v_indice = v_indice + 1
      END FOREACH
    ...
    --DISPLAY "Busca la respuesta FICO"
      ...
      CALL fn_credito_vivienda(v_arr_despliegue_det[ARR_CURR( )].id_derechohabiente, 1)
      ...
      -- despliega la informacion del Crédito
      DISPLAY BY NAME v_num_credito,v_tipo_credito_desc,v_estado_credito,
      v_f_otorga,v_tpo_credito
...
#Da formato a una fecha, el campo original es una cadena en fomato internacional aaaammdd y se regresa en formato México dd-mm-aaaa
FUNCTION fn_fecha_reg(p_fecha_text)
   DEFINE p_fecha_text VARCHAR(20)
      ,v_fecha varchar(8)
   IF LENGTH(p_fecha_text) > 0 THEN
      LET v_fecha = p_fecha_text[7,11],p_fecha_text[1,2],p_fecha_text[4,5]
   ELSE
      LET v_fecha = ""
   END IF
   RETURN v_fecha
