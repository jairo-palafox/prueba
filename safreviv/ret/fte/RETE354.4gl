#############################################################################
#Módulo          => RET                                                     #        
#Programa        => RETE354.4gl                                             #
#Objetivo        => Programa carga de archivo notificacion TIPN_BUT_VAL     #
...
DEFINE p_nom_archivo       STRING,
   ...
   LET v_cadena_registros = p_usuario CLIPPED, ".", "RETE354.log"
   ...
   DISPLAY "Generando tabla temporal tmp_ret_tipn_but_val"

   -- cambio a base de datos temporal
   DATABASE safre_tmp

   -- se regenera la tabla temporal
   LET v_sql = "\nDROP TABLE IF EXISTS tmp_ret_tipn_but_val"
   ...
   LET v_sql = "\nCREATE TABLE tmp_ret_tipn_but_val (  ", 
               "\n nss              CHAR(11),          ",
               "\n marca_grupo      CHAR(08),          ",
               "\n estatus          CHAR(05)        ); "
   ...
   WHILE ( v_ch_archivo.read([v_registro_archivo.*]) )
       ...
       --IF (v_reg_archivo >= 6) AND (v_registro_archivo.nss[1,1] <> '-' ) THEN 
           ...
           INSERT INTO tmp_ret_tipn_but_val values (
