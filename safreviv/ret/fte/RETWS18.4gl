...
DEFINE v_resultado        INTEGER, -- recibe el resultado de la ejecucion del servicio 
  ...
  LET v_ruta_log = "/safreviv_log/ret/RETWS18."
  ...
  IF num_args() = 2 AND arg_val(1) = "-W" THEN
      ...
      CALL fn_crea_servicio_marcaje_AE(TRUE)
      EXIT PROGRAM
  ELSE 
    IF num_args() = 2 AND arg_val(1) = "-S" THEN
      ...
      OPEN WINDOW w WITH FORM "RETWE030" ATTRIBUTES(TEXT = "Solcitud de marcaje Amortizaciones Excedentes") --, STYLE="naked")
  ...
  -- se crea el servicio
  CALL ERRORLOG("Invoca creacion de servicio de solicitud de marcaje Amortizaciones Excedentes")
  CALL fn_crea_servicio_marcaje_AE(FALSE)

  -- se inicia el servidor
  DISPLAY "Iniciando servidor de Marcaje para retiro Amortizaciones Excedentes..."
...
======================================================================
Clave: 
Nombre: fn_crea_servicio_marcaje_AE
...
FUNCTION fn_crea_servicio_marcaje_AE(generaWSDL)
...
DEFINE generaWSDL           SMALLINT,
  ...
  TRY
    -- =============================
    -- se crea el servicio
    LET serv = com.WebService.CreateWebService("retiroMarcaAE", v_service_NameSpace)
    ...
    CALL serv.publishOperation(op, "fn_marcaje_ae")
    ...
    ELSE
       ...
       CALL ERRORLOG("Se registro el servicio retiroMarcaAE")
    END IF
    
  CATCH -- en caso de error
    DISPLAY("No se pudo crear el servicio 'Solicitud de marcaje Amortizaciones Excedentes':" || STATUS)
    EXIT PROGRAM
  END TRY
