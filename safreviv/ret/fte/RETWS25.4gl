...
#MODULO            => RET                                                     #
#PROGRAMA          => RETWS25                                                 #
#OBJETIVO          => WS CONSULTA DE TRAMITES AMORTIZACIONES EXCEDENTES 
#FECHA INICIO      => 10-DIC-2019                                             #
###############################################################################
...
DEFINE v_resultado       INTEGER, -- recibe el resultado de la ejecucion del servicio 
    ...
    LET v_ruta_log = v_ruta_ejecutable CLIPPED, "/RETWS25."
    ...
    IF num_args() = 2 AND arg_val(1) = "-W" THEN
        ...
        CALL fn_crea_servicio_consulta_tramites_ae(TRUE)
        EXIT PROGRAM
    ELSE 
        IF num_args() = 2 AND arg_val(1) = "-S" THEN
            ...
            OPEN WINDOW w WITH FORM "RETWE030" ATTRIBUTES(TEXT = "Retiro Consulta Trámites Amort. Excedentes service") --, STYLE="naked")
    ...
    CALL fn_crea_servicio_consulta_tramites_ae(FALSE)

    -- se inicia el servidor
    CALL ERRORLOG("Iniciando servidor de Consulta de Trámites Amortizaciones Excedentes ...")
...
======================================================================
Clave: 
Nombre: fn_crea_servicio_consulta_tramites_ae
Fecha creacion: Diciembre 10, 2019
Autor: Ricardo Perez
Narrativa del proceso que realiza:
Genera el servicio web de amortizaciones excedentes que consulta los trámites realizados
...
FUNCTION fn_crea_servicio_consulta_tramites_ae(p_generar_WSDL)
...
DEFINE v_urn                STRING -- URN
    ...
    TRY
        -- =============================
        -- se crea el servicio
        LET v_webservice = com.WebService.CreateWebService("retiroConsultaTramitesAE", v_service_NameSpace)

        -- =============================
        -- Publicacion de las funciones

        -- fn_retiro 
        LET op = com.WebOperation.CreateDOCStyle("fn_ret_consulta_tramites_ae","fn_ret_consulta_tramites_ae",ws_ret_cons_tramites_in,ws_ret_cons_tramites_out)
        ...
        CALL v_webservice.publishOperation(op, "fn_ret_consulta_tramites_ae")
        ...
        ELSE
            ...
            CALL ERRORLOG("Se registro el servicio consulta de trámites para retiro Amortizaciones Excedentes")
        END IF
    
        CATCH -- en caso de error
            DISPLAY("No se pudo crear el servicio 'Consulta de trámites para retiro Amortizaciones Excedentes': " || STATUS)
            EXIT PROGRAM
    END TRY
...
======================================================================
Clave: 
Nombre: fn_ret_consulta_tramites_ae
Fecha creacion: Diciembre 10, 2019
...
FUNCTION fn_ret_consulta_tramites_ae()
...
END RECORD 
   ...
   ELSE 
      ...
      LET v_consulta = "\n SELECT a.id_solicitud,  a.nss, b.ap_paterno_af, b.ap_materno_af,  ",
                       "\n         b.nombre_af, ABS(c.total_importe),                        ",
