...
#Modulo            => RET                                                      #
#Programa          => RETP1259                                                 #
#Objetivo          => Programa para integrar la respuesta de archivos SAP      #
#                     de cancelacion de cuentas por pagar                      #
...
DEFINE l_s_qryTxt                      STRING, -- guarda una sentencia SQL a ejecutar
   ...
   CALL fn_display_proceso(0, "INTEGRACION DE RESPUESTA DE SOLICITUD DE ANULACION")
      ...
      WHERE  modulo_cod = "ret"
   {
      ...
      LET v_archivo_key = v_nom_archivo.subString(1,v_nom_archivo.getLength() - 4), ".KEY"
      ...
      LET v_comando = "export sha=`cat ", v_archivo_key, "`; verifica_sha_ret_generico.sh ", v_archivo_txt, " $sha"
   ...
   {   
      ...
      IF ( v_hash_es_correcto ) THEN
         ...
         RETURNING g_folio
         --
         DISPLAY "EJECUTANDO STORE INTEGRACIÓN: ",TODAY
         -- se prepara y ejecuta el procedimiento de integracion
         PREPARE prp_carga_disp
            FROM "EXECUTE FUNCTION sp_integra_sap(?,?,?,?)"
         ...
         IF ( v_error < 0 ) THEN
            DISPLAY "Error al ejecutar el SP de integración de respuesta de SAP"     
         ...
         ELSE
            ...
            LET p_mensaje = "El proceso de integración de la respuesta de SAP ha finalizado correctamente."
            DISPLAY "\n", p_mensaje
                                 
            -- se crea el titulo del mensaje
            LET p_titulo = "Finalización de operación - INTEGRACIÓN ARCHIVO SAP"     
         END IF
      ELSE
         -- EL HASH NO SE VALIDO CORRECTAMENTE. EL PROCESO SE MARCA EN ERROR
         LET p_mensaje = "El archivo ", v_nom_archivo, " de respuesta de SAP no cumple con la validación del archivo KEY. No se procesará el archivo. Es necesario regenerar y reenviar el archivo."
         DISPLAY p_mensaje
   ...
   -- se envia la cadena que indica el fin de la etapa
   CALL fn_display_proceso(1, "INTEGRACION DE RESPUESTA DE SAP")
