...
#Modulo            => ret                                                     #
#Programa          => RETWE05                                                 #
#Objetivo          => ws Solicitud de retiro Fondo Ahorro actualizacion de    #
#                      pago o no pago fico                                    #
#Fecha Inicio      => 17-May-2012                                             #
###############################################################################
...
MAIN
  ...
  ELSE 
    IF num_args() = 2 AND arg_val(1) = "-S" THEN
      ...
      CLOSE WINDOW SCREEN
      OPEN WINDOW w WITH FORM "RETWE050" ATTRIBUTES(TEXT = "Aprobacion fico service", STYLE="naked")
...
FUNCTION CreateFicoService(generateWSDL)
  ...
  TRY  
    ...
    #
    LET serv = com.WebService.CreateWebService("Aprobficofondo",serviceNS)
    ...
    # fn_aprob_fico 
    LET op = com.WebOperation.CreateRPCStyle("fn_aprob_fico","fn_aprob_fico",ret_aprob_fico,NULL)
    ...
    ELSE
      ...
      display_status("Aprobacion fico Service registered")
    END IF
    
  CATCH
    display_status("Unable to create 'Aprobacion fico' Web Service :"||STATUS)
    EXIT PROGRAM
  END TRY
...
FUNCTION valida_aprob()
 ...
 SELECT id_derechohabiente, id_afi_fondo72
   INTO g_id_derechohabiente, g_id_afi_fondo72
   FROM afi_fondo72 
  WHERE nss = g_nss
...
DEFINE  v_count SMALLINT 
 ...
 SELECT COUNT(id_referencia)
   INTO  v_count
   FROM  cta_fondo72
  WHERE  movimiento = 182
    AND  id_afi_fondo72 = g_id_afi_fondo72
...
LET v_s_sql = "EXECUTE PROCEDURE sp_reversa_solicitud_teso(?,?,?)"
