...
MAIN
   ...
   LET g_opera_cod   = 2           --sisxix-2
   LET p_folio       = 0           --sisxix-2
   LET g_nom_archivo = " "         --sisxix-2
   
--sisxix-2   LET v_fecha_consulta = TODAY - 1
--   LET v_fecha_consulta =  "11/14/2018"  --sisxix-2
   LET v_fecha_consulta =  "01/14/2019"  --sisxix-2-2
   
   -- se crea el archivo log
   CALL STARTLOG(p_usuario_cod CLIPPED|| ".PAGC81T.log")

   DISPLAY "Iniciando generación de Reporte de extracción de pagos para preca"

   LET v_sql = "\n SELECT glo.folio         ",
               "\n FROM   glo_folio glo, bat_his_ctr_operacion bat ",            --sisxix-2-2
...
--sisxix-2      CALL fn_actualiza_opera_fin(g_pid,g_proceso_cod,g_opera_cod) RETURNING r_resultado_opera
--sisxix-2      LET v_mensaje = "El reporte de extracción ha finalizado correctamente"

--sisxix-2      CALL fn_correo_proceso(g_pid,
--sisxix-2                             g_proceso_cod,
--sisxix-2                             g_opera_cod,
--sisxix-2                             '', # Archivo adjunto
--sisxix-2                             'Finalización de operación - '||v_proceso_desc CLIPPED||' - REPORTE EXTRACTOR PRECA',
--sisxix-2                              v_mensaje
--sisxix-2                             )
