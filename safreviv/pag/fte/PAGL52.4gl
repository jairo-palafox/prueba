...
#Modulo            =>PAG                                           #
#Programa          =>PAGL52                                        #
#Objetivo          =>Programa lanzador del proceso de integracion  #
...
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".PAGL52.log")
   ...
   CALL fn_valida_operacion(v_d_pid,g_proceso_cod_pag_registro_pagos_fa,g_opera_cod_pag_integracion) RETURNING r_b_valida
   ...
   OPEN WINDOW w_inte_acred WITH FORM "PAGL521"
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         ON ACTION ACCEPT
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
                ...
                SELECT COUNT(*),
                       SUM(monto_individualizar)
                INTO   v_arr_integrados[v_i_iter].total_registros,
                       v_imp_ap_pat_aux
                FROM  safre_tmp:tmp_pag_det_fa
         ...
         ON ACTION integrar
           ...
           CALL fn_actualiza_opera_ini(v_d_pid
            ...
            IF r_b_valida = 0 THEN
               ...
               LET v_s_comando = " nohup time fglrun ",v_c_ruta_bin_acr CLIPPED,"/PAGP52 ",
                                                      p_v_usuario, " ",
                                                      v_d_pid, " ",
                                                      g_proceso_cod_pag_registro_pagos_fa, " ",
                                                      g_opera_cod_pag_integracion, " ",
