...
-- Modulo       => PAG                                                                 
-- Programa     => PAGS80
-- Objetivo     => Genera archivo plano de extracción de pagos para la preca
-- Autor        => GERARDO ALFONSO VEGA PAREDES                                        
-- Fecha inicio => 05 de Febrero de 2014
--========================================================================================
-- Actualización=> Se parametrizo la subcuenta y el movimiento 6 oct 2015
--========================================================================================
-- Fecha cambio  => 29 Marzo de 2019
-- Descripción   => Cambiar FIRST PAGE HEADER porque no imprimia nada por ON EVERY ROW
-- Calve cambio  => bug-01
--========================================================================================
...
MAIN
   ...
   --DISPLAY "Sale de fn_genera_extractor, FIN MAIN"
END MAIN
...
FUNCTION fn_genera_extractor()
   ...
   --Lee fecha de inicio de proceso 12 diciembre 2014
   INITIALIZE v_fecha_inicio TO NULL 
   CALL fn_obtener_fec_ini_extraccion() RETURNING v_fecha_inicio
...
FUNCTION fn_genera_extraccion(p_fecha_inicio)
   ...
   IF v_cuantos > 0 THEN
      ...
      LET v_sql = "\n SELECT det.origen_archivo,                             ",
                  ...
                  "\n WHERE pag.f_consulta BETWEEN '",p_fecha_inicio,"' AND TODAY - 1 ",                  
      ...
      --DISPLAY  "ACTUALIZA cod_estado=2 para que no se genere nuevamente"
   ...
   CALL fn_actualiza_fecha_extraccion()
...
FUNCTION fn_obtener_fec_ini_extraccion()
   DEFINE v_rec_pag_ctr_folio_preca RECORD LIKE pag_ctr_folio_preca.*
   DEFINE v_query STRING

   INITIALIZE v_rec_pag_ctr_folio_preca.* TO NULL
   INITIALIZE v_query TO NULL

   LET v_query = " SELECT * FROM pag_ctr_folio_preca"
   PREPARE prp_pag_ctr_folio_preca FROM v_query
   EXECUTE prp_pag_ctr_folio_preca INTO v_rec_pag_ctr_folio_preca.*

   RETURN v_rec_pag_ctr_folio_preca.f_extraccion
END FUNCTION


FUNCTION fn_actualiza_fecha_extraccion()
   DEFINE v_query STRING

   INITIALIZE v_query TO NULL

   LET v_query = " UPDATE pag_ctr_folio_preca ", 
                 " SET f_extraccion = TODAY "
   PREPARE prp_update_f_extraccion FROM v_query
   EXECUTE prp_update_f_extraccion
