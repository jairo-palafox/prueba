...
#                    archivo de entrada Solo Infonavit             #
#Autor             =>EFP                                           #
...
MAIN
   ...
   CALL STARTLOG (p_v_usuario CLIPPED|| ".PAGL28.log")
   ...
   --LET v_i_proceso_cod = 1403 -- recepción rechazo de saldos pag
   ...
   CALL fn_valida_operacion(v_d_pid,g_proceso_cod_pag_registro_pagos_SINF,g_opera_cod_pag_integracion) RETURNING r_b_valida
         ...
         ON ACTION ACCEPT
            ...
            FOR v_i_iter = 1 TO v_i_num_arch
               ...
               LET v_s_qryTxt = " SELECT *\n",
                                ...
                                "  WHERE operacion = ",v_i_operacion,"\n",
                                "    AND estado = 10" -- cargado

                PREPARE prp_info_archivo FROM v_s_qryTxt
                EXECUTE prp_info_archivo INTO v_r_acr_ctr_arch.*

                -- se asigna la informacion del archivo integrado
                LET v_arr_integrados[v_i_iter].nom_archivo = v_v_nomArch_proc
                LET v_arr_integrados[v_i_iter].fecha_trans = v_r_acr_ctr_arch.f_lote
                LET v_arr_integrados[v_i_iter].lote = v_r_acr_ctr_arch.lote
                LET v_arr_integrados[v_i_iter].fecha_proceso = TODAY
                LET v_arr_integrados[v_i_iter].tot_registros = v_r_acr_ctr_arch.tot_registros}
                LET v_v_nomArch_proc = v_arr_arch_int[v_i_iter]
                LET v_arr_integrados[v_i_iter].nom_archivo = v_v_nomArch_proc

                SELECT COUNT(*), SUM(imp_ap_pat), SUM(imp_am_cre_ifv)
         ...
         ON ACTION integrar
           ...
           CALL fn_actualiza_opera_ini(v_d_pid
            ...
            IF r_b_valida = 0 THEN
               ...
               LET v_s_comando = " nohup time fglrun ",v_c_ruta_bin_acr CLIPPED,"/PAGP14 ",
                                                      p_v_usuario, " ",
                                                      v_d_pid, " ",
                                                      g_proceso_cod_pag_registro_pagos_SINF, " ",
                                                      g_opera_cod_pag_integracion, " ",
