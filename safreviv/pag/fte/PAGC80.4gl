...
GERARDO VEGA    23-jun-2014       Se agrego movimiento 681 para aceptar pagos RISS
...
---==================================================================================
            ...
            FOREACH c_pag INTO v_bim_pago,
               ...
               SELECT COUNT(*)
                ...
                WHERE folio_liquida = v_folio
                  ...
                  AND subcuenta  IN (SELECT subcuenta  FROM cat_sub_extractor where vigencia_cod =1)  --(4,44,55,60)   
                  AND movimiento IN (SELECT movimiento FROM cat_mov_extractor where vigencia_cod =1) --(1,61,221,41,51,81,451,681,691,701,711,761,771,791,741,781,811,731,801,831,751,821) 
        ...
        --250314
                  AND id_derechohabiente = v_id_derechohabiente
                  AND subcuenta  IN (SELECT subcuenta  FROM cat_sub_extractor where vigencia_cod =1)  --(4,44,55,60)   
                  AND movimiento IN (SELECT movimiento FROM cat_mov_extractor where vigencia_cod =1) --IN (1,61,221,41,51,81,451,681,691,701,711,761,771,791,741,781,811,731,801,831,751,821) 
...
FUNCTION fn_consulta_pago_saci(v_nss,v_folio_sua,v_fecha_pago,v_nrp,v_bim_pago,v_periodo_pago,l_folio, l_id_referencia)
   ...
   AND    mov.subcuenta  IN (SELECT subcuenta  FROM cat_sub_extractor where vigencia_cod =1)   --(4,44,55,60) 
   AND    mov.movimiento IN (SELECT movimiento FROM cat_mov_extractor where vigencia_cod =1)  --(1,61,221,41,51,81,451,681,691,701,711,761,771,791,741,781,811,731,801,831,751,821) 
