...
-- 13Feb2013 Antes de consultar, se verifica si existe un proceso LQINFO ejecutando. De ser
--           asi, no se permite la consulta
--===============================================================
-----------------------------------------------------------------------------------------
-- Modulo        => PAG                                                                    
-- Programa      => PAGC19                                                                
-- Objetivo      => Consulta Detalle del Pago.
-- Autor         => Ilhuitemoc Ricardo Ortiz.
-- Fecha inicio  => 11 de Abril de 2012.
-- Requerimiento =>
-----------------------------------------------------------------------------------------
-- Modificación => Se modifican pantallas y reporte para mostrar información necesaria.
-- Fecha        => 22 de Junio de 2012.
-- Autor        => Rubén Haro Castro.
-- Clave cambio =>
-----------------------------------------------------------------------------------------
-- Modificación => Se agrega como parametros folio e id_referencia. Si se reciben se envia
--              => directo a la consulta sin pasar por la captura de datos. 
-- Fecha        => 10 Sep 2012. 
-- Autor        => Rubén Haro Castro.
-- Clave cambio =>
-----------------------------------------------------------------------------------------
-- Modificación => Quitar pagos "0-Normales" de la pantalla.
-- Fecha        => 3 de Julio de 2018. 
-- Autor        => GERARDO ALFONSO VEGA PAREDES.
-- Clave cambio => saci2018-50.
-----------------------------------------------------------------------------------------
-- Modificación => sustituir base "DATABES por SCHEMA" y agregar "Connect 'safreviv_af' "
-- Fecha        => 11 Marzo de 2019.
-- Autor        => GERARDO ALFONSO VEGA PAREDES.
-- Clave cambio => sisxix-1
-----------------------------------------------------------------------------------------

--DATABASE safre_viv  --sisxix-1
SCHEMA safre_viv      --sisxix-1
...
DEFINE p_usuario_cod    LIKE seg_usuario.usuario_cod, -- clave del usuario firmado
   ...
   IF ( f_existe_proceso_operacion_ejecutando(g_proceso_cod_pag_registro_pagos_LQINFO, NULL) ) THEN
...
--            id_referencia            LIKE cta_his_pagos.id_referencia,
   ...
   OPEN WINDOW w_consulta_registros WITH FORM "../../pag/bin/PAGC191"
   LET  v_ventana = UI.WINDOW.GETCURRENT()
   CALL v_ventana.SETTEXT("Consulta detalle de pago")

   CONNECT TO "safre_viv"                      --sisxix-1
      ...
      INPUT v_nss FROM ed_nss ATTRIBUTES(UNBUFFERED, WITHOUT DEFAULTS)
         ...
         ON ACTION ACCEPT
            ...
            IF (v_nss IS NULL) THEN -- OR (v_folio IS NULL) THEN 
         	   CALL fn_mensaje("Consulta","Debe de ingresar un NSS","about")
               NEXT FIELD ed_nss         	 
            ELSE
               ...
               IF v_id_derechohabiente IS NULL OR 
                  ...
                  LET INT_FLAG = TRUE
                  CALL fn_mensaje("Consulta","No existen registros para el NSS ingresado","about")
                  CONTINUE INPUT
               ELSE
                  LET INT_FLAG = FALSE
                  EXIT INPUT
...
--                     "\n           WHEN d.destino_ap_viv = '3' THEN ",
--                     "\n              a.int_gen_pgo_ext  ",
--                     "\n           ELSE a.aiv_gen_pgo_ext ",
--                     "\n        END                     ,",                     
                     "\n        a.aiv_gen_pgo_ext       ,",
                     "\n        a.ind_liquidacion       ,",
                     "\n        a.result_operacion      ,",                     
                     "\n        d.id_derhab_nuevo        ",
                     "\n  FROM  cta_his_pagos a, pag_tpo_archivo b, cta_pag_complemento d, OUTER pag_tpo_aclaracion c ",
...
---------que siempre NO                     "\n   AND  a.ind_liquidacion <> 0 ",                    --saci2018-50, 0=Aportación Normal
...
--            id_referencia            LIKE cta_his_pagos.id_referencia,
     ...
     OPEN WINDOW w_complementarios WITH FORM "../../pag/bin/PAGC192"
     LET  v_ventana = UI.WINDOW.GETCURRENT()
     ...
     MENU ""
        BEFORE MENU
--      	   DISPLAY  arr_registros.localiza_trabajador   TO localiza_trabajador 
           ...
           IF arr_registros.destino_ap_viv = 1 THEN
             ...
             --DISPLAY "@ v_desc_destino_ap_viv ", v_desc_destino_ap_viv
           END IF 
   ...
   IF INT_FLAG = 1 THEN
      ...
      IF fgl_report_loadCurrentSettings("../../pag/bin/PAGC19.4rp") THEN 
      ...
      ELSE         
         DISPLAY "No fue posible generar el reporte. No se encuentra la plantilla PAGC19.4rp"
         EXIT PROGRAM 
      END IF   
