--==============================================================================
-- Version: 1.0.0
-- Fecha ultima modificacion: 28/06/2015
--==============================================================================
################################################################################
#Modulo            => PRT                                                      #
#Programa          => PRTL22                                                   #
#Descripcion       => Preliquidación de devoluciones de saldos cedente         #
#Autor             => Hugo César Ramírez García                                #
#Fecha inicio      => 28 Junio 2015                                            #
################################################################################
DATABASE safre_viv
...
MAIN
   ...
   CALL fn_recupera_folio_devolucion_cedente()
...
DEFINE v_consulta STRING
   ...
   LET v_consulta = " SELECT COUNT(*),",
                    "        CASE tpo_operacion WHEN '01' THEN 'TRANSFERENCIA TOTAL' WHEN '02' THEN 'TRANSFERENCIA PARCIAL' WHEN '03' THEN 'DEVOLUCIÓN' ELSE 'NO CLASIFICADO'  END CASE,",
                    "        SUM(mto_pesos_infonavit97),",
                    "        SUM(mto_aivs_infonavit97)",
...
FUNCTION fn_recupera_folio_devolucion_cedente()
...
CONSTANT v_sr_elegido    = "sr_elegido" # Se asigna el nombre de la tabla de archivos elegidos
   ...
   OPEN WINDOW vtna_preliq_dev_cedente WITH FORM v_ruta_ejecutable CLIPPED||"/PRTL221"
      ...
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         BEFORE DIALOG
            EXECUTE prp_recupera_pid USING C_PROCESO_COD_DEV_SDO_CEDENTE
                                      INTO g_pid
         ...
         ON ACTION aceptar
            ...
            CALL fn_preliquida_devolucion(v_folio_preliquidar) RETURNING v_continua
            IF( v_continua )THEN
   ...
   CLOSE WINDOW vtna_preliq_dev_cedente
...
DEFINE p_folio_preliquidar LIKE glo_folio.folio,
       ...
       v_saldos_preliquidar_aux RECORD
         ...
         v_monto_pesos   LIKE prt_traspaso_cedente.mto_pesos_infonavit97,
         v_monto_aivs    LIKE prt_traspaso_cedente.mto_aivs_infonavit97
       END RECORD,
   ...
   FOREACH cur_cifras_preliquidar USING p_folio_preliquidar 
      ...
      LET v_saldos_preliquidar[v_indice].v_registros     = v_saldos_preliquidar_aux.v_registros
      LET v_saldos_preliquidar[v_indice].v_tpo_operacion = v_saldos_preliquidar_aux.v_tpo_operacion      
      LET v_saldos_preliquidar[v_indice].v_monto_pesos   = v_saldos_preliquidar_aux.v_monto_pesos
      LET v_saldos_preliquidar[v_indice].v_monto_aivs    = v_saldos_preliquidar_aux.v_monto_aivs
      LET v_indice = v_indice + 1
   END FOREACH
...
FUNCTION fn_preliquida_devolucion(p_folio_preliquidar)
DEFINE p_folio_preliquidar STRING,
   ...
   # suponiendo que se realiza la preliquidación
   LET v_continua = FALSE
   
   # se despliega los posibles saldos a separar
   DISPLAY ARRAY v_cifras_preliquidar TO sr_cifras.* ATTRIBUTES(ACCEPT = FALSE, CANCEL = FALSE, UNBUFFERED)
      ...
      ON ACTION preliquidar
         ...
         IF( r_confirma )THEN
            ...
            ELSE
               CALL fn_ejecuta_preliquidacion_devolucion_cedente(p_folio_preliquidar) RETURNING r_error
               IF( r_error )THEN
                  LET v_continua = TRUE
               END IF
...
FUNCTION fn_ejecuta_preliquidacion_devolucion_cedente(p_folio_preliquidar)
DEFINE p_folio_preliquidar STRING, 
       v_comando          STRING,
   ...
   ELSE
      ...
      LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/PRTE22.42r ",p_usuario_cod, " ",
                                                                               g_pid, " ",
                                                                               C_PROCESO_COD_DEV_SDO_CEDENTE," ",
                                                                               C_OPERA_COD_PRELIQUIDACION," ",
                                                                               p_folio_preliquidar,
                      ...
                      " 1>", v_ruta_listados CLIPPED,"/nohup:",g_pid USING "&&&&&",":",
                                                               C_PROCESO_COD_DEV_SDO_CEDENTE USING "&&&&&",":",
