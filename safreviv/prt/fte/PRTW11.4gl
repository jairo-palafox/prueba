...
#                     Procesar para la solicitud de traspaso receptora         #
...
GLOBALS "PRTW10.inc"        #Archivo con variables globales del propio programa
GLOBALS "PRTW11.inc"        #Archivo con variables globales del cliente de WS generado con Genero
...
PRIVATE DEFINE v_tipo_movimiento       CHAR(2)
...
PRIVATE DEFINE v_tipo_operacion        CHAR(2)
PRIVATE DEFINE v_id_credito_infonavit  CHAR(10)
PRIVATE DEFINE v_id_credito_fovissste  CHAR(10)
PRIVATE DEFINE v_mto_credito_infonavit DECIMAL(22,2)
PRIVATE DEFINE v_mto_credito_fovissste DECIMAL(22,2)
PRIVATE DEFINE v_fecha_credito         CHAR(8)
PRIVATE DEFINE v_valor_aiv_inf97       DECIMAL(9,6)
PRIVATE DEFINE v_fecha_valor_aiv_inf97 CHAR(8)
PRIVATE DEFINE v_aivs_inf97            DECIMAL(9,6)
PRIVATE DEFINE v_pesos_inf97           DECIMAL(10,2)
PRIVATE DEFINE v_valor_aiv_fov08       DECIMAL(9,6)
PRIVATE DEFINE v_fecha_valor_aiv_fov08 CHAR(8)
PRIVATE DEFINE v_aivs_fov08            DECIMAL(9,6)
PRIVATE DEFINE v_pesos_fov08           DECIMAL(10,2)
PRIVATE DEFINE v_fecha_transferencia   CHAR(8)
...
PRIVATE FUNCTION fn_envia_solicitud()
   ...
   ELSE     #FIN VALIDACION DE TRAMITE CON ERROR
      ...
      IF v_id_bus_tramite > 0 AND v_id_cat_bus_contrato > 0 THEN
         ...
         LET recibeSolTraspInfonavit.idssn.idCanal          = v_id_canal
         LET recibeSolTraspInfonavit.idssn.idCliente        = v_id_cliente
         LET recibeSolTraspInfonavit.idssn.idEbusiness      = v_id_ebusiness
         LET recibeSolTraspInfonavit.idssn.idPortafolio     = v_id_portafolio
         LET recibeSolTraspInfonavit.idssn.idServicio       = v_id_servicio
         LET recibeSolTraspInfonavit.idssn.idSistema        = v_id_sistema
         LET recibeSolTraspInfonavit.idssn.codoperCliente   = COD_OPER_CLIENTE CLIPPED
         LET recibeSolTraspInfonavit.idssn.fecha            = CURRENT YEAR TO FRACTION(5)
...
PRIVATE FUNCTION fn_envia_mensaje_bus()
   ...
   LET recibeSolicitudTraspaso_recibeSolicitudTraspasoPortTypeLocation = v_url

   LET v_num_intento = 0
   WHILE v_num_intento <= v_num_reintento
      
      CALL recibeSolTraspInfonavit_g() RETURNING wsStatus
      IF wsStatus = 0 THEN
...
PRIVATE FUNCTION fn_procesa_respuesta()
   ...
   LET v_ind_error = 0
   LET v_r_cod_respuesta     = recibeSolTraspInfonavitResponse.codresp
   LET v_r_cod_respuesta_opr = recibeSolTraspInfonavitResponse.codrespopr
   LET v_r_desc_respuesta    = recibeSolTraspInfonavitResponse.descresp
   LET v_r_cod_oper_cliente  = recibeSolTraspInfonavitResponse.codopercliente
   LET v_folio_procesar      = recibeSolTraspInfonavitResponse.folioprocesar
   ...
   FOR v_indice = 1 TO recibeSolTraspInfonavitResponse.rechazo.element.getLength()
      LET v_ind_error = 1
      CALL v_r_rechazos_respuesta.appendElement()
      LET v_r_rechazos_respuesta[v_indice].cod_rechazo = recibeSolTraspInfonavitResponse.rechazo.element[v_indice].cod_rechazo
      LET v_r_rechazos_respuesta[v_indice].desc_rechazo = recibeSolTraspInfonavitResponse.rechazo.element[v_indice].desc_rechazo
...
PRIVATE FUNCTION fn_ejecuta_negocio()
   ...
   LET v_comando = "cd ", v_rura_exe CLIPPED, ";nohup ./ejecuta_negocio.sh ","'", v_sp_bus_ejecuta_negocio CLIPPED, "' 1> ", v_ruta_log CLIPPED, "/resp_iniciador_PRTW11-", v_consecutivo CLIPPED, ".log 2>&1 &"
   DISPLAY v_comando
   RUN v_comando
...
PRIVATE FUNCTION fn_asigna_valor(p_nombre, p_valor)
   ...
   CASE p_nombre
      WHEN "folioCliente"
         LET recibeSolTraspInfonavit.solicitudtraspaso.foliocliente = p_valor
      WHEN "institutoOrigen"
         LET recibeSolTraspInfonavit.solicitudtraspaso.origensolicitud = p_valor
      WHEN "tipoMovimiento"
         LET recibeSolTraspInfonavit.solicitudtraspaso.tipomovimiento = p_valor
      WHEN "nss"
         LET recibeSolTraspInfonavit.solicitudtraspaso.nss = p_valor
      WHEN "apellidoPaterno"
         LET recibeSolTraspInfonavit.solicitudtraspaso.apellidopaterno = p_valor
      WHEN "apellidoMaterno"
         LET recibeSolTraspInfonavit.solicitudtraspaso.apellidomaterno = p_valor
      WHEN "nombre"
         LET recibeSolTraspInfonavit.solicitudtraspaso.nombre = p_valor
      WHEN "curp"
         LET recibeSolTraspInfonavit.solicitudtraspaso.curp = p_valor
      WHEN "tipoOperacion"
         LET recibeSolTraspInfonavit.solicitudtraspaso.tipooperacion = p_valor
      WHEN "identificadorDeCreditoInfonavit"
         LET recibeSolTraspInfonavit.solicitudtraspaso.idcreditoinfonavit = p_valor
      WHEN "IdentificadorDeCreditoFovissste"
         LET recibeSolTraspInfonavit.solicitudtraspaso.idcreditofovissste = p_valor
      WHEN "montoDelCreditoInfonavit"
         LET recibeSolTraspInfonavit.solicitudtraspaso.mtocreditoinfonavit = p_valor
      WHEN "montoDelCreditoFovissste"
         LET recibeSolTraspInfonavit.solicitudtraspaso.mtocreditofovissste = p_valor
      WHEN "fechaDelCredito"
         LET recibeSolTraspInfonavit.solicitudtraspaso.fechacredito = p_valor
      WHEN "valorAivInfonavit97"
         LET recibeSolTraspInfonavit.solicitudtraspaso.valoraivinf97 = p_valor
      WHEN "fechaDeValorAivsInfonavit97"
         LET recibeSolTraspInfonavit.solicitudtraspaso.fechavaloraivinf97 = p_valor
      WHEN "numeroAivsInfonavit97"
         LET recibeSolTraspInfonavit.solicitudtraspaso.aivsinf97 = p_valor
      WHEN "montoEnPesosInfonavit97"
         LET recibeSolTraspInfonavit.solicitudtraspaso.pesosinf97 = p_valor
      WHEN "valorAivsFovissste2008"
         LET recibeSolTraspInfonavit.solicitudtraspaso.valoraivfov08 = p_valor
      WHEN "fechaDeValorAivsFovissste2008"
         LET recibeSolTraspInfonavit.solicitudtraspaso.fechavaloraivfov08 = p_valor
      WHEN "numeroAivsFovissste2008"
         LET recibeSolTraspInfonavit.solicitudtraspaso.aivsfov08 = p_valor
      WHEN "montoEnPesosFovissste2008"
         LET recibeSolTraspInfonavit.solicitudtraspaso.pesosfov08 = p_valor
      WHEN "fechaValorDeLaTransferencia"
         LET recibeSolTraspInfonavit.solicitudtraspaso.fechatransferencia = p_valor
