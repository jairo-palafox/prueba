...
#Modulo       => PRT                                                           #
#Programa     => PRTX33                                                        #
#Objetivo     => Integración  acpetadoscontingente viv 92                       #
#Fecha inicio => 12 Dic 2018                                                   #
################################################################################
DATABASE safre_viv
...
CONSTANT v_sr_elegido    = "sr_elegido" # Se asigna el nombre de la tabla de archivos elegidos
   ...
   OPEN WINDOW vtna_integra_trasp_receptora WITH FORM v_ruta_ejecutable CLIPPED||"/PRTX361"
      ...
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         BEFORE DIALOG
            EXECUTE prp_recupera_pid USING "2819" --C_PROCESO_COD_TRANS_SDO_RECEPTORA
                                      INTO g_pid

            CALL fn_valida_operacion(g_pid,
                                     2819,            
                                     --C_PROCESO_COD_TRANS_SDO_RECEPTORA,
                                     2) RETURNING r_valida
...
DEFINE v_archivos_pendientes DYNAMIC ARRAY OF STRING,
   ...
   FOREACH cur_recupera_pendientes USING "2819", --C_PROCESO_COD_TRANS_SDO_RECEPTORA,
                                         "1"
                                         --C_OPERA_COD_CARGA
                                    INTO v_archivo
...
DEFINE p_archivo  STRING,
   ...
   LET v_consulta = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/PRTX37.42r '",p_usuario_cod CLIPPED, "' ",
                                                                              g_pid CLIPPED, " ",
