...
DEFINE v_bnd_municipio    BOOLEAN
   ...
   OPEN WINDOW w_mant_mandatos WITH FORM v_ruta_ejecutable CLIPPED||"/PRTM111"
      INPUT v_desc_mandato, v_desc_larga_mandato FROM desc_mandato, desc_larga_mandato ATTRIBUTES (UNBUFFERED)
         BEFORE INPUT
            ...
            SELECT desc_tpo_entidad
              INTO v_tpo_mandato_desc
              FROM prt_tpo_entidad
             WHERE tpo_entidad = p_tipo_mandato
   ...
   OPEN WINDOW w_mant_mandatos_detalle WITH FORM v_ruta_ejecutable CLIPPED||"/PRTM112"
...
FUNCTION fn_almacena_registro_det()
  ...
  DEFINE v_id_cat_mandato_paquete  INTEGER
   ...
   --PASO 1: Se inserta en mdt_cat_mandato
       SELECT (NVL(MAX(id_cat_entidad),0) +1) INTO V_max_mdt_cat_mandato
      ...
      CASE v_arr_detalle[v_indice_detalle].etiqueta CLIPPED
         ...
         WHEN "MUNICIPIO"
            ...
            IF(v_tpo_mandato = 1)THEN # solo se inserta si es de tipo predial
               INSERT INTO prt_cat_entidad_paquete(id_cat_entidad,cve_entidad)
   ...
   SELECT (NVL(MAX(id_prt_notifica),0) + 1) INTO v_max_id_mdt_notifica
   ...
   IF(SQLCA.SQLCODE <> 0)THEN
      DISPLAY "ERROR SQL(INSERT INTO prt_notifica_entidad): ",SQLCA.SQLCODE
      --LET v_error = TRUE
   END IF
