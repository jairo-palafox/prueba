...
#Modulo            => PRT                                                      #
#Programa          => PRTE21                                                   #
#Objetivo          => Batch de integración de devoluciones de saldos cedente   # 
...
DEFINE v_proceso_desc    LIKE cat_proceso.proceso_desc,
   ...
   PREPARE prp_elimina_tabla_tmp FROM "DROP TABLE IF EXISTS tmp_prt_dev_integrados_ced"
   EXECUTE prp_elimina_tabla_tmp
   --DROP TABLE IF EXISTS tmp_prt_integrados
   CREATE TABLE tmp_prt_dev_integrados_ced(tpo_operacion  CHAR(2),
                                           nss            CHAR(11),
                                           rechazado      SMALLINT,
   ...
   CREATE TEMP TABLE tmp_prt_dev_rechazados_ced(registro CHAR(213))
   ...
   LET v_consulta = "EXECUTE FUNCTION fn_prt_integra_devolucion_sdo_cedente(?,?,?,?,?,?)"
   ...
   # si ocurrió un error con la actualizacion de la operación
   # muestra el mensaje
   IF( r_resultado_opera )THEN
      CALL fn_desplega_inc_operacion(r_resultado_opera)
   ELSE
      ...
      CALL fn_correo_proceso(p_pid, 
                             ...
                             p_opera_cod, 
                             '',
                             'Integración de devolución de saldos cedente',
                             'ID Proceso   : '||p_pid||
      ...
      DISPLAY "v_total_procesados:",v_total_procesados 
