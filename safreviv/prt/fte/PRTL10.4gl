...
DEFINE v_consulta STRING
   ...
   LET v_consulta = " SELECT CASE trs.tpo_operacion WHEN '01' THEN 'TRANSFERENCIA DE SALDO TOTAL VIV 97' WHEN '02' THEN 'TRANSFERENCIA PARCIAL DE SALDO VIV 97' ELSE 'NO ESPECIFICADO' END CASE,",
                    ...
                    " UNION ALL ",
                    " SELECT CASE trs.tpo_operacion WHEN '01' THEN 'TRANSFERENCIA DE SALDO TOTAL VIV 92' WHEN '02' THEN 'TRANSFERENCIA PARCIAL DE SALDO VIV 92' ELSE 'NO ESPECIFICADO' END CASE,",
                    "        SUM(NVL(trs.mto_aivs_infonavit92_afo,0)),",
                    "        SUM(NVL(trs.mto_pesos_infonavit92_afo,0)),",
                    "        SUM(NVL(trs.mto_aivs_infonavit92_recalculado,0)),",
                    "        SUM(NVL(trs.mto_pesos_infonavit92_recalculado,0)),",
                    "        SUM(NVL(trs.mto_aivs_infonavit92_cedido,0)),",
                    "        SUM(NVL(trs.mto_pesos_infonavit92_cedido,0))",
                    "   FROM prt_traspaso_cedente trs JOIN prt_solicitud_cedente sol",
                    "     ON sol.id_prt_solicitud_cedente = trs.id_prt_solicitud_cedente",
                    "  WHERE trs.estado = ?",
                    "    AND sol.tipo_portabilidad = ?",
...
DEFINE r_valida   SMALLINT,
       ...
       v_traspasos DYNAMIC ARRAY OF RECORD
          v_tpo_operacion  VARCHAR(100),--LIKE prt_traspaso_cedente.tpo_operacion,
   ...
   OPEN WINDOW vtna_preliq_trasp_cednete WITH FORM v_ruta_ejecutable CLIPPED||"/PRTL101"
      ...
      DISPLAY ARRAY v_traspasos TO sr_traspasos.* ATTRIBUTES(ACCEPT = FALSE, CANCEL = FALSE)
         ...
         BEFORE DISPLAY
            ...
            IF(v_traspasos.getLength() = 0)THEN
               CALL fn_mensaje(p_cad_ventana,"No hay información para preliquidar","about")
               EXIT DISPLAY
            END IF 

         ON ACTION preliquidar
            CALL fn_ventana_confirma(p_cad_ventana,"¿Preliquidar traspasos?","question") RETURNING r_confirma
            IF( r_confirma )THEN
               CALL fn_valida_operacion(g_pid,
                                        C_PROCESO_COD_TRANS_SDO_CEDENTE,
      ...
      END DISPLAY
   CLOSE WINDOW vtna_preliq_trasp_cednete
...
DEFINE v_indice   SMALLINT,
   ...
   FOREACH cur_recupera_traspasos USING C_ESTADO_TRASPASO_NOTIFICADA_CED,
                                        C_TIPO_PRT_CRED_NUEVO,
                                        C_ESTADO_SDO_NOTIFICADO_PRO,
                                        C_ESTADO_TRASPASO_NOTIFICADA_CED,
                                        C_TIPO_PRT_CRED_NUEVO,
                                        C_ESTADO_SDO_NOTIFICADO_PRO
                                   ...
                                   INTO v_traspaso.*
      LET v_traspasos[v_indice].v_tpo_operacion  = v_traspaso.v_tpo_operacion
      LET v_traspasos[v_indice].v_aivs_afore     = v_traspaso.v_aivs_afore
      LET v_traspasos[v_indice].v_pesos_afore    = v_traspaso.v_pesos_afore
      LET v_traspasos[v_indice].v_aivs_inf_reca  = v_traspaso.v_aivs_inf_reca
      LET v_traspasos[v_indice].v_pesos_inf_reca = v_traspaso.v_pesos_inf_reca
      LET v_traspasos[v_indice].v_aivs_cedido    = v_traspaso.v_aivs_cedido
      LET v_traspasos[v_indice].v_pesos_cedido   = v_traspaso.v_pesos_cedido
...
DEFINE v_comando          STRING,
   ...
   LET v_error = FALSE
   CALL fn_genera_pid(C_PROCESO_COD_TRANS_SDO_CEDENTE,
   ...
   ELSE
      ...
      LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/PRTE10.42r ",p_usuario_cod, " ",
                                                                               g_pid, " ",
                                                                               C_PROCESO_COD_TRANS_SDO_CEDENTE," ",
                                                                               C_OPERA_COD_PRELIQ_TANS_CED," ",
                                                                               "0 ",
                      ...
                      " 1>", v_ruta_listados CLIPPED,"/nohup:",g_pid USING "&&&&&",":",
                                                               C_PROCESO_COD_TRANS_SDO_CEDENTE USING "&&&&&",":",
