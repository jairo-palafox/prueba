...
#Modulo            => PRT                                                      #
#Programa          => PRTX38                                                   #
#Objetivo          => Genera reporte de integración aceptados contingente      # 
   ...
   IF(fgl_report_loadCurrentSettings(r_ruta_ejecutable CLIPPED||"/PRTI021.4rp"))THEN
      ...
      DISPLAY "p_proceso_cod:",p_proceso_cod
         ...
         LET v_consulta = " SELECT CASE tmp.tpo_operacion WHEN '01' THEN 'TRANSFERENCIA TOTAL' WHEN '02' THEN 'TRANSFERENCIA PARCIAL' WHEN '03' THEN 'DEVOLUCIÓN' ELSE 'N/C' END CASE,",
                          ...
                          "        SUM(monto_pesos)",                          
                          v_tabla,
                          "        LEFT OUTER JOIN prt_diagnostico cat",
                          "     ON tmp.motivo_rechazo = cat.diagnostico_externo",
                          "  WHERE 1 = 1",
                          "  AND   cat.destino_diagnostico = 'T-I' ",                          
                          "  GROUP BY 1,2,3"
         --DISPLAY v_consulta
...
DEFINE p_folio      LIKE glo_folio.folio,
   ...
   FORMAT
      ...
      FIRST PAGE HEADER
         LET v_titulo_rpt = "REPORTE INTEGRACIÓN ACEPTADOS CONTINGENTE SAR 92"
