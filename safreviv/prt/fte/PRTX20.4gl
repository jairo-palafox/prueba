--==============================================================================
-- Version: 1.0.0
-- Fecha ultima modificacion: 30/06/2015
--==============================================================================

################################################################################
#Modulo          => PRT                                                        #
#Programa        => PRTX20                                                     #
#Objetivo        => Programa batch de validación devoluciones de saldos cedente#
...
DEFINE v_conteo_det_total INTEGER,
   ...
   SELECT COUNT(*)
     ...
     FROM safre_tmp:tmp_prt_det_dev_cedente
    WHERE 1 = 1

   SELECT total_registros
     INTO v_total_sum
     FROM safre_tmp:tmp_prt_sum_dev_cedente
    WHERE 1 = 1
...
# Descripción: Validación de los campos del detalle de archivo para devolucion de saldos cedente
...
DEFINE v_consulta         STRING,
   ...
   PREPARE prp_crea_tbl_tmp_rch FROM "CREATE TEMP TABLE tmp_prt_dev_rechazados_ced(registro VARCHAR(213))"
   EXECUTE prp_crea_tbl_tmp_rch
   PREPARE prp_crea_tbl_tmp FROM "CREATE TEMP TABLE tmp_prt_dev_inconsistencias_ced(registro DECIMAL(9,0),rechazo_cod CHAR(3))"
   EXECUTE prp_crea_tbl_tmp

   LET v_consulta = "EXECUTE PROCEDURE sp_prt_valida_devolucion_cedente()"
   PREPARE prp_valida_carga FROM v_consulta
   TRY 
      EXECUTE prp_valida_carga
   CATCH
      DISPLAY "OCURRIÓ UN ERROR AL EJECUTAR EL PROCEDIMIENTO DE VALIDACIÓN"
   ...
   SELECT COUNT(*) 
     INTO v_conteo_inc
     FROM tmp_prt_inconsistencias_ced
    WHERE 1 = 1
   ...
   IF(v_conteo_inc > 0)THEN
      ...
      LET v_consulta = " SELECT tmp.registro,",
                       "        cat.rechazo_desc",
                       "   FROM tmp_prt_dev_inconsistencias_ced tmp LEFT OUTER JOIN prt_cat_rch_traspasos cat",
      ...
      LET v_consulta = " SELECT registro",
                       "   FROM tmp_prt_dev_rechazados_ced",
                       "  WHERE 1 = 1"
