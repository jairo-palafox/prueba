...
DEFINE r_folio_procesar LIKE bus_tramite.folio_procesar,
       ...
       r_tramite DYNAMIC ARRAY OF RECORD
         ...
         n_caso              LIKE prt_solicitud_cedente.n_caso         
       END RECORD,
...
DEFINE p_folio_procesar LIKE bus_tramite.folio_procesar,
       ...
       v_tramite RECORD
         ...
         v_folio_ack         LIKE bus_respuesta_tramite.folio_ack,
         n_caso              LIKE prt_solicitud_cedente.n_caso  ,
         v_id_solicitud_tramite LIKE bus_solicitud_tramite.id_bus_solicitud_tramite        
       END RECORD,
   ...
   LET v_consulta = " SELECT 'SOLICITUD DE MARCA',b.nss,b.curp,b.n_caso, max(a.id_prt_nuevo) ",
                    " FROM prt_his_id_folio a , ",
                    "      prt_solicitud_cedente b ",
                    " WHERE b.estado = 30 ",
                    " AND   b.id_prt_solicitud_cedente = a.id_prt_origen ",                    
                    " AND  (b.folio_procesar is null OR b.folio_procesar in ('',' ')) ",
                    " GROUP BY 1,2,3,4 ",
                    " UNION ALL ",                    
                    " SELECT 'SOLICITUD DE TRANSFERENCIA',b.nss,b.curp,c.n_caso, max(a.id_prt_nuevo) ",
                    " FROM prt_his_id_folio a , ",
                         " prt_traspaso_cedente b , ",
                         " prt_solicitud_cedente c ",
                    " WHERE b.estado = 10 ",
                    " AND   b.id_prt_traspaso_cedente = a.id_prt_origen ",
                    " AND  (b.folio_procesar is null OR b.folio_procesar in ('',' ')) ",
                    " and   b.id_prt_solicitud_cedente = c.id_prt_solicitud_cedente ",
                    " and   c.estado in (44,50) ",                    
                    " and   a.tpo_solicitud = 2 ",
                    " GROUP BY 1,2,3,4 ",
                    " UNION ALL ",
                    " SELECT 'NOTIFICACION  TRANSFERENCIA',b.nss,b.curp,c.n_caso, max(a.id_prt_nuevo) ",
                    " FROM prt_his_id_folio a , ",
                    "      prt_traspaso_cedente b , ",
                    "      prt_solicitud_cedente c ",
                    " WHERE b.estado = 10 ",
                    " AND   b.id_prt_traspaso_cedente = a.id_prt_origen ",
                    " AND   b.folio_procesar is not null ",
                    " AND   b.folio_procesar not in ('',' ') ",
                    " and   b.id_prt_solicitud_cedente = c.id_prt_solicitud_cedente ",
                    " and   c.estado in (50) ",
                    " and   a.tpo_solicitud = 2 ",
                    " GROUP BY 1,2,3,4 "
   ...
   FOREACH cur_recupera_tramite INTO v_tramite.v_proceso    ,
                                     v_tramite.v_nss        ,
                                     v_tramite.v_curp       ,     
                                     v_tramite.n_caso       ,                                
                                     v_tramite.v_folioCliente

            
             SELECT MAX(id_bus_solicitud_tramite)
             INTO   v_tramite.v_id_solicitud_tramite             
             FROM   bus_detalle_solicitud
             WHERE  nombre_campo = "folioCliente"
             AND    valor = v_tramite.v_folioCliente

                                     
            SELECT a.folio_transaccion ,
                   a.f_operacion       ,
                   a.h_operacion       
            INTO   v_tramite.v_folio_transaccion,
                   v_tramite.v_fecha_tran,
                   v_tramite.v_h_operacion_tran 
            FROM   bus_solicitud_tramite a
            WHERE  a.id_bus_solicitud_tramite = v_tramite.v_id_solicitud_tramite
                         
            LET v_indice = v_indice + 1

            LET v_tramites[v_indice].v_consecutivo       = v_indice
            LET v_tramites[v_indice].v_desc_proceso      = v_tramite.v_proceso
            ...
            LET v_tramites[v_indice].v_fecha_tran        = v_tramite.v_fecha_tran
            LET v_tramites[v_indice].v_h_operacion_tran  = v_tramite.v_h_operacion_tran
            LET v_tramites[v_indice].v_folio_ack         = v_tramite.v_folio_ack
            LET v_tramites[v_indice].n_caso              = v_tramite.n_caso
...
DEFINE p_modulo_desc   LIKE seg_modulo.modulo_desc,
   ...
   IF( fgl_report_loadCurrentSettings(v_ruta_ejecutables CLIPPED ||"/PRTI12.4rp") )THEN
...
DEFINE p_modulo_desc          LIKE seg_modulo.modulo_desc,
   ...
   FORMAT
      ...
      BEFORE GROUP OF p_tramite.v_desc_proceso
         PRINTX p_tramite.v_desc_proceso
