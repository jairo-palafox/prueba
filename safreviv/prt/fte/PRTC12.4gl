...
DEFINE v_desc_tpo_mandato LIKE mdt_tpo_mandato.desc_tpo_mandato,
   ...
   OPEN WINDOW w_arbol_mandato WITH FORM "PRTC122"
      ...
      DISPLAY ARRAY tree TO scr_tree.*
         ...
         BEFORE DISPLAY
            ...
            WHILE v_rec_datos 
               ...
               IF(p_v_tpo_mdt IS NULL OR LENGTH(p_v_tpo_mdt CLIPPED) = 0 OR p_v_tpo_mdt = 0)THEN
                  ...
                  CALL fn_filtra_consulta_entidades() RETURNING v_id_tpo_mandato,v_minucipio,v_entidad_fed,v_bnd_salir
                  IF(v_bnd_salir)THEN
         ...
         ON ACTION alta
            ...
            IF tree[ARR_CURR()].pid IS NULL OR tree[ARR_CURR()].pid = 0 THEN
               CALL fn_mensaje("Advertencia","Es requerido que seleccione el tipo de Entidad con Convenio","error")
            ELSE
               IF tree[ARR_CURR()].pid <> 99 THEN
                  LET v_ejecuta = "fglrun PRTM11 ", p_v_usuario CLIPPED," ",
                                                    p_b_tipo_carga," ",
                                                    "Alta ", 
                                                    tree[ARR_CURR()].pid
         ...
         ON ACTION baja
            IF (tree[ARR_CURR()].pid IS NULL OR tree[ARR_CURR()].pid = 0) OR
               ...
               CALL fn_mensaje("Advertencia","Es requerido que seleccione  la Entidad con Convenio","error")
            ELSE
               LET v_ejecuta = "fglrun PRTM12 ", p_v_usuario CLIPPED," ",
                                                 p_b_tipo_carga," ",
                                                 "Baja ", 
                                                 tree[ARR_CURR()].pid, " ",
                                                 tree[ARR_CURR()].id
         ...
         ON ACTION modificar
            IF (tree[ARR_CURR()].pid IS NULL OR tree[ARR_CURR()].pid = 0) OR
               ...
               CALL fn_mensaje("Advertencia","Es requerido que seleccione  la Entidad con Convenio","error")
            ELSE
               LET v_ejecuta = "fglrun PRTM13 ", p_v_usuario CLIPPED," ",
                                                 p_b_tipo_carga," ",
                                                 "Modificar ", 
                                                 tree[ARR_CURR()].pid, " ",
                                                 tree[ARR_CURR()].id
            ...
            END IF
{
         ON ACTION movimientos
            IF( tree[ARR_CURR()].id = 0 OR tree[ARR_CURR()].id = 99 OR
               ...
               CALL fn_mensaje("Advertencia","Es requerido que seleccione  la entidad","error")
            ELSE
               IF(tree[ARR_CURR()].pid = 99)THEN # registro seleccionado es un grupo de mandato
                  # se muestra consulta global ya que seleccionó un grupo de mandatdo
...
#Fecha inicio      => 06 Septiembre 2012                           #
####################################################################
FUNCTION fn_filtra_consulta_entidades()
   ...
   INPUT v_tpo_mandato, 
    ...
    FROM cb_tpo_mandato,
      ...
      BEFORE INPUT
         ...
         CALL fn_llena_cb_tpo_entidad(v_cb_tpo_mandato)
...
#Fecha inicio      => 06 Septiembre 2012                           #
####################################################################
FUNCTION fn_llena_cb_tpo_entidad(v_cb_tpo_mandato)
DEFINE v_cb_tpo_mandato   ui.ComboBox,
   ...
   LET v_consulta = "\n SELECT tpo_entidad, desc_tpo_entidad",
                    "\n   FROM prt_tpo_entidad",
                    "\n  WHERE 1 = 1"
...
DEFINE v_s_qry    STRING,
   ...
   IF(p_id_tpo_mandato IS NOT NULL AND p_id_tpo_mandato <> 0)THEN
      ...
      LET v_filtro_tpo = "cat.tpo_entidad = ",p_id_tpo_mandato CLIPPED
   ...
   ELSE
      LET v_filtro_param = " cat.tpo_entidad = ", p_tpo_mdt 
   END IF
   ...
   LET v_s_qry =
      "\n(SELECT DISTINCT 0 id_entidad, '', desc_tpo_entidad, '', '', tpo_entidad, today, 0",
      --"\n   FROM mdt_tpo_mandato tpo",
      "\n   FROM prt_tpo_entidad cat",
      ...
      "\n  UNION ALL ",
      "\n SELECT DISTINCT cat.*",
      "\n   FROM prt_cat_entidad cat JOIN prt_cat_atributo_nivel nvl",
      "\n     ON nvl.id_cat_entidad = cat.id_cat_entidad",
      "\n        JOIN prt_cat_instancia_entidad ins",
      "\n     ON ins.id_atr_nivel = nvl.id_atr_nivel",
      "\n        JOIN prt_cat_gpo_etiqueta etq",
   ...
   # consulta para recupeerar el municipio d la entidad
   LET v_consulta = "\n SELECT etq.etiqueta,ins.valor_etiqueta",
                    "\n   FROM prt_cat_entidad mdt JOIN prt_cat_atributo_nivel nvl",
                    "\n     ON nvl.id_cat_entidad = mdt.id_cat_entidad",
                    "\n        JOIN prt_cat_instancia_entidad ins",
                    "\n     ON ins.id_atr_nivel = nvl.id_atr_nivel",
                    "\n        JOIN prt_cat_gpo_etiqueta etq",
                    "\n     ON etq.id_gpo_etiqueta = nvl.id_gpo_etiqueta",
                    "\n  WHERE etq.etiqueta = 'MUNICIPIO'",
                    "\n    AND mdt.id_cat_entidad = ?"
   ...
   FOREACH Curr_ObtenerListaArbol INTO v_r_mdt.*
      ...
      IF  v_i_indice = 1 THEN
         LET tree[v_i_indice].name = 'TIPOS DE CONVENIO'
...
FUNCTION fn_consulta_detalle(p_tpo_mdt, p_cat_mdt) 
   ...
   OPEN WINDOW w_con_mandato WITH FORM "PRTC12"
...
#############################################################################
# Funcion           => fn_carga_arr_mdt - Carga detalle d la entidad         #
# Propietario       => E.F.P                                                #
# Sistema           => HPS                                                  #
# Entrada:          => Ninguno                                              #
# Salida:           => Ninguno                                              #
...
FUNCTION init_combo_mandato(p_tpo_mdt)
   ...
   IF p_tpo_mdt IS NULL THEN
      LET v_s_qry = ""
   ELSE
      LET v_s_qry = " AND tpo_entidad = ", p_tpo_mdt
   END IF
...
#############################################################################
# Funcion           => fn_carga_arr_img - Carga imagenes d la entidad        #
# Propietario       => E.F.P                                                #
# Sistema           => HPS                                                  #
# Entrada:          => Ninguno                                              #
# Salida:           => Ninguno                                              #
...
Narrativa del proceso que realiza:
 Función para recuperar montos totales de movimientos d la entidad
...
"\n SELECT NVL(SUM(tab1.monto_pesos),0), ",
 ...
 # Solo relaciona que exista  la entidad para el atributo seleccionado
