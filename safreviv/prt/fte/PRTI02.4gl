...
DEFINE v_consulta         STRING,
   ...
   IF(fgl_report_loadCurrentSettings(r_ruta_ejecutable CLIPPED||"/PRTI021.4rp"))THEN
      ...
      START REPORT fn_rpt_integracion_trasp_recept TO XML HANDLER v_manejador_rpt
         ...
         DISPLAY "C_PROCESO_COD_DEV_SDO_RECEPTORA:",C_PROCESO_COD_DEV_SDO_RECEPTORA
         CASE p_proceso_cod
            ...
            WHEN C_PROCESO_COD_TRANS_SDO_SUBSEC_RECEPTORA
               LET v_tabla = "   FROM safre_tmp:tmp_prt_integrados_subsec tmp"
               SELECT COUNT(*)
                 INTO v_total_procesados
                 FROM safre_tmp:tmp_prt_integrados_subsec
                WHERE 1 = 1   

            WHEN C_PROCESO_COD_DEV_SDO_RECEPTORA
               LET v_tabla = "   FROM safre_tmp:tmp_prt_dev_integrados tmp"
               SELECT COUNT(*)
                 INTO v_total_procesados
                 FROM safre_tmp:tmp_prt_dev_integrados
                WHERE 1 = 1  
                
            WHEN C_PROCESO_COD_DEV_SDO_CEDENTE
               LET v_tabla = "   FROM safre_tmp:tmp_prt_dev_integrados_ced tmp"
               SELECT COUNT(*)
                 INTO v_total_procesados
                 FROM safre_tmp:tmp_prt_dev_integrados_ced
                WHERE 1 = 1
         ...
         LET v_consulta = " SELECT CASE tmp.tpo_operacion WHEN '01' THEN 'TRANSFERENCIA TOTAL' WHEN '02' THEN 'TRANSFERENCIA PARCIAL' WHEN '03' THEN 'DEVOLUCIÓN' ELSE 'N/C' END CASE,",
                          ...
                          "        SUM(monto_pesos)",                          
                          v_tabla,
                          "        LEFT OUTER JOIN prt_cat_rch_traspasos cat",
                          "     ON tmp.motivo_rechazo = cat.rechazo_cod",
                          "  WHERE 1 = 1",
                          "  GROUP BY 1,2,3"
         --DISPLAY v_consulta
...
DEFINE p_folio      LIKE glo_folio.folio,
   ...
   FORMAT
      ...
      FIRST PAGE HEADER
         CASE p_proceso_cod
            WHEN C_PROCESO_COD_TRANS_SDO_RECEPTORA
               LET v_titulo_rpt = "REPORTE INTEGRACIÓN DE TRASPASOS DE SALDOS RECEPTORA"
               
            WHEN C_PROCESO_COD_TRANS_SDO_SUBSEC_RECEPTORA
               LET v_titulo_rpt = "REPORTE INTEGRACIÓN DE TRASPASOS SUBSECUENTES RECEPTORA"
               
            WHEN C_PROCESO_COD_DEV_SDO_RECEPTORA
               LET v_titulo_rpt = "REPORTE INTEGRACIÓN DE DEVOLUCIÓN DE SALDOS RECEPTORA"
               
            WHEN C_PROCESO_COD_DEV_SDO_CEDENTE
               LET v_titulo_rpt = "REPORTE INTEGRACIÓN DE DEVOLUCIÓN DE SALDOS CEDENTE"
