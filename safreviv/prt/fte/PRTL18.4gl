...
#Modulo            => PRT                                                      #
#Programa          => PRTL18                                                   #
#Descripcion       => Preliquidación de devolución de saldos receptora         #
...
MAIN
   ...
   CALL fn_recupera_folio_devolucion_receptora()
...
DEFINE v_consulta STRING
   ...
   LET v_consulta = " SELECT CASE tpo_operacion WHEN '01' THEN 'TRANSFERENCIA TOTAL'",
                    " WHEN '02' THEN 'TRANSFERENCIA PARCIAL' ",
                    " WHEN '03' THEN 'DEVOLUCIÓN' ",
                    " ELSE 'NO CLASIFICADO'  END CASE,",
...
FUNCTION fn_recupera_folio_devolucion_receptora()
...
CONSTANT v_sr_elegido    = "sr_elegido" # Se asigna el nombre de la tabla de archivos elegidos
   ...
   OPEN WINDOW vtna_preliq_dev_receptora WITH FORM v_ruta_ejecutable CLIPPED||"/PRTL181"
      ...
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         BEFORE DIALOG
            EXECUTE prp_recupera_pid USING C_PROCESO_COD_DEV_SDO_RECEPTORA
                                      INTO g_pid
            CALL fn_valida_operacion(g_pid,
                                     C_PROCESO_COD_DEV_SDO_RECEPTORA,
   ...
   CLOSE WINDOW vtna_preliq_dev_receptora
   ...
   DISPLAY ARRAY v_cifras_preliquidar TO sr_cifras.* ATTRIBUTES(ACCEPT = FALSE, CANCEL = FALSE)
      ...
      ON ACTION preliquidar
         ...
         IF( r_confirma )THEN
            ...
            ELSE
               CALL fn_ejecuta_preliquidacion_devolucion(p_folio_preliquidar) RETURNING r_error
               IF( r_error )THEN
                  LET v_continua = TRUE
               END IF
...
FUNCTION fn_ejecuta_preliquidacion_devolucion(p_folio_preliquidar)
DEFINE p_folio_preliquidar STRING, 
       v_comando          STRING,
   ...
   ELSE
      ...
      LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/PRTE18.42r ",p_usuario_cod, " ",
                                                                               g_pid, " ",
                                                                               C_PROCESO_COD_DEV_SDO_RECEPTORA," ",
                                                                               C_OPERA_COD_PRELIQUIDACION," ",
                                                                               p_folio_preliquidar,
                      ...
                      " 1>", v_ruta_listados CLIPPED,"/nohup:",g_pid USING "&&&&&",":",
                                                               C_PROCESO_COD_DEV_SDO_RECEPTORA USING "&&&&&",":",
