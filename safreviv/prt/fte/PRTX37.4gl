...
#Modulo            => PRT                                                      #
#Programa          => PRTX34                                                   #
#Objetivo          => Batch aceptados contingente viv 92                       # 
#Fecha inicio      => 10 Dic 2018                                              #
################################################################################
DATABASE safre_viv
...
DEFINE r_contingente RECORD LIKE safre_tmp:tmp_prt_rac_cont.*
...
END RECORD
   ...
   DATABASE safre_tmp   
   PREPARE prp_elimina_tabla_tmp FROM "DROP TABLE IF EXISTS tmp_prt_integrados"
   EXECUTE prp_elimina_tabla_tmp
   --EXECUTE "DROP TABLE IF EXISTS tmp_prt_integrados"
   --DROP TABLE IF EXISTS tmp_prt_integrados
   CREATE TABLE tmp_prt_integrados(tpo_operacion  CHAR(2),
                                   nss            CHAR(11),
                                   rechazado      SMALLINT,
                                   monto_pesos    DECIMAL(22,2),
   ...
   SELECT COUNT(*)
     ...
     FROM safre_tmp:tmp_prt_rac_cont
   ...
   LET v_consulta = " SELECT * FROM safre_tmp:tmp_prt_rac_cont "
   PREPARE prp_contingente FROM v_consulta 
   DECLARE cur_contingente CURSOR FOR prp_contingente 
   FOREACH cur_contingente INTO r_contingente.*
   
        SELECT a.tpo_operacion  ,
               a.nss  ,
               a.mto_pesos_infonavit92  
        INTO   r_reporte.*
        FROM   prt_traspaso_cedente a
        WHERE  a.nss = r_contingente.nss 
        AND    a.estado = 10
        
      IF r_contingente.diagnostico = "000" THEN
      
         UPDATE prt_traspaso_cedente
         SET    folio_procesar = r_contingente.folio_procesar
         WHERE  nss    = r_contingente.nss
         AND    estado = 10
         ...
         INSERT INTO safre_tmp:tmp_prt_integrados VALUES (r_reporte.tipo_operacion,
                                                r_reporte.nss,
                                                "0" ,
                                                r_reporte.monto_pesos,
                                                "000")
   ...
   DISPLAY ""
   DISPLAY "TOTAL DE REGISTROS ACEPTADOS  : ",v_total_procesados USING "##,###,##&"
...
--   DISPLAY "TOTAL DE REGISTROS PROCESADOS: ",v_total_procesados USING "##,###,##&" 
   ...
   ELSE
      ...
      CALL fn_correo_proceso(p_pid, 
                             ...
                             p_opera_cod, 
                             '',
                             'Integración de traspaso de saldos cedente',
                             'ID Proceso   : '||p_pid||
      ...
      IF( v_total_procesados > 0 )THEN
         ...
         LET v_comando = "fglrun ",v_ruta_ejecutable CLIPPED,"/PRTX38.42r '",
                                   p_usuario CLIPPED, "' ",
                                   p_pid CLIPPED, " ",
