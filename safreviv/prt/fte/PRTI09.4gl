...
#MODULO           => PRT                                                       #
#PROGRAMA         => PRTI09                                                    #
#OBJETIVO         => REPORTE DE CANCELACION SOLICITUDES PORTABILIDAD           #
...
MAIN
   ...
   --MANEJO DE ERRORES
    ...
    CALL fn_display_proceso(0,"GENERACIÓN DE REPORTE CANCELACIÓN SOLICITUDES PORTABILIDAD")
   --Se Procede a llenar el filto
   SELECT a.nss,a.num_caso,(SELECT estatus_cancelacion_desc FROM prt_cat_estatus_cancelacion WHERE estatus_cancelacion = a.estatus_cancelacion),a.f_ini,a.f_fin 
      INTO t_filtro.*
      FROM tmp_cancelacion_filtro a
   ...
   LET i = 1
   PREPARE prp_llena_t FROM "SELECT * FROM tmp_consulta_cancelacion_portabilidad ORDER BY 1"
   ...
   IF SQLCA.SQLCODE != 0 THEN
       LET r_cod_error = SQLCA.SQLCODE
       LET r_mensaje_error = SQLCA.SQLERRM
       DISPLAY "Error al generar el reporte: Problema con la tabla temporal de consulta: ",SQLCA.SQLCODE,r_cod_error,r_mensaje_error 
       CALL fn_error_opera(g_pid, g_proceso_cod, g_opera_cod)  RETURNING v_estado
    ELSE
       IF(fgl_report_loadCurrentSettings(v_ruta_bin CLIPPED||"/PRTI091.4rp"))THEN
         LET v_ruta_reporte = fn_nombra_reporte(v_ruta_lst,g_usuario_cod,"PRTI09",g_pid,g_proceso_cod,g_opera_cod)
       ...
       IF (r_cod_error = 0) THEN
          ...
          CALL fn_actualiza_opera_fin(g_pid,
                                 ...
                                 g_opera_cod)
                                 RETURNING v_estado
          DISPLAY "Generación de Reporte de Cancelación de Portabilidad exitosa"
          CALL fn_display_proceso(1,"GENERACIÓN DE REPORTE CANCELACIÓN SOLICITUDES PORTABILIDAD")
       ELSE
     ...
     END IF
    END IF
    --Borrando tablas temporales del proceso PRTC09
