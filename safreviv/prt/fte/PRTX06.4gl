...
#Modulo            => PRT                                                      #
#Programa          => PRTX06                                                   #
#Objetivo          => Programa batch de validacion traspaso de saldos          #
#                     subsecuentes receptora                                   #
...
DEFINE v_consulta         STRING,
   ...
   LET v_consulta = "EXECUTE PROCEDURE sp_prt_valida_trasp_subsec_receptora()"
   PREPARE prp_valida_carga FROM v_consulta
   EXECUTE prp_valida_carga
   IF(SQLCA.SQLCODE <> 0)THEN
      DISPLAY "Ocurrió un error en validación de carga de traspaso saldos subsecuentes receptora"
   END IF
