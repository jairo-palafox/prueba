...
DEFINE v_bnd_municipio    BOOLEAN
   ...
   SELECT ruta_bin
     ...
     FROM seg_modulo
    WHERE modulo_cod = "prt"
DISPLAY "entrando al PRTM13"
   # busca si hay una instruccion realcionada con direccion
   LET v_consulta = "\n SELECT COUNT(*)",
                    "\n   FROM prt_cat_gpo_etiqueta et JOIN prt_cat_gpo gpo",
   ...
   OPEN WINDOW w_mant_mandatos WITH FORM v_ruta_ejecutable CLIPPED||"/PRTM131"
      INPUT v_desc_mandato, v_desc_larga_mandato FROM desc_mandato, desc_larga_mandato ATTRIBUTES (UNBUFFERED, ACCEPT = FALSE, CANCEL = FALSE)
         BEFORE INPUT
            ...
            LET v_consulta = "\n SELECT FIRST 1 tpo.desc_tpo_entidad, cat.desc_entidad, cat.desc_larga_entidad",
                             "\n   FROM prt_cat_entidad cat JOIN prt_tpo_entidad tpo",
                             "\n     ON tpo.tpo_entidad = cat.tpo_entidad",
                             "\n  WHERE cat.tpo_entidad = ?",
                             "\n    AND cat.id_cat_entidad = ?"
   ...
   OPEN WINDOW w_mant_mandatos_detalle WITH FORM v_ruta_ejecutable CLIPPED||"/PRTM132"
                  ...
                  IF(LENGTH(v_imagen_docto[ARR_CURR()].nombre_imagen CLIPPED) > 0 )THEN
                     ...
                     CASE 
                        WHEN v_imagen_docto[ARR_CURR()].v_operacion = 'A' AND  # se esta agregando un documento nuevo por primera vez (en modificacion)
                           ...
                           LET v_imagen_docto[ARR_CURR()].v_documento_int   = "<a gwc:attributes=\"href resourceuri('",v_imagen_docto[ARR_CURR()].v_documento CLIPPED,"','prtdocto')\" target='nueva'>",v_imagen_docto[ARR_CURR()].v_documento CLIPPED,"</a>"
                     ...
                     IF(v_imagen_docto[ARR_CURR()].v_nombre_original <> v_imagen_docto[ARR_CURR()].v_documento AND (v_imagen_docto[ARR_CURR()].v_operacion <> 'A' OR v_imagen_docto[ARR_CURR()].v_documento IS NOT NULL))THEN
                        FOR v_indice = 1 TO v_imagen_docto_general.getLength()
                           IF(v_imagen_docto_general[v_indice].id_imagen_docto = v_imagen_docto[ARR_CURR()].id_imagen_docto AND
                              ...
                              LET v_imagen_docto_general[v_indice].v_documento_int = "<a gwc:attributes=\"href resourceuri('",v_imagen_docto_general[ARR_CURR()].v_documento CLIPPED,"','prtdocto')\" target='nueva'>",v_imagen_docto_general[ARR_CURR()].v_documento CLIPPED,"</a>"
...
DEFINE v_r_detalle RECORD
  ...
  ,id_cat_gpo          LIKE mdt_cat_gpo_etiqueta.id_cat_gpo,
   ...
   v_nombre_original   STRING
END RECORD
DEFINE  v_r_imagen_docto_general  RECORD 
        id_imagen_docto   LIKE mdt_imagen_docto.id_imagen_docto,
        id_gpo_mandato    LIKE mdt_imagen_docto.id_gpo_mandato,
        id_cat_gpo        LIKE mdt_cat_gpo.id_cat_gpo,
        nombre_imagen     STRING, -- LIKE mdt_imagen_docto.nombre_imagen,
...
--   WHENEVER ERROR CONTINUE
   ...
   LET v_consulta = " SELECT cat.id_cat_gpo, cat.descripcion, ", 
                    "        NVL(CASE                         ",
                    "              WHEN mto.id_cat_gpo IS NULL THEN 0 ",
                    "              WHEN mto.id_cat_gpo IS NOT NULL THEN 1 ",
                    "            END,0) ",
                    "   FROM prt_cat_gpo cat LEFT OUTER JOIN prt_gpo_entidad mto",
                    "     ON mto.id_cat_entidad = ? ",
                    "    AND cat.id_cat_gpo = mto.id_cat_gpo "

   PREPARE prp_grupos FROM v_consulta
   DECLARE cur_grupos CURSOR FOR prp_grupos   

   # Recupera los documentos del grupo
   LET v_consulta = " SELECT doc.id_imagen_docto, doc.id_gpo_entidad, doc.nombre_imagen, doc.desc_imagen",
                    "   FROM prt_imagen_docto doc JOIN prt_gpo_entidad gpo",
                    "     ON doc.id_gpo_entidad = gpo.id_gpo_entidad",
                    "  WHERE gpo.id_cat_entidad = ?",
                    "    AND gpo.id_cat_gpo = ?"
   ...
   FOREACH cur_grupos USING p_id_cat_mandato 
                      INTO  v_r_grupos.id_cat_gpo  ,
                            v_r_grupos.descripcion ,
                            v_r_grupos.v_existe
                            
       IF v_r_grupos.v_existe IS NULL THEN LET v_r_grupos.v_existe = 0 END IF
                                         
            LET v_arr_grupos[v_indice].id_cat_gpo  = v_r_grupos.id_cat_gpo
            LET v_arr_grupos[v_indice].descripcion = v_r_grupos.descripcion
            LET v_arr_grupos[v_indice].v_existe    = v_r_grupos.v_existe
            
DISPLAY "id gpo: ",v_r_grupos.id_cat_gpo            
DISPLAY "desc gpo: ",v_r_grupos.descripcion
DISPLAY "existe: ",v_r_grupos.v_existe
      ...
      IF(v_arr_grupos[v_indice].v_existe = 1)THEN
         FOREACH cur_rec_docto_grupo USING p_id_cat_mandato, v_arr_grupos[v_indice].id_cat_gpo
                                     INTO  v_r_imagen_docto_general.id_imagen_docto,
                                           v_r_imagen_docto_general.id_gpo_mandato,
                                           v_c_nombre_imagen, -- v_imagen_docto_general[v_indice_docto].nombre_imagen,
                                           v_desc_imagen

            LET v_imagen_docto_general[v_indice_docto].id_imagen_docto = v_r_imagen_docto_general.id_imagen_docto
            LET v_imagen_docto_general[v_indice_docto].id_gpo_mandato = v_r_imagen_docto_general.id_gpo_mandato
            ...
            LET v_imagen_docto_general[v_indice_docto].v_documento_int   = "<a gwc:attributes=\"href resourceuri('",v_imagen_docto_general[v_indice_docto].v_documento CLIPPED,"','prtdocto')\" target='nueva'>",v_imagen_docto_general[v_indice_docto].v_documento CLIPPED,"</a>" 
   ...
   FROM prt_cat_gpo_etiqueta

   LET v_indice = 1
   FOREACH cur_elementos INTO v_r_elementos.id_gpo_etiqueta
                             ,v_r_elementos.id_cat_gpo
                             ,v_r_elementos.etiqueta

        LET v_arr_elementos[v_indice].id_gpo_etiqueta = v_r_elementos.id_gpo_etiqueta
        LET v_arr_elementos[v_indice].id_cat_gpo = v_r_elementos.id_cat_gpo
        LET v_arr_elementos[v_indice].etiqueta = v_r_elementos.etiqueta
   ...
   LET v_consulta = "\n SELECT nvl.id_atr_nivel, nvl.id_gpo_etiqueta, nvl.id_cat_entidad, nvl.id_gpo_entidad,",
                    "\n        nvl.orden, eti.etiqueta, ins.valor_etiqueta, eti.id_cat_gpo",
                    "\n   FROM prt_cat_atributo_nivel nvl JOIN prt_cat_instancia_entidad ins",
                    "\n     ON ins.id_atr_nivel = nvl.id_atr_nivel",
                    "\n   JOIN prt_cat_gpo_etiqueta eti",
                    "\n     ON eti.id_gpo_etiqueta = nvl.id_gpo_etiqueta",
                    "\n  WHERE nvl.id_cat_entidad = ? ",
                    "\n  ORDER BY nvl.id_gpo_entidad, nvl.orden"
   ...
   FOREACH cur_rec_atributos_mandato USING p_id_cat_mandato
                                     INTO  v_r_detalle.id_atr_nivel,
                                           v_r_detalle.id_gpo_etiqueta,
                                           v_r_detalle.id_cat_mandato,
                                           v_r_detalle.id_gpo_mandato,
                                           v_r_detalle.orden,
                                           v_r_detalle.etiqueta,
                                           v_r_detalle.valor_etiqueta,
                                           v_r_detalle.id_cat_gpo


                      LET v_arr_detalle[v_indice].id_atr_nivel    = v_r_detalle.id_atr_nivel
                      LET v_arr_detalle[v_indice].id_gpo_etiqueta = v_r_detalle.id_gpo_etiqueta
                      LET v_arr_detalle[v_indice].id_cat_mandato  = v_r_detalle.id_cat_mandato
                      LET v_arr_detalle[v_indice].id_gpo_mandato  = v_r_detalle.id_gpo_mandato
                      LET v_arr_detalle[v_indice].orden           = v_r_detalle.orden
                      LET v_arr_detalle[v_indice].etiqueta        = v_r_detalle.etiqueta
                      LET v_arr_detalle[v_indice].valor_etiqueta  = v_r_detalle.valor_etiqueta
                      LET v_arr_detalle[v_indice].id_cat_gpo      = v_r_detalle.id_cat_gpo
...
DEFINE v_r_detalle_tmp  RECORD
  ...
  ,id_cat_gpo          LIKE mdt_cat_gpo_etiqueta.id_cat_gpo,
   ...
   v_nombre_original   STRING
END RECORD
DEFINE v_r_imagen_docto   RECORD 
        id_imagen_docto   LIKE mdt_imagen_docto.id_imagen_docto,
        id_gpo_mandato    LIKE mdt_imagen_docto.id_gpo_mandato,
        id_cat_gpo        LIKE mdt_cat_gpo.id_cat_gpo,
        nombre_imagen     STRING, -- LIKE mdt_imagen_docto.nombre_imagen,
   ...
   FOREACH cur_rec_detalle USING p_id_cat_mandato, p_id_cat_gpo
                           INTO  v_r_detalle_tmp.id_atr_nivel,
                                 v_r_detalle_tmp.id_gpo_etiqueta,
                                 v_r_detalle_tmp.id_cat_mandato,
                                 v_r_detalle_tmp.id_gpo_mandato,
                                 v_r_detalle_tmp.orden,
                                 v_r_detalle_tmp.etiqueta,
                                 v_r_detalle_tmp.valor_etiqueta,
                                 v_r_detalle_tmp.id_cat_gpo

      LET v_arr_detalle_tmp[v_indice].id_atr_nivel    = v_r_detalle_tmp.id_atr_nivel
      LET v_arr_detalle_tmp[v_indice].id_gpo_etiqueta = v_r_detalle_tmp.id_gpo_etiqueta
      LET v_arr_detalle_tmp[v_indice].id_cat_mandato  = v_r_detalle_tmp.id_cat_mandato
      LET v_arr_detalle_tmp[v_indice].id_gpo_mandato  = v_r_detalle_tmp.id_gpo_mandato
      LET v_arr_detalle_tmp[v_indice].orden           = v_r_detalle_tmp.orden
      LET v_arr_detalle_tmp[v_indice].etiqueta        = v_r_detalle_tmp.etiqueta
      LET v_arr_detalle_tmp[v_indice].valor_etiqueta  = v_r_detalle_tmp.valor_etiqueta
      LET v_arr_detalle_tmp[v_indice].id_cat_gpo      = v_r_detalle_tmp.id_cat_gpo
   ...
   # Recupera los documentos del grupo
   LET v_consulta = "\n SELECT doc.id_imagen_docto, doc.id_gpo_entidad, doc.nombre_imagen, doc.desc_imagen",
                    "\n   FROM prt_imagen_docto doc JOIN prt_gpo_entidad gpo",
                    "\n     ON doc.id_gpo_entidad = gpo.id_gpo_entidad",
                    "\n  WHERE gpo.id_cat_entidad = ?",
                    "\n    AND gpo.id_cat_gpo = ?"
   ...
   FOREACH cur_rec_docto_grupo_seleccionado USING p_id_cat_mandato, p_id_cat_gpo
                                            INTO  v_r_imagen_docto.id_imagen_docto,
                                                  v_r_imagen_docto.id_gpo_mandato,
                                                  v_c_nombre_imagen, --v_imagen_docto[v_indice].nombre_imagen,
                                                  v_desc_imagen


      LET v_imagen_docto[v_indice].id_imagen_docto   = v_r_imagen_docto.id_imagen_docto
      LET v_imagen_docto[v_indice].id_gpo_mandato    = v_r_imagen_docto.id_gpo_mandato
      ...
      LET v_imagen_docto[v_indice].v_documento_int   = "<a gwc:attributes=\"href resourceuri('",v_imagen_docto[v_indice].v_documento CLIPPED,"','prtdocto')\" target='nueva'>",v_imagen_docto[v_indice].v_documento CLIPPED,"</a>"
      CALL v_imagen_docto_general.appendElement()
   ...
   IF v_tipo_operacion = "agregar" THEN 
      ...
      IF v_badera_existente = FALSE THEN
         ...
         SELECT id_gpo_entidad 
           ...
           FROM prt_gpo_entidad
          WHERE id_cat_entidad = p_id_cat_mandato
...
DEFINE --v_documento RECORD LIKE mdt_imagen_docto.*
   ...
   INSERT INTO prt_imagen_docto(id_imagen_docto,id_gpo_entidad,nombre_imagen,desc_imagen)
...
FUNCTION fn_registra_detalle_modificaciones()
   ...
   UPDATE prt_cat_entidad
      SET desc_entidad       = v_desc_mandato, 
          desc_larga_entidad = v_desc_larga_mandato
    WHERE id_cat_entidad     = p_id_cat_mandato
...
FUNCTION fn_almacena_registro_det_gpo(p_indice)
   ...
   LET v_indice = p_indice
         ...
         SELECT (NVL(MAX(id_gpo_entidad),0) +1) INTO v_max_id_gpo_mandato
         FROM prt_gpo_entidad
         
         INSERT INTO prt_gpo_entidad
         (id_gpo_entidad, id_cat_entidad, id_cat_gpo, orden, usuario)
         VALUES
         (
          v_max_id_gpo_mandato
         ,p_id_cat_mandato
   ...
   FOR v_indice_detalle = 1 TO  v_arr_detalle.getLength()
      IF v_arr_detalle[v_indice_detalle].id_gpo_mandato = v_max_id_gpo_mandato THEN
         ...
         SELECT (NVL(MAX(id_atr_nivel),0) +1) INTO v_max_id_atr_nivel
         FROM prt_cat_atributo_nivel
         INSERT INTO prt_cat_atributo_nivel
         (id_atr_nivel, id_gpo_etiqueta, id_cat_entidad, id_gpo_entidad, orden, id_habilita, usuario)
         VALUES
         (
          v_max_id_atr_nivel
         ,v_arr_detalle[v_indice_detalle].id_gpo_etiqueta
   ...
   FOR v_indice_detalle = 1 TO  v_arr_detalle.getLength()
      IF v_arr_detalle[v_indice_detalle].id_gpo_mandato = v_max_id_gpo_mandato THEN
         SELECT (NVL(MAX(id_instancia_entidad),0) +1) INTO v_id_instancia_mandato
         FROM prt_cat_instancia_entidad
         INSERT INTO prt_cat_instancia_entidad
         (id_instancia_entidad, id_atr_nivel, valor_etiqueta, usuario)
         VALUES
         (
          v_id_instancia_mandato
         ,v_arr_detalle[v_indice_detalle].id_atr_nivel
                  ...
                  UPDATE prt_cat_atributo_nivel
                   ...
                   IF(v_arr_detalle[v_indice].etiqueta = "MUNICIPIO")THEN
                      ...
                      IF(v_tpo_mandato = 1)THEN # solo se inserta si es de tipo predial
                         UPDATE prt_cat_entidad_paquete
                            SET cve_entidad = v_cve_mandato_paquete_aux
                          WHERE id_cat_entidad = p_id_cat_mandato
                         IF SQLCA.SQLCODE <> 0 THEN
                            LET v_error = TRUE
