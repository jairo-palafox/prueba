...
#Modulo            => PRT                                                      #
#Programa          => PRTE17                                                   #
#Objetivo          => Batch de integración de devoluciones de saldos receptora # 
...
DEFINE v_proceso_desc    LIKE cat_proceso.proceso_desc,
   ...
   DATABASE safre_tmp   
   PREPARE prp_elimina_tabla_tmp FROM "DROP TABLE IF EXISTS tmp_prt_dev_integrados"
   EXECUTE prp_elimina_tabla_tmp
   --EXECUTE "DROP TABLE IF EXISTS tmp_prt_integrados"
   --DROP TABLE IF EXISTS tmp_prt_integrados
   CREATE TABLE tmp_prt_dev_integrados(tpo_operacion  CHAR(2),
                                       nss            CHAR(11),
                                       rechazado      SMALLINT,
   ...
   CREATE TEMP TABLE tmp_prt_dev_rechazados(registro CHAR(213))
   ...
   SELECT COUNT(*)
     ...
     FROM safre_tmp:tmp_prt_det_dev_receptora
   ...
   # Ejecuta SP de integración traspasos receptora
   LET v_consulta = "EXECUTE FUNCTION fn_prt_integra_devolucion_sdo_receptora(?,?,?,?,?,?)"
   PREPARE prp_integra_devolucion_sdo_receptora FROM v_consulta
   EXECUTE prp_integra_devolucion_sdo_receptora USING p_usuario,
                                                      p_pid,
                                                      p_proceso_cod,
                                                      p_folio,
                                                      p_nom_archivo,
                                                      C_ESTADO_MARCADA_PRO # Marcada en cedente
                                                 INTO v_error_sql,
                                                      v_error_isam,
                                                      v_msg_error,
                                                      v_total_procesados,
   ...
   ELSE
      ...
      CALL fn_correo_proceso(p_pid, 
                             ...
                             p_opera_cod, 
                             '',
                             'Integración de devolución de saldos',
                             'ID Proceso   : '||p_pid||
                             'Proceso      : '||p_proceso_cod||
                             'Operación    : '||p_opera_cod||
