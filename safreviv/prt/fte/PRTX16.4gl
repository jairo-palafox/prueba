...
#Modulo            => PRT                                                      #
#Programa          => PRTX16                                                   #
#Objetivo          => Programa batch de validación devolución receptora        #
...
DEFINE v_conteo_det_total INTEGER,
   ...
   SELECT total_registros
     INTO v_total_sum
     FROM safre_tmp:tmp_prt_sum_dev_receptora
    WHERE 1 = 1
...
DEFINE v_consulta         STRING,
   ...
   PREPARE prp_crea_tbl_tmp_rch FROM "CREATE TEMP TABLE tmp_prt_dev_rechazados(registro VARCHAR(213))"
   EXECUTE prp_crea_tbl_tmp_rch
   PREPARE prp_crea_tbl_tmp FROM "CREATE TEMP TABLE tmp_prt_dev_inconsistencias(registro DECIMAL(9,0),rechazo_cod CHAR(3))"
   EXECUTE prp_crea_tbl_tmp

   LET v_consulta = "EXECUTE PROCEDURE sp_prt_valida_devolucion_receptora()"
   ...
   SELECT COUNT(*) 
     INTO v_conteo_inc
     FROM tmp_prt_dev_inconsistencias
    WHERE 1 = 1
   ...
   IF(v_conteo_inc > 0)THEN
      ...
      LET v_consulta = " SELECT tmp.registro,",
                       "        cat.rechazo_desc",
                       "   FROM tmp_prt_dev_inconsistencias tmp LEFT OUTER JOIN prt_cat_rch_traspasos cat",
