...
GLOBALS "PRTW05.inc"        #Archivo con variables globales del propio programa
GLOBALS "PRTW04.inc"        #Archivo con variables globales del cliente de WS generado con Genero
...
PRIVATE FUNCTION fn_envia_solicitud()
   ...
   ELSE     #FIN VALIDACION DE TRAMITE CON ERROR
      ...
      IF v_id_bus_tramite > 0 AND v_id_cat_bus_contrato > 0 THEN
         ...
         LET ns1solicitudMarcaDesmarcaRequest.idssn.idCanal          = v_id_canal
         LET ns1solicitudMarcaDesmarcaRequest.idssn.idCliente        = v_id_cliente
         LET ns1solicitudMarcaDesmarcaRequest.idssn.idEbusiness      = v_id_ebusiness
         LET ns1solicitudMarcaDesmarcaRequest.idssn.idPortafolio     = v_id_portafolio
         LET ns1solicitudMarcaDesmarcaRequest.idssn.idServicio       = v_id_servicio
         LET ns1solicitudMarcaDesmarcaRequest.idssn.idSistema        = v_id_sistema
         LET ns1solicitudMarcaDesmarcaRequest.idssn.codoperCliente   = COD_OPER_CLIENTE CLIPPED
         LET ns1solicitudMarcaDesmarcaRequest.idssn.fecha            = CURRENT YEAR TO FRACTION(5)
...
PRIVATE FUNCTION fn_envia_mensaje_bus()
   ...
   LET MarcaDesmarcaCuentaExt_MarcaDesmarcaCuentaExtPortLocation = v_url

   LET v_num_intento = 0
   WHILE v_num_intento <= v_num_reintento
      
      CALL solicitudMarcaDesmarca_g() RETURNING wsStatus
      IF wsStatus = 0 THEN
...
PRIVATE FUNCTION fn_procesa_respuesta()
   ...
   LET v_ind_error = 0
   LET v_r_cod_respuesta     = ns1solicitudMarcaDesmarcaResponse.ssnrop.codRespuesta
   LET v_r_cod_respuesta_opr = ns1solicitudMarcaDesmarcaResponse.ssnrop.codRespuestaOpr
   LET v_r_desc_respuesta    = ns1solicitudMarcaDesmarcaResponse.ssnrop.descRespuesta
   LET v_r_cod_oper_cliente  = ns1solicitudMarcaDesmarcaResponse.ssnrop.codoperCliente
   LET v_folio_procesar      = ns1solicitudMarcaDesmarcaResponse.objetoRespuesta.folioDeTramiteProcesar
   ...
   FOR v_indice = 1 TO ns1solicitudMarcaDesmarcaResponse.ssnrop.motivos.motivo.getLength()
      LET v_ind_error = 1
      CALL v_r_rechazos_respuesta.appendElement()
      LET v_r_rechazos_respuesta[v_indice].cod_rechazo = ns1solicitudMarcaDesmarcaResponse.ssnrop.motivos.motivo[v_indice].idMotivo
      LET v_r_rechazos_respuesta[v_indice].desc_rechazo = ns1solicitudMarcaDesmarcaResponse.ssnrop.motivos.motivo[v_indice].descripcion
...
PRIVATE FUNCTION fn_asigna_valor(p_nombre, p_valor)
   ...
   CASE p_nombre
      WHEN "folioCliente"
         LET ns1solicitudMarcaDesmarcaRequest.cuerpo.folioCliente = p_valor
      WHEN "origenSolicitud"
         LET ns1solicitudMarcaDesmarcaRequest.cuerpo.origenSolicitud = p_valor
      WHEN "tipoSolicitud"
         LET ns1solicitudMarcaDesmarcaRequest.cuerpo.tipoSolicitud = p_valor
      WHEN "nssImss"
         LET ns1solicitudMarcaDesmarcaRequest.cuerpo.nssImss = p_valor
         LET v_nss = p_valor
      WHEN "apellidoPaterno"
         LET ns1solicitudMarcaDesmarcaRequest.cuerpo.apellidoPaterno = p_valor
      WHEN "apellidoMaterno"
         LET ns1solicitudMarcaDesmarcaRequest.cuerpo.apellidoMaterno = p_valor
      WHEN "nombre"
         LET ns1solicitudMarcaDesmarcaRequest.cuerpo.nombre = p_valor
      WHEN "curp"
         LET ns1solicitudMarcaDesmarcaRequest.cuerpo.curp = p_valor
