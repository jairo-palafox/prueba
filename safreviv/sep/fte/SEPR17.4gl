...
#Modulo            => SEP                                                      #
#Programa          => SEPR17                                                   #
#Objetivo          => Batch reverso de integración diagnóstico operación 27    #
...
DEFINE v_proceso_desc LIKE cat_proceso.descripcion,
   ...
   LET v_opera_cod   = 2    # integracion de diagnóstico operacion 27
   ...
   LET v_consulta = "EXECUTE FUNCTION fn_sep_r_integrar_diag_op27(?,?)"
   ...
   ELSE
      IF(v_ind <> 0)THEN
         ...
         CASE v_diag
            ...
            WHEN "003" 
              DISPLAY "Error en funcion general de reverso de integración \n"
              LET v_mensaje = v_mensaje,"Error en funcion general de reverso de integración"
            WHEN "004" 
              DISPLAY "Error al actualizar operación \n"
              LET v_mensaje = v_mensaje,"Error al actualizar operación"
         END CASE
         CALL fn_error_opera(p_pid_rev,p_proceso_cod_rev,p_opera_cod_rev) RETURNING r_res_opera
         IF(r_res_opera <> 0)THEN
            CALL fn_desplega_inc_operacion(r_res_opera)
      ...
      ELSE
         ...
         DISPLAY "\nTOTAL REGISTROS REVERSADOS DETALLE 03: ",v_total_rev
