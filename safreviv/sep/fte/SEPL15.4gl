...
#Modulo            => SEP                                                      #
#Programa          => SEPL15                                                   #
#Objetivo          => Reverso integración de marca operación 27                #
...
DEFINE v_count_detalle INTEGER,
   ...
   LET v_proceso_cod_rev = 2227 # reverso de integracion marca operacion 27
   LET v_opera_cod_rev   = 1 # reverso de integracion marca operacion 27
   LET v_proceso_cod = 2201 # operacion 27
   ...
   LET v_comando = "EXECUTE FUNCTION fn_sep_vr_integrar_marca_op27()"
   PREPARE prp_val_rev_op27 FROM v_comando
   EXECUTE prp_val_rev_op27 INTO v_ind,
                                 v_diag,
                                 v_sql_error,
                                 v_isam_error,
                                 v_msg_error,
   ...
   ELSE
      IF(v_ind <> 0)THEN # inconsistencias
         CASE v_diag
            ...
            WHEN "003"
               LET v_mensaje = "folio no corresponde a marca Op 27"
            WHEN "004"
   ...
   OPEN WINDOW vtna_reversa_folio WITH FORM v_ruta_bin CLIPPED||"/SEPL151"
      ...
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         ON ACTION reverso            
            # en caso de no existir registros a procesar se informa que debe de haber al menos uno
            IF(v_folios_eleg.getLength() = 0)THEN
               CALL fn_mensaje("Aviso","No se seleccionado folio a reversar","stop")
               CONTINUE DIALOG
            END IF
            ...
            IF(v_confirma)THEN
                ...
                IF(r_res_opera = 0)THEN
                   ...
                   LET v_comando = "nohup fglrun ",v_ruta_bin CLIPPED,"/SEPR15.42r ",
                                                   p_usuario_cod, " ",
                                                   v_pid_rev, " ",
                                                   v_proceso_cod_rev," ",
                                                   v_opera_cod_rev," ",
