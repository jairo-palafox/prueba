...
#Modulo            => SEPE                                                     #
#Programa          => SEPE31                                                   #
#Objetivo          => Programa de integracion de operacion 28                  # 
...
MAIN
   ...
   ELSE
      ...
      ELSE
         ...
         ELSE
            ...
            DISPLAY "\nTOTAL DE REGISTOS A SER INTEGRADOS (DET02): ",v_tot_integra_det02
            ...
            LET v_s_qryTxt = "EXECUTE FUNCTION fn_sep_integra_op28(?,?,?,?)"
            ...
            LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/SEPI31.42r ",
                                            p_v_usuario, " ",
                                            p_d_pid, " ",
                                            p_i_proceso_cod," ",
                                            p_i_opera_cod," ",
            ...
            #************ Llamada a funcion para generar rechazos***************
            LET v_conteo_rechzadas = 0
            SELECT COUNT(*)
              INTO v_conteo_rechzadas
              FROM sep_det_02_op28
             WHERE folio = p_d_folio
               AND estado = 30 # rechazado
            # solo si hay rechazadas se llama la funcion para generar el archivo de rechazadas
            IF(v_conteo_rechzadas > 0)THEN
               # funcion en programa SEPS32
               CALL fn_archivo_rechazo_op28(p_d_folio,p_v_usuario)
   ...
   CALL fn_correo_proceso(p_d_pid, 
                          ...
                          p_i_opera_cod, 
                          '', -- TMP AHM adjunto ?
                          'Integrar archivo de Notificación de Separacion (op28)',
                          v_mensaje
                          {'ID Proceso   : '||p_d_pid||
...
DEFINE v_registro_rechazado VARCHAR(300), # El tamaño de tipo registro 03 es el mayor con 65
   ...
   DISPLAY "TOTAL RECHAZADOS         :",v_conteo
