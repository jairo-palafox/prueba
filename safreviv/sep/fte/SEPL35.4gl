...
#Modulo            => SEP                                                      #
#Programa          => SEPL35                                                   #
#Objetivo          => Reverso carga de archivo de  operacion 28                #
...
DEFINE p_usuario_cod   LIKE seg_usuario.usuario, # usuario logeado
       ...
       v_folios_pend   DYNAMIC ARRAY OF VARCHAR(40), # arreglo que contiene los folios pendientes 
       v_folios_eleg   DYNAMIC ARRAY OF VARCHAR(40), # arreglo que contiene los folios a reversar
...
DEFINE v_count_detalle INTEGER,
   ...
   LET v_proceso_cod_rev = 2224 # reverso de carga de archivo de operacion 28
   ...
   FOREACH cur_archivos INTO v_nombre_archivo
      # solo se procesa el ultimo folio de op 28
      LET v_folios_pend[v_indice] = v_nombre_archivo
      
      LET v_indice = v_indice + 1
   END FOREACH
   
   IF ( v_folios_pend.getLength() < 1 ) THEN
      -- no hay archivos para reversar
      CALL fn_mensaje("Atención","No se tienen archivos para reversar","stop")
      EXIT PROGRAM
   END IF
   ...
   OPEN WINDOW vtna_reversa_folio WITH FORM v_ruta_bin CLIPPED||"/SEPL351"
      ...
      DIALOG ATTRIBUTE(UNBUFFERED)
         ...
         ON ACTION reverso            
            CALL fn_ventana_confirma("Confimar","¿Reversar carga de archivo?","info") RETURNING v_confirma
            IF ( v_confirma ) THEN
                ...
                IF ( r_res_opera = 0 ) THEN
                   ...
                   LET v_comando = "nohup fglrun ",v_ruta_bin CLIPPED,"/SEPR35.42r ",
