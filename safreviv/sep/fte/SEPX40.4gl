...
#Modulo            => SEP                                                      #
#Programa          => SEPX40                                                   #
#Objetivo          => Programa batch de validacion para sumario y detalle op29 # 
...
DEFINE v_conteo_det_02    INTEGER,
       ...
       v_conteo_det_05    INTEGER,
       v_conteo_det_06    INTEGER,
       v_conteo_det_total INTEGER,
       v_total_sum_02     INTEGER,
       v_total_sum_03     INTEGER,
       v_total_sum_05     INTEGER,
       v_total_sum_06     INTEGER,
       v_total_sum        INTEGER,
   ...
   LET v_conteo_det_05 = 0
   LET v_conteo_det_06 = 0
   LET v_conteo_det_total = 0
   LET v_total_sum_02 = 0
   LET v_total_sum_03 = 0
   LET v_total_sum_05 = 0
   LET v_total_sum_06 = 0
   LET v_total_sum = 0
   LET v_edo_error = 3
   ...
   SELECT COUNT(*)
     INTO v_conteo_det_05
     FROM safre_tmp:tmp_sep_det_05_op29
    WHERE 1 = 1

   SELECT COUNT(*)
     INTO v_conteo_det_06
     FROM safre_tmp:tmp_sep_det_06_op29
    WHERE 1 = 1

   LET v_conteo_det_total = v_conteo_det_02 + 
                            v_conteo_det_03 + 
                            v_conteo_det_05 + 
                            v_conteo_det_06  
   ...
   IF(v_conteo_det_02 <> v_total_sum_02 OR 
      v_conteo_det_03 <> v_total_sum_03 OR
      v_conteo_det_05 <> v_total_sum_05 OR 
      v_conteo_det_06 <> v_total_sum_06 OR  
      v_conteo_det_total <> v_total_sum)THEN
      DISPLAY "\n================================================================="
...
DEFINE v_consulta   STRING,
   ...
   PREPARE prp_crea_tbl_tmp FROM "CREATE TEMP TABLE sep_tmp_inconsistencias_val_op29(mensaje CHAR(254))"
   EXECUTE prp_crea_tbl_tmp

   LET v_consulta = "EXECUTE PROCEDURE sp_sep_valida_op29()"
   ...
   SELECT COUNT(*) 
     INTO v_conteo_inc
     FROM sep_tmp_inconsistencias_val_op29
    WHERE 1 = 1
