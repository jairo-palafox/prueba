...
#Modulo            =>SEPE                                          #
#Programa          =>SEPI04                                        #
#Objetivo          =>Reporte de preliquidacion de operacion 40     # 
#Fecha inicio      =>11 MAYO    2018                               #
####################################################################
DATABASE safre_viv

MAIN
   DEFINE v_signo              SMALLINT
   DEFINE v_signo_aso              SMALLINT
   ...
   DEFINE v_r_reporte_det_agr DYNAMIC ARRAY OF RECORD
             ...
             v_invadido            STRING,
             v_asociado            STRING,
             v_saldo92_invadido    DECIMAL(16,2),
             v_saldo92_asociado    DECIMAL(16,2),
             v_saldo97_invadido    DECIMAL(16,2),
             v_saldo97_asociado    DECIMAL(16,2),
             v_saldo92             DECIMAL(16,2),
             v_saldo97             DECIMAL(16,2),
             v_total_parejas       STRING,
             v_ind_conciliar       CHAR(40)
          END RECORD
   DEFINE v_saldos_separar_aux RECORD
   	         gpo                  SMALLINT,
             clasifica_separacion LIKE sep_cat_resultado_operacion.resultado_operacion_desc,
             total_inv_92         DECIMAL(16,2),
             total_aso_92         DECIMAL(16,2),
             total_inv_97         DECIMAL(16,2),
             total_aso_97         DECIMAL(16,2),
             v_total              INTEGER
          END RECORD
   ...
   LET v_v_nom_reporte = p_v_usuario CLIPPED, "-SEPL05-", p_d_pid USING "&&&&&", "-", p_i_proceso_cod USING "&&&&&", "-", p_i_opera_cod USING "&&&&&"
   ...
   ELSE}
         ...
         IF fgl_report_loadCurrentSettings(r_ruta_bin CLIPPED||"/SEPI051.4rp") THEN
        ...
        --DISPLAY "folio = ",p_d_folio 
         ...
         LET v_consulta = " SELECT case det02.ind_conciliar when 1 THEN 1 ELSE 2 end case, ",
                          " det02.tipo_movimiento,",
                          "        sum(NVL(det02.saldo_viv_92,0)),",
                          "        sum(NVL(det03.saldo_viv_92,0)),",
                          "        sum(NVL(det02.saldo_viv_97,0)),",
                          "        sum(NVL(det03.saldo_viv_97,0)),",
                          "        count(*)",
                          "   FROM sep_det_02_op40 det02,",
                          "        sep_det_03_op40 det03",
                          "  WHERE det02.id_det_02_op40 = det03.id_det_02_op40",
         ...
         FOREACH cur_recupera_posibles_cuentas INTO v_saldos_separar_aux.*
            ...
            IF   v_r_reporte_det_agr[v_indice].v_clasificacion = 1 THEN          
                 LET v_signo = -1 
                 LET v_signo_aso = 1 
            ELSE 
                 LET v_signo = 1 
                 LET v_signo_aso = -1 
            END IF                 
            LET v_r_reporte_det_agr[v_indice].v_saldo92_invadido = v_saldos_separar_aux.total_inv_92 * v_signo --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo92_asociado = v_saldos_separar_aux.total_aso_92 * v_signo_aso --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo97_invadido = v_saldos_separar_aux.total_inv_97 * v_signo --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo97_asociado = v_saldos_separar_aux.total_aso_97 * v_signo_aso --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo92          = (v_saldos_separar_aux.total_inv_92*v_signo)+(v_saldos_separar_aux.total_aso_92 * v_signo_aso) --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo97          = (v_saldos_separar_aux.total_inv_97*v_signo)+(v_saldos_separar_aux.total_aso_97 * v_signo_aso) --USING '-########.##'
         ...
         LET v_consulta = " SELECT case det02.ind_conciliar when 1 THEN 3 ELSE 4 end case, det02.tipo_movimiento, invadido, asociado,",
                          "        NVL(det02.saldo_viv_92,0),",
                          "        NVL(det03.saldo_viv_92,0),",
                          "        NVL(det02.saldo_viv_97,0),",
                          "        NVL(det03.saldo_viv_97,0),",
                          "        case det02.ind_conciliar when 0 then 'NO existe pareja de separacion' ",
                                                          " when 1 then 'Aceptada' ",
                                                          " when 2 then 'Tipo Movimiento cargo inválido en det03' ",
                                                          " when 3 then 'Movimiento invadido/asociado viv92 diferentes' ",
                                                          " when 4 then 'Movimiento invadido/asociado viv97 diferentes' ",
                                                          " when 5 then 'Tipo Movimiento abono inválido en det03' ",
                                                          " when 6 then 'Saldo insuficiente en viv97' ",
                                                          " when 7 then 'Saldo insuficiente en viv92' end case ",
                          "   FROM sep_det_02_op40 det02,",
                          "        sep_det_03_op40 det03",
                          "  WHERE det02.id_det_02_op40 = det03.id_det_02_op40 ",
                          "  AND   det02.folio = ",p_d_folio,
         ...
         FOREACH cur_recupera_posibles_cuentas2 INTO v_saldos_separar_aux2.*
            ...
            LET v_r_reporte_det_agr[v_indice].v_saldo92_invadido = v_saldos_separar_aux2.total_inv_92 * v_signo --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo92_asociado = v_saldos_separar_aux2.total_aso_92 * v_signo_aso --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo97_invadido = v_saldos_separar_aux2.total_inv_97 * v_signo --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo97_asociado = v_saldos_separar_aux2.total_aso_97 * v_signo_aso --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo92          = (v_saldos_separar_aux2.total_inv_92 * v_signo)+(v_saldos_separar_aux2.total_aso_92 * v_signo_aso) --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_saldo97          = (v_saldos_separar_aux2.total_inv_97 * v_signo)+(v_saldos_separar_aux2.total_aso_97 * v_signo_aso) --USING '-########.##'
            LET v_r_reporte_det_agr[v_indice].v_ind_conciliar    = v_saldos_separar_aux2.ind_conciliar            
            LET v_indice = v_indice + 1
         END FOREACH
...
#Modulo            => SEP                                                      #
#Programa          => SEPI05                                                   #
#Objetivo          => Reporte de diagnostico operacion  40                     #
