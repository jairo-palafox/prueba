...
DEFINE p_usuario_cod   LIKE seg_usuario.usuario_cod,
       ...
       v_saldos_preliquidar DYNAMIC ARRAY OF RECORD
         ...
         v_saldo92_invadido dec(16,2),
         v_saldo92_asociado dec(16,2),
         v_saldo97_invadido dec(16,2),
         v_saldo97_asociado dec(16,2),
         v_saldo92          dec(16,2),
         v_saldo97          dec(16,2),
       ...
       DEFINE v_tot92_invadido dec(16,2)
       DEFINE v_tot92_asociado dec(16,2)
       DEFINE v_tot97_invadido dec(16,2)
       DEFINE v_tot97_asociado dec(16,2)
       DEFINE v_tot_saldo92    dec(16,2)
       DEFINE v_tot_saldo97    dec(16,2)
...
#Modulo            => SEP                                                      #
#Programa          => SEPL05                                                   #
#Descripcion       => Preliquidación de op40                                   #
#Fecha inicio      => 15 Mayo 2018                                             #
################################################################################
...
DEFINE v_ventana         ui.Window,
   ...
   LET g_opera_cod     = 3    # preliquidacion de Operación40 
   ...
   OPEN WINDOW vtna_integra_notficacion_separacion WITH FORM v_ruta_ejecutable CLIPPED||"/SEPL051"
...
DEFINE v_folio    LIKE glo_folio.folio,
   ...
   LET v_consulta = "\n SELECT det.folio",
                    "\n   FROM sep_det_02_op40 det JOIN sep_cza_op40 cza",
...
DEFINE v_folio_preliquidar LIKE glo_folio.folio,
   ...
   LET v_consulta = "\n SELECT det2.tipo_movimiento, SUM(det2.saldo_viv_92),",
                    "\n        SUM(det3.saldo_viv_92),SUM(det2.saldo_viv_97),",
                    "\n        SUM(det3.saldo_viv_97),SUM(det2.saldo_viv_92+det3.saldo_viv_92),",
                    "\n        SUM(det3.saldo_viv_97+det2.saldo_viv_97),COUNT(det2.id_det_02_op40)",
   ...
   FOREACH cur_cifras_preliquidar USING v_folio_preliquidar 
      ...
      IF  v_saldos_preliquidar[v_indice].v_clasificacion = 1 THEN
         LET v_signo = -1 
         LET v_signo_aso = 1
      ELSE
         LET v_signo = 1 
      ...
      LET v_saldos_preliquidar[v_indice].v_saldo92_invadido = v_saldos_preliquidar_aux.v_saldo92_invadido * v_signo --USING "-##,###,###.##"
      LET v_saldos_preliquidar[v_indice].v_saldo92_asociado = v_saldos_preliquidar_aux.v_saldo92_asociado * v_signo_aso --USING "-##,###,###.##"
      LET v_saldos_preliquidar[v_indice].v_saldo97_invadido = v_saldos_preliquidar_aux.v_saldo97_invadido * v_signo --USING "-##,###,###.##"
      LET v_saldos_preliquidar[v_indice].v_saldo97_asociado = v_saldos_preliquidar_aux.v_saldo97_asociado * v_signo_aso --USING "-##,###,###.##"
      
      --LET v_saldos_preliquidar[v_indice].v_saldo92          = v_saldos_preliquidar_aux.v_saldo92 USING "###,###,###.##"
      LET v_saldos_preliquidar[v_indice].v_saldo92          = (v_saldos_preliquidar_aux.v_saldo92_invadido * v_signo) + 
                                                              (v_saldos_preliquidar_aux.v_saldo92_asociado * v_signo_aso) --USING "-##,###,###.##"
      
      --LET v_saldos_preliquidar[v_indice].v_saldo97          = v_saldos_preliquidar_aux.v_saldo97 USING "###,###,###.##"
      LET v_saldos_preliquidar[v_indice].v_saldo97          = (v_saldos_preliquidar_aux.v_saldo97_invadido * v_signo) +
                                                              (v_saldos_preliquidar_aux.v_saldo97_asociado * v_signo_aso) --USING "-##,###,###.##"
      ...
      ON ACTION preliquidar
         ...
         IF(r_confirma)THEN
            ...
            IF(r_resultado_opera <> 0 )THEN
               ...
               CONTINUE DISPLAY
            ELSE
               CALL fn_actualiza_opera_ini(g_pid,g_proceso_cod,g_opera_cod,v_folio_preliquidar,'SEPL05','',p_usuario_cod)
...
#Modulo            => SEP                                                      #
#Programa          => SEPL33                                                   #
#Descripcion       => Ejecuta el programa que realiza la preliquidacion de op40#
#Fecha inicio      => 09 Mayo 2018                                             #
################################################################################
...
DEFINE v_folio_preliquidar STRING, 
       ...
       v_int_folio        DEC(10,0)

   LET v_int_folio = v_folio_preliquidar
       
   SELECT nombre_archivo 
   INTO   v_nombre_archivo   
   FROM   glo_ctr_archivo 
   WHERE  proceso_cod = 2237
   AND    folio = v_int_folio
       
   CALL fn_rutas("bat") RETURNING v_ruta_bin_aux, v_ruta_listados
   
   LET v_comando = "nohup fglrun ",v_ruta_ejecutable CLIPPED,"/SEPE05.42r "
                                 ,p_usuario_cod, " ",g_pid, " ",g_proceso_cod," "
                                 ,g_opera_cod," ",v_folio_preliquidar," ",v_nombre_archivo clipped,
