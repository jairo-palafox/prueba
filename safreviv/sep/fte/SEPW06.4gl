...
FUNCTION fn_recibe_datos_expediente(p_url_servidor, p_usuario, p_password, p_intentos, p_solicita)
   DEFINE p_url_servidor   STRING 
   DEFINE p_usuario        STRING 
   DEFINE p_password       STRING
   DEFINE p_intentos       INTEGER
   DEFINE p_solicita       tns2DatosExpedienteRequest
   ...
   LET ns1recibeDatosExpediente.datosExpedienteRequest.* = p_solicita.*
   
   WHILE v_num_intento <= p_intentos
      --Se ejecuta la funcion del cliente
      CALL  recibeDatosExpediente(p_url_servidor, p_usuario, p_password) RETURNING wsStatus

      IF wsStatus == 0 THEN
        --Se ejecuto correctamente el WS
        LET v_respuesta.* = ns1recibeDatosExpedienteResponse.recibeDatosExpedienteReturn.*
        EXIT WHILE
      ELSE
         ...
         IF v_num_intento = p_intentos THEN
            ...
            --Se reporta el error
            LET v_cve_cliente = 5         --Clave del cliente de Datos de expediente de separacion (wsv_cliente)
            LET v_f_error = TODAY
            LET v_ws_status = wsStatus
            LET v_cod_error = wsError.code
            LET v_desc_error = "Error al enviar los datos de expediente del NSS ", p_solicita.nssAsociado, ": ", wsError.description
