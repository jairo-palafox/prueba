--===============================================================
-- Version: 1.0.0
-- Fecha ultima modificacion:   /06/2012
--===============================================================

################################################################################
#Módulo          => SEP                                                        #
#Programa        => SEPF24                                                     #
#Objetivo        => Programa de Carga de Ajuste al Crédito                     #
...
#Modulo            => SEP                                                      #
#Programa          => SEPF24                                                   #
#Descripcion       => Carga de archivo Ajuste al Crédito                       #
#Autor             =>                                                          #
...
DEFINE v_continuar BOOLEAN,
   ...
   OPEN WINDOW vtna_carag_ajuste WITH FORM v_ruta_ejecutable CLIPPED||"/SEPF241"
      ...
      CALL v_forma.setElementHidden("gpo_ajuste",1)
      ...
      IF(v_continuar)THEN
         ...
         CALL v_forma.setElementHidden("gpo_ajuste",0)
         ...
         INPUT ARRAY v_documento FROM sr_documento.*  ATTRIBUTES(WITHOUT DEFAULTS,  APPEND ROW = FALSE, INSERT ROW = FALSE, DELETE ROW = FALSE)
            ...
            ON ACTION dialogtouched
             ...
             BEFORE FIELD tedi_seleccion
               IF v_documento[1].v_ruta_local IS NULL THEN             
         ...
         END INPUT
            ...
            IF(sin_archivo = 0)THEN
               IF(v_documento[1].v_ruta_local_exp IS NULL AND v_documento[1].v_nom_documento IS NULL)THEN
                  CALL fn_mensaje(p_cad_ventana,"Seleccione archivo de Ajuste al Crédito","information")
                  CONTINUE DIALOG
               END IF
               ...
               IF(r_confirma)THEN
                  ...
                  IF(v_documento[1].v_nom_documento IS NULL)THEN
                     ...
                     IF(v_continuar)THEN
                        ...
                        IF(r_error)THEN                        
                           ...
                           CONTINUE DIALOG
                        ELSE}
                           DISPLAY "EJECUTA CARGA 1"
                           DISPLAY "ARCHIVO:",v_documento[1].v_archivo
                           CALL fn_carga_credito(p_usuario_cod, v_documento[1].v_archivo, v_documento[1].v_id_expediente)
                  ...
                  ELSE
                     ...
                     IF(r_error)THEN                        
                        ...
                        CONTINUE DIALOG
                     ELSE}
                        DISPLAY "EJECUTA CARGA 2"
                        DISPLAY "ARCHIVO:",v_documento[1].v_nom_documento
                        CALL fn_carga_credito(p_usuario_cod, v_documento[1].v_nom_documento, v_documento[1].v_id_expediente)
            ...
            ELSE 
                {CALL fn_actualiza_maquinaria(v_documento[1].v_id_expediente) 
                 ...
                 ELSE}
                    ...
                    IF(v_indicador = 0)THEN
                       UPDATE sep_expediente
                          SET ind_ajuste = 4 # confirmado sin archivo
                        WHERE id_expediente = v_documento[1].v_id_expediente
                       CALL fn_mensaje(p_cad_ventana,"Datos Cargados...","information")
                       EXIT DIALOG
                    ELSE
   ...
   CLOSE WINDOW vtna_carag_ajuste
...
DEFINE v_filtro   STRING,
   ...
   IF(v_expediente_aux.v_id_expediente IS NOT NULL)THEN
      ...
      LET v_documento[1].v_nom_documento  = v_docto_ajuste
      LET v_documento[1].v_ruta_servidor  = v_ruta_docto CLIPPED,"/",v_docto_ajuste CLIPPED
...
DEFINE v_continuar BOOLEAN,
   ...
   RUN v_comando
   IF(STATUS)THEN
      DISPLAY "Error al eliminar archivo Ajuste al Crédito"
      RETURN v_continuar 
   END IF
   ...
   IF(STATUS)THEN
      --CALL fn_mensaje(p_cad_ventana,"Ocurrió un error al transferir el documento","information")
      DISPLAY "Error de transferencia de archivo Ajuste al Crédito"
   ELSE
      ...
      UPDATE sep_expediente
         SET docto_ajuste = v_documento[1].v_archivo
       WHERE id_expediente = v_documento[1].v_id_expediente
      ...
      ELSE
        DISPLAY "Error de actualizacion de registro de archivo Ajuste al Crédito"
      END IF
   END IF
   RETURN v_continuar
END FUNCTION
