...
#Modulo            => SEP                                                      #
#Programa          => SEPL14                                                   #
#Objetivo          => Reverso de validacion marca operación 27                 #
...
DEFINE p_usuario_cod   LIKE seg_usuario.usuario, # usuario logeado
   ...
   LET v_proceso_cod_rev = 2228  # reverso de marca operacion 27
   LET v_opera_cod_rev   = 1 # reverso de validacion marca operacion 27
   LET v_proceso_cod = 2201 # marca operacion 27
   LET v_opera_cod   = 1 # validacion marca operacion 27

   
   LET v_folio = 0      # Se indica folio 0 para que el proceso de historico genere un nuevo folio
   LET v_programa_cod = "SEPL14"

   # recuepra el pid del proceso a reversar
   --CALL fn_max_pid(v_proceso_cod,v_opera_cod) RETURNING v_pid
    SELECT NVL(MAX(a.pid),0) 
    INTO   v_pid
    FROM  bat_ctr_operacion a
    WHERE a.proceso_cod  = v_proceso_cod
    AND   a.opera_cod    = v_opera_cod
    AND   a.estado_cod   <> 10 -- distinto de reversado
   ...
   OPEN WINDOW vtna_reversa_folio WITH FORM v_ruta_bin CLIPPED||"/SEPL141"
         ...
         ON ACTION reverso            
            ...
            IF(v_confirma)THEN
                ...
                IF(r_res_opera = 0)THEN
                   ...
                   LET v_comando = "nohup fglrun ",v_ruta_bin CLIPPED,"/SEPR14.42r ",
                                                   p_usuario_cod, " ",
                                                   v_pid_rev, " ",
                                                   v_proceso_cod_rev," ",
                                                   v_opera_cod_rev," ",
