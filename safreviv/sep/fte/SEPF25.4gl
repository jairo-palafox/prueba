...
#Módulo          => SEP                                                        #
#Programa        => SEPF26                                                    #
#Objetivo        => Programa de Carga de Baja Notificación                           #
...
#Modulo            => SEP                                                      #
#Programa          => SEPF23                                                   #
#Descripcion       => Carga de archivo Baja Notificación                                #
#Autor             =>                               #
...
DEFINE v_continuar BOOLEAN,
   ...
   OPEN WINDOW vtna_carag_dictamen WITH FORM v_ruta_ejecutable CLIPPED||"/SEPF251"
       ...
       IF(v_continuar)THEN
         ...
         DIALOG ATTRIBUTES (UNBUFFERED)
          ...
          BEFORE  INPUT 
               ...
               IF(v_documento[1].v_ruta_local_exp IS NULL AND v_documento[1].v_nom_documento IS NULL)THEN
                ...
                END IF
          END INPUT
         INPUT ARRAY v_documento FROM sr_documento.*  ATTRIBUTES(   APPEND ROW = FALSE, INSERT ROW = FALSE, DELETE ROW = FALSE)
            ...
            ON ACTION dialogtouched
               ...
               # reemplaza los slash por pipes, para  poder hacer busqueda por toquen
         ...
         END INPUT 
            ...
            IF(sin_archivo=0)THEN
               IF(v_documento[1].v_ruta_local_exp IS NULL AND v_documento[1].v_nom_documento IS NULL)THEN
                  CALL fn_mensaje(p_cad_ventana,"Seleccione archivo de Baja Notificación","information")
                  CONTINUE DIALOG
               END IF
               ...
               IF(v_documento[1].v_archivo IS NULL)THEN
                  CALL fn_mensaje(p_cad_ventana,"Seleccione archivo de Baja Notificación","information")
               ELSE
                  # Actualiza estado del indicador
                  UPDATE sep_expediente
                     SET ind_baja_notificacion = 1
               ...
               IF(r_confirma)THEN
                  ...
                  IF(v_documento[1].v_nom_documento IS NULL)THEN
                     ...
                     IF(v_continuar)THEN
                        ...
                        IF(r_error)THEN                        
                           ...
                           CONTINUE DIALOG
                        ELSE}
                           CALL fn_carga_baja_notificacion(v_documento[1].v_id_expediente,p_usuario_cod,v_documento[1].v_archivo)
                  ...
                  ELSE
                     ...
                     IF(r_error)THEN                        
                        ...
                        CONTINUE DIALOG
                     ELSE}
                        CALL fn_carga_baja_notificacion(v_documento[1].v_id_expediente,p_usuario_cod,v_documento[1].v_nom_documento)
                             RETURNING v_indicador
                        IF(v_indicador = 0)THEN
                           CALL fn_mensaje(p_cad_ventana,"Datos Cargados...","information")
                           EXIT DIALOG
            ...
            ELSE 
                {CALL fn_actualiza_maquinaria(v_documento[1].v_id_expediente) 
                 ...
                 ELSE}
                    ...
                    IF(v_indicador = 0)THEN
                       UPDATE sep_expediente
                          SET ind_baja_notificacion = 4 # confirmado sin archivo
                        WHERE id_expediente = v_documento[1].v_id_expediente
                       CALL fn_mensaje(p_cad_ventana,"Datos Cargados...","information")
                       EXIT DIALOG
                    ELSE
          ...
          ON ACTION btn_quitar
               ...
               IF(r_confirma)THEN
                  ...
                  ELSE
                     UPDATE sep_expediente
                        SET docto_baja_notificacion = NULL
                      WHERE id_expediente = v_documento[1].v_id_expediente
                     IF(SQLCA.SQLCODE = 0)THEN
    ...
    WHILE (v_bnd_con)
      CONSTRUCT v_filtro ON exp.caso_adai, exp.id_expediente FROM edi_caso_adai,edi_id_expediente 
       ...
       ON ACTION aceptar
        ...
        ON ACTION cancelar
       -- CALL fn_mensaje(p_cad_ventana,"cancela2","information")
           LET v_bnd_con = FALSE
           EXIT CONSTRUCT
    END CONSTRUCT
...
DEFINE v_filtro   STRING,
       ...
       v_expediente_aux RECORD
         ...
         v_id_expediente LIKE sep_expediente.id_expediente
       END RECORD,
       v_docto_baja_notificacion  LIKE sep_expediente.docto_baja_notificacion,
       
       v_continua BOOLEAN,
       docto_baja_notificacion INTEGER
   ...
   LET v_consulta = "\n SELECT exp.caso_adai, flo.flujo_desc, ",
                    ...
                    "\n    AND exp.ind_baja_notificacion IN (0,1)",
                    "\n    AND ",v_filtro
   --DISPLAY "query: ",v_consulta
   ...
   IF(v_expediente_aux.v_id_expediente IS NOT NULL)THEN
      ...
      {CALL fn_mensaje("",SQLCA.sqlcode,"")
      CALL fn_mensaje("",v_expediente_aux.v_id_expediente,"")}
      # REVISAR PORQUE MARCA ERROR CON SQL ESTATICO
      LET v_consulta = "\nSELECT docto_baja_notificacion
        
        FROM sep_expediente
       WHERE id_expediente = ?"
      PREPARE prp_rec_docto FROM v_consulta
      EXECUTE prp_rec_docto USING v_expediente_aux.v_id_expediente
                             INTO v_docto_baja_notificacion
      ...
      LET v_documento[1].v_nom_documento  = v_docto_baja_notificacion CLIPPED
      LET v_documento[1].v_ruta_servidor  = v_ruta_docto CLIPPED,"/",v_docto_baja_notificacion CLIPPED
...
DEFINE v_continuar BOOLEAN,
   ...
   RUN v_comando
   IF(STATUS)THEN
      DISPLAY "Error al eliminar archivo Baja Notificación"
      RETURN v_continuar
   END IF
   ...
   IF(STATUS)THEN
      --CALL fn_mensaje(p_cad_ventana,"Ocurrió un error al transferir el documento","information")
      DISPLAY "Error de transferencia de archivo Baja Notificación"
   ELSE
      ...
      UPDATE sep_expediente
         SET docto_baja_notificacion = v_documento[1].v_archivo
       WHERE id_expediente = v_documento[1].v_id_expediente
      ...
      ELSE
        DISPLAY "Error de actualizacion de registro de archivo Baja Notificación"
      END IF
   END IF
   RETURN v_continuar
END FUNCTION
