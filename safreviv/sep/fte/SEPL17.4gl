...
#Modulo            => SEP                                                      #
#Programa          => SEPL17                                                   #
#Objetivo          => Reverso integración de diagnóstico operación 27          #
...
DEFINE v_count_detalle INTEGER,
   ...
   LET v_proceso_cod_rev = 2225 # reverso de integracion diagnóstico operacion 27
   LET v_opera_cod_rev   = 1 # reverso de integracion diagnóstico operacion 27
   LET v_proceso_cod = 2204 # operacion 27
   LET v_opera_cod   = 2 # integracion de operacion 27

   
   LET v_folio = 0      # Se indica folio 0 para que el proceso de historico genere un nuevo folio
   LET v_programa_cod = "SEPL17"
   ...
   LET v_comando = "EXECUTE FUNCTION fn_sep_vr_integrar_diag_op27(?)"
   PREPARE prp_val_rev_op27 FROM v_comando
   EXECUTE prp_val_rev_op27 USING v_proceso_cod
                             INTO v_ind,
                                  v_diag,
                                  v_sql_error,
                                  v_isam_error,
   ...
   ELSE
      IF(v_ind <> 0)THEN # inconsistencias
         CASE v_diag
            ...
            WHEN "003"
               LET v_mensaje = "folio no corresponde a diagnóstico Op 27"
            WHEN "004"
               LET v_mensaje = "no se encuentra el pid a ser reversado"
            WHEN "005"
               LET v_mensaje = "expediente ya se encuentra ligado a Op 27"
            WHEN "006"
               LET v_mensaje = "Operación 27 ya se encuentra en operación 28"
            WHEN "007"
               LET v_mensaje = "error en validación de reverso de proceso"
         END CASE 
   ...
   OPEN WINDOW vtna_reversa_folio WITH FORM v_ruta_bin CLIPPED||"/SEPL171"
         ...
         ON ACTION reverso            
            ...
            IF(v_confirma)THEN
                ...
                IF(r_res_opera = 0)THEN
                   ...
                   LET v_comando = "nohup fglrun ",v_ruta_bin CLIPPED,"/SEPR17.42r ",
                                                   p_usuario_cod, " ",
                                                   v_pid_rev, " ",
                                                   v_proceso_cod_rev," ",
                                                   v_opera_cod_rev," ",
