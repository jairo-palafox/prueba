...
#Modulo            => SEP                                                     #
#Programa          => SEPC22                                                  #
#Objetivo          => CONSULTA DE EXPEDIENTES                                 #
...
DEFINE v_r_captura_exp  RECORD-- LIKE sep_expediente.*
       ...
       {v_docto_restitucion_complementario_1 RECORD
         v_docto_complementario_1 LIKE sep_expediente.docto_ajuste,
         v_ind_complementario_1   LIKE sep_expediente.ind_ajuste,
         v_indicador_desc         LIKE sep_cat_indicador_exp.indicador_desc
       END RECORD,
       v_docto_restitucion_complementario_2 RECORD
         v_docto_complementario_2 LIKE sep_expediente.docto_ajuste,
         v_ind_complementario_2   LIKE sep_expediente.ind_ajuste,
         v_indicador_desc         LIKE sep_cat_indicador_exp.indicador_desc
       END RECORD,}
...
DEFINE v_c_sql     STRING,
   ...
   OPEN WINDOW w_consulta_expediente WITH FORM "SEPC221"
   
   LET v_bandera = TRUE
   
   WHILE v_bandera
      -- Apertura de menú principal del módulo para el filtrado de expedientes
      CONSTRUCT BY NAME v_c_sql ON caso_adai,
                                   e.id_expediente,
                                   nss,
                                   tipo_nss,
                                   estado
...
DEFINE p_id_expediente   LIKE sep_expediente.id_expediente,
   ...
   OPEN WINDOW w_consulta_det_expediente WITH FORM "SEPC222"
...
--   LET v_c_sql = "SELECT e.id_expediente, e.caso_adai, e.flujo_cod, e.f_captura, e.canal_cod, n.nss, n.tipo_trabajador",
   ...
   FOREACH CurExpPreliq INTO v_r_nss_cons.*, v_tipo_nss
      IF v_id_expediente <> v_r_nss_cons.id_expediente THEN
         -- cambio de registro de expediente
         LET v_pos = v_pos + 1
         ...
         LET v_id_expediente = v_r_nss_cons.id_expediente
         ...
         END IF
      END IF
      -- Conforme a no. de nss se almacena en registro del expediente
      CASE v_pos_nss
         WHEN 1
            LET v_arr_expediente[v_pos].v_nss_acred = v_r_nss_cons.nss
         WHEN 2
            LET v_arr_expediente[v_pos].v_nss_trab1 = v_r_nss_cons.nss
         -- 2012 Mayo 30
         -- AHM Solicitud de inhabilitarlo WHEN 3
         -- AHM Solicitud de inhabilitarlo    LET v_arr_expediente[v_pos].v_nss_trab2 = v_r_nss_cons.nss
         -- AHM Solicitud de inhabilitarlo WHEN 4
         -- AHM Solicitud de inhabilitarlo    LET v_arr_expediente[v_pos].v_nss_trab3 = v_r_nss_cons.nss
         -- AHM Solicitud de inhabilitarlo WHEN 5
         -- AHM Solicitud de inhabilitarlo    LET v_arr_expediente[v_pos].v_nss_trab4 = v_r_nss_cons.nss
      END CASE
